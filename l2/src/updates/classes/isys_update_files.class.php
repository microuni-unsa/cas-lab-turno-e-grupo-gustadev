...
class isys_update_files extends isys_update
{
                ...
                if (!empty($l_files->delete->file)) {
                    ...
                    foreach ($l_files->delete->file as $l_file) {
                        ...
                        if (file_exists($l_delete) && is_file($l_delete)) {
                            $l_file_exists = true;
                        } elseif (file_exists($g_absdir . '/' . $l_delete) && is_file($g_absdir . '/' . $l_delete)) {
                            $l_file_exists = true;
                            $l_delete = $g_absdir . '/' . $l_delete;
                        }
                    ...
                    if (!empty($l_files->delete->dir)) {
                        foreach ($l_files->delete->dir as $l_dir) {
                            ...
                            if (file_exists($l_delete) && is_dir($l_delete)) {
                                $l_dir_exists = true;
                            } elseif (file_exists($g_absdir . '/' . $l_delete) && is_dir($g_absdir . '/' . $l_delete)) {
                                $l_dir_exists = true;
                                $l_delete = $g_absdir . '/' . $l_delete;
                            }
    ...
    public function copy()
    {
        ...
        $this->m_path = rtrim(str_replace('\\', '/', $this->get_path()), '/') . '/';

        $l_absdir = rtrim(str_replace('\\', '/', $g_absdir), '/');
        $l_log = isys_update_log::get_instance();
        ...
        if (is_writable($g_absdir)) {
            // @see ID-11896 Move compatibility file of updated version to update directory (for other functions).
            $this->copyIncompatibilityFile($l_absdir, $l_log);
                ...
                foreach ($l_filearray as $l_value) {
                    ...
                    if (file_exists($l_source_file)) {
                        ...
                        if (!is_dir($l_source_file)) {
                            ...
                            mkdir($l_dest_file, 0775, true);
                        }
                    }
                }

                array_map('unlink', glob(__DIR__ . '/../../temp/di_container_cache_mandator_*.php*'));
            } else {
    ...
    /**
     * @param string          $dir
     * @param isys_update_log $log
     * @return void
     * @throws Exception
     * @see ID-11896
     */
    private function copyIncompatibilityFile(string $dir, isys_update_log $log): void
    {
        $sourceCompatibilityFile = dirname($this->m_path) . '/addon-compatibilities.json';
        $targetCompatibilityFile = $dir . '/updates/addon-compatibilities.json';

        if (!file_exists($sourceCompatibilityFile)) {
            return;
        }

        if (@copy($sourceCompatibilityFile, $targetCompatibilityFile)) {
            $log->add('Copying "./addon-compatibilities.json"', C__MESSAGE, "indent", C__LOW, C__DONE);
        } else {
            throw new Exception('Could not copy addon-compatibilities.json');
        }
    }
    ...
    {
        $p_dir = rtrim(str_replace('\\', '/', $this->get_path()), '/') . '/';

        if (!empty($p_dir) && $p_dir != '/') {
            if (is_dir($p_dir)) {
     ...
     * @return bool|array
     */
    public function read_zip(string $zipFile, string $targetPath, bool $singleFile = false, bool $overwrite = false)
    {
        if (class_exists('ZipArchive')) {
            $zipArchive = new ZipArchive();
            if ($zipArchive->open($zipFile) === true) {
                $extractionResult = $zipArchive->extractTo($targetPath, $singleFile ?: null);
                $closeResult = $zipArchive->close();

                return ($extractionResult && $closeResult);
            }
        ...
        if (!function_exists('file_get_contents')) {
            ...
            if (($fileResource = @fopen($zipFile, 'rb')) !== false) {
                while (!feof($fileResource)) {
                    $data .= fread($fileResource, 4096);
                }
                fclose($fileResource);
            }
        } else {
        ...
        return $this->extract_zip($data, $targetPath, $singleFile, $overwrite);
    }
        ...
        for ($i = 0;$i < $l_zip_info['files'];$i++) {
            ...
            elseif ($p_destination !== null && !$p_single_file) {
                ...
                if (!file_exists($p_destination . '/' . $l_file_info['filename'])) {
                    mkdir($p_destination . '/' . $l_file_info['filename'], 0775);
                }
                $l_write = false;
            } else {
                $l_write = false;
            ...
            if ($l_write && $p_destination !== null) {
                ...
                if (substr($p_data, $l_file_info['offset'], 4) === 'PK' . chr(3) . chr(4)) {
                    ...
                    isys_file_put_contents($p_destination . '/' . $l_file_info["filename"], $l_file_info["data"]);
                } else {
                    return false;
                }
            }
