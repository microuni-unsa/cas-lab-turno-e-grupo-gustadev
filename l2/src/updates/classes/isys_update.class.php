...
class isys_update
{
    ...
    public static function get_module_dependencies($moduleIdentifier = null, $for = 'php')
    {
        ...
        if ($moduleIdentifier === null) {
            ...
            if ($for === 'php') {
                $extensions = defined('isys_application::REQUIRED_PHP_EXTENSIONS')
                    ? isys_application::REQUIRED_PHP_EXTENSIONS
                    : ['curl', 'gd', 'json', 'ldap', 'libxml', 'mbstring', 'mysqli', 'pcre', 'pdo', 'phar', 'session', 'simplexml', 'sockets', 'spl', 'xml', 'zip', 'zlib'];
        ...
        foreach ($packageFiles as $packageFile) {
            ...
            if (isset($jsonContent['dependencies'][$for]) && is_array($jsonContent['dependencies'][$for])) {
                foreach ($jsonContent['dependencies'][$for] as $dependency) {
                    $dependencies[$dependency][] = $language->get($jsonContent[$l_name_key]);
                }
            }
        }
    ...
    public function fetch_file($p_file)
    {
        if (function_exists("curl_init")) {
            ...
            if (!empty($l_error)) {
                ...
                $l_error_message = 'Error while connecting / cURL error: ' . curl_errno($l_sess_curl) . ' - ' . $l_error . PHP_EOL .
                    'Make sure this host is connected to the internet!' . PHP_EOL .
                    'Proxy settings can be configured in i-doit system settings' . PHP_EOL .
                    $l_proxy_config;
    ...
    public function get_isys_info()
    {
        ...
        $requiredVersion = $requiredRevision = $l_title = $l_revision = $l_version = '';
        ...
        while ($l_mrow = $g_comp_database_system->fetch_row_assoc($l_mret)) {
            ...
            if ($l_mrow['isys_db_init__key'] === 'required_revision') {
                $requiredRevision = $l_mrow['isys_db_init__value'];
            }

            if ($l_mrow['isys_db_init__key'] === 'required_version') {
                $requiredVersion = $l_mrow['isys_db_init__value'];
            }
        }
    ...
    public function get_available_updates($p_path)
    {
        ...
        $system = $this->get_isys_info();

        $l_arDirs = [];
        $l_resDir = opendir($p_path);

        while ($l_strDirValue = readdir($l_resDir)) {
            if (!str_starts_with($l_strDirValue, ".") && !in_array($l_strDirValue, ['images', 'classes', 'tpl'], true) && is_dir($p_path . $l_strDirValue)) {
        ...
        foreach ($l_arDirs as $l_val) {
            ...
            $update = $this->get_xml_data($p_path . $l_val . "/" . C__XML__SYSTEM);

            $updateIsNewer = $update["revision"] >= $system["revision"];
            $revisionIsCompatible = $update["requirement"]["revision"] <= $system["revision"];
            $noRevisionAndVersion = empty($update["revision"]) && empty($update["version"]);

            if (($updateIsNewer && $revisionIsCompatible) || $noRevisionAndVersion) {
                $l_dirs[$l_val] = $update;
                $l_dirs[$l_val]['disabled'] = !empty($update['revision']) && $update['revision'] < $update["requirement"]["revision"];

                if (!file_exists($p_path . $l_val . '/addon-compatibilities.json')) {
                    continue;
                }

                $incompatibleAddons = (new AddonVerify($p_path . $l_val . '/addon-compatibilities.json'))->getIncompatibleAddons();

                if (count($incompatibleAddons) === 0) {
                    $l_dirs[$l_val]['incompatible'] = null;
                    continue;
                }

                $l_dirs[$l_val]['disabled'] = true;
                $l_dirs[$l_val]['incompatible'] = PHP_EOL . implode(
                    PHP_EOL,
                    array_map(
                        fn ($addon) => "'{$addon['title']}' requires at least version {$addon['compatibleVersion']}, you have {$addon['currentVersion']}.",
                        $incompatibleAddons
                    )
                );
            }
        ...
        usort($l_dirs, fn ($a, $b) => strcasecmp($a['title'], $b['title']));
    ...
    public function update($p_system_database, $p_databases)
    {
        ...
        try {
            ...
            if ($p_system_database == $g_comp_database_system->get_db_name()) {
                ...
                try {
                    $l_db_update[$l_id] = $this->update_database($g_upd_dir . '/' . C__XML__SYSTEM, $g_comp_database_system, true, $daoSystem);
                ...
                foreach ($p_databases as $l_database) {
                    ...
                    try {
                        ...
                        if ($l_db_instance instanceof isys_component_database) {
                            ...
                            $l_db_update[] = $this->update_database($g_upd_dir . '/' . C__XML__DATA, $l_db_instance, true, $l_tenant_dao);
            ...
            if (strlen($g_upd_dir) > 0 && (!isset($_POST["no_file_update"]) || $_POST["no_file_update"] != "true")) {
                // ID-6346  Moved the "file copy" section somewhere else, will now be processed in an ajax request.

                // File delete.
                (new isys_update_files($g_file_dir))->delete();
            } else {
    ...
    public function update_database($p_file, isys_component_database &$p_database, $p_do_version_change = false, $p_dao = null)
    {
        ...
        foreach ($l_statements as $l_statement) {
            ...
            if ($l_check) {
                 ...
                 * Log current process
                 */
                $l_id = $l_log->add(isys_glob_str_stop($l_check_title, 85, "..") . " <strong class=\"text-neutral-400\">" . $l_check_result["error"] . "</strong>", C__MESSAGE, "indent");
                                ...
                                foreach ($l_tmp as $l_q) {
                                    ...
                                    if (!empty($l_q)) {
                                        try {
                                            ...
                                            if ($e->getCode() !== MySQL\Error\Server::ER_TOO_BIG_ROWSIZE) {
                                                throw $e;
                                            }

                                            $l_log->debug("Row format problem detected. Let us set it to `dynamic` and try again...");
                                            [$table] = $this->explode_tables((string)$l_statement['check']);

                                            $p_dao->get_database_component()->query("ALTER TABLE {$table} ROW_FORMAT=DYNAMIC;");
                                            $l_query = $p_dao->get_database_component()
                                                ->query($l_q);
    ...
    public function get_databases()
    {
        ...
        if ($g_comp_database_system->num_rows($l_ret) > 0) {
            $mandatorComponentDao = isys_component_dao_mandator::instance($g_comp_database_system);

            while ($l_row = $g_comp_database_system->fetch_row_assoc($l_ret)) {
                $tenantPass = (method_exists($mandatorComponentDao, 'getPassword')?
                    isys_component_dao_mandator::getPassword($l_row): $l_row["isys_mandator__db_pass"]);
