...
class isys_update_property_migration extends isys_update
{
    ...
    public function execute_sql()
    {
        ...
        // Force a commit to fix this in i-doit setup / update.
        $this->m_db->commit();
     ...
     * @return isys_update_property_migration
     */
    public function prepare_sql_queries(string $p_cat_type, bool $p_log = true)
    {
        $logger = new Logger('property-migration');

        if ($p_log) {
            $logger->pushHandler(new StreamHandler(BASE_DIR . 'log/property-migration_' . date('Y-m-d') . '.log'));
        }
        ...
        foreach ($this->m_cat_data[$p_cat_type] as $l_cat) {
            if (!class_exists($l_cat['class']) || (!defined($l_cat['const']) && $p_cat_type !== 'g_custom')) {
                $logger->notice('Skipping class "' . $l_cat['class'] . '"');
                $this->m_missing_classes[] = $l_cat['class'];
                continue;
            }

            if (!is_a($l_cat['class'], isys_cmdb_dao_category::class, true)) {
                $logger->warning('Skipping class "' . $l_cat['class'] . '", it does not extend "isys_cmdb_dao_category"');
                continue;
            }
            ...
            // Do not remove "$p_cat_type === 'g_custom'" for i-doit OPEN.
            if ($p_cat_type === 'g_custom' && $l_cat_dao instanceof isys_cmdb_dao_category_g_custom_fields) {
            ...
            if (is_array($l_cat_properties)) {
                foreach ($l_cat_properties as $l_prop => $l_property) {
                    ...
                    if (is_numeric($l_prop)) {
                        $logger->notice('Skipping class "' . $l_cat['class'] . '", property "' . $l_property[C__PROPERTY__INFO][C__PROPERTY__INFO__DESCRIPTION] . '"');
                    ...
                    if (isset($l_property[C__PROPERTY__DATA][C__PROPERTY__DATA__FIELD]) &&
                        (!str_starts_with($l_property[C__PROPERTY__DATA][C__PROPERTY__DATA__FIELD], 'isys_') ||
                    ...
                    if (!isset($l_property[C__PROPERTY__INFO][C__PROPERTY__INFO__DESCRIPTION])) {
                        $logger->notice('Class: "' . $l_cat['class'] . '", property key "' . $l_prop . ' has no info."');
                        $logger->notice('Migrating class "' . $l_cat['class'] . '", property key "' . $l_prop . '"');
                    } else {
                        $logger->notice('Migrating class "' . $l_cat['class'] . '", property "' . $l_property[C__PROPERTY__INFO][C__PROPERTY__INFO__DESCRIPTION] . '"');
                    }
                    ...
                    if (isset($l_property[C__PROPERTY__PROVIDES])) {
                        if ($l_property[C__PROPERTY__PROVIDES] instanceof PropertyProvides) {
