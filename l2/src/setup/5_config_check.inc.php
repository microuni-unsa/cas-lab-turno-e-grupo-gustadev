...
function process_after_posttransfer()
{
    ...
    foreach ($g_config as $l_key => $l_data) {
        ...
        switch ($l_key) {
            ...
            case "config.dir.src":
                if (is_dir($l_data["content"]) && is_writable($l_data["content"])) {
                    $l_success = true;
                    $l_message = "WRITEABLE";
                } else {
                    $l_success = false;
            ...
            case "config.db.root.password":
                ...
                if (!empty($l_dbHost) && !empty($l_dbPort)) {
                    ...
                    // @see ID-10539 Properly check database connection.
                    if (is_object($l_dbLink) && !$l_dbLink->connect_error) {
                        ...
                        $l_mysql = [
                            'innodb_buffer_pool_size' => [0, 1024],
                            'query_cache_size'        => [0, 16],
                            'max_allowed_packet'      => [0, 64],
                            'tmp_table_size'          => [0, 16],
                            'query_cache_limit'       => [0, 5]
                        ];
                        ...
                        try {
                            ...
                            if (!checkVersion(getVersion($l_db_version), $minVersion, $maxVersion)) {
                                ...
                                if (checkVersionIsAbove(getVersion($l_db_version), $maxVersion)) {
                                    ...
                                    // DB requirement is no met!
                                    $dbVersionCheckMessage = 'Not between ' . $minVersion . ' and ' . $maxVersion . ' - ' . $recommendedVersion . ' recommended.';

                                    // Set this false to stop installation.
                                    $l_success = false;
                                }
                            } elseif (version_compare($l_db_version, $deprecatedBelowVersion, '<')) {
                                $dbVersionCheckClass = 'stepLineStatusBad';
                                $dbVersionCheckMessage = 'We discourage the use of ' . $dbTitle . ' version below ' . $deprecatedBelowVersion . '.
            ...
            case "config.mandant.name":
                ...
                if (is_object($l_dbLink) && !$l_dbLink->connect_error) {
                    ...
                    $l_success = false;
                    $l_message = "DATABASE CONNECTION ISSUE";
                }
                break;
