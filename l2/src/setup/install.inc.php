...
$extensionsNeed = isys_application::REQUIRED_PHP_EXTENSIONS;
$extensionsHave = array_intersect($extensionsNeed, array_map('strtolower', get_loaded_extensions()));
$extensionsOthersNum = count(get_loaded_extensions()) - count($extensionsHave);
$extensionsMissing = array_diff($extensionsNeed, $extensionsHave);

if (count($extensionsMissing) > 0) {
    sort($extensionsNeed);
    sort($extensionsHave);
    sort($extensionsMissing);

    $extensionsNeedList = implode(', ', array_map(fn ($extension) => "<code>{$extension}</code>", $extensionsNeed));
    $extensionsHaveList = implode(', ', array_map(fn ($extension) => "<code>{$extension}</code>", $extensionsHave));
    $extensionsMissingList = implode(', ', array_map(fn ($extension) => "<code>{$extension}</code>", $extensionsMissing));

    startup_die('Not all needed extensions are installed.<br />' .
        "I need: {$extensionsNeedList}<br />" .
        "I have: {$extensionsHaveList} (and {$extensionsOthersNum} others)<br />" .
        "I am missing: {$extensionsMissingList}");
}
...
$g_config = [
    ...
    'config.crypt.hash'         => [
        ...
        "name"    => 'Crypto hash used as key for encryption with phpseclib'
    ],
    ...
    'config.admin.enable_gui_update' => [
        "content" => "1",
        "name"    => "Enable the update for the GUI (1 = enabled, 0 = disabled)"
    ]
];
