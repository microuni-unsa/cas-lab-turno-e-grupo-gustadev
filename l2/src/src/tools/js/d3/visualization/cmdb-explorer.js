...
window.CMDB_Explorer_Graph = Class.create(window.CMDB_Explorer, {
    ...
    render: function (list_array) {
        ...
        // @see ID-11412 Note the shown object IDs down.
        $('C_VISUALIZATION_VISIBLE_OBJECTS').setValue(this.data.map((d) => '0>' + d.data.obj_id).uniq().join(','));
...
window.CMDB_Explorer_Tree = Class.create(CMDB_Explorer, {
    ...
    findDataById: function (id) {
        var data = this.data.filter((d) => d.id == id);
    ...
    findChildrenByParentId: function (id) {
        var data = this.data.filter((d) => d.parent && d.parent == id);
    ...
    render: function () {
        ...
        this.dataCache.sort((a, b) => {
            if (this.profile.hasOwnProperty('sort-by-object-type') && this.profile['sort-by-object-type']) {
                // @see ID-9249 Sort by object type title + object title.
                return (a.data.obj_type_title + a.name).localeCompare((b.data.obj_type_title + b.name));
            }
        ...
        var that       = this,
            root       = d3.stratify()
                           ...
                           .parentId(function(d) { return d.parent; })(this.dataCache),
        ...
        var diagonal = function (s, d) {
            ...
            if (that.options.vertical) {
                if (that.options.mirrored) {
                    sourceX = d.x;
                    sourceY = -Math.abs(d.y) + (that.options.nodeHeight / 2) - nodeMargin;
                    targetX = s.x;
                    targetY = -Math.abs(s.y) - (that.options.nodeHeight / 2) + nodeMargin;
                } else {
                    sourceX = s.x;
                ...
                return 'M' + sourceX + ' ' + sourceY +
                       ...
                       ',' + (sourceX + targetX) / 2 + ' ' + (sourceY + targetY) / 2 +
                       'C' + targetX + ' ' + (targetY - delta) +
                       ',' + targetX + ' ' + targetY +
                       ',' + targetX + ' ' + targetY;
            }

            if (that.options.mirrored) {
                sourceX = -Math.abs(d.y) + (that.options.nodeWidth / 2) - nodeMargin;
                sourceY = d.x;
                targetX = -Math.abs(s.y) - (that.options.nodeWidth / 2) + nodeMargin;
                targetY = s.x;
            } else {
            ...
            return 'M' + sourceX + ' ' + sourceY +
                   'C' + (sourceX + delta / 2) + ' ' + sourceY +
                   ',' + (sourceX + targetX) / 2 + ' ' + (sourceY + targetY) / 2 +
                   ',' + (sourceX + targetX) / 2 + ' ' + (sourceY + targetY) / 2 +
                   'C' + (sourceX + targetX) / 2 + ' ' + targetY +
                   ',' + targetX + ' ' + targetY +
                   ',' + targetX + ' ' + targetY;
        };
