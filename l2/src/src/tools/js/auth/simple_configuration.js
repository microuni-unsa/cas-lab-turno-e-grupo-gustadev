var SimpleAuthConfiguration = Class.create({
	...
	initialize: function (element, options) {
		...
		for (var i in this.options.rights) {
            ...
            thead.insert(new Element('th', { className: 'p5', style: 'width:30px;' }).update(
		...
		this.element.update(
			new Element('table', { className: 'listing w100' }).update(
				thead
			).insert(
				new Element('tbody')
			)
	...
	display_path: function (module, rights, inherited) {
		...
		for (i in this.options.rights) {
			if (this.options.rights.hasOwnProperty(i)) {
                ...
                tr.insert(new Element('td', {className: 'center'}).update(new Element('input', {type: 'checkbox', disabled: (!this.options.edit_mode || inherited), className: 'right-checkbox m5', name: 'right_' + this.path_counter + '[]', value: right.value, checked: rights.in_array(parseInt(right.value))})));
			}
		}
		...
		tr
			...
			.insert(new Element('td', {className:'border-left'})
				.update((this.options.edit_mode && ! inherited) ? new Element('button', {className: 'btn remove-path-button', type: 'button', title: idoit.Translate.get('LC__UNIVERSAL__REMOVE')}).update(new Element('img', {src: window.dir_images + 'axialis/basic/symbol-cancel.svg'})) : ''));
	...
	create_new_path: function () {
		...
		tr
			.insert(new Element('td', {className:'border-left'}).update(new Element('span', {className: 'mr5'}).update(idoit.Translate.get('LC__AUTH_GUI__REFERS_TO'))).insert(options))
			.insert(new Element('td', {className:'border-left'})
				.update((this.options.edit_mode) ? new Element('button', {className: 'btn remove-path-button', type: 'button', title: idoit.Translate.get('LC__UNIVERSAL__REMOVE')}).update(new Element('img', {src: window.dir_images + 'axialis/basic/symbol-cancel.svg'})) : ''));
