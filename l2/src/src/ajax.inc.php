...
switch ($_GET["request"]) {
    case "breadcrumb":
        ...
        isys_module_manager::instance()->init(isys_module_request::get_instance());

        $moduleId = $_GET[C__GET__MODULE_ID] ?? defined_or_default('C__MODULE__CMDB', 2);

        $breadcrumbHtml = (new isys_component_template_breadcrumb)
            ->includeHome(true)
            ->process(false, (int)$moduleId);

        echo stripslashes($breadcrumbHtml);
        die;
