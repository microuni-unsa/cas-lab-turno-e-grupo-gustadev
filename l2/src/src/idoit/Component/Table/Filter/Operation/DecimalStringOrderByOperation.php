...
class DecimalStringOrderByOperation extends Operation
{
    ...
    {
        // @see ID-9865 cast to decimal if property is a custom category property from type f_numeric
        if ($typeAndKey = isys_cmdb_dao_category_g_custom_fields::getTypeAndKey($name)) {
            if ($typeAndKey[0] === 'f_numeric') {
                $orderBy = "CAST({$name} AS DECIMAL)";
