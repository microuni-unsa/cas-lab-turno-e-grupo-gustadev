...
class Interval
{
     ...
     * @return Carbon
     */
    public function getCheckDate(): Carbon
    {
     ...
     * @return $this
     */
    public function setCheckDate(DateTime $dateTime): static
    {
     ...
     */
    public function isIntervalExpired(): bool
    {
        $this->prepare();
     ...
     * @return bool
     */
    public function hasLastIntervalDate(): bool
    {
        $this->prepare();
     ...
     * @return DateTime
     */
    public function getLastIntervalDate(): DateTime
    {
        $this->prepare();
     ...
     * @return bool
     */
    public function hasNextIntervalDate(): bool
    {
        $this->prepare();
     ...
     */
    public function getAllPreviousIntervals(DateTime $until, array &$dates = []): ?array
    {
        try {
     ...
     * @return null|DateTime[]
     */
    public function getAllUpcomingIntervals(DateTime $until, array &$dates = []): ?array
    {
        try {
            if ($this->isIntervalExpired() || $this->getCheckDate()
    ...
    {
        $details = (array)$this->config->getRepeatDetails();
    ...
    private function iteratePeriod(DatePeriod $period, ?callable $formatter = null)
    {
        ...
        foreach ($period as $dates) {
            // @see ID-9040 In some instances the period contains 'DateTime' objects instead of 'Carbon'.
            if (is_array($dates)) {
            ...
            } elseif (is_object($dates) && !($dates instanceof Carbon)) {
                $dates = Carbon::instance($dates);
            }
            ...
            foreach ($dates as $date) {
                ...
                if (((int)$date->diffInDays($this->checkDate, false)) === 0) {
                    $this->eventCounter++;
     ...
     * @return Carbon
     */
    private function getRelativeDayInMonth(Carbon $originalDate, Carbon $searchDate): Carbon
    {
