...
class Ip
{
     ...
     */
    public static function calc_cidr_suffix($p_subnet_mask): int
    {
     ...
     * @author  Van Quyen Hoang<qhoang@synetics.de>
     */
    public static function calc_cidr_suffix_ipv6($p_net_mask): int
    {
     ...
     * @author  Van Quyen Hoang <qhoang@synetics.de>
     */
    public static function calc_ip_range($p_net_address, $p_subnet_mask, $p_fullrange = false): ?array
    {
     ...
     * @return array
     */
    public static function calc_ip_range_ipv6(string $address, $cidr): array
    {
     ...
     */
    public static function calculate_next_ipv6($p_address): string
    {
     ...
     * @author  Leonard Fischer <lfischer@i-doit.org>
     */
    public static function calculate_prev_ipv6($p_address): string
    {
        // If we got a shortened IPv6 address, create the "full lenght".
        if (strlen($p_address) < 39) {
            $p_address = self::validate_ip($p_address);
     ...
     * @author  Van Quyen Hoang<qhoang@synetics.de>
     */
    public static function calc_subnet_by_cidr_suffix($p_cidr_suffix): string
    {
     ...
     * @author  Van Quyen Hoang<qhoang@synetics.de>
     */
    public static function calc_subnet_by_cidr_suffix_ipv6($p_cidr_suffix, $p_full_mask = false): string
    {
        $l_bits = 16;
     ...
     */
    public static function ip2long($ipAddress): ?int
    {
        if (empty($ipAddress)) {
            return null;
        }
        ...
        // @see ID-9247 Make sure all values are integers.
        $ipArray = array_map('intval', $ipArray);

        return $ipArray[0] * pow(256, 3) +
            $ipArray[1] * pow(256, 2) +
            $ipArray[2] * 256 +
            $ipArray[3];
    }
     ...
     * @author  Van Quyen Hoang <qhoang@synetics.de>
     */
    public static function is_ip_in_range($p_ip, $p_range_from, $p_range_to): bool
    {
        if (strpos($p_ip, '.')) {
     ...
     * @author  Leonard Fischer <lfischer@i-doit.org>
     */
    public static function is_ipv6_in_range($p_address, $p_from, $p_to): bool
    {
     ...
     * @author  Van Quyen Hoang <qhoang@i-doit.org>
     */
    public static function long2ip($p_iplong): ?string
    {
        if ($p_iplong < 1) {
            return '0.0.0.0';
        }
     ...
     * @author  Van Quyen Hoang <qhoang@synetics.de>
     */
    public static function set_ip_in_range($p_ip, $p_range_from, $p_range_to): string
    {
     ...
     * @author  Van Quyen Hoang<qhoang@synetics.de>
     */
    public static function validate_ip($p_ip): bool
    {
     ...
     * @return  string|null
     */
    public static function validate_ipv6($address, bool $shorten = false): ?string
    {
        // @see ID-9025 We use this library because our code had issues with certain addresses and returned wrong values.
        $ipv6 = IPv6::parseString($address);

        if ($ipv6 === null) {
            return null;
        }

        return $ipv6->toString(!$shorten);
     ...
     * @author  Van Quyen Hoang <qhoang@synetics.de>
     */
    public static function validate_net_ip($p_net_ip, $p_subnetmask = null, $p_cidr_suffix = null, $p_next_net_ip = false): mixed
    {
        if (empty($p_net_ip)) {
            return null;
     ...
     * @author  Van Quyen Hoang <qhoang@synetics.de>
     */
    public static function validate_net_ipv6($p_address, $p_cidr_suffix): string
    {
     ...
     * @author  Van Quyen Hoang <qhoang@synetics.de>
     */
    public static function validate_subnetmask($p_net_mask, $p_get_next_mask = false): ?string
    {
        if (empty($p_net_mask)) {
            return null;
        }
     ...
     * @author  Van Quyen Hoang <qhoang@synetics.de>
     */
    public static function validate_subnetmask_ipv6($p_ipv6_address): string
    {
     ...
     * @author  Leonard Fischer <lfischer@i-doit.com>
     */
    public static function virtual_supernet_range_instance($p_from, $p_to): array
    {
     ...
     * @author  Leonard Fischer <lfischer@i-doit.com>
     */
    public static function get_free_ranges_in_virtual_supernet(): array
    {
        $l_return = [];
     ...
     * @author  Leonard Fischer <lfischer@i-doit.com>
     */
    public static function nslookup($p_hostname, array $p_dns_server = []): mixed
    {
        if (empty($p_hostname)) {
            return false;
        }
     ...
     * @author  Leonard Fischer <lfischer@i-doit.com>
     */
    public static function reverse_nslookup($p_ip): mixed
    {
        if (empty($p_ip)) {
            return false;
        }
     ...
     * @author  Leonard Fischer <lfischer@i-doit.com>
     */
    public static function nmap_ping($p_ip_from, $p_ip_to = null): array
    {
        $l_return = [];
        ...
        if (!in_array($l_nmap_option, array_keys(\isys_tenantsettings::NMAP_OPTIONS))) {
            $l_nmap_option = 'sP';
        }
     ...
     * @author  Leonard Fischer <lfischer@i-doit.com>
     */
    public static function fping_ping($ipFrom, $ipTo = null): array
    {
        $l_return = [];
     ...
     * @return array
     */
    public static function parseHexIpv6(string $ipv6Address): array
    {
        $addressParts = explode(':', $ipv6Address);
        $ipv6Array = [];
        for ($i=0; $i<8; $i++) {
            $ipv6Array[] = hexdec($addressParts[$i]);
        }
        return $ipv6Array;
    }

    /**
     * @param array $supernet_range_longs
     * @param array $subnet_range_longs
     * @return bool
     */
    public static function checkSubnetInRange(array $supernet_range_longs, array $subnet_range_longs): bool
    {
        for ($i=0; $i<8; $i++) {
            if (!(($supernet_range_longs['from'][$i] <= $subnet_range_longs['from'][$i]) && ($supernet_range_longs['to'][$i] >= $subnet_range_longs['to'][$i]))) {
                return false;
            }
        }
        return true;
     ...
     * @return bool
     */
    public static function checkIPv6InRange(string $ipv6Address, array $subnet_range_longs): bool
    {
        $ipv6Address = self::validate_ipv6($ipv6Address);
        if (!$ipv6Address) {
            return false;
        }
        $ipv6Array = self::parseHexIpv6($ipv6Address);
        for ($i=0; $i<8; $i++) {
            if (!(($subnet_range_longs['from'][$i] <= $ipv6Array[$i]) && ($subnet_range_longs['to'][$i] >= $ipv6Array[$i]))) {
                return false;
            }
        }
        return true;
