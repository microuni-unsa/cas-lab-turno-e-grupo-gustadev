...
class isys_notification_license_expiration extends isys_notification
{
    ...
    protected function handle_notification($p_notification)
    {
        ...
        $skipPastExpirations = (bool)$tenantSettings->get('idoit.notifications.license-expiration.skip-expired', 0);
        $skipInbetweenThresholdAndNow = (bool)$tenantSettings->get('idoit.notifications.license-expiration.skip-between-threshold-and-now', 0);
        ...
        foreach ($l_unit_parameters as $l_parameter) {
            if ($l_parameter[$l_unit['table'] . '__id'] == $p_notification['threshold_unit']) {
                ...
                $singular = $l_notification_threshold == 1;
                $thresholdModificationString = match($l_parameter[$l_unit['table'] . '__const']) {
                    'C__CMDB__UNIT_OF_TIME__SECOND' => $singular ? 'second' : 'seconds',
                    'C__CMDB__UNIT_OF_TIME__MINUTE' => $singular ? 'minute' : 'minutes',
                    'C__CMDB__UNIT_OF_TIME__HOUR' => $singular ? 'hour' : 'hours',
                    'C__CMDB__UNIT_OF_TIME__DAY' => $singular ? 'day' : 'days',
                    'C__CMDB__UNIT_OF_TIME__MONTH' => $singular ? 'month' : 'months',
                    'C__CMDB__UNIT_OF_TIME__YEAR' => $singular ? 'year' : 'years',
                };

                $thresholdModificationString = "{$l_notification_threshold} {$thresholdModificationString}";
                break;
            }
        }
        ...
        foreach ($l_objects as $l_object) {
            ...
            while ($catData = $result->get_row()) {
                if (isset($l_licenses[$l_object['isys_obj__id']])) {
                    continue;
                }
                ...
                if (!isset($l_expiration_date) || $l_expiration_date === '0000-00-00 00:00:00' || $l_expiration_date === '0000-00-00' || $l_expiration_date === C__FALLBACK_EMPTY_DATE) {
                ...
                $l_formatted_threshold_date = date('Y-m-d', $l_calculated_threshold);
                ...
                // @see ID-9081 Skip 'expired' licenses, if the tenant setting says so.
                if ($skipPastExpirations && $l_now > $l_expiration_date) {
                    $this->m_log->debug(sprintf('Threshold date exceeded in the past (%s). Skip license.', date('Y-m-d', $l_expiration_date)));
                    continue;
                }

                if ($skipInbetweenThresholdAndNow && date('Y-m-d', $l_calculated_threshold) !== date('Y-m-d', $l_now)) {
                    $this->m_log->debug(sprintf(
                        'Calculated threshold date (%s minus %s) is not today (%s). Skip license.',
                        date('Y-m-d', $l_expiration_date),
                        $thresholdModificationString,
                        date('Y-m-d', $l_now)
                    ));
                    continue;
                ...
                $l_licenses[$l_object['isys_obj__id']] = $l_object;
            }
        }
    ...
    {
        $availableInterval = ['YEAR', 'DAY', 'MONTH', 'HOUR', 'SECOND', 'MINUTE'];
        $unitConstant = $this->getUnitParameters()['isys_unit_of_time__const'] ?? null;

        if ($unitConstant === null) {
            return;
        }

        $availableInterval = array_filter($availableInterval, fn ($interval) => strpos($unitConstant, $interval));
        if (empty($availableInterval)) {
            return;
        }

        $modifier = current($availableInterval);

        foreach ($properties as $property) {
            if (isset($property['s']['C__CATS__LICENCE_LIST'])) {
                $alias = $reportQuery->getAliasFromJoins('isys_cats_lic_list');

                if ($alias !== '') {
                    $reportQuery->addCondition(
                        " AND {$alias}.isys_cats_lic_list__expire <= NOW() + INTERVAL {$this->getThreshold()} {$modifier}"
                    );
                }
                break;
            }
