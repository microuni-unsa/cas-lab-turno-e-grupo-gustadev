...
class isys_library_fileupload extends qqFileUploader
{
     ...
     *
     * @param $p_upload_dir
     * @param $p_replace_old_file
     *
     * @return array|string[]|true[]
     * @throws Exception
     */
    public function handleUpload($p_upload_dir, $p_replace_old_file = true)
    {
        $language = isys_application::instance()->container->get('language');

        if (!is_dir($p_upload_dir) && !mkdir($p_upload_dir, 0775, true)) {
            return [
                'error' => $language->get('LC__UNIVERSAL__FILE_UPLOAD__NO_DIR_WRITE_PERMISSIONS')
            ];
        }

        if (!is_writable($p_upload_dir)) {
            return [
                'error' => $language->get('LC__UNIVERSAL__FILE_UPLOAD__NO_WRITE_PERMISSIONS')
            ];
        }

        if (!$this->file) {
            return [
                'error' => $language->get('LC__UNIVERSAL__FILE_UPLOAD__NO_FILE_SELECTED')
            ];
        }
        ...
        if ($this->sizeLimit > 0 && $size > $this->sizeLimit) {
            return [
                'error' => $language->get('LC__UNIVERSAL__FILE_UPLOAD__FILE_SIZE_TOO_BIG')
            ];
        }
        ...
        if ($this->allowedExtensions && !in_array(strtolower($ext), $this->allowedExtensions)) {
            ...
            return [
                'error' => $language->get('LC__UNIVERSAL__FILE_UPLOAD__EXTENSION_ERROR') . ' - ' . $these
            ];
        }
        ...
        if ($this->file->save($p_upload_dir . $filename)) {
            ...
            return [
                'error' => $language->get('LC__UNIVERSAL__FILE_UPLOAD__FILE_UPLOADED_PARTIALLY')
            ];
        }
    }
