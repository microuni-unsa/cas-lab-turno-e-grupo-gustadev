<div id="widget-config-popup" class="p10">
	...
	<button type="button" id="widget-config-add-button" class="btn ml5">
		<img src="[{$dir_images}]axialis/basic/symbol-add.svg" /><span>[{isys type="lang" ident="LC__MODULE__DASHBOARD__WIDGET_CONFIGURATION__ADD_SELECTED_WIDGET"}]</span>
	</button>

	<div class="bg-neutral-200 mt10 p5 border border-bottom-none">[{isys type="lang" ident="LC__MODULE__DASHBOARD__WIDGET_CONFIGURATION__SELECTED_WIDGETS"}]</div>
	<ul id="widget-config-list" class="border">
		[{foreach from=$widget_list item=widget}]
		<li data-id="[{$widget.row_id}]">
			<span class="handle">&nbsp;&nbsp;&nbsp;</span>
			<span>[{$widget.title}]</span>
			<button type="button" class="btn btn-secondary delete ml-auto" title="[{isys type="lang" ident="LC__WIDGET__REMOVE"}]" data-tooltip="1">
	...
	<div class="bg-neutral-200 mt10 p5 border">[{isys type="lang" ident="LC__AUTH_GUI__CONFIGURE_DASHBOARD_OF_OTHERS__HEADLINE"}]</div>
		...
		<tr>
			<td class="vat">
				...
				<button type="button" class="btn btn-small mt10" id="define-standard-button">
                    <img src="[{$dir_images}]axialis/basic/gear.svg" alt="" /><span>[{isys type="lang" ident="LC__AUTH_GUI__CONFIGURE_DASHBOARD_OF_OTHERS__DEFINE_DEFAULT__BUTTON"}]</span>
				</button>
			</td>
			<td class="vat">
				...
				<button type="button" class="btn btn-small mt10" id="overwrite-others-button">
					<img src="[{$dir_images}]axialis/basic/tool-pencil.svg" /><span>[{isys type="lang" ident="LC__AUTH_GUI__CONFIGURE_DASHBOARD_OF_OTHERS__DEFINE_OTHERS__BUTTON"}]</span>
				</button>
			</td>
		</tr>
	</table>
...
<script type="text/javascript">
    ...
    var $defineStandardButton   = $('define-standard-button'),
        overwrite_others_button = $('overwrite-others-button');

    const $widgetDialog = $('widget-config-dialog');
    ...
    $('widget-config-add-button').on('click', function () {
        var widget_li = new Element('li', {'data-id': $widgetDialog.getValue()})
            .update(new Element('span', {className: 'handle'}).update('&nbsp;&nbsp;&nbsp;'))
            .insert(new Element('span').update($widgetDialog.down(':selected').innerText))
            .insert(new Element('button', { type: 'button', className: 'btn btn-secondary delete ml-auto', title: '[{isys type="lang" ident="LC__WIDGET__REMOVE"}]', 'data-tooltip': 1 })
                .update(new Element('img', { src: window.dir_images + 'axialis/basic/symbol-cancel.svg', alt: '' })));

        $('widget-config-list').insert(widget_li);

        reset_observer();
        // @see ID-10431 Update tooltips after changes to the DOM.
    ...
    if ($defineStandardButton) {
        $defineStandardButton.on('click', function () {
                ...
                onSuccess:  function (response) {
                    ...
                    if (json.success) {
                        result_container
                            ...
                            .update('[{isys type="lang" ident="LC__AUTH_GUI__CONFIGURE_DASHBOARD_OF_OTHERS__DEFINE_DEFAULT__SAVED"}]')
            ...
            new Ajax.Request('[{$overwrite_dashboard_ajax_url}]', {
                ...
                onSuccess:  function (response) {
                    ...
                    if (json.success) {
                        result_container
                            .addClassName('box-green')
                            .update('[{isys type="lang" ident="LC__AUTH_GUI__CONFIGURE_DASHBOARD_OF_OTHERS__DEFINE_OTHERS__SAVED"}]')
                            .removeClassName('hide');
                    } else {
                        result_container
                            .addClassName('box-red')
