...
class isys_dashboard_widgets_properties extends isys_dashboard_widgets
{
    ...
    {
        $list = $this->prepareTable($p_unique_id);
     ...
     * @throws isys_exception_database
     */
    public function prepareTable($p_unique_id)
    {
        $propertyDao = new isys_cmdb_dao_category_property_ng($this->database);
        $attributes = [];
        $header = [];

        if (is_array($this->m_config) && is_array($this->m_config['config']) && count($this->m_config['config'])) {
        ...
        $list->disableCheckboxes()
            ...
            ->setUseColumnFormatter(false)
            ->addToRouteRenderBlockList(['func'])
            ->setUseColumnFormatter(false)
            ->createTempTable();
    ...
    private function formatProperties(array $row)
    {
        foreach ($row as &$property) {
            $property = preg_replace([
                ...
                '<img class="vam" src="\' + window.dir_images + \'axialis/construction/house-4.svg">'
            ], $property);
        }
