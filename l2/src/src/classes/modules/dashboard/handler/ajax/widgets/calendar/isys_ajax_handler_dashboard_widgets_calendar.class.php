...
class isys_ajax_handler_dashboard_widgets_calendar extends isys_ajax_handler_dashboard
{
    ...
    public static function get_planning_data(array $p_params)
    {
        ...
        $l_dao = isys_cmdb_dao_category_g_planning::instance(isys_application::instance()->container->get('database'));
    ...
    public static function get_maintenance_data(array $p_params)
    {
        ...
        $l_object_data = $l_dao->get_object($p_params['obj_id'])
            ->get_row();
        $l_color = (string)$l_dao
            ->retrieve('SELECT isys_cmdb_status__color FROM isys_cmdb_status WHERE isys_cmdb_status__const = "C__CMDB_STATUS__UNDER_REPAIR";')
            ->get_row_value('isys_cmdb_status__color');

        return isys_application::instance()->container->get('template')
            ->assign('color', isys_helper_color::unifyHexColor($l_color))
            ->assign('data', [
            ...
            foreach ($p_events as $l_event) {
                if ($l_event['type'] == isys_component_calendar_event::TYPE_NOTE || $l_event['type'] == isys_component_calendar_event::TYPE_ALERT) {
                    ...
                    if ($l_event['type'] == isys_component_calendar_event::TYPE_CALLBACK && isset($l_event['callback'])) {
                        // @see ID-8944 Only allow specific callbacks, based on classes and methods.
                        $isCallbackAlright = is_array($l_event['callback']['callback'])
                            && substr($l_event['callback']['callback'][0], 0, 5) === 'isys_'
                            && class_exists($l_event['callback']['callback'][0])
                            && is_callable($l_event['callback']['callback']);

                        if ($isCallbackAlright) {
                            $l_output[] = [
                            ...
                            $l_output[] = [
                                'data' => isys_application::instance()->template->assign('message', $l_message)
                                    ->fetch(isys_dashboard_widgets_calendar::get_tpl_dir() . 'events/error.tpl')
                            ];
                        }
                    }
                }
