...
class isys_ajax_handler_dashboard extends isys_ajax_handler
{
    ...
    protected function load_widget($p_identifier, $p_unique_id, $p_widget_id, $p_default = true, $p_config = null)
    {
        ...
        if (class_exists($l_classname)) {
            $widgetInstance = isys_dashboard_widgets::factory($l_classname);

            // @see ID-11578 Close the session to unblock requests.
            if ($widgetInstance instanceof isys_dashboard_widgets && !$widgetInstance->needsActiveSession()) {
                isys_application::instance()->container->get('session')->write_close();
            }

            if (isset($l_config['title'])) {
                $l_config['title'] = isys_application::instance()->container->get('language')->get($l_config['title']);
            }
