...
class isys_dashboard_dao extends isys_module_dao
{
     ...
     */
    public function getWidgetConfigByConstant(string $widgetConstant): array
    {
        $query = 'SELECT * FROM isys_widgets_config
            INNER JOIN isys_widgets ON isys_widgets__id = isys_widgets_config__isys_widgets__id
            WHERE isys_widgets__const = ' . $this->convert_sql_text($widgetConstant);
        $result = $this->retrieve($query);
        $configurations = [];
        $userId = isys_application::instance()->container->get('session')->get_user_id();

        while ($row = $result->get_row()) {
            $configurations[$row['isys_widgets_config__isys_obj__id']] = $row;
        }
