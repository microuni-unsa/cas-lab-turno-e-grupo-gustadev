...
use idoit\Module\Templates\Exception\ValidationException;
 ...
 */
class isys_module_templates extends isys_module implements AuthableInterface
{
    /**
     *
     */
    ...
    /**
     * Defines the limit of iterations of calling getObjectTitleWithReplacedPlaceholders
     */
    const OBJECT_TITLE_ITERATION_LIMIT = 10;
    ...
    {
        if (isys_application::isPro() && defined('C__MODULE__TEMPLATES') && defined('C__MODULE__CMDB')) {
            return [
                'MASS_CHANGES' => [
                    'LC__MASS_CHANGE',
                    ...
                    C__MODULE__CMDB,
                    isys_application::instance()->www_path . 'images/axialis/basic/pages-stack.svg',
                ]
            ];
        }
    ...
    public function mydoit_get(&$p_text, &$p_link)
    {
        ...
        $explodedParameters = array_map(function ($p_arg) {
            $l_return = [];
        ...
        $l_params = array_pop($explodedParameters);
    ...
    /**
     * Method to disconnect specific signals for the usage of the template system
     *
     * @throws Exception
     * @see ID-8936 This method will disconnect some signals in 'template' context.
     */
    public static function disconnectSignalsForTemplates()
    {
        // Only disconnect the signals in 'template' context.
        if (Context::instance()->getGroup() !== Context::CONTEXT_GROUP_TEMPLATE) {
            return;
        }
        ...
        // @see ID-8772 and ID-8634 disconnect signal for autoinventory and for specific category person, person group and organization
        $signal
            ->disconnect('mod.cmdb.objectCreated', [isys_cmdb_dao_category_g_accounting::instance($database), 'signal_auto_inventory_no'])
            ->disconnect('mod.cmdb.objectCreated', [isys_cmdb_dao_category_s_person_master::instance($database), 'signalObjectCreated'])
            ->disconnect('mod.cmdb.objectCreated', [isys_cmdb_dao_category_s_organization_master::instance($database), 'signalObjectCreated'])
            ->disconnect('mod.cmdb.objectCreated', [isys_cmdb_dao_category_s_person_group_master::instance($database), 'signalObjectCreated']);
    }
    ...
    public function create_from_template(
        ...
        // @see ID-9060 Update the 'old' logging
        $l_log = isys_log::get_instance('template');
        $l_log->set_verbose_level(isys_log::C__NONE);
        $l_log->set_log_level(isys_log::C__ALL & ~isys_log::C__DEBUG);
        $l_log->set_log_file(BASE_DIR . '/log/template_' . date('Y-m-d') . '.log');
        ...
        if ($p_html_output) {
            ...
            isys_application::instance()->container->get('template')->display("head.tpl");
            echo "<div class=\"p10\">";
        }
        ...
        if (defined("C__MODULE__EXPORT") && class_exists("isys_module_export")) {
            ...
            if (class_exists($l_export_type)) {
                ...
                if (class_exists("isys_import_handler_cmdb")) {
                    ...
                    // Fixes ID-2087
                    $p_title = htmlspecialchars($p_title, ENT_COMPAT, BASE_ENCODING);
                    $l_dao = isys_cmdb_dao_category_g_global::instance(isys_application::instance()->container->get('database'));
                    $objectIds = [];
                    $config = Config::factory(null, null, null, null, 'isys_catg_global_list');
                    ...
                    for ($i = $l_start; $i < $p_count + $l_start; $i++) {
                        ...
                        $l_title = isys_application::instance()->container->get('idoit.component.placeholder-replacer')->replacePlaceholder(
                            $l_title,
                            $config
                        );
                        ...
                        // @see ID-9096 Move this INTO the loop, since the placeholder function needs to call this before category data was created for the next object.
                        // @see ID-9156 Do not pass the "last created" object, but all created objects.
     ...
     * @throws Exception
     */
    private function getObjectTitleWithReplacedPlaceholders(isys_log $log, string $placeholderPattern, string $objectTitle, int $objectId, int $objectTypeId, string $sysId, ?int $iteration = 0): string
    {
        $title = isys_application::instance()->container->get('idoit.component.placeholder-replacer')
            ->replacePlaceholder($placeholderPattern, Config::factory($objectId, $objectTypeId, $objectTitle, $sysId, 'isys_obj'));

        $daoGeneral = isys_cmdb_dao_category_g_global::instance(isys_application::instance()->container->get('database'));
        $daoGeneral->set_object_id($objectId);
        $validationErrors = $daoGeneral->validate(['title' => $title]);

        if (is_array($validationErrors)) {
            if (isys_tenantsettings::get('import.validation.break-on-error', true)) {
                // @see ID-11309 revert last update of custom counters
                isys_application::instance()->container->get('idoit.component.placeholder-replacer')
                    ->revertLastCustomCountersUpdate($placeholderPattern, 'isys_obj');
                throw ValidationException::titleAlreadyInUse($title, $objectId);
            }

            if ($iteration > self::OBJECT_TITLE_ITERATION_LIMIT) {
                throw ValidationException::iterationLimitExceeded($placeholderPattern, $objectId);
            }

            $log->warning("Title '{$title}' already in use generating new title with placeholder '{$placeholderPattern}' for object ID {$objectId}.");

            return $this->getObjectTitleWithReplacedPlaceholders($log, $placeholderPattern, $objectTitle, $objectId, $objectTypeId, $sysId, $iteration++);
        }
    ...
    public function apply_mass_change($p_objects, $p_template, $p_empty_fields, $p_multivalue_categories, $p_html_output = true, $overwriteCmdbStatus = true)
    {
        ...
        $l_log->set_verbose_level(constant('isys_log::' . isys_application::instance()->container->get('request')->get('log-level')));
        ...
        $l_xml_template = $l_export->setOverwriteCmdbStatusOnMassChange($overwriteCmdbStatus)
            ->export(
                ...
                true,
                false,
                ((int)$p_empty_fields) === isys_import_handler_cmdb::C__KEEP
            )
            ->parse()
            ->get_export();
        ...
        $l_dao = isys_cmdb_dao_category_g_accounting::instance(isys_application::instance()->container->get('database'));

        $l_template_object = $l_dao->get_object_by_id($p_template)
            ->__to_array();

        $titleWithPlaceholders = null;
        if (isys_cmdb_dao_category_g_accounting::has_placeholders($l_template_object['isys_obj__title'])) {
            $titleWithPlaceholders = $l_template_object['isys_obj__title'];
        }
        ...
        try {
            foreach ($p_objects as $i => $l_object_id) {
                // @see ID-10906 Check if there is at least 4 MB of memory available.
                Memory::instance()
                    ->outOfMemoryBreak(1024 * 1024 * 4);

                // Fetch object information:
                $l_object = $l_dao->get_object_by_id($l_object_id)
                    ->__to_array();
                $oldTitle = $l_object['isys_obj__title'];

                if ($titleWithPlaceholders !== null) {
                    $l_object['isys_obj__title'] = $this->getObjectTitleWithReplacedPlaceholders(
                        $l_log,
                ...
                $l_modified_template = $this->modify_xml_header(
                    $l_xml_template,
                    htmlspecialchars($l_object['isys_obj__title'], ENT_COMPAT, BASE_ENCODING),
                ...
                $l_string .= $this->language->get("LC__MASS_CHANGE__APPLIED", ["var1" => $oldTitle]) . "<br>";

                $l_object_link = $this->prepareLink($l_object);

                $l_string .= '<a class="" target="_blank" href="' . $l_object_link . '">' . $this->language->get($l_object['isys_obj_type__title']) . ': ' .
    ...
    {
        // @see ID-9929 Use a different method to fetch affected categories as efficient as possible.
        $affectedCategories = $this->loadCategories(
            (int)$p_ar_data['isys_obj__id'],
            (int)$p_ar_data['isys_obj__isys_obj_type__id']
        );

        $exportUrl = isys_helper_link::create_url([
            C__GET__MODULE_ID => $this->get_module_id(),
            'export' => $p_ar_data['isys_obj__id']
        ]);

        $p_ar_data['affected_categories'] = '<div style="white-space:normal;">' . $affectedCategories . '</div>';
        $p_ar_data['title'] = '<strong>' . $p_ar_data['isys_obj__title'] . '</strong><br />' . $p_ar_data['isys_obj_type__title'];
        $p_ar_data['created'] = $p_ar_data['isys_obj__created'] . " (" . $p_ar_data['isys_obj__created_by'] . ")";
        $p_ar_data['export'] = '<a href="' . $exportUrl . '" class="btn btn-small" data-tooltip="1" title="LC__CMDB__EXPORT__DOWNLOAD">' .
            '<img src="' . isys_application::instance()->www_path . 'images/axialis/basic/symbol-download.svg" alt="Download" />' .
            '</a>';
    }
    ...
    /** @var array */
    private static array $objectTypeCategories = [];
     ...
     * @see ID-9929
     */
    private function prepareQueries(int $objectTypeId): array
    {
        $dao = isys_application::instance()->container->get('cmdb_dao');
        $queries = [];
        $typeEdit = $dao->convert_sql_int(isys_cmdb_dao_category::TYPE_EDIT);
        $statusNormal = $dao->convert_sql_int(C__RECORD_STATUS__NORMAL);

        // This query should fetch ALL categories (including sub-categories) assigned to the given object type ID.
        $query = "SELECT 'g' AS 'type', isysgui_catg__id AS 'id', isysgui_catg__title AS 'title', isysgui_catg__source_table AS 'table',
            isysgui_catg__class_name as 'class'
            FROM isysgui_catg
            ...
            AND isysgui_catg__type & {$typeEdit}
            AND isysgui_catg__source_table NOT IN ('isys_catg_virtual', 'isys_catg_logb')
            AND isysgui_catg__status = {$statusNormal}

            UNION

            SELECT 'g' AS 'type', isysgui_catg__id AS 'id', isysgui_catg__title AS 'title', isysgui_catg__source_table AS 'table',
            isysgui_catg__class_name as 'class'
            FROM isysgui_catg
            WHERE isysgui_catg__parent IN (
                SELECT isysgui_catg__id
                FROM isysgui_catg
                INNER JOIN isys_obj_type_2_isysgui_catg ON isys_obj_type_2_isysgui_catg__isysgui_catg__id = isysgui_catg__id
                WHERE isys_obj_type_2_isysgui_catg__isys_obj_type__id = {$objectTypeId}
                AND isysgui_catg__status = {$statusNormal}
            ...
            SELECT 's' AS 'type', isysgui_cats__id AS 'id', isysgui_cats__title AS 'title', isysgui_cats__source_table AS 'table',
            isysgui_cats__class_name as 'class'
            FROM isysgui_cats
            INNER JOIN isys_obj_type ON isys_obj_type__isysgui_cats__id = isysgui_cats__id
            WHERE isys_obj_type__id = {$objectTypeId}
            AND isysgui_cats__type & {$typeEdit}
            AND isysgui_cats__source_table NOT IN ('isys_catg_virtual_list', 'isys_cats_virtual', 'isys_cats_virtual_list')
            AND isysgui_cats__status = {$statusNormal}
            ...
            isysgui_cats__class_name as 'class'
            FROM isysgui_cats
            WHERE isysgui_cats__id IN (
                SELECT isysgui_cats__id
                FROM isysgui_cats
                INNER JOIN isys_obj_type ON isys_obj_type__isysgui_cats__id = isysgui_cats__id
                WHERE isys_obj_type__id = {$objectTypeId}
                AND isysgui_cats__status = {$statusNormal}
            ...
            SELECT 'c' AS 'type', isysgui_catg_custom__id AS 'id', isysgui_catg_custom__title AS 'title', isysgui_catg_custom__source_table AS 'table',
            'isys_cmdb_dao_category_g_custom_fields' as 'class'
            ...
            AND isysgui_catg_custom__status = {$statusNormal};";

        $result = $dao->retrieve($query);

        while ($row = $result->get_row()) {
            $table = $row['table'] . ($row['type'] === 'g' ? '_list' : '');
            $title = $dao->convert_sql_text($row['title']);

            if (!isset(self::$existingTables[$table])) {
                // @see ID-10534 Check if specific fields exist.
                self::$existingTables[$table] = $dao->fieldsExistsInTable($table, ["{$table}__isys_obj__id", "{$table}__status"]);
            }

            if (!self::$existingTables[$table]) {
                continue;
            }

            $customCategoryCondition = $row['type'] === 'c'
                ? 'AND isys_catg_custom_fields_list__isysgui_catg_custom__id = ' . $dao->convert_sql_id($row['id'])
                : '';
            $objectIdField = "{$table}__isys_obj__id";

            if (!$dao->fieldsExistsInTable($table, [$table . '__isys_obj__id'])) {
                $className = $row['class'];
                $categoryDao = new $className(isys_application::instance()->container->get('database'));
                if ($categoryDao instanceof isys_cmdb_dao_category_g_custom_fields) {
                    $categoryDao->set_catg_custom_id($row['id']);
                }
                $objectIdField = $categoryDao->get_object_id_field();
            }

            $queries[$table] = "SELECT COUNT(1) AS cnt, {$title} AS category
                    FROM {$table}
                    WHERE {$objectIdField} = :objectId
                    AND {$table}__status = {$statusNormal}
     ...
     * @see ID-9929
     */
    private function loadCategories(int $objectId, int $objectTypeId): string
    {
        $dao = isys_application::instance()->container->get('cmdb_dao');
        $categoryStrings = [];

        // Only prepare the queries, if they have not yet been prepared.
        if (!isset(self::$objectTypeCategories[$objectTypeId])) {
            self::$objectTypeCategories[$objectTypeId] = $this->prepareQueries($objectTypeId);
        }

        $countQueries = self::$objectTypeCategories[$objectTypeId];

        if (count($countQueries) === 0) {
            return '';
        }

        // Merge all queries into one and replace the object ID.
        $result = $dao->retrieve(str_replace(':objectId', $objectId, implode(' UNION ', $countQueries)));
        ...
        while ($row = $result->get_row()) {
            ...
            $categoryStrings[] = $row['category'] . ' (' . $row['cnt'] . ')';
        }

        natcasesort($categoryStrings);
    ...
    private function process($p_get, $p_post)
    {
        ...
        if (isset($p_get["export"]) && $p_get["export"] > 0) {
            ...
            $l_affected_categories = $this->get_affected_categories(
                $p_get["export"],
                isys_cmdb_dao::instance($this->m_db)->get_objTypeID($p_get["export"]),
                true
            );

            echo (new isys_export_cmdb_object())
                ->export($p_get["export"], $l_affected_categories, C__RECORD_STATUS__NORMAL)
                ->parse()
                ->get_export();
            die;
        }
        ...
        switch ($p_get[C__GET__SETTINGS_PAGE]) {
            case TPL_PID__NEW:
                if ($p_get[C__GET__MODULE] == C__MODULE__CMDB) {
                    ...
                    $template
                        ->assign('content_title', $this->language->get('LC__MASS_CHANGE__CREATE_NEW_TEMPLATE'))
                    ...
                    $template
                        ->activate_editmode()
                        ->assign('content_title', $this->language->get('LC__TEMPLATES__NEW_TEMPLATE'))
                        ->smarty_tom_add_rules('tom.content.bottom.content', $rules)
            ...
            case TPL_PID__NEW_OBJET:
                ...
                $template->activate_editmode()
                    ->assign('content_title', $this->language->get('LC__TEMPLATES__NEW_OBJECT_FROM_TEMPLATE'))
            ...
            case TPL_PID__MASS_CHANGE:
                if (!isys_application::isPro()) {
                    throw new isys_exception_general("The 'mass change' is only available in i-doit.");
                }
                ...
                // @see ID-10969 Always add ranking options.
                $l_rules['templates']['p_arData'] = $this->getExtraMenuItems();

                if (count($l_template_array) === 0) {
                    $l_rules['apply_mass_change']['p_bDisabled'] = true;
                    $template->assign('field_disabled', 'disabled="disabled"');
                } else {
                    // Add divider.
                    $l_rules['templates']['p_arData'] += [$this->language->get('LC__MASS_CHANGE__DIVIDER') => []];
                    // Add templates.
                    $l_rules['templates']['p_arData'] += $l_template_array;
                }

                // Options:
                $template->activate_editmode()
                    ->assign('content_title', $this->language->get('LC__MASS_CHANGE'))
            ...
            default:
                ...
                if (is_array($p_post[C__GET__ID]) && !empty($p_post[C__GET__ID]) &&     // Check whether entries are selected
    ...
    protected function changeRank($object, $rank)
    {
        ...
        while ($statusDiff !== 0) {
            ...
            $success = $dao->rank_records([$id], $direction, 'isys_obj');
            if (!$success) {
                return false;
            }
        ...
        if ($success) {
            // Update the 'last change' of the object:
            $dao->object_changed($id);
        }
    ...
    public function disconnectSignals()
    {
        isys_application::instance()->container->get('signals')
            ->disconnect('mod.cmdb.templatesApplied', [
                isys_cmdb_dao_category_g_accounting::instance($this->m_db),
                'templateAppliedAutoInventory'
            ]);
    }
}
