<table class="contentTable">
    ...
    [{if !$catFilter}]
    <!-- For now we only allow this, if NO category filter is set. Maybe in future we can update this feature to also work with category filters? -->
    <tr>
        <td class="key">[{isys type="f_label" name="C__CUSTOMIZE_OBJECT_BROWSER__OBJECT_TYPE_FILTER_FOR_CREATION" ident="LC__CMDB__TREE__SYSTEM__CMDB_EXPLORER__OBJECT_TYPE_FILTER_FOR_CREATION"}]</td>
        <td class="value">[{isys type="f_dialog" name="C__CUSTOMIZE_OBJECT_BROWSER__OBJECT_TYPE_FILTER_FOR_CREATION"}]</td>
    </tr>
    [{/if}]
</table>
