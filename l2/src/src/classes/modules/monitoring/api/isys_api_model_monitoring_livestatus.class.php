...
class isys_api_model_monitoring_livestatus extends isys_api_model_monitoring implements isys_api_model_interface
{
    ...
    public function read($parameters)
    {
        ...
        } elseif (isset($parameters['ids']) && is_array($parameters['ids'])) {
    ...
    public function create($parameters)
    {
        if (! isset($parameters['data']['title']) || empty($parameters['data']['title'])) {
            throw new isys_exception_api_validation('Mandatory parameter "title" not found in your request.', -32602);
        }

        if (! isset($parameters['data']['connection']) || empty($parameters['data']['connection'])) {
            throw new isys_exception_api_validation('Mandatory parameter "connection" not found in your request.', -32602);
        }
    ...
    public function update($parameters)
    {
        ...
        if (isset($parameters['id']) && is_numeric($parameters['id']) && $parameters['id'] > 0) {
            if ($this->dao->get_data($parameters['id'], C__MONITORING__TYPE_LIVESTATUS)->count() === 0) {
                throw new isys_exception_api_validation('The given ID does not exist.', -32602);
            }
        }
        ...
        if (isset($parameters['data']['connection'])) {
            ...
            if ($parameters['data']['connection'] !== 'tcp' && $parameters['data']['connection'] !== 'unix') {
                throw new isys_exception_api_validation('The given Livestatus connection "' . $parameters['data']['connection'] . '" is not valid. Please provide "unix", "unix socket" or "tcp".', -32602);
            }
