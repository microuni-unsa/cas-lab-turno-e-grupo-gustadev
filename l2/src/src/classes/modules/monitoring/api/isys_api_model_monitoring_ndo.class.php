...
class isys_api_model_monitoring_ndo extends isys_api_model_monitoring implements isys_api_model_interface
{
    ...
    public function create($parameters)
    {
        ...
        if (! isset($parameters['data']['database']) || empty($parameters['data']['database'])) {
            throw new isys_exception_api_validation('Mandatory parameter "database" not found in your request.', -32602);
        }

        if (! isset($parameters['data']['username']) || empty($parameters['data']['username'])) {
            throw new isys_exception_api_validation('Mandatory parameter "username" not found in your request.', -32602);
        }

        if (! isset($parameters['data']['password']) || empty($parameters['data']['password'])) {
            throw new isys_exception_api_validation('Mandatory parameter "password" not found in your request.', -32602);
        }
