...
class Main extends Base
{
    ...
    public function process(ModuleBase $module, ComponentTemplate $template, DaoBase $model)
    {
        ...
        // @see ID-8977 We need to check this in our next release.
        ...
        if (!isys_module_multiedit::getAuth()->is_allowed_to(isys_auth::VIEW, 'multiedit')) {
            $this->paths['contentbottomcontent'] = $module::getPath() . 'templates/auth-error.tpl';

            $template->assign('errorMessage', $language->get('LC__AUTH__MULTIEDIT_EXCEPTION__MISSING_RIGHT_FOR_MULTIEDIT', $language->get('LC__AUTH__RIGHT_VIEW')));
        ...
        if ($preselection) {
            ...
            try {
                ...
                asort($categories[$language->get('LC__CMDB__GLOBAL_CATEGORIES')]);
                asort($categories[$language->get('LC__CMDB__SPECIFIC_CATEGORIES')]);
                asort($categories[$language->get('LC__CMDB__CUSTOM_CATEGORIES')]);
            } catch (CategoryDataException $e) {
                isys_notify::error($e->getMessage(), ['sticky' => true]);
            }
        }

        // @see ID-9453 Select 'Extras' if no object type was selected.
        if (isset($_GET[C__CMDB__GET__OBJECTTYPE]) && is_numeric($_GET[C__CMDB__GET__OBJECTTYPE])) {
            $objTypeGroupId = $container->get('cmdb_dao')
                ->get_objtype($_GET[C__CMDB__GET__OBJECTTYPE])
        ...
        \isys_component_template_navbar::getInstance()
            ...
            ->set_icon('axialis/documents-folders/document-type-new.svg', C__NAVBAR_BUTTON__EDIT)
        ...
        $template
            ...
            ->assign('content_title', $language->get('LC__MULTIEDIT__MULTIEDIT'))
