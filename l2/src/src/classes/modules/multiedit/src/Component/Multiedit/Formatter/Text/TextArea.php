 ...
 */
class TextArea extends Formatter implements FormatterInterface
{
    /**
     * @var string
     */
    protected static $type = C__PROPERTY__INFO__TYPE__TEXTAREA;
    ...
    public static function formatCell($valueFormatter)
    {
        ...
        $plugin = new isys_smarty_plugin_f_textarea();
        ...
        // @see ID-12179 Do not work with 'sprintf' in case the field contains '%'.
        return "<td data-cell-type='{$type}' data-old-value='{$value->getViewValue()}' data-sort='{$value->getViewValue()}' data-key='{$valueFormatter->getPropertyKey()}' class='multiedit-table-td'>" .
            $plugin->navigation_edit(\isys_application::instance()->container->get('template'), $params) .
            "</td>";
    }
