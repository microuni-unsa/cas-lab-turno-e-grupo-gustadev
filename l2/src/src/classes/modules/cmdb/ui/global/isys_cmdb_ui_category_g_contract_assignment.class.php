...
class isys_cmdb_ui_category_g_contract_assignment extends isys_cmdb_ui_category_global
{
    ...
    public function process(isys_cmdb_dao_category $p_cat)
    {
        ...
        if ((!empty($l_catdata["isys_catg_contract_assignment_list__contract_start"])
                && $l_catdata["isys_catg_contract_assignment_list__contract_start"] !== C__FALLBACK_EMPTY_DATETIME
                && $l_catdata["isys_catg_contract_assignment_list__contract_start"] !== C__FALLBACK_EMPTY_DATETIME_TZ) ||
            (!empty($l_catdata["isys_catg_contract_assignment_list__contract_end"])
                && $l_catdata["isys_catg_contract_assignment_list__contract_end"] !== C__FALLBACK_EMPTY_DATETIME
                && $l_catdata["isys_catg_contract_assignment_list__contract_end"] !== C__FALLBACK_EMPTY_DATETIME_TZ)) {
            $l_subcontract = true;
        }
        ...
        if ($l_connected_contract_id) {
            ...
            $dateFields = [
                ...
                'isys_cats_contract_list__notice_date',
                'notice_end',
                'maintenance_end',
            ];
            ...
            if ((!empty($l_catdata["isys_catg_contract_assignment_list__contract_start"]) || !empty($l_contract_data["isys_cats_contract_list__start_date"])) &&
                ...
                $l_universal_startdate = (!empty($l_catdata["isys_catg_contract_assignment_list__contract_start"])
                    && $l_catdata["isys_catg_contract_assignment_list__contract_start"] != C__FALLBACK_EMPTY_DATETIME
                    && $l_catdata["isys_catg_contract_assignment_list__contract_start"] != C__FALLBACK_EMPTY_DATETIME_TZ) ? $l_catdata["isys_catg_contract_assignment_list__contract_start"] : $l_contract_data["isys_cats_contract_list__start_date"];

                if ($l_universal_startdate != C__FALLBACK_EMPTY_DATETIME && $l_universal_startdate != C__FALLBACK_EMPTY_DATETIME_TZ) {
                    $l_rules["C__CATG__CONTRACT_ASSIGNMENT__MAINTENANCE_PERIOD"]['p_strValue'] = $l_contract_dao->calculate_maintenanceperiod(
            ...
            foreach ($dateFields as $dateField) {
                if (isset($l_contract_data[$dateField]) && strtotime($l_contract_data[$dateField]) !== false) {
                    $l_contract_data[$dateField] = $locale->fmt_date($l_contract_data[$dateField]);
                } else {
                    $l_contract_data[$dateField] = '';
                }
            }
