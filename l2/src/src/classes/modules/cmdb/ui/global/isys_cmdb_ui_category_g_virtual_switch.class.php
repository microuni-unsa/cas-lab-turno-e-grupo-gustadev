    ...
    public function process(isys_cmdb_dao_category $p_cat)
    {
        ...
        if ($l_id) {
            ...
            while ($l_row = $l_pgs->get_row()) {
                ...
                $l_clients = $p_cat->get_connected_clients($clusterMembership, $l_row["isys_virtual_port_group__id"]);

                while ($l_rowClient = $l_clients->get_row()) {
                    $l_title = $l_quickinfo->get_quick_info(
                        $l_rowClient["isys_obj__id"],
                        $language->get($p_cat->get_objtype_name_by_id_as_string($l_rowClient["isys_obj__isys_obj_type__id"])) . " >> " . $l_rowClient["isys_obj__title"],
                        C__LINK__OBJECT
                    );
                ...
                if ($_POST[C__GET__NAVMODE] == C__NAVMODE__EDIT) {
                    $l_pgData .= '<td>' .
                        '<button type="button" class="btn btn-small fr" onclick="remove_port_group(' . $l_pgCount . ');" title="' . $language->get('LC__UNIVERSAL__REMOVE') . '" data-tooltip="1">' .
            ...
            while ($l_row = $l_scps->get_row()) {
                ...
                if ($_POST[C__GET__NAVMODE] == C__NAVMODE__EDIT) {
                    $l_scpData .= '<td>' .
                        '<button type="button" class="btn btn-small fr" onclick="remove_service_console_port(' . $l_scpCount . ');" title="' . $language->get('LC__UNIVERSAL__REMOVE') . '" data-tooltip="1">' .
                        '<img src="' . $g_dirs['images'] . 'axialis/basic/symbol-cancel.svg" />' .
                        '</button>' .
                        '</td>';
                }
            ...
            while ($l_row = $l_vmks->get_row()) {
                ...
                if ($_POST[C__GET__NAVMODE] == C__NAVMODE__EDIT) {
                    $l_vmkData .= '<td>' .
                        '<button type="button" class="btn btn-small fr" onclick="remove_vmkernel_port(' . $l_vmkCount . ');" title="' . $language->get('LC__UNIVERSAL__REMOVE') . '" data-tooltip="1">' .
                        '<img src="' . $g_dirs['images'] . 'axialis/basic/symbol-cancel.svg" />' .
                        '</button>' .
                        '</td>';
                }
