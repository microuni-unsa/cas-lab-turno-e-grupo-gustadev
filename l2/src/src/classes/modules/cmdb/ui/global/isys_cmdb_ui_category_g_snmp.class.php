...
class isys_cmdb_ui_category_g_snmp extends isys_cmdb_ui_category_global
{
    ...
    public function process(isys_cmdb_dao_category $p_cat)
    {
        ...
        if ($l_primary_ip !== null) {
            ...
            try {
                $this->get_template_component()
                    ->assign('snmp', new isys_library_snmp($l_primary_ip, $l_catdata['isys_snmp_community__title']));
            } catch (Exception $e) {
        ...
        // @see ID-10440 Only try to unserialize, if content exists.
        if (!empty($l_catdata['isys_catg_snmp_list__oids'])) {
            $l_oids = Unserialize::toArray($l_catdata['isys_catg_snmp_list__oids']);
