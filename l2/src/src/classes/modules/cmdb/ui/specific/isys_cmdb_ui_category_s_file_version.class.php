        ...
        if (is_array($l_active_file)) {
            ...
            if ($l_filepath && file_exists($l_filepath)) {
                $downloadUrl = isys_application::instance()->container->get('route_generator')
                    ->generate('system.file-download', ['type' => 'cmdb.global-file-category', 'identifier' => $l_active_file['isys_file_version__isys_file_physical__id']]);

                $l_filesize = filesize($l_filepath);

                if ($l_filesize < 100000) {
                    $fileSize = isys_convert::memory($l_filesize, 'C__MEMORY_UNIT__KB', C__CONVERT_DIRECTION__BACKWARD);
                    $unit = 'LC__CMDB__MEMORY_UNIT__KB';
                } else {
                    $fileSize = isys_convert::memory($l_filesize, 'C__MEMORY_UNIT__MB', C__CONVERT_DIRECTION__BACKWARD);
                    $unit = 'LC__CMDB__MEMORY_UNIT__MB';
                }

                $l_rules['C__CATS__FILE_SIZE']['p_strValue'] = isys_convert::formatNumber($fileSize) . ' ' . $language->get($unit);
            }
        }
