...
class isys_cmdb_action_category_rank extends isys_cmdb_action_category implements isys_cmdb_action
{
    ...
    public function handle(isys_cmdb_dao $p_dao, &$p_data)
    {
        ...
        if (!$this->object_is_locked()) {
            ...
            // @see ID-10911 Special case for 'file version' since it's no ordinary category.
            if ($p_table === 'isys_file_version_list') {
                $p_table = 'isys_file_version';
            }
                    ...
                    if (count($p_posts) > 0) {
                        ...
                        if (!($l_result = $this->m_cat_dao->rank_records($p_posts, $p_direction, $p_table, [
                            ...
                            throw new isys_exception_cmdb(
                                "Could not delete category entries (" . var_export($p_posts, true) . ") (CMDB-DAO->rank_records)",
                        ...
                        $this->m_cat_dao->object_changed($this->m_cat_dao->get_object_id());
