...
class isys_cmdb_action_object_create implements isys_cmdb_action
{
    ...
    public function handle(isys_cmdb_dao $p_dao, &$p_data)
    {
        ...
        try {
            ...
            // Create the object.
            $title = '';
            $l_new_objid = $p_dao->insert_new_obj($p_objtype_id, false, $title);

            if ($l_default_template && $l_template_module instanceof isys_module_templates) {
                $l_obj_title = '';
