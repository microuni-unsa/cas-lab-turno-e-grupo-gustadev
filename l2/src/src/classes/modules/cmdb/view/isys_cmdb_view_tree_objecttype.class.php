...
class isys_cmdb_view_tree_objecttype extends isys_cmdb_view_tree
{
    ...
    public function tree_build()
    {
        ...
        $l_root = $this->m_tree->add_node(
            0,
            C__CMDB__TREE_NODE__PARENT,
            $l_roottitle,
            $l_root_link,
            '',
            $g_dirs['images'] . 'axialis/user-interface/interface-3-filled.svg',
            false,
            '',
            '',
            true,
        ...
        $l_dao_object_types = \idoit\Module\Cmdb\Model\CiTypeCache::instance(isys_application::instance()->container->get('database'));
        ...
        if ($objectTypeSorting == C__CMDB__VIEW__OBJECTTYPE_SORTING__AUTOMATIC && count($l_type_data_arr)) {
        ...
        foreach ($l_type_data_arr as $l_typedata) {
            ...
            $l_issel = $l_typedata["isys_obj_type__id"] == $l_objtypeid;
            ...
            $this->m_tree->add_node(
                ...
                $l_link,
                '',
                isys_application::instance()->container->get('route_generator')->generate('cmdb.object-type.icon', ['objectTypeId' => (int)$l_typedata["isys_obj_type__id"]]),
                $l_issel,
                '',
                '',
                true,
        ...
        $l_tpl->assign('treeHide', (int)$userSettings->get('gui.tree.hide-empty-object-types', false));
