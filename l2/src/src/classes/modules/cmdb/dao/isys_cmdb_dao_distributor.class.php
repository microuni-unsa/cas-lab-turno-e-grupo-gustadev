    ...
    public function __construct(isys_component_database $p_db, $p_objid, $p_disttype = C__CMDB__CATEGORY__TYPE_GLOBAL, $p_list_id = null, $p_categories = null)
    {
        ...
        if ($p_objid > 0 && $l_cattype) {
            ...
            while ($l_row = $l_cats->get_row()) {
                ...
                $sourceTable = $l_row["{$l_cattable}__source_table"];
                $daoClassName = $l_row["{$l_cattable}__class_name"];

                // Skip categories, that might not exist anymore.
                if (!class_exists($daoClassName)) {
                    continue;
                }

                // Retrieve category id if category is single value and $p_list_id was not given.
                if ($l_row[$l_cattable . "__list_multi_value"] == "0" && (($l_num_cats == 1 && empty($l_list_id)) || $l_num_cats > 0)) {
                    if ($p_disttype == C__CMDB__CATEGORY__TYPE_SPECIFIC || $p_disttype == C__CMDB__CATEGORY__TYPE_CUSTOM) {
                        ...
                        $l_table = "{$sourceTable}_list";
                    }

                    if (!$this->table_exists($l_table)) {
                        continue;
                    }

                    if ($p_disttype === C__CMDB__CATEGORY__TYPE_CUSTOM) {
                        $l_query = "SELECT {$l_table}__data__id AS id
                            FROM {$l_table}
                            WHERE {$l_table}__isys_obj__id = '" . $this->m_db->escape_string($p_objid) . "'
                            AND {$l_table}__isysgui_catg_custom__id = '" . $this->m_db->escape_string($l_cat_id) . "'
                            LIMIT 1;";
                    } else {
                        $daoInstance = $daoClassName::instance($p_db);

                        // @see ID-9959 Implement a method to get specific queries.
                        if ($daoInstance instanceof HasDistinctEntryIdQuery) {
                            $l_query = $daoInstance->getEntryIdQuery((int)$p_objid);
                        } else {
                            $l_query = "SELECT {$l_table}__id AS id
                                FROM {$l_table}
                                WHERE {$l_table}__isys_obj__id = '" . $this->m_db->escape_string($p_objid) . "'
                                LIMIT 1;";
                        }
                ...
                if ($p_objid != null) {
                    ...
                    if ($l_catobj != null) {
                        // @see ID-11255 Fix list configuration for OPEN users, caused by ID-10116.
                        if ($p_disttype == C__CMDB__CATEGORY__TYPE_CUSTOM && $l_catobj instanceof isys_cmdb_dao_category_g_custom_fields) {
                            $l_catobj->set_catg_custom_id($l_row['isysgui_catg_custom__id']);
                            $l_catobj->set_catgory_const($l_row['isysgui_catg_custom__const']);
                        }
