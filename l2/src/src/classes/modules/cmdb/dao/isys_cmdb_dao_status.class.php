...
class isys_cmdb_dao_status extends isys_cmdb_dao
{
    ...
    {
        /*
        // @todo  Come up with a better solution, because this causes errors.
    ...
    {
        // @see API-436 Properly format the constant.
        $p_const = strtoupper(str_replace('-', '_', isys_helper_upload::prepare_filename($p_const)));

        if (trim($p_const) === '') {
            $p_const = 'C__CMDB_STATUS__' . time();
        }
        ...
        $l_sql = 'UPDATE isys_cmdb_status
			...
			isys_cmdb_status__color = ' . $this->convert_sql_text(isys_helper_color::unifyHexColor($p_color)) . '
     ...
     * @return  mixed
     */
    public function create($p_const, $p_title, $p_color = '#ffffff')
    {
        // @see API-436 Properly format the constant.
        $p_const = strtoupper(str_replace('-', '_', isys_helper_upload::prepare_filename($p_const)));
     ...
     * @return  mixed
     */
    public function get_cmdb_status_color($p_status_id): string
    {
        return isys_helper_color::unifyHexColor($this->get_cmdb_status($p_status_id)->get_row_value('isys_cmdb_status__color'));
    }
}
