...
class isys_cmdb_dao_list_catg_share_access extends isys_component_dao_category_table_list implements isys_cmdb_dao_list_interface
{
    ...
    public function modify_row(&$p_row)
    {
        if ($p_row['isys_catg_share_access_list__isys_catg_relation_list__id']) {
            $sql = 'SELECT isys_obj__status AS status
                FROM isys_catg_relation_list 
                INNER JOIN isys_obj ON isys_obj__id = isys_catg_relation_list__isys_obj__id
                WHERE isys_catg_relation_list__id = ' . $this->convert_sql_id($p_row['isys_catg_share_access_list__isys_catg_relation_list__id']) . ';';

            $relationObjectStatus = $this->retrieve($sql)->get_row_value('status');
        ...
        $p_row['object'] = isys_ajax_handler_quick_info::instance()->getQuickInfoReplacement(
            $p_row['isys_connection__isys_obj__id'],
            isys_application::instance()->container->get('cmdb_dao')->get_obj_name_by_id_as_string($p_row['isys_connection__isys_obj__id'])
        );
    }
    ...
    public function get_fields()
    {
        return [
            ...
            'isys_catg_share_access_list__description' => 'LC__CMDB__CAT__COMMENTARY',
        ];
    }
}
