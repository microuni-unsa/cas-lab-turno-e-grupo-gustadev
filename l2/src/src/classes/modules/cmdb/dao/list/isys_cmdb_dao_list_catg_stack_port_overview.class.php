...
class isys_cmdb_dao_list_catg_stack_port_overview extends isys_component_dao_category_table_list implements isys_cmdb_dao_list_interface
{
    ...
    public function modify_row(&$p_row)
    {
        ...
        $p_row['isys_catg_port_list__title'] = $quickinfo->get_quick_info(
            $p_row['isys_obj__id'],
            $p_row['isys_catg_port_list__title'],
            C__LINK__CATG,
            false,
            [C__CMDB__GET__CATLEVEL => $p_row['isys_catg_port_list__id'], C__CMDB__GET__CATG => defined_or_default('C__CATG__NETWORK_PORT')]
        );
        $p_row['isys_obj__title'] = $quickinfo->getQuickInfoReplacement(
            $p_row['isys_obj__id'],
            isys_application::instance()->container->get('language')->get($p_row['isys_obj_type__title']) . ' >> ' . $p_row['isys_obj__title']
        );
    }
