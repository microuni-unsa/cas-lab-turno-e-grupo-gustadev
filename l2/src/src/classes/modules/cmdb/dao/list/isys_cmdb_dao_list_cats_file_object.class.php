...
class isys_cmdb_dao_list_cats_file_object extends isys_component_dao_category_table_list
{
    ...
    public function get_result($table = null, $objectId = null, $recordStatus = null)
    {
        ...
        $sql = "SELECT
                connectionList.connection as connectionId,
            ...
            FROM (
                SELECT -- Select for file
                    fl.isys_catg_file_list__isys_connection__id AS connection,
                    fl.isys_catg_file_list__isys_obj__id target,
                    fl.isys_catg_file_list__id catId
                FROM isys_catg_file_list fl
                WHERE fl.isys_catg_file_list__status = {$recordStatus}

                UNION

                SELECT -- Select for manuals
                    ml.isys_catg_manual_list__isys_connection__id,
                    ml.isys_catg_manual_list__isys_obj__id,
                    ml.isys_catg_manual_list__id
                FROM isys_catg_manual_list ml
                WHERE ml.isys_catg_manual_list__status = {$recordStatus}

                UNION

                SELECT -- Select for emergency plan
                    epl.isys_catg_emergency_plan_list__isys_connection__id,
                ...
                WHERE epl.isys_catg_emergency_plan_list__status = {$recordStatus}
            ) connectionList
            ...
            WHERE fileObject.isys_obj__id = {$objectId}";

        // @see ID-11363 Implement the same check as in ID-8287.
        if (!isys_tenantsettings::get('cmdb.only-show-ranked-entries-as-such', 0)) {
            $sql .= " AND targetObject.isys_obj__status = {$recordStatus}";
        }
    ...
    public function modify_row(&$row)
    {
        ...
        $row['objectTitle'] = isys_ajax_handler_quick_info::instance()->getQuickInfoReplacement(
            $row['objectId'],
            $row['objectTitle']
        );
