...
class isys_cmdb_dao_list_catg_ldevclient extends isys_component_dao_category_table_list
{
    ...
    public function modify_row(&$p_arrRow)
    {
        ...
        if ($p_arrRow['isys_catg_sanpool_list__capacity'] > 0) {
            if ($p_arrRow['isys_catg_sanpool_list__isys_memory_unit__id'] > 0) {
                ...
                $p_arrRow['storage_capacity'] = isys_convert::formatNumber($p_arrRow['storage_capacity']) . ' ' . $language->get($l_unit_name['isys_memory_unit__title']);
            } else {
                $p_arrRow['storage_capacity'] = number_format($p_arrRow['isys_catg_sanpool_list__capacity'], 2, ',', '');
            }
        }

        if ($p_arrRow['isys_catg_ldevclient_list__isys_catg_sanpool_list__id'] > 0) {
            $l_ldev_row = isys_cmdb_dao_category_g_sanpool::instance($this->m_db)
                ->get_ldevserver_by_obj_id_or_ldev_id(null, $p_arrRow['isys_catg_ldevclient_list__isys_catg_sanpool_list__id'])
                ->get_row();

            if (is_array($l_ldev_row)) {
                $objectTypeTitle = $language->get($l_ldev_row['isys_obj_type__title']);
                $objectTitle = $l_ldev_row['isys_obj__title'];
                $objectId = $l_ldev_row['isys_obj__id'];

                $p_arrRow['ldev_server'] = "{$objectTypeTitle} &raquo; {$objectTitle} {{$objectId}}";
            }
        }
    }
    ...
    public function get_fields()
    {
        ...
        return [
            ...
            'storage_capacity'                       => $language->get_in_text('LC__CATG__STORAGE_CAPACITY (LC__CMDB__CATG__LDEV_SERVER)'),
            'ldev_server'                            => 'LC__CMDB__CATG__LDEV_SERVER',
            'isys_catg_sanpool_list__lun'            => 'LC__CATD__SANPOOL_LUN',
            'isys_catg_ldevclient_list__description' => 'LC__CMDB__CAT__COMMENTARY',
        ];
    }
}
