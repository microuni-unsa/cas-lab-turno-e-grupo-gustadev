...
class isys_cmdb_dao_list_cats_person_contact_assign extends isys_component_dao_category_table_list
{
    ...
    public function get_result($p_str = null, $p_objID = null, $p_cRecStatus = null)
    {
        ...
        $sql = "SELECT
                ...
                o1.*, isys_contact_tag.*, isys_obj_type__title,
                (CASE WHEN isys_catg_contact_list__primary_contact = 1 THEN 'LC__UNIVERSAL__YES'
                    WHEN isys_catg_contact_list__primary_contact = 0 THEN 'LC__UNIVERSAL__NO'
            ...
            LEFT JOIN isys_obj_type ON isys_obj_type__id = o1.isys_obj__isys_obj_type__id
        ...
        if ($l_cRecStatus > 0) {
            ...
            if (isys_tenantsettings::get('cmdb.only-show-ranked-entries-as-such', 0)) {
                $sql .= "AND isys_catg_contact_list__status = {$recordStatus}";
            } else {
                $sql .= "AND (
                    (o1.isys_obj__status >= isys_catg_contact_list__status AND
                        o1.isys_obj__status >= o2.isys_obj__status AND
                        o1.isys_obj__status = {$recordStatus}) OR

                    (o2.isys_obj__status >= isys_catg_contact_list__status AND
                        o2.isys_obj__status >= o1.isys_obj__status AND
                        o2.isys_obj__status = {$recordStatus}) OR

                    (isys_catg_contact_list__status >= o1.isys_obj__status AND
                        isys_catg_contact_list__status >= o2.isys_obj__status AND
                        isys_catg_contact_list__status = {$recordStatus})
                )";
            }
        }
