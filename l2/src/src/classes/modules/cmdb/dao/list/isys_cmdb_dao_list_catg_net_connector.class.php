...
class isys_cmdb_dao_list_catg_net_connector extends isys_component_dao_category_table_list
{
    ...
    public function get_fields()
    {
        ...
        return [
            ...
            'port_range'            => $language->get('Port') . ' / ' . $language->get('LC__UNIVERSAL__PORT_RANGE'),
    ...
    public function modify_row(&$p_row)
    {
        ...
        if ($p_row['isys_catg_net_connector_list__isys_catg_net_listener_list__id'] > 0) {
            ...
            if (isset($l_listener_data['isys_obj__id'])) {
                ...
                $objectType = isys_application::instance()->container->get('language')->get($l_listener_data['isys_obj_type__title']);

                $p_row['connected_to_listener'] = $quickinfo->get_quick_info(
                    $l_listener_data['isys_obj__id'],
                    $l_listener_data['isys_obj__title'] . ' (' . $objectType . ')',
                    $l_listener_url
                );
            }

            $l_protocol = $l_listener_dao
                ->get_dialog('isys_net_protocol', $p_row['isys_catg_net_listener_list__isys_net_protocol__id'])
                ->__to_array();
        ...
        if ($p_row['isys_catg_net_listener_list__opened_by']) {
            $p_row['opened_by'] = $quickinfo->getQuickInfoReplacement(
