...
class isys_cmdb_dao_list_objects extends isys_cmdb_dao_list
{
     ...
     * @var bool
     */
    protected $useColumnFormatter = true;
    ...
    public function format_result(isys_component_dao_result $p_result, $p_list_config = []): array
    {
        ...
        while ($l_row = $p_result->get_row()) {
            foreach ($p_list_config as $l_config) {
                [$l_property_type, $l_propkey, $l_rowfield, $l_title, $l_get_properties_method, $l_dynamic_callback, $l_cat_name, $l_custom_cat_const] = $l_config;
                // Category class
                ...
                if ($l_property_type == C__PROPERTY_TYPE__STATIC) {
                    ...
                    if (is_callable($l_property_callback)) {
                        ...
                        if (defined($l_custom_cat_const) && $l_instance instanceof isys_cmdb_dao_category_g_custom_fields) {
                        ...
                        if ($l_property = $l_instance->get_property_by_key($l_propkey)) {
                            if ($l_property[C__PROPERTY__INFO][C__PROPERTY__INFO__TYPE] == C__PROPERTY__INFO__TYPE__DATE ||
                                ...
                                $l_row[$l_rowfield] = isys_application::instance()->container->get('locales')
     ...
     * @return  string
     */
    public function get_additional_selects(): string
    {
        if (empty($this->additional_selects)) {
            return '';
        }
     ...
     * @return $this
     */
    public function set_additional_selects(array $select)
    {
    ...
    /**
     * @param string|null $objectTypeConst
     *
     * @return Config|null
    ...
    {
        $type = $constant ?? $this->m_object_type['isys_obj_type__const'] ?? null;

        if (isys_tenantsettings::has("cmdb.default-object-table.config.{$type}")) {
            $l_config = isys_tenantsettings::get("cmdb.default-object-table.config.{$type}");

            // @see ID-10384 It's possible that the tenant-settings will decode the JSON prematurely.
            if (is_array($l_config)) {
                $l_config = isys_format_json::encode($l_config);
            }

            if (is_string($l_config) && isys_format_json::is_json_array($l_config)) {
                return Config::fromJson($l_config);
            }
        }
        return null;
    }
     ...
     * @throws isys_exception_general
     */
    public function getSystemTableConfig(?string $constant = null): ?Config
    {
        $type = $constant ?? $this->m_object_type['isys_obj_type__const'] ?? null;

        if ($type && isys_tenantsettings::has("cmdb.base-object-table.config.{$type}")) {
            $l_config = isys_tenantsettings::get("cmdb.base-object-table.config.{$type}");
    ...
    {
        return $this->getTenantTableConfig() ?? $this->getSystemTableConfig() ??
            (new Config())->setRowClickable(true)
    ...
    public function get_table_query($p_offset = 0, $p_length = 0)
    {
        ...
        if ($this->tableConfiguration !== null) {
             ...
             */
            if (is_array($l_list_config) && isys_format_json::is_json_array($l_list_config['isys_obj_type_list__table_config'])) {
                $l_table_conf = Config::fromJson($l_list_config['isys_obj_type_list__table_config']);
        ...
        return $this->addConditionsToQuery(
            $l_return,
    ...
    {
        // @see ID-11788 Use proper logic, when working with temp tables.
        $tempTableName = ($this instanceof isys_component_list)
            ? $this->getTempTableName()
            : null;

        // @see ID-11512 Re-introduce 'expensive' logic in order to consider filtered lists.
        if ($tempTableName || !empty($this->get_additional_conditions()) || !empty($this->get_additional_having_conditions())) {
            // Revert the original code from before ID-11230.
            $this->deactivate_order_by();
            $query = $this->get_table_query(false);
            ...
            $l_rowcount_sql = 'SELECT COUNT(*) AS cnt FROM (' . rtrim($query, ';') . ') AS tmp;';

            try {
                $l_rowcount = $this->retrieve($l_rowcount_sql)->get_row_value('cnt');
            } catch (Exception $e) {
                // If our first try fails because we broke the SQL, we use this here...
                $l_rowcount = $this->retrieve($query)->count();
            }

            return (int)$l_rowcount;
        }

        // @see ID-11230 Re-use the 'get_rec_counts' logic because it is very performant and ALREADY fetches the amount of objects.
        $statusCounts = $this->get_rec_counts();

        $recordStatus = ($this->get_rec_status() ? $this->get_rec_status() : C__RECORD_STATUS__NORMAL);

        if (is_array($l_cmdb_status = $this->get_cmdb_status())) {
            if (defined('C__CMDB_STATUS__IDOIT_STATUS_TEMPLATE') && is_numeric(array_search(constant('C__CMDB_STATUS__IDOIT_STATUS_TEMPLATE'), $l_cmdb_status))) {
                $recordStatus = C__RECORD_STATUS__TEMPLATE;
            }
        }

        return (int)($statusCounts[$recordStatus] ?? 0);
    }
    ...
    public function get_rec_counts()
    {
        ...
        // Build SQL-Statement
        $l_sql = 'SELECT
            SUM(isys_obj__status = ' . C__RECORD_STATUS__NORMAL . ') AS COUNT_NORMAL,
        ...
        // Build array
        $l_array = [
            C__RECORD_STATUS__NORMAL   => (int)$l_row['COUNT_NORMAL'],
            C__RECORD_STATUS__ARCHIVED => (int)$l_row['COUNT_ARCHIVED'],
            C__RECORD_STATUS__DELETED  => (int)$l_row['COUNT_DELETED'],
        ];

        // Add C__STATUS__TEMPLATE
        if (isset($l_row['COUNT_TEMPLATE'])) {
            $l_array[C__RECORD_STATUS__TEMPLATE] = (int)$l_row['COUNT_TEMPLATE'];
        }

        return $this->recordStatusCount = $l_array;
    }
    ...
    /**
     * @param bool $useColumnFormatter
     *
     * @return $this
     */
    public function setUseColumnFormatter(bool $useColumnFormatter): isys_cmdb_dao_list_objects
    {
        $this->useColumnFormatter = $useColumnFormatter;
        return $this;
    }
     ...
     * @return bool
     */
    public function useColumnFormatter(): bool
    {
        return $this->useColumnFormatter;
    }
}
