...
class isys_cmdb_dao_list_cats_application_assigned_obj extends isys_component_dao_category_table_list
{
    ...
    {
        $l_quick_info = isys_ajax_handler_quick_info::instance();
        $l_relation_type = $this->m_dao_relation->get_relation_type($p_row['isys_catg_relation_list__isys_relation_type__id'])
            ->get_row();

        $p_row['main_obj_title'] = $l_quick_info->getQuickInfoReplacement(
            $p_row['main_obj_id'],
            $p_row['main_obj_title']
        );
        $p_row['rel_obj_title'] = $l_quick_info->getQuickInfoReplacement(
            $p_row['rel_obj_id'],
            $p_row['slave_title'] . ' ' . isys_application::instance()->container->get('language')->get($l_relation_type['isys_relation_type__slave']) . ' ' . $p_row['master_title']
        );
        ...
        if ($p_row['isys_catg_application_list__isys_cats_lic_list__id'] > 0) {
            $l_row = isys_cmdb_dao_category_s_lic::instance($this->m_db)
                ->get_data($p_row['isys_catg_application_list__isys_cats_lic_list__id'])
                ->get_row();

            $p_row['assigned_license'] = $l_quick_info->getQuickInfoReplacement(
                $l_row['isys_obj__id'],
                $l_row['isys_obj__title']
            );
        }

        $p_row['assigned_version'] = (!empty($p_row['isys_catg_version_list__title']) ? $p_row['isys_catg_version_list__title'] : '') .
    ...
    public function get_fields()
    {
        return [
            ...
            'version_sort'                        => false,
            'target_schema_title'                 => 'LC__CMDB__CATS__DATABASE_GATEWAY__TARGET_SCHEMA',
            'app_db_rel_obj_title'                => 'LC__CATG__APPLICATION_ASSIGNED_DATABASES',
            'isys_catg_application_list__install_date' => 'LC__CMDB__CATG__APPLICATION__INSTALLDATE',
