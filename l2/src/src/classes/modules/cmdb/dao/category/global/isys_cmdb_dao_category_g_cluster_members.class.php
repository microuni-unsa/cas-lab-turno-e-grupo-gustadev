 ...
 */
class isys_cmdb_dao_category_g_cluster_members extends isys_cmdb_dao_category_global implements ObjectBrowserReceiver, ObjectBrowserAssignedEntries
{
    /**
        ...
        return [
            'member' => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_g_cluster_memberships::connected_object'
                ],
                C__PROPERTY__DATA     => [
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            $value = $p_category_data['properties']['member'][C__DATA__VALUE];
            if (!is_array($value)) {
                $value = [$value];
            }
            ...
            if ($p_status === isys_import_handler_cmdb::C__CREATE && $p_object_id > 0) {
                ...
                foreach ($unassignedObjects as $member) {
                    $lastId = $this->create(
                        $p_object_id,
        ...
        if ($this->update($l_strSql)) {
            if ($this->apply_update()) {
                ...
                $l_relation_dao->handle_relation(
                    $p_cat_level,
                    "isys_catg_cluster_members_list",
                    defined_or_default('C__RELATION_TYPE__CLUSTER_MEMBERSHIPS'),
