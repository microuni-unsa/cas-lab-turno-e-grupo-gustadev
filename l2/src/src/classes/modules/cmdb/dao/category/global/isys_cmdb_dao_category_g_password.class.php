...
use idoit\Component\Property\Type\PasswordProperty;
...
class isys_cmdb_dao_category_g_password extends isys_cmdb_dao_category_global
{
    ...
    {
        // @see ID-9502 Go sure that the password is actually a string!
        if (isset($p_data['password']) && is_string($p_data['password']) && strpos(base64_decode($p_data['password']), isys_helper_crypt::DELIMITER) === false) {
    ...
    {
        return [
            '_password' => new DynamicProperty(
                'LC__LOGIN__PASSWORD',
                'isys_catg_password_list__password',
