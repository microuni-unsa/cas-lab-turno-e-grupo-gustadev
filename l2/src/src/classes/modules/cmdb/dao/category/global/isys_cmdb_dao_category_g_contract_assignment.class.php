...
class isys_cmdb_dao_category_g_contract_assignment extends isys_cmdb_dao_category_global
{
    ...
    public function create(
        ...
        $l_sql = 'INSERT INTO isys_catg_contract_assignment_list SET
            isys_catg_contract_assignment_list__isys_obj__id = ' . $this->convert_sql_id($p_obj_id) . ',
            ...
            isys_catg_contract_assignment_list__isys_connection__id = ' . $this->convert_sql_id($l_dao_connection->add_connection($p_connected_contract)) . ',
            ...
            isys_catg_contract_assignment_list__status = ' . $this->convert_sql_int($p_rec_status) . ';';
    ...
    public function save(
        ...
        $l_sql = 'SELECT isys_catg_contract_assignment_list__isys_obj__id,
            ...
            isys_catg_contract_assignment_list__isys_catg_relation_list__id
        ...
        $l_sql = 'UPDATE isys_catg_contract_assignment_list SET
            isys_catg_contract_assignment_list__contract_start = ' . $this->convert_sql_datetime($p_contract_start) . ',
            isys_catg_contract_assignment_list__contract_end = ' . $this->convert_sql_datetime($p_contract_end) . ',
        ...
        return [
            'connected_contract' => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_s_contract_allocation::assigned_object'
                ],
                C__PROPERTY__DATA     => [
            ...
            'contract_start'     => array_replace_recursive(isys_cmdb_dao_category_pattern::date(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT IFNULL(' . self::build_query_date_format('isys_catg_contract_assignment_list__contract_start') . ', IFNULL(' . self::build_query_date_format('isys_cats_contract_list__start_date') . ', \'-\'))' .
                        ' FROM isys_catg_contract_assignment_list
                        INNER JOIN isys_connection ON isys_connection__id = isys_catg_contract_assignment_list__isys_connection__id
                        INNER JOIN isys_obj ON isys_obj__id = isys_connection__isys_obj__id
                        LEFT JOIN isys_cats_contract_list on isys_cats_contract_list__isys_obj__id = isys_obj__id',
                ...
                C__PROPERTY__DEPENDENCY => [
                    C__PROPERTY__DEPENDENCY__PROPKEY => 'connected_contract'
                ]
            ]),
            'contract_end'       => array_replace_recursive(isys_cmdb_dao_category_pattern::date(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT IFNULL(' . self::build_query_date_format('isys_catg_contract_assignment_list__contract_end') . ', IFNULL(' . self::build_query_date_format('isys_cats_contract_list__end_date') . ', \'-\'))' .
                        ' FROM isys_catg_contract_assignment_list
                        INNER JOIN isys_connection ON isys_connection__id = isys_catg_contract_assignment_list__isys_connection__id
                        INNER JOIN isys_obj ON isys_obj__id = isys_connection__isys_obj__id
                        LEFT JOIN isys_cats_contract_list on isys_cats_contract_list__isys_obj__id = isys_obj__id',
    ...
    {
        return [
            // @see ID-12079 Create dynamic property.
            '_contract_start' => new DynamicProperty(
                'LC__CMDB__CATG__CONTRACT_ASSIGNMENT__CONTRACT_START',
                'isys_catg_contract_assignment_list__id',
                'isys_catg_contract_assignment_list',
                [
                    $this,
                    'dynamic_property_callback_contract_start'
                ]
            ),
            // @see ID-12079 Create dynamic property.
            '_contract_end' => new DynamicProperty(
                'LC__CMDB__CATG__CONTRACT_ASSIGNMENT__CONTRACT_END',
                'isys_catg_contract_assignment_list__id',
                'isys_catg_contract_assignment_list',
                [
                    $this,
                    'dynamic_property_callback_contract_end'
                ]
            )
        ];
    }
     ...
     */
    public function dynamic_property_callback_contract_start(array $row): string
    {
        $entryId = (int)$row['isys_catg_contract_assignment_list__id'];
        ...
        $sql = "SELECT
            isys_catg_contract_assignment_list__contract_start AS localStart,
            isys_cats_contract_list__start_date AS assignedContractStart
            FROM isys_catg_contract_assignment_list
			INNER JOIN isys_obj ON isys_catg_contract_assignment_list__isys_obj__id = isys_obj__id
			LEFT JOIN isys_connection ON isys_connection__id = isys_catg_contract_assignment_list__isys_connection__id
			LEFT JOIN isys_cats_contract_list ON isys_cats_contract_list__isys_obj__id = isys_connection__isys_obj__id
			WHERE isys_catg_contract_assignment_list__id = {$entryId}
			LIMIT 1;";

        $entryData = $this->retrieve($sql)->get_row();

        return $entryData['localStart'] ?? $entryData['assignedContractStart'] ?? '';
    }
     ...
     * @throws isys_exception_database
     */
    public function dynamic_property_callback_contract_end(array $row): string
    {
        $entryId = (int)$row['isys_catg_contract_assignment_list__id'];
        ...
        $sql = "SELECT
            isys_catg_contract_assignment_list__contract_end AS localEnd,
            isys_cats_contract_list__end_date AS assignedContractEnd
            FROM isys_catg_contract_assignment_list
			INNER JOIN isys_obj ON isys_catg_contract_assignment_list__isys_obj__id = isys_obj__id
			LEFT JOIN isys_connection ON isys_connection__id = isys_catg_contract_assignment_list__isys_connection__id
			LEFT JOIN isys_cats_contract_list ON isys_cats_contract_list__isys_obj__id = isys_connection__isys_obj__id
        ...
        return $entryData['localEnd'] ?? $entryData['assignedContractEnd'] ?? '';
    }
