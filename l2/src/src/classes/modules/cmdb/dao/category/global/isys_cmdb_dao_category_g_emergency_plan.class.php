 ...
 */
class isys_cmdb_dao_category_g_emergency_plan extends isys_cmdb_dao_category_global implements ObjectBrowserAssignedEntries
{
    /**
        ...
        return [
            ...
            'emergency_plan'   => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_s_emergency_plan_assigned_obj::object'
                ],
                C__PROPERTY__DATA     => [
    ...
    {
        $l_strSql = "UPDATE isys_catg_emergency_plan_list SET 
             isys_catg_emergency_plan_list__isys_connection__id = " . $this->convert_sql_id($this->handle_connection($p_cat_level, $p_connectedObjID)) . ", 
             isys_catg_emergency_plan_list__title = " . $this->convert_sql_text($p_title) . ", 
             isys_catg_emergency_plan_list__description	 = " . $this->convert_sql_text($p_description) . ", 
             isys_catg_emergency_plan_list__status = " . $this->convert_sql_id($p_newRecStatus) . " 
             WHERE isys_catg_emergency_plan_list__id = " . $this->convert_sql_id($p_cat_level);

        if ($this->update($l_strSql) && $this->apply_update()) {
            // Create connection if necessary
            if ($p_connectedObjID > 0) {
                $data = $this->get_data($p_cat_level)->get_row();
    ...
    public function create($p_objID, $p_newRecStatus, $p_title, $p_connectedObjID, $p_description)
    {
        ...
        $l_strSql = "INSERT INTO isys_catg_emergency_plan_list SET 
              isys_catg_emergency_plan_list__title = " . $this->convert_sql_text($p_title) . ", 
              isys_catg_emergency_plan_list__isys_obj__id  = " . $this->convert_sql_id($p_objID) . ", 
              isys_catg_emergency_plan_list__description	 = " . $this->convert_sql_text($p_description) . ", 
              isys_catg_emergency_plan_list__status = " . $this->convert_sql_id($p_newRecStatus) . ", 
