...
class isys_cmdb_dao_category_g_overview extends isys_cmdb_dao_category_global
{
     ...
     * @var int
     */
    private int $savedCategories;

    /**
     * @return int
     * @see ID-9307 Get the amount of all saved categories.
     */
    public function getSavedCategories(): int
    {
    ...
    public function save_element($p_cat_level, &$p_intOldRecStatus = C__RECORD_STATUS__BIRTH)
    {
        ...
        // @see ID-9307 Count all saved categories.
        $savedCategories = 0;
            ...
            if (is_array($p_categories)) {
                foreach ($p_categories as $l_cat) {
                    ...
                    if ($l_dist && $l_dist->count() > 0) {
                        ...
                        if ($p_cat_type === C__CMDB__CATEGORY__TYPE_CUSTOM && $l_current_cat instanceof isys_cmdb_dao_category_g_custom_fields) {
                        ...
                        $rightEvaluation = isys_auth_cmdb::instance()
                            ->has_rights_in_obj_and_category(isys_auth::EDIT, $p_object_id, $l_catg_const);

                        // Check for create right if no entry exists
                        if ($l_catdata === null) {
                            $rightEvaluation = $rightEvaluation || isys_auth_cmdb::instance()
                                    ->is_allowed_to(isys_auth_cmdb::CREATE, 'CATEGORY/' . $l_catg_const);
                        }
                        ...
                        $l_changes = $l_action_update->format_changes($_POST, $l_current_cat);
                        ...
                        // ID-8078 retrieve the correct object id field
                        $objectIdField = $l_current_cat->get_object_id_field();
                        // Ask CMDB for category entry count.
                        $l_catcount = $this->cat_count_by_status($p_cat_type, $l_gui_cat_id, $l_catdata[$objectIdField]);

                        // Prepare some variables to check, if we can skip this category.
                        $daoClassName = get_class($l_current_cat);

                        // If we find no changes for the current category, we skip further process
                        // @see  ID-6693  We skip the 'isys_cmdb_dao_category_s_layer2_net' DAO because i-doit might not recognize changes of "IP helpers".
                        if (!$l_changes->hasAnyChanges() && !in_array($daoClassName, ['isys_cmdb_dao_category_g_global', 'isys_cmdb_dao_category_s_layer2_net'])) {
                            continue;
                        }

                        // @see ID-9468 Move the sanitation down here in case the category gets skipped because of missing changes.
                        ...
                        // @see ID-8161 In order to fix this issue, the 'beforeCategoryEntrySave' event needs to be emitted.
                        $signals->emit('mod.cmdb.beforeCategoryEntrySave', $l_current_cat, $p_cat_level, $p_object_id, $_POST, $l_changes->getCurrentReformatedChanges());

                        $l_current_cat->logbook_update(
                        ...
                        if ($l_current_cat->get_object_browser_category() === true && ($l_objBrowser_key = $l_current_cat->get_object_browser_property()) !== '' &&
                            ...
                            $signals->emit("mod.cmdb.afterCategoryEntrySave", $l_current_cat, $p_cat_level, true, $p_object_id, $_POST, $l_changes->getCurrentReformatedChanges());
                        } else {
                            // Save the category.
                                ...
                                if ($l_catcount === 0) {
                                    ...
                                    if (method_exists($l_current_cat, "create_connector")) {
                                        ...
                                        isys_component_signalcollection::get_instance()
                                            ->emit('mod.cmdb.categoryEntryJustCreated', $l_current_cat, $l_category_id, true, $p_object_id, $_POST, $l_changes->getCurrentReformatedChanges());
                                    }
                                }

                                $l_saved = $l_current_cat->save_element($p_cat_level, $l_category_id, $l_category_id ? false : true);
                            } else {
                                $l_saved = $l_current_cat->save_user_data(false);
                            }

                            if ($l_current_cat->is_multivalued() && $l_catcount === 0 && $this->cat_count_by_status($p_cat_type, $l_gui_cat_id, $p_object_id)) {
                                // Emit category signal (afterCreateCategoryEntry).
                                isys_component_signalcollection::get_instance()
                                    ->emit("mod.cmdb.afterCreateCategoryEntry", $l_gui_cat_id, $l_category_id ?? $l_saved, (($l_category_id ?? $l_saved) > 0), $p_object_id, $l_current_cat, $l_changes->getCurrentReformatedChanges());
                            } else {
                                // Emit category signal (afterCategoryEntrySave).
                                $signals->emit("mod.cmdb.afterCategoryEntrySave", $l_current_cat, $p_cat_level, $l_saved, $p_object_id, $_POST, $l_changes->getCurrentReformatedChanges());
                            }
                        }

                        // @see ID-9307 Increase the 'saved categories' counter after a successful save.
     ...
     */
    public function getOrderedOverviewCategories(string $objectTypeConstant): isys_component_dao_result
    {
        $sql = 'SELECT gui.isysgui_catg__const AS constant, ov.isys_obj_type_2_isysgui_catg_overview__sort AS sort
            FROM isys_obj_type_2_isysgui_catg_overview AS ov
            INNER JOIN isys_obj_type AS ot ON ot.isys_obj_type__id = ov.isys_obj_type__id
            INNER JOIN isysgui_catg AS gui ON gui.isysgui_catg__id = ov.isysgui_catg__id
            WHERE ot.isys_obj_type__const = ' . $this->convert_sql_text($objectTypeConstant) . '

            UNION

            SELECT gui.isysgui_catg_custom__const AS constant, ov.isys_obj_type_2_isysgui_catg_custom_overview__sort AS sort
            FROM isys_obj_type_2_isysgui_catg_custom_overview AS ov
            INNER JOIN isys_obj_type AS ot ON ot.isys_obj_type__id = ov.isys_obj_type__id
            INNER JOIN isysgui_catg_custom AS gui ON gui.isysgui_catg_custom__id = ov.isysgui_catg_custom__id
            WHERE ot.isys_obj_type__const = ' . $this->convert_sql_text($objectTypeConstant) . ';';
