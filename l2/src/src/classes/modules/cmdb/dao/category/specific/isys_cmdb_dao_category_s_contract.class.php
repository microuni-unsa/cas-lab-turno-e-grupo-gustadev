...
class isys_cmdb_dao_category_s_contract extends isys_cmdb_dao_category_specific
{
    ...
    public static function dynamic_property_callback_date($p_value)
    {
        ...
        return '<span data-date="' . $p_value . '" class="hide"></span>' . isys_application::instance()->container->get('locales')->fmt_date($p_value);
    }
    ...
    public function create_data($p_data)
    {
        ...
        if (!isset($p_data['run_time']) && isset($p_data['start_date']) && isset($p_data['end_date'])) {
            ...
            if (!isset($p_data['run_time_unit'])) {
                $p_data['run_time_unit'] = defined_or_default('C__GUARANTEE_PERIOD_UNIT_DAYS', 2);
            }
            $p_data['run_time'] = isys_convert::seconds_to_period(strtotime($p_data['end_date']) - strtotime($p_data['start_date']), $p_data['run_time_unit']);
        }

        if (empty($p_data['end_date']) && isset($p_data['start_date']) && isset($p_data['run_time'])) {
            if (!isset($p_data['run_time_unit'])) {
                $p_data['run_time_unit'] = defined_or_default('C__GUARANTEE_PERIOD_UNIT_DAYS');
            }
            $p_data['end_date'] = $this->calculate_maintenanceperiod($p_data['start_date'], $p_data['run_time'], $p_data['run_time_unit']);
        }
    ...
    {
        $objId = ($p_row['isys_cats_contract_list__isys_obj__id'] ?: ($p_row['isys_obj__id'] ?: null));
        if (!is_numeric($objId)) {
            $objId = $this->getContractIdByObjectIDAndContractTitle($p_row['isys_cats_contract_list__isys_obj__id'], $p_row['isys_obj__id']);
        }
        if ($objId === null) {
            return isys_tenantsettings::get('gui.empty_value', '-');
        }

        return $this->dynamicMonetaryFormatter($p_row, 'isys_cats_contract_list__costs', $objId);
    }

    public function getContractIdByObjectIDAndContractTitle(string $title, int $objId)
    {
        $sql = 'SELECT j3.isys_obj__id AS id FROM `isys_obj` AS main
            JOIN isys_catg_contract_assignment_list AS j1 ON j1.isys_catg_contract_assignment_list__isys_obj__id = main.isys_obj__id
            JOIN isys_connection AS j2 ON j2.isys_connection__id = j1.isys_catg_contract_assignment_list__isys_connection__id
            JOIN isys_obj AS j3 ON j3.isys_obj__id = j2.isys_connection__isys_obj__id
            WHERE main.isys_obj__id = ' . $this->convert_sql_id($objId) . ' AND j3.isys_obj__title = ' . $this->convert_sql_text($title) . ' LIMIT 1;';
    ...
    public function calculate_noticeperiod($p_contract_end_date = null, $p_noticeperiod_value = null, $p_noticeperiod_unit = null)
    {
        ...
        if (!empty($p_contract_end_date) && !empty($p_noticeperiod_unit) && is_numeric($p_noticeperiod_value)) {
            if ($p_noticeperiod_unit == defined_or_default('C__GUARANTEE_PERIOD_UNIT_MONTH', 1)) {
                ...
                return $locale->fmt_date($l_new_date);
                ...
                return $locale->fmt_date(strtotime($p_contract_end_date) - isys_convert::period_to_seconds($p_noticeperiod_value, $p_noticeperiod_unit));
            }
        } else {
            return null;
        }
    ...
    public function calculate_next_contract_end_date($p_expiration_date = null, $p_noticeperiod_value = null, $p_noticeperiod_unit = null)
    {
        ...
        if (!empty($p_expiration_date) && !empty($p_noticeperiod_unit) && !empty($p_noticeperiod_value)) {
            ...
            return $locale->fmt_date(strtotime($p_expiration_date) + isys_convert::period_to_seconds($p_noticeperiod_value, $p_noticeperiod_unit));
        }
    ...
    public function calculate_maintenanceperiod($p_startdate = null, $p_maintenanceperiod_value = null, $p_maintenanceperiod_unit = null)
    {
        ...
        if (!empty($p_startdate) && !empty($p_maintenanceperiod_value) && !empty($p_maintenanceperiod_unit)) {
            ...
            return $locale->fmt_date(strtotime($p_startdate) + isys_convert::period_to_seconds($p_maintenanceperiod_value, $p_maintenanceperiod_unit));
        }
    ...
    protected function dynamic_properties()
    {
        ...
        return [
            '_notice_period' => new DynamicProperty(
            ...
            '_maintenance_period' => new DynamicProperty(
            ...
            '_start_date' => new DynamicProperty(
                'LC__CMDB__CATS__CONTRACT__START_DATE',
                'isys_cats_contract_list__start_date',
                'isys_cats_contract_list',
                [
                    $this,
            ...
            '_end_date' => new DynamicProperty(
                'LC__CMDB__CATS__CONTRACT__END_DATE',
            ...
            '_notice_date' => new DynamicProperty(
                'LC__CMDB__CATS__CONTRACT__NOTICE_DATE',
                'isys_cats_contract_list__id',
                'isys_cats_contract_list',
                [
                    $this,
            ...
            '_costs' => new DynamicProperty(
                'LC__CMDB__CATS__CONTRACT__COSTS',
    ...
    protected function properties()
    {
        return [
            ...
            'run_time' => (new IntWithUnitProperty(
                'C__CATS__CONTRACT__RUNTIME',
                'LC__CMDB__CATG__LEASING__RUNTIME',
                'isys_cats_contract_list__runtime',
                'isys_cats_contract_list',
            ...
            'run_time_unit'           => (new DialogProperty(
                'C__CATS__CONTRACT__RUNTIME_UNIT',
                'LC__CMDB__CATS__CONTRACT__RUNTIME_UNIT',
                'isys_cats_contract_list__runtime_unit',
                'isys_cats_contract_list',
                'isys_guarantee_period_unit'
            ))
            ...
            'run_time_period' => (new IntWithUnitProperty(
                'C__CATS__CONTRACT__RUNTIME_PERIOD',
                'LC__CATS__CONTRACT__RUNTIME_PERIOD',
                'isys_cats_contract_list__runtime_period',
                'isys_cats_contract_list',
                'isys_guarantee_period_unit',
                'run_time_period_unit',
                [
                    'isys_export_helper',
                    'timeperiod',
                    [null]
                ],
                'isys_cats_contract_list__runtime_period_unit'
            ))
                ->mergePropertyProvides([
                    C__PROPERTY__PROVIDES__SEARCH    => false,
                ])
            ...
            'run_time_period_unit'           => (new DialogProperty(
                'C__CATS__CONTRACT__RUNTIME_PERIOD_UNIT',
                'LC__CATS__CONTRACT__RUNTIME_PERIOD_UNIT',
                'isys_cats_contract_list__runtime_period_unit',
                'isys_cats_contract_list',
                'isys_guarantee_period_unit'
            ))
            ...
            'maintenance_period'      => (new IntWithUnitProperty(
                'C__CATS__CONTRACT__MAINTENANCE_PERIOD',
                'LC__CMDB__CATS__CONTRACT__MAINTENANCE_PERIOD',
                'isys_cats_contract_list__maintenance_period',
                'isys_cats_contract_list',
                'isys_guarantee_period_unit',
            ...
            'maintenance_end' => new VirtualProperty(
                'C__CATS__CONTRACT__MAINTENANCE_END',
                'LC__CMDB__CATS__CONTRACT__MAINTENANCE_END'
            ),
            'notice_end' => new VirtualProperty(
                'C__CATS__CONTRACT__NOTICE_END_DATE',
                'LC__CMDB__CATS__CONTRACT__CONTRACT_NOTICE_END'
            ),
    ...
    public function save_data($p_category_data_id, $p_data)
    {
        ...
        if (in_array(Context::instance()->getGroup(), [Context::CONTEXT_GROUP_IMPORT, Context::CONTEXT_GROUP_MULTIEDIT], true)) {
        ...
        } elseif (Context::instance()->getOrigin() === Context::ORIGIN_GUI) {
        ...
        if (empty($p_data['run_time']) && $p_data['start_date'] !== '' && $p_data['end_date'] !== '') {
            // Calculate run_time with start_date and end_date
            if (!isset($p_data['run_time_unit'])) {
                $p_data['run_time_unit'] = defined_or_default('C__GUARANTEE_PERIOD_UNIT_DAYS');
            }
        ...
        if (empty($p_data['end_date']) && $p_data['start_date'] !== '' && $p_data['run_time'] > 0) {
            if (!isset($p_data['run_time_unit'])) {
                $p_data['run_time_unit'] = defined_or_default('C__GUARANTEE_PERIOD_UNIT_DAYS');
            }
            $p_data['end_date'] = $this->calculate_maintenanceperiod($p_data['start_date'], $p_data['run_time'], $p_data['run_time_unit']);
