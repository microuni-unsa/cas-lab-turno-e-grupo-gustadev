 ...
 */
class isys_cmdb_dao_category_g_virtual_object extends isys_cmdb_dao_category_g_virtual implements ObjectBrowserReceiver, ObjectBrowserAssignedEntries
{
    /**
     ...
     * @var string
     */
    protected $m_object_id_field = 'isys_catg_location_list__parentid';

    /**
     * @var string
     */
    protected $m_connected_object_id_field = 'isys_catg_location_list__isys_obj__id';

    /**
     * Const for logging action type
     */
    const IS_OBJECT_ADDED = 1;

    /**
     * Const for logging action type
     */
    const IS_OBJECT_REMOVED = 2;
        ...
        return [
            'assigned_object' => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_g_location::parent',
                ],
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        ...
                        idoit\Module\Report\SqlQuery\Structure\SelectGroupBy::factory(['isys_catg_location_list__parentid']),
                ...
                C__PROPERTY__PROVIDES => [
                    // C__PROPERTY__PROVIDES__VIRTUAL    => true, // @see ID-11956
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        ...
        if (isys_import_handler_cmdb::C__CREATE) {
            ...
            if (is_array($l_connected_object)) {
                $output = $this->attachObjects($p_object_id, $l_connected_object, true);
                return count(array_filter($output)) === count($output);
            }
     ...
     * @throws isys_exception_database
     */
    public function attachObjects($objectId, array $selectedObjects, $withOutput = false)
    {
        $l_location_dao = new isys_cmdb_dao_category_g_location($this->m_db);

        // Bugfix: Sometimes "get_assigned_objects" returns no array.
        $l_assigned_objects_plain = (array)$this->get_assigned_objects($objectId, true);
        $l_assigned_objects = array_flip($l_assigned_objects_plain);
        ...
        // logging newly assigned servers
        $newChildObjects = array_diff($selectedObjects, $l_assigned_objects_plain);

        foreach ($newChildObjects as $newChildObjectID) {
            // @see ID-10358 Check for recursion, by running the validation of 'Location' category.
            $validation = isys_cmdb_dao_category_g_location::instance($this->m_db)->validate([
                'object' => [
                    C__DATA__VALUE => $newChildObjectID
                ],
                'parent' => [
                    C__DATA__VALUE => $objectId
                ]]);

            if (is_array($validation)) {
                // Remove the affected object from the list of objects.
                $selectedObjects = array_filter($selectedObjects, fn ($object) => $object != $newChildObjectID);
                $validationIssues[] = $this->get_obj_name_by_id_as_string($newChildObjectID);
            }
        }
        ...
        if (count($validationIssues)) {
            ...
            if (count($validationIssues) === 1) {
                $validationMessage = $language->get('LC__CATG__LOCATION__VALIDATION__RECURSION_ONE', implode('', $validationIssues));
            } else {
                $validationMessage = $language->get('LC__CATG__LOCATION__VALIDATION__RECURSION_MANY', '"' . implode('", "', $validationIssues) . '"');
            }

            isys_notify::error($language->get('LC__VALIDATION_ERROR') . ' ' . $validationMessage, ['sticky' => true]);
        }

        $return = [];

        foreach ($selectedObjects as $l_popup_object) {
            ...
            if ($l_popup_object != $objectId && $l_allowed_to_assign) {
                $l_sql = "SELECT isys_catg_location_list__id, isys_catg_location_list__parentid
					FROM isys_catg_location_list
                ...
                if (!$l_row) {
                    $return[] = $l_location_dao->create($l_popup_object, $objectId);
                } else {
                    // We only need to save the data, if the object is beeing assigned newly.
                    if ($l_row['isys_catg_location_list__parentid'] != $objectId) {
                        if ($l_location_dao->save($l_row["isys_catg_location_list__id"], $l_popup_object, $objectId)) {
                            $return[] = $l_row["isys_catg_location_list__id"];
                        }
                    }
                }
                ...
                if (!$l_allowed_to_assign) {
                    ...
                    isys_component_template_infobox::instance()
                        ->set_message('Error: Attaching own object is prohibitted.', 1, defined_or_default('C__LOGBOOK__ALERT_LEVEL__3'));
                }
            }
        }
        ...
        if (count($l_not_allowed_objects) > 0) {
            ...
            isys_component_template_infobox::instance()
                ->set_message($l_message, 1, defined_or_default('C__LOGBOOK__ALERT_LEVEL__3'));
        }
        ...
        if (count($l_assigned_objects) > 0) {
            ...
            foreach ($l_assigned_objects as $l_delObj => $l_val) {
                ...
                // logging removed servers from a rack
                $this->logbookEntryForServer($l_delObj, self::IS_OBJECT_REMOVED);
            }
        }

        return ($withOutput === true ? $return: true);
    }
     ...
     * @throws isys_exception_dao
     * @throws isys_exception_database
     * @deprecated Generic logbook is now handling it
     */
    public function logbookEntryForServer(int $objectID, int $type)
    {
        return;
    }
    ...
    public function getAttachedEntries($id, $tag = '', $asId = false): CollectionInterface
    {
        ...
        $query = 'SELECT isys_catg_location_list__id as id, isys_obj__id as objId, isys_obj__title as title, isys_obj__id as sysid, isys_obj_type__title as objType
			FROM isys_catg_location_list
			INNER JOIN isys_obj ON isys_obj__id = isys_catg_location_list__isys_obj__id
            INNER JOIN isys_obj_type ON isys_obj_type__id = isys_obj__isys_obj_type__id
			WHERE isys_catg_location_list__parentid IN (' . implode(', ', array_map([$this, 'convert_sql_int'], $id)) . ');';
    ...
    {
        return $this->get_assigned_objects($request->get_object_id(), true);
    }
}
