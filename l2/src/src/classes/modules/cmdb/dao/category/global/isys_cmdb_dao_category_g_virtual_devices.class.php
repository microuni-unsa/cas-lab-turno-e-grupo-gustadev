...
class isys_cmdb_dao_category_g_virtual_devices extends isys_cmdb_dao_category_global
{
     ...
     * @author  Van Quyen Hoang <qhoang@synetics.de>
     */
    public function callback_property_host_stor_device(isys_request $p_request, $class)
    {
        ...
        if ($typeId == defined_or_default('C__OBJTYPE__CLUSTER')) {
            ...
            while ($l_row = $l_res->get_row()) {
                if (!empty($l_return)) {
                    $l_puffer = $this->get_dialog_stor_content_by_host($l_row['isys_obj__id'], $class);
                    if (!empty($l_puffer)) {
                        $l_return = array_merge_recursive($l_return, $l_puffer);
                    }
                } else {
                    $l_return = $this->get_dialog_stor_content_by_host($l_row['isys_obj__id'], $class);
                }
            }
        } else {
            if ($l_host_obj_id > 0) {
                $l_return = $this->get_dialog_stor_content_by_host($l_host_obj_id, $class);
            }
        }
    ...
    public function callback_property_general(isys_request $p_request, $p_class)
    {
        ...
        if (class_exists($p_class)) {
            /** @var isys_cmdb_dao_category $p_class */
            $l_dao_general = new $p_class($this->get_database_component());
            $l_class_table = $l_dao_general->get_source_table() ?? $l_dao_general->get_table();
        ...
        if ($l_dao_vm->get_objTypeID($l_host_obj_id) == defined_or_default('C__OBJTYPE__CLUSTER')) {
            ...
            while ($l_row = $l_res->get_row()) {
                ...
                while ($l_row2 = $l_res2->get_row()) {
                    $portGroups = $l_dao_vs->get_port_groups($l_row2['isys_catg_virtual_switch_list__id']);
                    while ($portGroup = $portGroups->get_row()) {
                        $l_return[$l_row2["isys_catg_virtual_switch_list__title"]][$portGroup["isys_virtual_port_group__id"]] = $portGroup["isys_virtual_port_group__title"];
                    }
                }
            }
        } else {
            ...
            while ($l_row = $l_res->get_row()) {
                $portGroups = $l_dao_vs->get_port_groups($l_row['isys_catg_virtual_switch_list__id']);
                while ($portGroup = $portGroups->get_row()) {
                    $l_return[$l_row["isys_catg_virtual_switch_list__title"]][$portGroup["isys_virtual_port_group__id"]] = $portGroup["isys_virtual_port_group__title"];
                }
            }
        }
    ...
    {
        return [
            '_host_resource' => new DynamicProperty(
                'LC__CMDB__CATG__VD__HOST_RESOURCE',
            ...
            '_type' => new DynamicProperty(
                'LC__CMDB__CATG__VD__TYPE',
                'isys_catg_virtual_device_list__id',
                'isys_catg_virtual_device_list',
                [
                    $this,
            ...
            '_local_device' => new DynamicProperty(
                'LC__CMDB__CATG__VD__LOCAL_DEVICE',
                'isys_catg_virtual_device_list__id',
                'isys_catg_virtual_device_list',
                [
                    $this,
    ...
    {
        $dao = isys_cmdb_dao_category_g_virtual_devices::instance(isys_application::instance()->container->get('database'));
    ...
    private function getHostStorageDevice($id)
    {
        $query = 'SELECT isys_obj__id AS \'objId\', isys_obj__title AS \'objTitle\', isys_catg_stor_list__title AS \'resourceTitle\' FROM isys_virtual_device_host
            ...
            WHERE isys_virtual_device_host__isys_catg_virtual_device_list__id = ' . $this->convert_sql_id($id) . '
            UNION
            SELECT isys_obj__id AS \'objId\', isys_obj__title AS \'objTitle\', isys_catg_raid_list__title AS \'resourceTitle\' FROM isys_virtual_device_host
            INNER JOIN isys_catg_raid_list ON isys_catg_raid_list__id = isys_virtual_device_host__isys_catg_raid_list__id
            INNER JOIN isys_obj ON isys_obj__id = isys_catg_raid_list__isys_obj__id
            WHERE isys_virtual_device_host__isys_catg_virtual_device_list__id = ' . $this->convert_sql_id($id) . '
            UNION
            SELECT NULL AS \'objId\', isys_virtual_device_host__cluster_storage AS \'objTitle\', NULL AS \'resourceTitle\'
            FROM isys_virtual_device_host
            WHERE isys_virtual_device_host__isys_catg_virtual_device_list__id = ' . $this->convert_sql_id($id);
        $resourceData = $this->retrieve($query)->get_row();

        $resourceTitle = '';
        if (!empty($resourceData['resourceTitle'])) {
            $resourceTitle = ' >> ' . $resourceData['resourceTitle'];
        }

        $return = empty($resourceData['objTitle']) ? isys_tenantsettings::get('gui.empty_value', '-') : $resourceData['objTitle'] . $resourceTitle;
        return $return;
    }
    ...
    private function getHostNetworkDevice($id, $type)
    {
        if ($type == defined_or_default('C__NETWORK_TYPE__VIRTUALSWITCH')) {
            ...
            $query = 'SELECT isys_virtual_port_group__title FROM isys_virtual_device_host
               INNER JOIN isys_virtual_port_group ON isys_virtual_port_group__id = isys_virtual_device_host__isys_virtual_port_group__id
                WHERE isys_virtual_device_host__isys_catg_virtual_device_list__id = ' .
                $this->convert_sql_id($id);
            $return = "Switch Port Group: " . $this->retrieve($query)->get_row_value('isys_virtual_port_group__title');
        } else {
            // Get Port
    ...
    protected function properties()
    {
        return [
            ...
            'host_raid'            => new DialogProperty(
                'C__CMDB__CATG__VD__HOST_RAID_ARRAY',
                'LC__CMDB__CATG__VD__HOST_RAID_ARRAY',
                'isys_virtual_device_host__isys_catg_raid_list__id',
            ...
            'switch_port_group'    => array_replace_recursive(isys_cmdb_dao_category_pattern::dialog(), [
                ...
                C__PROPERTY__DATA     => [
                    C__PROPERTY__DATA__FIELD => 'isys_virtual_device_host__isys_virtual_port_group__id',
                ],
                C__PROPERTY__UI       => [
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        ...
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            ...
            $l_host_raid = $this->get_property('host_raid');
            $deviceType = $this->get_property('device_type') ?? $this->determineDeviceType($p_category_data['properties']);
            switch ($p_status) {
                    ...
                    if (($p_category_data['data_id'] = $this->create(
                        ...
                        if ($this->get_property('local_stor_device')) {
                            ...
                            if ($l_host_stor_device) {
                                $this->create_host_device($p_category_data['data_id'], $l_host_stor_device, null, null, null, "", null, null, null, null);
                            } elseif ($l_host_ldev_client) {
                                $this->create_host_device($p_category_data['data_id'], null, $l_host_ldev_client, null, null, "", null, null, null, null);
                            } elseif ($l_host_drive) {
                                $this->create_host_device($p_category_data['data_id'], null, null, $l_host_drive, null, "", null, null, null, null);
                            } elseif ($l_host_raid) {
                                $this->create_host_device($p_category_data['data_id'], null, null, null, null, "", null, null, null, $l_host_raid);
                            }
                        }
                        if ($this->get_property('local_port')) {
                            ...
                            if ($l_host_port_device) {
                                $this->create_host_device($p_category_data['data_id'], null, null, null, $l_host_port_device['ref_id'], "", null, null, null, null);
                            } elseif (($l_switch_port_group = $this->get_property('switch_port_group'))) {
                                $this->create_host_device($p_category_data['data_id'], null, null, null, null, $l_switch_port_group, null, null, null, null);
                            }
                        }
                        if ($this->get_property('local_interface')) {
                            ...
                            if ($l_cluster_interface) {
                                $this->create_host_device($p_category_data['data_id'], null, null, null, null, null, null, null, $l_cluster_interface['ref_title'], null);
                            } elseif (($l_host_interface = $this->get_property('host_interface'))) {
                                $this->create_host_device($p_category_data['data_id'], null, null, null, null, null, $l_host_interface, null, null, null);
                            }
                        }
                        $l_indicator = true;
                    }
                ...
                case isys_import_handler_cmdb::C__UPDATE:
                    ...
                    if ($this->get_property('local_stor_device')) {
                        ...
                        $l_ref_id = null;
                        $this->save_host_device(null, $l_host_stor_device, null, null, null, "", null, null, null, $p_category_data['data_id'], null);
                        $this->save_host_device(null, null, $l_host_ldev_client, null, null, "", null, null, null, $p_category_data['data_id'], null);
                        $this->save_host_device(null, null, null, $l_host_drive, null, "", null, null, null, $p_category_data['data_id'], null);
                        $this->save_host_device(null, null, null, null, null, "", null, null, null, $p_category_data['data_id'], $l_host_raid);
                    }
                    if ($this->get_property('local_port')) {
                        ...
                        if ($l_host_port_device) {
                            $this->save_host_device(null, null, null, null, $l_host_port_device['ref_id'], null, null, null, null, $p_category_data['data_id'], null);
                        } elseif (($l_switch_port_group = $this->get_property('switch_port_group'))) {
                            $this->save_host_device(null, null, null, null, null, $l_switch_port_group, null, null, null, $p_category_data['data_id'], null);
                        }
                    }
                    if ($this->get_property('local_interface')) {
                        $this->save_local_device(null, null, null, $this->get_property('local_interface'), null, null, $p_category_data['data_id']);
                        if ($l_cluster_interface) {
                            $this->save_host_device(null, null, null, null, null, null, null, null, $l_cluster_interface['ref_title'], $p_category_data['data_id'], null);
                        } elseif ($l_host_interface = $this->get_property('host_interface')) {
                            $this->save_host_device(null, null, null, null, null, null, $l_host_interface, null, null, $p_category_data['data_id'], null);
                        }
                    }
                    break;
            }
     ...
     * @return bool
     */
    public static function isStorageDeviceType(array $data): bool
    {
        $keys = [
            'isys_virtual_device_local__isys_catg_stor_list__id',
            'local_stor_device',
            'isys_virtual_device_host__cluster_storage',
            'cluster_storage',
            'isys_virtual_device_host__isys_catg_stor_list__id',
            'host_stor_device',
            'isys_virtual_device_host__isys_catg_ldevclient_list__id',
            'host_ldev_client',
            'isys_virtual_device_host__isys_catg_drive_list__id',
            'host_drive',
        ];
        return !empty(array_filter($keys, fn ($key) => isset($data[$key])));
    }
     ...
     * @return bool
     */
    public static function isNetworkDeviceType(array $data): bool
    {
        $keys = [
            'isys_virtual_device_local__isys_catg_port_list__id',
            'local_port',
            'isys_virtual_device_host__isys_catg_port_list__id',
            'host_port',
            'isys_virtual_device_host__isys_virtual_port_group__id',
            'switch_port_group',
        ];
        return !empty(array_filter($keys, fn ($key) => isset($data[$key])));
    }
     ...
     * @return bool
     */
    public static function isInterfaceDeviceType(array $data)
    {
        $keys = [
            'isys_virtual_device_local__isys_catg_ui_list__id',
            'local_interface',
            'isys_virtual_device_host__cluster_ui',
            'cluster_interface',
            'isys_virtual_device_host__isys_catg_ui_list__id',
            'host_interface',
        ];
        return !empty(array_filter($keys, fn ($key) => isset($data[$key])));
    }
     ...
     * @return  integer
     */
    public function determineDeviceType($p_catdata)
    {
        if (self::isStorageDeviceType($p_catdata)) {
            return C__VIRTUAL_DEVICE__STORAGE;
        } elseif (self::isNetworkDeviceType($p_catdata)) {
            return C__VIRTUAL_DEVICE__NETWORK;
        } elseif (self::isInterfaceDeviceType($p_catdata)) {
    ...
    public function create_host_device(
        ...
        $l_sql = "INSERT INTO isys_virtual_device_host SET " . "isys_virtual_device_host__isys_catg_virtual_device_list__id = " .
            ...
            $this->convert_sql_id($p_port_id) . ", " . "isys_virtual_device_host__isys_virtual_port_group__id = " . $this->convert_sql_id($p_switch_port) . ", " .
            ...
            "isys_virtual_device_host__isys_catg_raid_list__id = " . $this->convert_sql_id($p_raid_id) . ";";
    ...
    public function save_host_device(
        ...
        $l_sql = "UPDATE isys_virtual_device_host SET " . "isys_virtual_device_host__isys_catg_ldevclient_list__id = " . $this->convert_sql_id($p_ldevclient_id) . ", " .
            ...
            "isys_virtual_device_host__isys_virtual_port_group__id = " . $this->convert_sql_id($p_switch_port) . ", " . "isys_virtual_device_host__cluster_storage = " .
            $this->convert_sql_text($p_cluster_storage) . ", " . "isys_virtual_device_host__cluster_ui = " . $this->convert_sql_text($p_cluster_ui) . ", " .
            "isys_virtual_device_host__isys_catg_ui_list__id = " . $this->convert_sql_id($p_ui_id) . ", " . "isys_virtual_device_host__isys_catg_raid_list__id = " .
            $this->convert_sql_id($p_raid_id) . " ";
    ...
    public function save_element(&$p_cat_level, &$p_status, $p_create = false)
    {
        ...
        if ($l_dao_vm->get_objTypeID($l_host_system) === defined_or_default('C__OBJTYPE__CLUSTER')) {
            $l_is_cluster = true;
        } else {
            $l_is_cluster = false;
        }
        ...
        if (count($l_host_storage) == 2) {
            ...
            } elseif ($l_host_storage[1] == defined_or_default('C__CATG__RAID')) {
                $l_host_raid = $l_host_storage[0];
            }
        }
        ...
        if ($_POST['C__CMDB__CATG__VD__NETWORK_TYPE'] == defined_or_default('C__NETWORK_TYPE__VIRTUALSWITCH')) {
            $switchPort = html_entity_decode($_POST['C__CMDB__CATG__VD__SWITCH_PORT_GROUP'], ENT_COMPAT, BASE_ENCODING);
            $portId = null;
        }
     ...
     * @author  Van Quyen Hoang <qhoang@synetics.de>
     */
    private function get_dialog_stor_content_by_host($objectId, $class)
    {
        $daoStorage = isys_cmdb_dao_category_g_stor::instance($this->get_database_component());
        $daoLdev = isys_cmdb_dao_category_g_ldevclient::instance($this->get_database_component());
        $daoDrive = isys_cmdb_dao_category_g_drive::instance($this->get_database_component());
        $dialogData = [];

        $contextOrigin = Context::instance()->getOrigin();

        $storageData = $this->getStorageData($daoStorage, $objectId);
        if ($contextOrigin === Context::ORIGIN_API && $class === get_class($daoStorage)) {
            return $storageData;
        }

        if ($storageData) {
            $dialogData[isys_application::instance()->container->get('language')
                ->get('LC__STORAGE_DEVICE')] = $storageData;
        }

        $ldevData = $this->getLdevData($daoLdev, $objectId);
        if ($contextOrigin === Context::ORIGIN_API && $class === get_class($daoLdev)) {
            return $ldevData;
        }

        if ($ldevData) {
            $dialogData[isys_application::instance()->container->get('language')
                ->get('LC__CMDB__CATG__LDEV_CLIENT')] = $ldevData;
        }

        $driveData = $this->getDriveData($daoDrive, $objectId);
        if ($contextOrigin === Context::ORIGIN_API && $class === get_class($daoDrive)) {
            return $driveData;
        }

        if ($driveData) {
            $dialogData[isys_application::instance()->container->get('language')
                ->get('LC__STORAGE_DRIVE')] = $driveData;
        }
     ...
     * @throws Exception
     */
    private function getStorageData($dao, $objectId)
    {
        ...
        while ($data = $result->get_row()) {
            $id = $contextOrigin === Context::ORIGIN_API ? $data["isys_catg_stor_list__id"] :
                $data["isys_catg_stor_list__id"] . "," . defined_or_default('C__CATG__STORAGE');

            $dialogData[$id] = $data["isys_catg_stor_list__title"] . " (" . isys_application::instance()->container->get('language')
                    ->get($data["isys_stor_manufacturer__title"]) . ")";
        }
     ...
     * @throws Exception
     */
    private function getLdevData($dao, $objectId)
    {
        $result = $dao->get_data(null, $objectId);
        $dialogData = [];
        $contextOrigin = Context::instance()->getOrigin();
        ...
        while ($data = $result->get_row()) {
            $id = $contextOrigin === Context::ORIGIN_API ? $data["isys_catg_ldevclient_list__id"] :
                $data["isys_catg_ldevclient_list__id"] . "," . defined_or_default('C__CATG__LDEV_CLIENT');

            $dialogData[$id] = $data["isys_catg_ldevclient_list__title"] . " (" . isys_application::instance()->container->get('language')
                    ->get($data["isys_catg_sanpool_list__title"]) . ")";
        }
     ...
     * @throws Exception
     */
    private function getDriveData($dao, $objectId)
    {
        $result = $dao->get_data(null, $objectId);
        $dialogData = [];
        $contextOrigin = Context::instance()->getOrigin();
        ...
        while ($data = $result->get_row()) {
            $id = $contextOrigin === Context::ORIGIN_API ? $data["isys_catg_drive_list__id"] :
                $data["isys_catg_drive_list__id"] . "," . defined_or_default('C__CATG__DRIVE');

            $dialogData[$id] = $data["isys_catg_drive_list__title"] . " (" . isys_application::instance()->container->get('language')
                    ->get($data["isys_catg_drive_list__driveletter"]) . ")";
        }
     ...
     * @return array
     */
    public function get_raid_list($p_object_id)
    {
        $return = [];
        /**
         * @var $raid_dao isys_cmdb_dao_category_g_raid
         */
        $raid_dao = isys_cmdb_dao_category_g_raid::instance(isys_application::instance()->container->get('database'));
        $raids = $raid_dao->get_raids(null, null, $p_object_id);
        while ($l_row = $raids->get_row()) {
            $l_id = $l_row['isys_catg_raid_list__id'] . '_' . defined_or_default('C__CATG__RAID', 'C__CATG__RAID');
            $return[$l_id] = $l_row['isys_catg_raid_list__title'];
        }
        return $return;
    }
}
