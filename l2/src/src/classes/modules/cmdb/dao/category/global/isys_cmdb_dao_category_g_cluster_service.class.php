...
class isys_cmdb_dao_category_g_cluster_service extends isys_cmdb_dao_category_global
{
     ...
     * @throws isys_exception_database
     */
    public function dynamicPropertyCallbackRunsOn(array $row)
    {
        $query = 'SELECT isys_catg_cluster_service_list__isys_obj__id as objId
            FROM isys_catg_cluster_service_list
            WHERE isys_catg_cluster_service_list__id = ' . $this->convert_sql_id($row['isys_catg_cluster_service_list__id']);

        $objectId = $this->retrieve($query)->get_row_value('objId');
        $request = (isys_request::factory())
            ->set_object_id($objectId)
            ->set_category_data_id($row['isys_catg_cluster_service_list__id']);

        $data = $this->callback_property_runs_on($request);
        $data = array_filter($data, fn ($item) => $item['sel']);

        if (count($data) == 0) {
            return '';
        }
        $return = array_map(fn ($item) => $item['val'], $data);
     ...
     * @throws isys_exception_database
     */
    public function dynamicPropertyCallbackDefaultServer(array $row)
    {
        $query = 'SELECT isys_catg_cluster_service_list__isys_obj__id as objId, isys_catg_cluster_service_list__cluster_members_list__id as defaultServer
            FROM isys_catg_cluster_service_list
            WHERE isys_catg_cluster_service_list__id = ' . $this->convert_sql_id($row['isys_catg_cluster_service_list__id']);

        $result = $this->retrieve($query)->get_row();
        $objectId = $result['objId'];
        $defaultServer = $result['defaultServer'];

        $request = (isys_request::factory())
            ->set_object_id($objectId)
            ->set_category_data_id($row['isys_catg_cluster_service_list__id']);

        $data = $this->callback_property_default_server($request);
        return $data[$defaultServer] ?? isys_tenantsettings::get('gui.empty_value', '-');
    }
    ...
    public function properties()
    {
        return [
            'type'                     => (new DialogProperty(
                'C__CMDB__CATG__CLUSTER_SERVICE__TYPE',
                'LC__CMDB__CATG__CLUSTER_SERVICE__TYPE',
                'isys_catg_cluster_service_list__isys_cluster_type__id',
                'isys_catg_cluster_service_list',
                'isys_cluster_type'
            ))->mergePropertyProvides([Property::C__PROPERTY__PROVIDES__REPORT => true]),
            'cluster_service'          => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_s_cluster_service::cluster'
                ],
                C__PROPERTY__DATA     => [
            ...
            'drives'                   => (new ObjectBrowserMultiselectProperty(
                'C__CMDB__CATG__CLUSTER_SERVICE__DRIVES',
                'LC__CMDB__CATG__CLUSTER_SERVICE__VOLUMES',
                'isys_catg_cluster_service_list__id',
                'isys_catg_cluster_service_list',
                ...
                '',
                'CONCAT(ref.isys_catg_drive_list__title, \' (\', ref.isys_catg_drive_list__driveletter, \') \')'
            ))->mergePropertyUiParams(
                [
                    'p_strPopupType' => 'browser_cat_data',
                    isys_popup_browser_object_ng::C__DATARETRIEVAL  => 'isys_cmdb_dao_category_g_drive::catdata_browser'
                ]
            )->setPropertyFormatOffset(
                \idoit\Component\Property\Property::C__PROPERTY__FORMAT__REQUIRES,
                'cluster_service'
            )->mergePropertyProvides([
                C__PROPERTY__PROVIDES__REPORT  => false,
                C__PROPERTY__PROVIDES__VIRTUAL => true
            ]),
            'shares'                   => (new ObjectBrowserMultiselectProperty(
                'C__CMDB__CATG__CLUSTER_SERVICE__SHARES',
                'LC__CMDB__CATG__CLUSTER_SERVICE__SHARES',
                'isys_catg_cluster_service_list__isys_obj__id',
                'isys_catg_cluster_service_list',
                ...
                '',
                'CONCAT(ref.isys_catg_shares_list__title, \' (\', ref.isys_catg_shares_list__unc_path, \') \')'
            ))->mergePropertyUiParams(
                [
                    'p_strPopupType' => 'browser_cat_data',
                    isys_popup_browser_object_ng::C__DATARETRIEVAL  => 'isys_cmdb_dao_category_g_shares::catdata_browser'
                ]
            )->setPropertyFormatOffset(
                \idoit\Component\Property\Property::C__PROPERTY__FORMAT__REQUIRES,
                'cluster_service'
    ...
    public function dynamic_properties()
    {
        return [
            '_runs_on' => new DynamicProperty(
                ...
                [
                    'isys_cmdb_dao_category_g_cluster_service',
                    'dynamicPropertyCallbackRunsOn'
                ]
            ),
            '_default_server' => new DynamicProperty(
                'LC__CMDB__CATG__CLUSTER_SERVICE__DEFAULT_SERVER',
                'isys_catg_cluster_service_list__id',
                'isys_catg_cluster_service_list',
                [
                    'isys_cmdb_dao_category_g_cluster_service',
                    'dynamicPropertyCallbackDefaultServer'
                ]
            ),
        ];
    }
     ...
     * @throws  isys_exception_database
     */
    public function get_assigned_objects_and_relations($p_cat_id, $p_assigned_obj__id, $p_status = C__RECORD_STATUS__NORMAL)
    {
