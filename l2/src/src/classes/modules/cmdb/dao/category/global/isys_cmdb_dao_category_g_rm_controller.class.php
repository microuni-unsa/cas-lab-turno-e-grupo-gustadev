...
class isys_cmdb_dao_category_g_rm_controller extends isys_cmdb_dao_category_global
{
     ...
     * @var string
     */
    protected $m_list = 'isys_cmdb_dao_list_catg_rm_controller';
    ...
    public function dynamic_property_callback_remote_url($p_row)
    {
        ...
        if (!isset($p_row['isys_connection__isys_obj__id'])) {
            $p_row['isys_connection__isys_obj__id'] = isys_cmdb_dao_category_g_rm_controller::instance($database)
                ->get_data(null, $p_row['isys_obj__id'])
                ->get_row_value('isys_connection__isys_obj__id');
        }

        $data = isys_cmdb_dao_category_g_access::instance($database)
            ->get_primary_element($p_row['isys_connection__isys_obj__id'])
            ->get_row();
        ...
        return isys_helper_link::handle_url_variables($data['isys_catg_access_list__url'], $p_row['isys_connection__isys_obj__id']);
    }
    ...
    public function save($p_cat_level, $p_status = C__RECORD_STATUS__NORMAL, $p_assigned_object = null, $p_description = '')
    {
        ...
        if ($this->update($l_sql) && $this->apply_update()) {
            ...
            $l_relation_dao->handle_relation(
                ...
                defined_or_default('C__RELATION_TYPE__RM_CONTROLLER'),
        ...
        return [
            'remote_url'       => array_replace_recursive(isys_cmdb_dao_category_pattern::text(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        isys_cmdb_dao_category_g_access::build_formatted_url_query() . '
                        	LEFT JOIN isys_connection ON isys_connection__isys_obj__id = acc.isys_catg_access_list__isys_obj__id
                        	LEFT JOIN isys_catg_rm_controller_list ON isys_catg_rm_controller_list__isys_connection__id = isys_connection__id',
                        'isys_catg_rm_controller_list',
                        '',
                        'isys_catg_rm_controller_list__isys_obj__id',
                        '',
                        ...
                        SelectGroupBy::factory(['isys_catg_rm_controller_list__isys_obj__id'])
                    )
                ],
            ...
            'connected_object' => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_g_rm_controller_backward::connected_object'
                ],
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT           => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        ...
                        'isys_catg_rm_controller_list',
                        'isys_catg_rm_controller_list__id',
                        'isys_catg_rm_controller_list__isys_obj__id',
                        '',
                        '',
                        null,
     ...
     */
    public function rank_records($ids, $p_direction = C__CMDB__RANK__DIRECTION_DELETE, $p_table = "isys_obj", $p_checkMethod = null, $p_purge = false)
    {
        if ($p_direction === C__CMDB__RANK__DIRECTION_DELETE) {
            return parent::rank_records($ids, $p_direction, $p_table, $p_checkMethod, true);
        }
