 ...
 */
class isys_cmdb_dao_category_g_cpu extends isys_cmdb_dao_category_global implements ComparableCategory
{
    /**
     ...
     * @return array
     */
    public function prepareCsvImportData(array $data): array
    {
        if (isset($data['frequency']) && !isset($data['frequency_unit'])) {
            $frequency_data = explode(" ", $data['frequency']['value']);
            if (is_array($frequency_data) && count($frequency_data) > 1) {
                $data['frequency']['value'] = $frequency_data[0];
                $data['frequency']['id'] = $frequency_data[0];
                if(isset($frequency_data[1])) {
                    switch (strtoupper($frequency_data[1])) {
                        case 'KHZ':
                            $unit = C__FREQUENCY_UNIT__KHZ;
                            break;
                        case 'MHZ':
                            $unit = C__FREQUENCY_UNIT__MHZ;
                            break;
                        case 'GHZ':
                            $unit = C__FREQUENCY_UNIT__GHZ;
                            break;
                        case 'THZ':
                            $unit = C__FREQUENCY_UNIT__THZ;
                            break;
                        default:
                            $unit = C__FREQUENCY_UNIT__GHZ;
                    }
                    $data['frequency_unit']['value'] = $unit;
                    $data['frequency_unit']['id'] = $unit;
                }
            }
        }
    ...
    public function save($p_cat_level, $p_newRecStatus, $p_title, $p_manufacturerID, $p_frequency, $p_typeID, $p_description, $p_cores = null, $p_frequency_unit = null)
    {
        ...
        $l_strSql = 'UPDATE isys_catg_cpu_list SET
			...
			isys_catg_cpu_list__isys_frequency_unit__id = ' . $this->convert_sql_id($p_frequency_unit ?? defined_or_default('C__FREQUENCY_UNIT__GHZ', 3)) . '
    ...
    public function save_element(&$p_cat_level, &$p_intOldRecStatus, $p_create = false)
    {
        ...
        if ($p_create) {
            ...
            $l_nQuantity = $_POST["C__CATG__CPU_NUMBER2CREATE"] ?? 1;
    ...
    {
        $context = Context::instance()->getContextCustomer();
        $contextxOrigin = Context::instance()->getOrigin();

        $objId = ($data['isys_catg_cpu_list__isys_obj__id'] ?: ($data['isys_obj__id'] ?: null));
        $status = null;
        ...
        if ($context === Context::CONTEXT_REPORT && $contextxOrigin !== Context::ORIGIN_CONSOLE) {
            $reportQuery = isys_application::instance()->container->get('idoit.report.container')->getReportQuery();
            $statusData = $reportQuery->getMultivalueStatusFilter();
            if (!empty($statusData)) {
                $status = current($statusData);
            }
        }

        $result = $this->get_data(null, $objId, '', null, $status);
        $cores = 0;
    ...
    {
        return [
            '_frequency' =>   new DynamicProperty(
                'LC__CMDB__CATG__CPU',
            ...
            '_total_cores' => new DynamicProperty(
                'LC__CMDB__CATG__CPU__TOTAL_CORES',
            ...
            '_total_cpus' => new DynamicProperty(
                'LC__CMDB__CATG__CPU__TOTAL_CPUS',
                'isys_catg_cpu_list__isys_obj__id',
                'isys_catg_cpu_list',
                [
                    $this,
    ...
    {
        $count = (int)$_POST['C__CATG__CPU_NUMBER2CREATE'];

        if ($count > 1) {
            for ($i = 1; $i <= $count; $i++) {
                parent::logbook_update(
                    $constant,
                    $changer,
            ...
            'frequency'      => array_replace_recursive(isys_cmdb_dao_category_pattern::float(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__CONDITION => function (isys_cmdb_dao_list_objects $dao, $name, $value) {
                        ...
                        $dao->add_additional_having_conditions("{$column} LIKE '%@{$value} %'");
            ...
            'frequency_unit' => array_replace_recursive(isys_cmdb_dao_category_pattern::dialog(), [
                ...
                C__PROPERTY__UI       => [
                    ...
                    C__PROPERTY__UI__PARAMS => [
                        ...
                        'order'        => 'isys_frequency_unit__sort',
                        'p_bSort'      => false,
                        'p_bDbFieldNN' => '1',
                        'p_strClass'   => 'input-mini',
                    ],
                    C__PROPERTY__UI__DEFAULT => defined_or_default('C__FREQUENCY_UNIT__GHZ', 3),
                ],
                C__PROPERTY__PROVIDES => [
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            // @see ID-10513 This happens if no unit gets passed when using the API.
            if (empty($p_category_data['properties']['frequency_unit'][C__DATA__VALUE])) {
                $unitProperty = $this->get_property_by_key('frequency_unit');

                // Fetch the UI default value.
                $p_category_data['properties']['frequency_unit'][C__DATA__VALUE] = $unitProperty[C__PROPERTY__UI][C__PROPERTY__UI__DEFAULT];
            }
