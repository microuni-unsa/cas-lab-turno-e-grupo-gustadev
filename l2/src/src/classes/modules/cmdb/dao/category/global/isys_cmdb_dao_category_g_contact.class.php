...
use idoit\Module\Cmdb\Event\CiObject\Ranked;
 ...
 */
class isys_cmdb_dao_category_g_contact extends isys_cmdb_dao_category_global implements ObjectBrowserReceiver, ObjectBrowserAssignedEntries, CiEventListenerInterface
{
    /**
        ...
        return [
            ...
            'contact_object'  => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_s_organization_contact_assign::object;isys_cmdb_dao_category_s_person_contact_assign::object;isys_cmdb_dao_category_s_person_group_contact_assign::object'
                ],
                C__PROPERTY__DATA     => [
                    C__PROPERTY__DATA__FIELD            => 'isys_catg_contact_list__isys_connection__id',
            ...
            'contact_list_with_roles'    => array_replace_recursive(isys_cmdb_dao_category_pattern::virtual(), [
                C__PROPERTY__INFO     => [
                    C__PROPERTY__INFO__TITLE       => 'LC__CATG__CONTACT_AND_ROLE',
                    C__PROPERTY__INFO__DESCRIPTION => 'Contacts (Role)'
                ],
                C__PROPERTY__DATA     => [
                    C__PROPERTY__DATA__FIELD  => 'isys_catg_contact_list__isys_connection__id',
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT CONCAT(isys_obj__title, \' {\', isys_obj__id, \'}\', \' (\', isys_contact_tag__title, \')\')
                            FROM isys_catg_contact_list
                            INNER JOIN isys_connection ON isys_connection__id = isys_catg_contact_list__isys_connection__id
                            INNER JOIN isys_obj ON isys_obj__id = isys_connection__isys_obj__id AND isys_obj__status = ' . $this->convert_sql_int(C__RECORD_STATUS__NORMAL) .'
                            LEFT JOIN isys_contact_tag ON isys_contact_tag.isys_contact_tag__id = isys_catg_contact_list.isys_catg_contact_list__isys_contact_tag__id',
                        'isys_catg_contact_list',
                        'isys_catg_contact_list__id',
                        'isys_catg_contact_list__isys_obj__id',
                        '',
    ...
    {
        // @see ID-10459 Jump out with 'nothing has to be done'.
        if (!is_array($p_category_data) || !isset($p_category_data['properties']) || empty($p_category_data['properties'])) {
            return true;
        }
        ...
        if (is_array($l_value['contact'])) {
            ...
            $l_contact = $l_value['contact'] ?? null;
        }

        if (($l_contact === null || $l_contact === 0 || $l_contact === '0') && isset($p_category_data['properties']['contact_object'])) {
            ...
            if (is_array($l_contact)) {
                ...
                if ($p_status === isys_import_handler_cmdb::C__UPDATE && $p_category_data['data_id']) {
                    $this->save(
        ...
        if ($p_status === isys_import_handler_cmdb::C__CREATE && $p_object_id && $l_contact > 0) {
        ...
        // @see ID-10459 Set default return value to 'nothing has to be done'.
        $l_indicator = true;

        // Primary:
        if (is_numeric($p_category_data['properties']['primary'][C__DATA__VALUE]) && $l_contact > 0) {
            $l_indicator = ((int)$p_category_data['properties']['primary'][C__DATA__VALUE])
                ? $this->make_primary($p_object_id, $p_category_data['data_id'])
    ...
    public function save($p_cat_level, $p_connected_obj_id, $p_role_id = null, $p_description = null, $p_record_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        if ($p_connected_obj_id > 0) {
            ...
            return $this->delete((int)$p_cat_level);
        }
    }

    /**
     * @param integer|null $p_id
    ...
    public function delete($p_id = null, $p_obj_id = null)
    {
        ...
        $l_sql = "DELETE FROM isys_catg_contact_list WHERE ";
        $conditions = [];

        if ($p_id) {
            $conditions[] = " isys_catg_contact_list__id = " . $this->convert_sql_id($p_id);
        }

        if ($p_obj_id) {
            $conditions[] = " isys_catg_contact_list__isys_obj__id = " . $this->convert_sql_id($p_obj_id);
        }
    ...
    public function save_contact_tag($p_catg_contact_id, $p_contact_tag)
    {
        ...
        $l_query = "UPDATE isys_catg_contact_list
            SET isys_catg_contact_list__isys_contact_tag__id = " . $this->convert_sql_id($p_contact_tag) . "
        ...
        if ($this->update($l_query)) {
            ...
            $this->object_changed($l_data['isys_obj__id']);

            // Emit category signal afterCategoryEntrySave
            isys_component_signalcollection::get_instance()
                ->emit('mod.cmdb.afterCategoryEntrySave', $this, $p_catg_contact_id, ($p_catg_contact_id > 0), $l_data['isys_obj__id'], $_POST, []);
    ...
    {
        if (is_array($p_contact_tag_id) && !empty($p_contact_tag_id)) {
            $l_delete = 'WHERE isys_contact_tag__id IN (' . implode(',', array_map([$this, 'convert_sql_id'], $p_contact_tag_id)) . ')';
     ...
     * @author Van Quyen Hoang <qhoang@i-doit.com>
     */
    public function get_assigned_objects_by_contact($p_contact_obj_id = null, $p_email = null, $status = null)
    {
        ...
        if ($status !== null) {
            $l_sql .= ' AND isys_catg_contact_list__status = ' . $this->convert_sql_int($status);
        }
    ...
    public function getAttachedEntries($id, $tag = '', $asId = true): CollectionInterface
    {
        return isys_cmdb_dao_connection::instance(isys_application::instance()->container->get('database'))
            ->getConnectedObjects('isys_catg_contact_list', $id);
    }
    ...
    public static function getEventListeners(): EventListenerCollection
    {
        ...
        $collection->addEventListener(
            new EventListenerContainer(
                Ranked::NAME,
                'onObjectRanked',
                isys_cmdb_dao_category_g_contact::class
            )
        );
     ...
     * @return void
     */
    public function onObjectRanked(Ranked $event): void
    {
        $objectId = $event->getId();
        $data = $event->getData();

        if (!isset($data['previousStatus']) || !isset($data['newStatus'])) {
            return;
        }

        $objectTitle = $this->obj_get_title_by_id_as_string($objectId);
        $objectTypeId = $this->get_objTypeID($objectId);
        $source = $data['source'];
        $contactsResult = $this->get_data(null, null, "AND isys_connection__isys_obj__id = {$this->convert_sql_id($objectId)}");

        if ($contactsResult->count() === 0) {
            return;
        }

        if ($data['newStatus'] === C__RECORD_STATUS__NORMAL) {
            $eventType = 'C__LOGBOOK_EVENT__CATEGORY_RECYCLED';
        } else {
            $eventType = match((int)$data['newStatus']) {
        ...
        while ($row = $contactsResult->get_row()) {
            isys_event_manager::getInstance()->triggerCMDBEvent(
                $eventType,
                "Entry with contact {$objectTitle} changed from {$source}",
                $row['isys_catg_contact_list__isys_obj__id'],
                $this->get_objTypeID($row['isys_catg_contact_list__isys_obj__id']),
                'LC__CMDB__CATG__CONTACT'
            );
        }
    }
