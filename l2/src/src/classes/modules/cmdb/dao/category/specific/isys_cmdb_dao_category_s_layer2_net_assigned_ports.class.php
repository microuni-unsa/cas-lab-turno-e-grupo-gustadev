 ...
 */
class isys_cmdb_dao_category_s_layer2_net_assigned_ports extends isys_cmdb_dao_category_specific implements ObjectBrowserReceiver, ObjectBrowserAssignedEntries
{
    /**
     ...
     */
    protected $m_category_const = 'C__CATS__LAYER2_NET_ASSIGNED_PORTS';

    /**
     * @var string
     */
    protected $m_object_id_field = 'isys_cats_layer2_net_assigned_ports_list__isys_obj__id';

    /**
     * @var string
     */
    protected $m_connected_object_id_field = 'isys_catg_port_list__isys_obj__id';
        ...
        return [
            ...
            'isys_catg_port_list__id'  => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_g_network_port::layer2_assignment',
                ],
                C__PROPERTY__DATA     => [
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            if ($p_status === isys_import_handler_cmdb::C__CREATE || $p_status === isys_import_handler_cmdb::C__UPDATE) {
                $value = isset($p_category_data['properties']['isys_catg_port_list__id']['ref_id']) ?
                    $p_category_data['properties']['isys_catg_port_list__id']['ref_id'] : $p_category_data['properties']['isys_catg_port_list__id'][C__DATA__VALUE];
                ...
                if (($l_id = $this->create($p_object_id, $value))) {
                    return $l_id;
                }
            }
        }
     ...
     * @author  Leonard Fischer <lfischer@i-doit.org>
     */
    public function create($p_obj_id, $p_port_id, $p_default_vlan = 0, $p_status = C__RECORD_STATUS__NORMAL)
    {
    ...
    public function attachObjects($objectId, array $portIds)
    {
        ...
        $this->delete_entries_by_obj_id((int) $objectId);
     ...
     * @throws isys_exception_database
     */
    public function getAttachedEntries($ids, $tag = '', $asId = true): CollectionInterface
    {
        ...
        $query = 'SELECT
            con.isys_catg_port_list__id as id,
            isys_obj__id as objId,
            isys_catg_port_list__title as title,
            con.*
            FROM isys_cats_layer2_net_assigned_ports_list as main
                INNER JOIN isys_catg_port_list as con ON con.isys_catg_port_list__id = main.isys_catg_port_list__id
                INNER JOIN isys_obj ON isys_obj__id = con.isys_catg_port_list__isys_obj__id
                INNER JOIN isys_obj_type ON isys_obj_type__id = isys_obj__isys_obj_type__id
            WHERE main.isys_cats_layer2_net_assigned_ports_list__isys_obj__id IN (%s);';

        if ($tag === 'isys_catg_port_list__id') {
            $separator = isys_tenantsettings::get('gui.separator.connector', ' > ');
            $query = "SELECT
                isys_catg_port_list__id as id,
                isys_catg_port_list__isys_obj__id as objId,
                isys_catg_port_list__title as title,
                CONCAT(isys_obj__title, {$this->convert_sql_text($separator)}, isys_catg_port_list__title) as title,
                isys_catg_port_list.*
              FROM isys_catg_port_list
                INNER JOIN isys_obj ON isys_obj__id = isys_catg_port_list__isys_obj__id
                WHERE isys_catg_port_list__id IN (%s);";
        }
        ...
        while ($row = $result->get_row()) {
            $collection->addEntry(Entry::factory($row['id'], $row['objId'], $row['title'], $row));
        }
