...
class isys_cmdb_dao_category_s_cluster_service extends isys_cmdb_dao_category_specific
{
    ...
    /**
     * @see ID-10478 Configure category as multivalue to make the parent constructor set '$this->m_list'.
     * @var bool
     */
        ...
        return [
            'cluster'                  => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_g_cluster_service::cluster_service'
                ],
                C__PROPERTY__DATA     => [
    ...
    {
        $dao = isys_cmdb_dao_category_g_cluster_service::instance($this->get_database_component());
        $currentState = intval(isys_glob_get_param("cRecStatus"));
        ...
        if ($direction === C__CMDB__RANK__DIRECTION_DELETE) {
            $newState = match ($currentState) {
                ...
                default => C__RECORD_STATUS__PURGE
            };
        } else {
            $newState = match ($currentState) {
                C__RECORD_STATUS__ARCHIVED => C__RECORD_STATUS__NORMAL,
                C__RECORD_STATUS__DELETED => C__RECORD_STATUS__ARCHIVED
            };
        }

        if ($newState === C__RECORD_STATUS__PURGE) {
            return $dao->delete($entryId);
        }

        parent::rank_record($entryId, $direction, $table, $checkMethod, $purge);

        return $dao->set_status($entryId, $newState);
    }
