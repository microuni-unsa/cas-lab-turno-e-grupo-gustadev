...
class isys_cmdb_dao_category_g_global extends isys_cmdb_dao_category_global
{
    ...
    public static function dynamic_property_callback_cmdb_status(array $p_row)
    {
        ...
        return '<div class="cmdb-marker" style="background-color:' . isys_helper_color::unifyHexColor((string)$l_row["isys_cmdb_status__color"]) . ';"></div> ' .
    ...
    public function dynamic_property_callback_created(array $p_row)
    {
        ...
        if (!empty($p_row["isys_obj__created_by"])) {
            ...
            return '<span data-date="' . $p_row["isys_obj__created"] . '" class="hide"></span>' . $locales->fmt_date($p_row["isys_obj__created"]) . " (" . $p_row["isys_obj__created_by"] . ")";
        } else {
            return '<span data-date="' . $p_row["isys_obj__created"] . '" class="hide"></span>' . $locales->fmt_date($p_row["isys_obj__created"]);
        }
    }
    ...
    public function dynamic_property_callback_changed(array $p_row)
    {
        ...
        if (!empty($p_row["isys_obj__updated_by"])) {
            return '<span data-date="' . $p_row["isys_obj__updated"] . '" class="hide"></span>' . $locales->fmt_datetime($p_row["isys_obj__updated"]) . " (" . $p_row["isys_obj__updated_by"] . ")";
        } else {
            return '<span data-date="' . $p_row["isys_obj__updated"] . '" class="hide"></span>' . $locales->fmt_datetime($p_row["isys_obj__updated"], true, false);
        }
    }
    ...
    public function save_title($p_object_id, $p_title)
    {
        ...
        $p_title = str_replace("\n", ' ', $this->generate_unique_obj_title($p_title, (int)$p_object_id));
     ...
     * @param int    $p_catID
     * @param int    $p_purposeID
     * @param string $p_description
     * @param int    $p_obj_id
     * @param bool   $p_overwrite_sysid
     * @param int    $p_cmdb_status_id
    ...
    public function save(
        ...
        $oldObjectTitle = $this->get_obj_name_by_id_as_string($p_obj_id);
        $oldObjectTypeId = $this->get_objTypeID($p_obj_id);
        ...
        if (isys_cmdb_dao_category_g_accounting::has_placeholders($p_title) && $p_status != C__RECORD_STATUS__TEMPLATE && $p_status != C__RECORD_STATUS__MASS_CHANGES_TEMPLATE) {
            $p_title = isys_cmdb_dao_category_g_accounting::instance($this->get_database_component())
                ->replace_placeholders(
                    $p_title,
                    $p_obj_id, // @see ID-4968
                    (($p_objtype_id === null) ? $oldObjectTypeId : $p_objtype_id),
                    null,
        ...
        // @see ID-9930 Prevent line-breaks in object titles.
        $p_title = str_replace("\n", ' ', $this->generate_unique_obj_title($p_title, (int)$p_obj_id));

        $l_strSql = 'UPDATE isys_obj
        ...
        if ((!C__SYSID__READONLY && !empty($p_sysid)) || $p_overwrite_sysid) {
            $l_strSql .= 'isys_obj__sysid = ' . $this->convert_sql_text($p_sysid) . ', ';
        }

        /**
         * @see ID-8388  Only process further if the object type has changed.
         */
        if (!empty($p_objtype_id) && is_numeric($p_objtype_id) && $oldObjectTypeId != $p_objtype_id) {
            $l_create_object_types = isys_auth_cmdb_object_types::instance()->get_allowed_objecttypes(isys_auth::CREATE);

            if (in_array($p_objtype_id, $l_create_object_types)) {
                ...
                isys_notify::error(
                    sprintf(
                        ...
                        $lang->get($this->get_objtype_name_by_id_as_string($p_objtype_id))
                    )
                );
        ...
        $l_strSql .= 'isys_obj__status = ' . $this->convert_sql_int($p_status) . ' WHERE isys_obj__id = ' . $this->convert_sql_id($p_obj_id) . ';';

        $l_strSql2 = 'UPDATE isys_catg_global_list
            SET isys_catg_global_list__isys_purpose__id = ' . $this->convert_sql_id($p_purposeID) . ',
    ...
    {
        $l_catdata = $this->get_data_by_object($_GET[C__CMDB__GET__OBJECT])->get_row();
        $justCreated = (int) $l_catdata['isys_obj__status'] === C__RECORD_STATUS__BIRTH;
        ...
        if ($l_success) {
            ...
            $currentObjectStatus = (int)$l_catdata['isys_obj__status'];

            // Handle templates and potential new status.
            $newObjectStatus = (int)$this->handle_template_status($currentObjectStatus, $l_catdata["isys_obj__id"]);

            $objectId = $this->convert_sql_id($l_catdata["isys_obj__id"]);

            $isStatusBirth = C__RECORD_STATUS__BIRTH == $currentObjectStatus;
            $newStatusTemplate = C__RECORD_STATUS__TEMPLATE == $newObjectStatus;
            $newStatusMassChangeTemplate = C__RECORD_STATUS__MASS_CHANGES_TEMPLATE == $newObjectStatus;
            $autoInventoryNumber = trim(isys_tenantsettings::get('cmdb.objtype.' . $_POST["C__OBJ__TYPE"] . '.auto-inventory-no', ''));

            // Replace placeholders and update accounting inventory number
            if ($isStatusBirth && !$newStatusTemplate && !$newStatusMassChangeTemplate && $autoInventoryNumber !== '') {
                if (strpos($autoInventoryNumber, '%OBJTITLE%') !== false) {
                    $l_sql_inventory = "SELECT isys_catg_accounting_list__inventory_no
                        FROM isys_catg_accounting_list
						WHERE isys_catg_accounting_list__isys_obj__id = {$objectId};";

                    $l_inventory_no = $this
                        ->retrieve($l_sql_inventory)
                        ->get_row_value('isys_catg_accounting_list__inventory_no');

                    /*
                     * @see ID-9937 Get the current object title in order to use this in 'replace_placeholders'.
                     * Doing it manually because '$this->get_obj_name_by_id_as_string()' will result in an empty string.
                     */
                    $currentObjectTitle = $this
                        ->retrieve("SELECT isys_obj__title FROM isys_obj WHERE isys_obj__id = {$objectId} LIMIT 1;")
                        ->get_row_value('isys_obj__title');

                    $accountingDao = isys_cmdb_dao_category_g_accounting::instance($this->m_db);

                    if ($l_inventory_no) {
                        $newInventoryNumber = $this->convert_sql_text($accountingDao->replace_placeholders(
                            $l_inventory_no,
                            $l_catdata["isys_obj__id"],
                            $_POST["C__OBJ__TYPE"],
                            $currentObjectTitle ?: $_POST["C__CATG__GLOBAL_TITLE"],
                            $_POST["C__CATG__GLOBAL_SYSID"]
                        ));
                    } else {
                        $newInventoryNumber = $this->convert_sql_text($accountingDao->replace_placeholders(
                    ...
                    $l_update = "UPDATE isys_catg_accounting_list
                        SET isys_catg_accounting_list__inventory_no = {$newInventoryNumber}
                        WHERE isys_catg_accounting_list__isys_obj__id = {$objectId};";

                    $this->update($l_update);
                }
            } elseif ($isStatusBirth && ($newStatusTemplate || $newStatusMassChangeTemplate)) {
                $l_update = "UPDATE isys_catg_accounting_list
                    SET isys_catg_accounting_list__inventory_no = ''
                    WHERE isys_catg_accounting_list__isys_obj__id = {$objectId};";
                $this->update($l_update);
            }
            ...
            $objectTypeDao = isys_cmdb_dao_object_type::instance($this->m_db);

            $isOrganizationCategoryAssigned = $objectTypeDao->has_cat($this->get_objTypeID($l_catdata["isys_obj__id"]), ['C__CATS__ORGANIZATION']);
            $isPersonGroupCategoryAssigned = $objectTypeDao->has_cat($this->get_objTypeID($l_catdata["isys_obj__id"]), ['C__CATS__PERSON_GROUP']);
            $isPersonCategoryAssigned = $objectTypeDao->has_cat($this->get_objTypeID($l_catdata["isys_obj__id"]), ['C__CATS__PERSON']);

            if (!isset($_POST["C__CONTACT__ORGANISATION_TITLE"]) && $isOrganizationCategoryAssigned) {
                isys_cmdb_dao_category_s_organization_master::instance($this->m_db)
                    ->set_organization_title($l_catdata["isys_obj__id"], $_POST["C__CATG__GLOBAL_TITLE"]);
            } elseif (!isset($_POST['C__CONTACT__GROUP_TITLE']) && $isPersonGroupCategoryAssigned) {
                isys_cmdb_dao_category_s_person_group_master::instance($this->m_db)
                    ->set_person_group_title($l_catdata["isys_obj__id"], $_POST["C__CATG__GLOBAL_TITLE"]);
            } elseif (!isset($_POST['C__CONTACT__PERSON_FIRST_NAME']) && !isset($_POST['C__CONTACT__PERSON_LAST_NAME']) && $isPersonCategoryAssigned) {
            ...
            if ($justCreated) {
                // Emit just created object signal
                isys_application::instance()->container->get('signals')
                    ->emit(
                        'mod.cmdb.objectJustCreated',
                        $l_catdata['isys_obj__id'],
                        $l_catdata['isys_obj__sysid'],
                        $l_catdata['isys_obj__isys_obj_type__id'],
                        $_POST["C__CATG__GLOBAL_TITLE"],
                    );
            }
            return null;
    ...
    protected function dynamic_properties()
    {
        return [
            ...
            '_tag' => new DynamicProperty(
                'LC__CMDB__CATG__GLOBAL_TAG',
                'isys_obj__id',
                'isys_obj',
                [
                    $this,
    ...
    protected function properties()
    {
        return [
            ...
            'status' => (new DialogDataProperty(
                'C__OBJ__STATUS',
                'LC__UNIVERSAL__CONDITION',
                'isys_obj__status',
                'isys_obj',
                new isys_callback([
                ...
                Property::C__PROPERTY__DATA__SELECT   => SelectSubSelect::factory('(CASE ' .
            ...
            'created'     => array_replace_recursive(isys_cmdb_dao_category_pattern::datetime(), [
                    C__PROPERTY__INFO     => [
            ...
            'changed'     => array_replace_recursive(isys_cmdb_dao_category_pattern::datetime(), [
                C__PROPERTY__INFO     => [
                    C__PROPERTY__INFO__TITLE       => 'LC__UNIVERSAL__DATE_OF_CHANGE',
    ...
    /**
     * @param array  $data
     * @param string $prependTableField
     *
     * @return array|mixed|true
     * @throws Exception
     */
    public function validate(array $data = [], $prependTableField = false)
    {
        $existingData = [];

        if ($this->m_object_id) {
            $existingData = $this
                ->get_data(null, $this->m_object_id)
                ->get_row();
        }

        if (!$this->m_object_type_id && !empty($existingData)) {
            $this->set_object_type_id($existingData['isys_obj_type__id']);
        }

        // @see ID-10888 Perform replacement preview, necessary for API / list edit.
        if (!empty($data['title'])) {
            // @see ID-11308 Small fix for the list-edit to at least not fail in PHP errors:
            if (isset($data['title'][C__DATA__VALUE])) {
                $data['title'] = $data['title'][C__DATA__VALUE];
            }

            $data['title'] = isys_application::instance()->container->get('idoit.component.placeholder-replacer')
                ->replacePlaceholder(
                    $data['title'],
                    Config::factory(
                        $this->m_object_id,
                        $this->m_object_type_id,
                        $data['title'],
                        $existingData['isys_obj__sysid'] ?? '',
                        'isys_catg_global_list',
                        true
                    )
                );
        ...
        return parent::validate($data, $prependTableField);
    }
    ...
    public function validate_user_data()
    {
        ...
        if (trim($_POST["C__CATG__GLOBAL_TITLE"]) === '') {
            $l_arrTomAdditional["C__CATG__GLOBAL_TITLE"]["message"] = $language->get('LC__UNIVERSAL_ERROR__SELECT_NAME');
            $l_retValid = false;
        } else {
            // @see ID-10888 Check title, based on 'preview'.
            $previewTitle = isys_application::instance()->container->get('idoit.component.placeholder-replacer')->replacePlaceholder(
                $_POST["C__CATG__GLOBAL_TITLE"],
                Config::factory(
                    $l_data['isys_obj__id'],
                    $l_data['isys_obj_type__id'],
            ...
            $validationResult = $this->validate(['title' => $previewTitle]);

            // If the validation failed, we'll notify the user.
            if (isset($validationResult['title'])) {
                $l_arrTomAdditional['C__CATG__GLOBAL_TITLE']['message'] = $validationResult['title'];
                $l_retValid = false;
            }
        }
        ...
        if ($l_retValid) {
            $this->set_object_id($l_data['isys_obj__id']);
    ...
    public function getAssignedTags(array $data)
    {
        ...
        $objectId = ($data["isys_catg_global_list__isys_obj__id"] ?: ($data["isys_obj__id"] ?: null));
