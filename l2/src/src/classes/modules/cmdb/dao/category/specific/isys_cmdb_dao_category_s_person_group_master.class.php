 ...
 */
class isys_cmdb_dao_category_s_person_group_master extends isys_cmdb_dao_category_specific implements CiEventListenerInterface
{
    /**
     ...
     * @var string
     */
    protected $categoryTitle = 'LC__CONTACT__TREE__PERSON_GROUP';
    ...
    public function save($p_catlevel, $p_title, $p_mail, $p_phone, $p_ldapGroup, $p_description, $p_status = C__RECORD_STATUS__NORMAL)
    {
        ...
        $changes = [
            'isys_cats_person_group_list__status' => $this->convert_sql_int($p_status),
        ];

        if ((int) $l_old_data['isys_obj__status'] !== C__RECORD_STATUS__TEMPLATE) {
            $changes['isys_obj__status'] = $this->convert_sql_int($p_status);
        }
    ...
    {
        $existingData = $this->get_data_by_object($p_object_id);

        // @see ID-10833 Check if an entry exists, before creating another one.
        if (count($existingData) > 0) {
            return $existingData->get_row_value('isys_cats_person_group_list__id');
        }
    ...
    /**
     * @param        $p_ldapGroupSID
     * @param string $p_ldapGroupName
     *
     * @return mixed|null
     * @throws isys_exception_database
     */
    public function getGroupIDByLdapSyncID($p_ldapGroupSID, $p_ldapGroupName = '')
    {
        $sqlOR = '';
        if ($p_ldapGroupName !== '') {
            $sqlOR = " OR LCASE(isys_cats_person_group_list__ldap_group) = " . $this->convert_sql_text(strtolower($p_ldapGroupName)) . " ";
        }

        $sql = "select isys_cats_person_group_list__isys_obj__id as id
            from isys_cats_person_group_list
            where isys_cats_person_group_list__ldap_group = " . $this->convert_sql_text($p_ldapGroupSID) .
            $sqlOR .
            " limit 1";
        return $this->retrieve($sql)->get_row_value('id');
    }
    ...
    {
        if (($categoryId = defined_or_default('C__CATS__PERSON_GROUP')) === null) {
            return;
        }
        ...
        isys_cmdb_dao_category_s_person_group_master::instance(isys_application::instance()->container->get('database'))
            ->create(
                $objectId,
                $title,
                '',
    ...
    public static function getEventListeners(): EventListenerCollection
    {
        ...
        $collection->addEventListener(
            new EventListenerContainer(
                Updated::NAME,
                'onObjectUpdated',
                isys_cmdb_dao_category_s_person_group_master::class
            )
        );
