...
class isys_cmdb_dao_category_g_ldevclient extends isys_cmdb_dao_category_global
{
    ...
    /**
     * @param integer|null $p_ldevclient_id
     * @param integer|null $p_sanpool_id
    ...
    {
        $l_sql = "DELETE FROM isys_catg_ldevclient_list WHERE ";
        $conditions = [];

        if (!is_null($p_ldevclient_id)) {
            $conditions[] = " isys_catg_ldevclient_list__id = " . $this->convert_sql_id($p_ldevclient_id);
        }

        if (!is_null($p_sanpool_id)) {
            $conditions[] = " isys_catg_ldevclient_list__isys_catg_sanpool_list__id = " . $this->convert_sql_id($p_sanpool_id);
        }

        if (!is_null($p_obj_id)) {
            $conditions[] = " isys_catg_ldevclient_list__isys_obj__id = " . $this->convert_sql_id($p_obj_id);
        }
     ...
     */
    public function create($p_objID, $p_newRecStatus, $p_title, $p_sanpool_id, $p_hbaID, $p_fc_paths, $p_fc_prim, $p_multipathID, $p_description = '')
    {
    ...
    public function save($p_id, $p_newRecStatus, $p_title, $p_sanpool_id, $p_hbaID, $p_fc_paths, $p_fc_prim, $p_multipathID, $p_description)
    {
    ...
    public function attach_paths($p_id, $p_path)
    {
        ...
        if (is_countable($l_path_arr) && count($l_path_arr) > 0) {
            $l_update = "INSERT INTO isys_ldevclient_fc_port_path (
                isys_ldevclient_fc_port_path__isys_catg_ldevclient_list__id,
                isys_ldevclient_fc_port_path__isys_catg_fc_port_list__id
            ) VALUES ";
            ...
            foreach ($l_path_arr as $l_path) {
                if ((int)$l_path > 0) {
                    $inserts[] = "(" . $this->convert_sql_id($p_id) . ", " . $this->convert_sql_id($l_path) . ")";
                }
            }
