 ...
 */
class isys_cmdb_dao_category_g_ip extends isys_cmdb_dao_category_global implements ComparableCategory
{
    /**
    ...
    public function callback_property_use_standard_gateway(isys_request $p_request)
    {
        ...
        if (empty($l_catdata) && $p_request->get_category_data_id()) {
            $query = 'SELECT isys_cats_net_ip_addresses_list__isys_obj__id, isys_catg_ip_list__id FROM isys_catg_ip_list
                ...
                WHERE isys_catg_ip_list__id = ' . $this->convert_sql_int($p_request->get_category_data_id());
            $l_catdata = $this->retrieve($query)->get_row();
        }
    ...
    public function callback_property_ports(isys_request $p_request)
    {
        ...
        // @see  ID-7620  Use natural sorting.
        foreach ($l_return as &$portCollection) {
            if (is_array($portCollection) && count($portCollection)) {
                uasort($portCollection, 'strnatcasecmp');
            }
        }

        // @see API-360 Don't return nested lists in API context.
        if (Context::instance()->getOrigin() === Context::ORIGIN_API) {
            $apiReturn = [];

            if (isset($l_return[$l_port])) {
                $apiReturn = array_merge($apiReturn, $l_return[$l_port]);
            }

            if (isset($l_return[$l_log_port])) {
                $apiReturn = array_merge($apiReturn, $l_return[$l_log_port]);
            }
    ...
    /**
     * Callback method for the zone dialog-field.
     *
     * @param isys_request $p_request
     *
     * @return array
     * @throws Exception
     */
    public function callback_property_zone(isys_request $p_request): array
    {
        $zoneDao = isys_cmdb_dao_category_s_net_zone::instance(isys_application::instance()->container->get('database'));
        $zones = [];

        // @see ID-8878 use context report condition to show all available zone objects
        if (in_array(Context::instance()->getContextTechnical(), [Context::CONTEXT_REPORT_CONDITION, Context::CONTEXT_REPORT_SELECTION])) {
            $zoneResult = $zoneDao->get_data(null, null, '', null, C__RECORD_STATUS__NORMAL);
            while ($zoneData = $zoneResult->get_row()) {
                $zones[$zoneData['isys_obj__id']] = $zoneData['isys_obj__title'];
            }

            return $zones;
        }

        $data = $this->get_data(null, $p_request->get_object_id())->get_row();
        if ($data['isys_cats_net_ip_addresses_list__isys_obj__id'] > 0) {
            $l_zone_row = $zoneDao->get_data(null, $data['isys_cats_net_ip_addresses_list__isys_obj__id'], '', null, C__RECORD_STATUS__NORMAL)->get_row();

            // @see ID-10457 Do not add empty values.
            if ($l_zone_row !== null) {
                $zones[$l_zone_row['isys_obj__id']] = $l_zone_row['isys_obj__title'];
            }
        }
    ...
    public function compare_category_data(
        ...
        $checkAssignment = function ($dataset, $importData) {
            return ($importData[isys_import_handler_cmdb::C__PROPERTIES]['ipv4_assignment']['value'] == $dataset['isys_ip_assignment__id'] &&
                    $importData[isys_import_handler_cmdb::C__PROPERTIES]['ipv4_assignment']['title_lang'] == $dataset['isys_ip_assignment__title']) ||
                ($importData[isys_import_handler_cmdb::C__PROPERTIES]['ipv6_assignment']['value'] == $dataset['isys_ipv6_assignment__id'] &&
                    $importData[isys_import_handler_cmdb::C__PROPERTIES]['ipv6_assignment']['title_lang'] == $dataset['isys_ipv6_assignment__title']);
        };

        $checkIp = function ($address, $netObject, $currentDataset, $importDataset) {
            return $address === $currentDataset['isys_cats_net_ip_addresses_list__title'] && $netObject === $currentDataset['isys_cats_net_ip_addresses_list__isys_obj__id']
                && trim($importDataset[isys_import_handler_cmdb::C__PROPERTIES]['hostname']['value']) == $currentDataset['isys_catg_ip_list__hostname']
                && trim($importDataset[isys_import_handler_cmdb::C__PROPERTIES]['domain']['value']) == $currentDataset['isys_catg_ip_list__domain'];
        };
        ...
        foreach ($p_object_category_dataset as $l_dataset_key => $l_dataset) {
            ...
            $checks[] = ($l_import_ip === $l_dataset['isys_cats_net_ip_addresses_list__title'] &&
            ...
            $checks[] = ($l_import_ip === $l_dataset['isys_cats_net_ip_addresses_list__title'] &&
                $l_net_object !== $l_dataset['isys_cats_net_ip_addresses_list__isys_obj__id'] &&
                trim($p_category_data_values[isys_import_handler_cmdb::C__PROPERTIES]['hostname']['value']) == $l_dataset['isys_catg_ip_list__hostname'] &&
                trim($p_category_data_values[isys_import_handler_cmdb::C__PROPERTIES]['domain']['value']) == $l_dataset['isys_catg_ip_list__domain']);

            if ($checkIp($l_import_ip, $l_net_object, $l_dataset, $p_category_data_values)
                && $checkAssignment($l_dataset, $p_category_data_values)
            ) {
                // Entry found
                $p_comparison[isys_import_handler_cmdb::C__COMPARISON__SAME][$l_dataset_key] = $p_dataset_id;

                return;
            } elseif (in_array(true, $checks, true)) {
    ...
    public function save(
        ...
        // @see ID-10501 Trim the IP-address.
        ...
        $l_content = [
            ...
            "isys_obj__id__zone"                  => (is_numeric($p_zone_obj_id) && $p_zone_obj_id > 0) ? $p_zone_obj_id : null
        ];
        ...
        if ($p_net_type == defined_or_default('C__CATS_NET_TYPE__IPV4')) {
            ...
            if (isset($p_data[$p_key])) {
                if (is_array($p_data[$p_key])) {
                    $l_result[C__IP__ADDRESS] = trim(implode('.', $p_data[$p_key]));
                } else {
                    $l_result[C__IP__ADDRESS] = trim($p_data[$p_key]);
                }
            ...
            if (array_key_exists($p_key, $p_data)) {
                $l_result[C__IP__ADDRESS] = Ip::validate_ipv6(trim($p_data[$p_key]));
            }
        }
     ...
     * @param int    $p_id
     * @param int    $p_obj_id
     * @param bool   $p_primary_only
     * @param bool   $p_short_fields
     *
     * @return isys_component_dao_result
     * @throws isys_exception_database
     */
    public function get_ips_by_connection_table($p_table_name = "isys_catg_port_list", $p_id = null, $p_obj_id = null, $p_primary_only = false, $p_short_fields = false)
    {
        $l_query = 'SELECT * FROM isys_catg_ip_list
            ...
            INNER JOIN isys_catg_ip_list_2_' . $p_table_name . ' ON ';
        ...
        if (!empty($p_obj_id)) {
            $l_query .= ' AND isys_catg_ip_list__isys_obj__id = ' . $this->convert_sql_id($p_obj_id);
        }
        ...
        if (!empty($p_id)) {
            if (!$p_short_fields) {
                $l_query .= ' AND isys_catg_ip_list_2_' . $p_table_name . '__' . $p_table_name . '__id = ' . $this->convert_sql_id($p_id);
            } else {
                $l_query .= ' AND isys_catg_ip_list_2_' . $p_table_name . '.' . $p_table_name . '__id = ' . $this->convert_sql_id($p_id);
            }
        }
        ...
        switch ($p_context) {
            ...
            case isys_popup_browser_object_ng::C__CALL_CONTEXT__PREPARATION:
                ...
                if (!empty($p_preselection) && is_array($p_preselection)) {
                    $l_sql = "SELECT isys_obj__id, isys_obj__isys_obj_type__id, isys_catg_ip_list__id, isys_cats_net_ip_addresses_list__title, isys_obj_type__title
                        FROM isys_catg_ip_list
                        INNER JOIN isys_cats_net_ip_addresses_list ON isys_catg_ip_list__isys_cats_net_ip_addresses_list__id = isys_cats_net_ip_addresses_list__id
                    ...
                    while ($l_row = $l_res->get_row()) {
                        ...
                        $l_preselection['first'][] = $l_row['isys_obj__id'];
     ...
     *
     * @param integer $p_cat_id
     * @param integer|array $dnsServer
     *
     * @return  boolean
     * @throws isys_exception_dao
     * @author  Van Quyen Hoang <qhoang@i-doit.de>
     */
    public function attach_dns_server($p_cat_id, $dnsServer)
    {
        if (empty($p_cat_id) || empty($dnsServer)) {
            return true;
        }

        $value = '(' . $this->convert_sql_id($p_cat_id) . ', ' . $this->convert_sql_id($dnsServer) . ')';

        if (is_array($dnsServer)) {
            $categoryEntryId = $this->convert_sql_id($p_cat_id);
            array_walk($dnsServer, [$this, 'convert_sql_id']);
            $value = implode(',', array_map(function ($item) use ($categoryEntryId) {
                return '(' . $categoryEntryId . ', ' . $item . ')';
            }, $dnsServer));
        }

        $l_insert = 'INSERT INTO isys_catg_ip_list_2_isys_catg_ip_list (isys_catg_ip_list__id, isys_catg_ip_list__id__dns)
            VALUES ' . $value;
     ...
     * @return  boolean
     */
    public function attach_gateway($p_net_id, $p_net_obj_id, $p_cat_id)
    {
        $l_update = 'UPDATE isys_cats_net_list SET isys_cats_net_list__isys_catg_ip_list__id = ' . $this->convert_sql_id($p_cat_id) . ' WHERE ';

        if ($p_net_id > 0) {
            $l_update .= 'isys_cats_net_list__id = ' . $this->convert_sql_id($p_net_id);
        } elseif ($p_net_obj_id) {
            $l_update .= 'isys_cats_net_list__isys_obj__id = ' . $this->convert_sql_id($p_net_obj_id);
        } else {
            return false;
        }
     ...
     * @throws isys_exception_database
     */
    public function get_primary_ip_by_object_id(?int $objectId = null, ?int $status = C__RECORD_STATUS__NORMAL): isys_component_dao_result
    {
        $query = 'SELECT isys_catg_ip_list__id
            FROM isys_catg_ip_list
            WHERE isys_catg_ip_list__primary = 1';

        // @see ID-11404 Giving a status is optional.
        if ($status !== null) {
            $query .= " AND isys_catg_ip_list__status = {$status}";
        }

        if ($objectId !== null) {
            $query .= " AND isys_catg_ip_list__isys_obj__id = {$objectId}";
        }
            ...
            for ($i = $netAreaFrom;$i <= $netAreaTo;$i++) {
                ...
                if ($assignmentType && count($netDhcpRanges)) {
                    ...
                    if (count($netDhcpRanges)) {
                        foreach ($netDhcpRanges as $dhcpRange) {
                            if ($dhcpRange['from'] <= $i && $dhcpRange['to'] >= $i) {
                                // ID-8106 set counter to last dhcp-range
                                $i = $dhcpRange['to'];
                                continue 2;
                            }
                        }
    ...
    public function update_ip_assignment_by_ip_range($p_net_id, $p_ip_assignment, $p_from, $p_to = null)
    {
        ...
        $l_sql = 'SELECT isys_cats_net_ip_addresses_list__id
            ...
            AND isys_cats_net_ip_addresses_list__ip_address_long BETWEEN ' . $this->convert_sql_int(Ip::ip2long($p_from)) . ' AND ' . $this->convert_sql_int(Ip::ip2long($p_to)) . ';';
     ...
     * @throws isys_exception_database
     */
    public function update_primary_hostaddress(int $entryId): bool
    {
        $query = "SELECT isys_catg_ip_list__isys_obj__id
            FROM isys_catg_ip_list
            WHERE isys_catg_ip_list__id = {$entryId};";

        $objectId = (int)$this->retrieve($query)->get_row_value('isys_catg_ip_list__isys_obj__id');

        $query = "UPDATE isys_catg_ip_list
            SET isys_catg_ip_list__primary = '0'
            WHERE isys_catg_ip_list__isys_obj__id = {$objectId}
            AND isys_catg_ip_list__id != {$entryId};";
     ...
     */
    public static function unique_handling(isys_cmdb_dao $p_cmdb_dao, $p_direction, array $p_objectIDs)
    {
        $l_ips = [];
        $l_dao = isys_cmdb_dao_category_g_ip::instance($p_cmdb_dao->get_database_component());

        // @see ID-9755 Fix unmigrated code.
        $isGlobalUnique = false;

        if (class_exists(AttributeSettings::class)) {
            $result = AttributeSettings::instance($p_cmdb_dao->get_database_component())->getByCategoryConstant('C__CATG__IP');

            while ($row = $result->get_row()) {
                if (($row['propertyReference'] === 'C__CATG__IP::ipv4_address' || $row['propertyReference'] === 'C__CATG__IP::ipv6_address') && $row['uniqueGlobal']) {
                    $isGlobalUnique = true;
                }
            }
        }
        ...
        if ($p_direction == 2 && $_POST['cRecStatus'] == C__RECORD_STATUS__ARCHIVED && count($p_objectIDs)) {
                ...
                if (is_countable($l_data) && count($l_data)) {
                    while ($l_row = $l_data->get_row()) {
                        ...
                        $l_catLevel = $l_dao->in_use($l_row['isys_cats_net_ip_addresses_list__isys_obj__id'], /* Net */
                            ...
                            !$isGlobalUnique); /* GlobalNet ignore switch */

                        if ($l_catLevel) {
                            /* Here we have an ip that is detected as duplicate and needs special-handling */
                            $l_dao->update_catlevel((int)$l_row['isys_catg_ip_list__id'], C__RECORD_STATUS__ARCHIVED);
            ...
            if (count($l_ips)) {
                ...
                isys_application::instance()->container->get('notify')->error($l_strInfo);
            }
        }
    }
    ...
    {
        $ipData = $this->get_data($p_row['isys_catg_ip_list__id'], null, '', '', C__RECORD_STATUS__NORMAL)->__as_array();
        $language = isys_application::instance()->container->get('language');
        if (!empty($ipData)) {
            ...
            return $useStandardGateway === 0 ? $language->get('LC__UNIVERSAL__NO') : $language->get('LC__UNIVERSAL__YES');
        }
        return $language->get('LC__UNIVERSAL__YES');
    }
    ...
    {
        return [
            '_primary_ip' => (new DynamicProperty(
                'LC__CATG__IP__PRIMARY_ADDRESS',
            ...
            ))->mergePropertyData([C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                '',
                '',
                '',
                '',
            ...
            '_primary_hostname' => (new DynamicProperty(
                'LC__CATP__IP__PRIMARY_HOSTNAME',
                'isys_catg_ip_list__isys_obj__id',
                'isys_catg_ip_list',
                [
                    $this,
            ...
            '_fqdn' => new DynamicProperty(
                'LC__CATP__IP__HOSTNAME_FQDN',
            ...
            '_dns_domain' => new DynamicProperty(
                'LC__CMDB__CATS__NET__SEARCH_DOMAIN',
                'isys_catg_ip_list__id',
                'isys_catg_ip_list',
                [
                    $this,
            ...
            '_use_standard_gateway' => (new DynamicYesNoProperty(
                'LC__CATG__IP__DEFAULT_GATEWAY_FOR_THE_NET',
                'isys_catg_ip_list__id',
                'isys_catg_ip_list',
                [
                    $this,
            ...
            '_aliases' => (new DynamicProperty(
                'LC__CATG__IP__ALIASES',
                'isys_catg_ip_list__id',
                'isys_catg_ip_list',
                [
                    $this,
                    'retrieveHostaddressAliase'
                ]
            ))
        ];
    }
     ...
     * @return string
     */
    public function retrieveHostaddressAliase($row)
    {
        $result = $this->get_hostname_pairs($row['isys_catg_ip_list__id']);

        if ($result instanceof isys_component_dao_result && count($result)) {
            $aliases = [];
            while ($ipData = $result->get_row()) {
                $aliases[] = $ipData['isys_hostaddress_pairs__hostname'] . (!empty($ipData['isys_hostaddress_pairs__domain']) ?
                    '.' . $ipData['isys_hostaddress_pairs__domain'] : '');
            }
    ...
    {
        return [
            'primary_layer3_net'   => array_replace_recursive(isys_cmdb_dao_category_pattern::text(), [
                C__PROPERTY__INFO     => [
                    C__PROPERTY__INFO__TITLE       => 'LC__CATG__IP__LAYER3_NET',
                    C__PROPERTY__INFO__DESCRIPTION => 'Primary net title',
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT     => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT o.isys_obj__title
                              FROM isys_catg_ip_list c
                                INNER JOIN isys_cats_net_ip_addresses_list l ON c.isys_catg_ip_list__isys_cats_net_ip_addresses_list__id = l.isys_cats_net_ip_addresses_list__id
                                INNER JOIN isys_obj as o ON l.isys_cats_net_ip_addresses_list__isys_obj__id = o.isys_obj__id',
                        'isys_catg_ip_list',
                        'isys_catg_ip_list__id',
                        'isys_catg_ip_list__isys_obj__id',
                        '',
            ...
            'zone'                  => array_replace_recursive(isys_cmdb_dao_category_pattern::dialog(), [
                ...
                C__PROPERTY__UI     => [
                    ...
                    C__PROPERTY__UI__PARAMS  => [
                        'p_strTable' => 'isys_cats_net_zone_list',
                        'p_arData'   => new isys_callback([
                            'isys_cmdb_dao_category_g_ip',
                            'callback_property_zone'
                        ]),
                ...
                C__PROPERTY__DEPENDENCY => [
                    ...
                    C__PROPERTY__DEPENDENCY__SMARTYPARAMS => [
                        C__PROPERTY__DEPENDENCY__CONDITION      => 'isys_cats_net_zone_list__isys_obj__id IN (%s)',
            ...
            'ipv6_assignment'       => array_replace_recursive(isys_cmdb_dao_category_pattern::dialog(), [
                ...
                C__PROPERTY__UI       => [
                    ...
                    C__PROPERTY__UI__PARAMS => [
                        ...
                        'p_bDbFieldNN' => 1,
                        C__PROPERTY__UI__DEFAULT => defined_or_default('C__CMDB__CATG__IP__STATIC')
                    ],
                    C__PROPERTY__UI__DEFAULT => defined_or_default('C__CMDB__CATG__IP__STATIC')
                ],
                C__PROPERTY__PROVIDES => [
            ...
            'use_standard_gateway'  => array_replace_recursive(isys_cmdb_dao_category_pattern::dialog(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT      => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        ...
                        '',
                        idoit\Module\Report\SqlQuery\Structure\SelectCondition::factory([]),
                        idoit\Module\Report\SqlQuery\Structure\SelectGroupBy::factory(['isys_catg_ip_list__id'])
                    ),
                    C__PROPERTY__DATA__JOIN        => [
                        idoit\Module\Report\SqlQuery\Structure\SelectJoin::factory('isys_catg_ip_list', 'LEFT', 'isys_catg_ip_list__isys_obj__id', 'isys_obj__id'),
                        idoit\Module\Report\SqlQuery\Structure\SelectJoin::factory(
            ...
            'aliases'               => array_replace_recursive(isys_cmdb_dao_category_pattern::text(), [
                ...
                C__PROPERTY__UI       => [
                    C__PROPERTY__UI__ID     => 'C__CATP__IP__HOSTNAME_ADDITIONAL',
                ],
                C__PROPERTY__DATA     => [
                    C__PROPERTY__DATA__FIELD  => 'isys_catg_ip_list__id',
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
    ...
    public function rank_record($entryId, $direction, $table, $checkMethod = null, $purge = false)
    {
        ...
        if (class_exists(AttributeSettings::class)) {
            $result = AttributeSettings::instance($this->get_database_component())->getByCategoryConstant('C__CATG__IP');
        ...
        if ($isGlobalUnique && $direction == C__CMDB__RANK__DIRECTION_RECYCLE) {
            $row = $this->get_data($entryId)->get_row();

            if ($this->in_use($row['isys_cats_net_ip_addresses_list__isys_obj__id'], $row['isys_cats_net_ip_addresses_list__title'], $entryId)) {
                isys_notify::warning(
                    $language->get('LC__CATG__IP__UNIQUE_IP_WARNING_RECYCLE', [$row['isys_cats_net_ip_addresses_list__title'], $row['isys_obj__title'], $row['isys_obj__id']]),
                    ['sticky' => true]
                );
            }
        }
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            ...
            $customer = Context::instance()->getContextCustomer();

            // @see ID-9869 set default gateways to 0 as we want to keep the original default gateway
            if ($customer === Context::CONTEXT_DUPLICATE
                && isset($p_category_data['properties']['use_standard_gateway'][C__DATA__VALUE])
                && $p_category_data['properties']['use_standard_gateway'][C__DATA__VALUE] > 0
            ) {
                $p_category_data['properties']['use_standard_gateway'][C__DATA__VALUE] = 0;
            }

            $checkArray = [
                'ipv4_address' => 'validate_ip',
                'ipv6_address' => 'validate_ipv6',
            ];

            $l_address = '';
            foreach ($checkArray as $propertyKey => $method) {
                if (isset($p_category_data['properties'][$propertyKey]['ref_title']) &&
                    Ip::$method($p_category_data['properties'][$propertyKey]['ref_title'])
                ) {
                    // @see ID-10501 IP addresses with spaces should not be possible.
                    $l_address = trim($p_category_data['properties'][$propertyKey]['ref_title']);
                    break;
                }

                if (isset($p_category_data['properties'][$propertyKey][C__DATA__VALUE]) &&
                    Ip::$method($p_category_data['properties'][$propertyKey][C__DATA__VALUE])
                ) {
                    // @see ID-10501 IP addresses with spaces should not be possible.
                    $l_address = trim($p_category_data['properties'][$propertyKey][C__DATA__VALUE]);
                    break;
                }
            }
    ...
    private function validateIpAddress($ipType, &$ipData, &$errors)
    {
        ...
        if (empty($ipAddress) || empty($netObject)) {
            return;
        }
        ...
        $uniquePerObject = (bool)($ipCheck[C__PROPERTY__CHECK__UNIQUE_OBJ] ?? false);
        $uniquePerObjectType = (bool)($ipCheck[C__PROPERTY__CHECK__UNIQUE_OBJTYPE] ?? false);
        $uniqueGlobally = (bool)($ipCheck[C__PROPERTY__CHECK__UNIQUE_GLOBAL] ?? false);

        $sqlConditions = null;
        $sqlBase = 'SELECT isys_catg_ip_list__id AS id, isys_obj__id AS objId, isys_obj__title AS objTitle, isys_obj_type__id AS objTypeId, isys_obj_type__title AS objTypeTitle
            FROM isys_cats_net_ip_addresses_list
        ...
        if ($uniqueGlobally) {
            ...
            $sqlConditions = ';';
        } elseif ($uniquePerObjectType && $this->m_object_type_id) {
        ...
        } elseif ($uniquePerObject && $this->m_object_id) {
        ...
        while ($objectRow = $result->get_row()) {
            // This is necessary to skip the current entry (in 'update' context).
            if ($this->get_list_id() > 0 && $objectRow['id'] == $this->get_list_id()) {
                continue;
            }
     ...
     * @see    ID-8820
     */
    public function getEntriesByNet(int $objectId): isys_component_dao_result
    {
        $sql = 'SELECT
                isys_obj.isys_obj__title AS netTitle,
                isys_catg_ip_list__id AS hostIpEntryId,
                isys_catg_ip_list__isys_net_type__id AS hostIpType,
                isys_catg_ip_list__isys_obj__id AS hostObjId,
                isys_catg_ip_list__isys_catg_relation_list__id AS relationId,
                isys_cats_net_ip_addresses_list__title AS ipAddressTitle,
                isys_cats_net_ip_addresses_list__isys_ip_assignment__id AS ipAddressAssignmentId,
                isys_cats_net_ip_addresses_list__id AS ipAddressId
            FROM isys_cats_net_ip_addresses_list
            INNER JOIN isys_catg_ip_list ON isys_catg_ip_list__isys_cats_net_ip_addresses_list__id = isys_cats_net_ip_addresses_list__id
            INNER JOIN isys_obj ON isys_obj__id = isys_cats_net_ip_addresses_list__isys_obj__id
            WHERE isys_cats_net_ip_addresses_list__isys_obj__id = ' . $this->convert_sql_id($objectId) . ';';
     ...
     * @see    ID-8820
     */
    public function onObjectPurge($objectId, isys_cmdb_dao $cmdbDao)
    {
        $language = isys_application::instance()->container->get('language');
        $eventManager = isys_event_manager::getInstance();
        $relationDao = isys_cmdb_dao_category_g_relation::instance($this->m_db);
        $ipAddressDao = isys_cmdb_dao_category_s_net_ip_addresses::instance($this->m_db);
        $results = $this->getEntriesByNet($objectId);

        $fromNet = $language->get_in_text($this->get_obj_type_name_by_obj_id($objectId) . ' >> ' . $this->obj_get_title_by_id_as_string($objectId));
        $toNet = [
            C__OBJ__NET_GLOBAL_IPV4 => $language->get_in_text($this->get_obj_type_name_by_obj_id(C__OBJ__NET_GLOBAL_IPV4) . ' >> ' . $this->obj_get_title_by_id_as_string(C__OBJ__NET_GLOBAL_IPV4)),
            C__OBJ__NET_GLOBAL_IPV6 => $language->get_in_text($this->get_obj_type_name_by_obj_id(C__OBJ__NET_GLOBAL_IPV6) . ' >> ' . $this->obj_get_title_by_id_as_string(C__OBJ__NET_GLOBAL_IPV6))
        ];

        while ($row = $results->get_row()) {
            $newGlobalNet = $row['hostIpType'] == C__CATS_NET_TYPE__IPV6 ? C__OBJ__NET_GLOBAL_IPV6 : C__OBJ__NET_GLOBAL_IPV4;

            // Write a logbook entry to inform the user WHY the net was changed (also the change itself).
            $eventManager->triggerCMDBEvent(
                strtr($language->get('LC__CMDB__LOGBOOK__CHANGE_BECAUSE_OF_NET_PURGE'), [':purgedNet' => $fromNet, ':newNet' => $toNet[$newGlobalNet]]),
                '',
                $row['hostObjId'],
                $this->get_objTypeID($row['hostObjId']),
                $this->getCategoryTitle(),
                serialize(['isys_cmdb_dao_category_g_ip::net' => [
                    'from' => $fromNet,
                    'to' => $toNet[$newGlobalNet]
                ]])
            );

            // Update the IP address in the new NET
            $ipAddressDao->save(
                $row['ipAddressId'],
                $row['ipAddressTitle'],
                $newGlobalNet,
                $row['ipAddressAssignmentId'],
                2
            );

            // Handle the implicit relation towards the new NET.
