...
class isys_cmdb_dao_category_g_file extends isys_cmdb_dao_category_global
{
    ...
    {
        $this->m_category = 'file';
        $this->m_multivalued = true;

        parent::__construct($p_db);

        $this->categoryTitle = 'LC__CMDB__CATG__FILE';
        $this->m_has_relation = true;
        $this->m_entry_identifier = 'file';
        $this->m_connected_object_id_field = 'isys_connection__isys_obj__id';
        $this->m_object_id_field = 'isys_catg_file_list__isys_obj__id';
    }
    ...
    public function save_element(&$p_cat_level, &$p_intOldRecStatus)
    {
        ...
        if ($l_bNewItem) {
            $l_id = $this->create(
                ...
                str_replace('&amp;', '&', Purify::purifyValue($_POST['C__CATG__FILE_LINK'])),
    ...
    {
        // Identifier string contains the physical file ID.
        $physicalFileId = (int)$identifier;

        $query = "SELECT isys_file_physical__filename AS filename
            FROM isys_file_physical
            WHERE isys_file_physical__id = {$physicalFileId}
            LIMIT 1;";

        $fileName = isys_application::instance()->container->get('cmdb_dao')->retrieve($query)->get_row_value('filename');

        if (!$physicalFileId || !$fileName) {
            return null;
        }

        return isys_application::instance()->getOrCreateUploadFileDir($fileName) . $fileName;
    }
}
