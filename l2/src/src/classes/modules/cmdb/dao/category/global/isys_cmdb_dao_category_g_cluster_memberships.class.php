 ...
 */
class isys_cmdb_dao_category_g_cluster_memberships extends isys_cmdb_dao_category_global implements ObjectBrowserReceiver, ObjectBrowserAssignedEntries, ComparableCategory
{
    /**
        ...
        return [
            'connected_object' => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_g_cluster_members::member'
                ],
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT           => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT CONCAT(isys_obj__title, \' {\', isys_obj__id, \'}\') FROM isys_catg_cluster_members_list
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            ...
            if (($p_status === isys_import_handler_cmdb::C__CREATE || isys_import_handler_cmdb::C__UPDATE) && $p_object_id > 0) {
                $lastId = true;

                $unassignedObjects = $this->filterUnassignedEntries($p_object_id, $value);

                foreach ($unassignedObjects as $cluster) {
                    $lastId = $this->create(
                        $cluster,
