...
class isys_cmdb_dao_category_g_ui extends isys_cmdb_dao_category_global
{
     ...
     * @var string
     */
    protected $categoryTitle = 'LC__CMDB__CATG__UNIVERSAL_INTERFACE';
        ...
        return [
            ...
            'assigned_connector' => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__LINKED_PROPERTY => 'isys_cmdb_dao_category_g_ui::title'
                ],
                C__PROPERTY__DATA     => [
     ...
     * @param int    $p_plugTypeID
     * @param int    $p_connectorAheadID
    ...
    public function save(
        ...
        $l_update = "UPDATE isys_catg_ui_list
            SET isys_catg_ui_list__title = " . $this->convert_sql_text($p_title) . ",
            isys_catg_ui_list__isys_ui_con_type__id = " . $this->convert_sql_id($p_conTypeID) . ",
            isys_catg_ui_list__isys_ui_plugtype__id = " . $this->convert_sql_id($p_plugTypeID) . ",
            isys_catg_ui_list__description = " . $this->convert_sql_text($p_description) . ",
            isys_catg_ui_list__status = " . $this->convert_sql_id($p_newRecStatus) . "
            WHERE isys_catg_ui_list__id = " . $this->convert_sql_id($p_cat_level);

        if ($this->update($l_update) && $this->apply_update()) {
            $connectorId = $this->get_connector($p_cat_level);
            ...
            $success = isys_cmdb_dao_cable_connection::instance($this->m_db)
                ->handleConnectorUpdate(
                    ...
                    ($p_cableName ?: $p_title),
                    $connectorUpdates,
                    'isys_cmdb_dao_category_g_ui::assigned_connector',
     ...
     *
     * @param $p_act_ui_id Return list of specified obj_type, if NULL all obj
     *
     * @return array
     * @deprecated
     */
     ...
     *
     * @author Denis Koroliov <dkorolov@i-doit.org>
     */
    public function getRelatedObjectByInterfaceName($title, $objectId)
    {
        $query = 'SELECT * FROM isys_obj
            INNER JOIN isys_obj_type ON isys_obj_type__id = isys_obj__isys_obj_type__id
            INNER JOIN isys_catg_ui_list ON isys_catg_ui_list__isys_obj__id = isys_obj__id
            WHERE isys_obj__id = ' . $this->convert_sql_id($objectId) . '
            AND isys_catg_ui_list__title = ' . $this->convert_sql_text($title);
