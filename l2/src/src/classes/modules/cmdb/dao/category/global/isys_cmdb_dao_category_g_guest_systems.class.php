 ...
 */
class isys_cmdb_dao_category_g_guest_systems extends isys_cmdb_dao_category_global implements ObjectBrowserReceiver, ObjectBrowserAssignedEntries
{
    /**
    ...
    public function create($p_object_id, $p_status, $p_guest_systems, $p_description, $p_runs_on = null)
    {
        ...
        $l_sql = 'INSERT INTO isys_catg_virtual_machine_list
            SET isys_catg_virtual_machine_list__status = ' . $this->convert_sql_int($p_status) . ',
            isys_catg_virtual_machine_list__description = ' . $this->convert_sql_text($p_description) . ',
            isys_catg_virtual_machine_list__isys_obj__id = ' . $this->convert_sql_id($p_guest_systems) . ',
            isys_catg_virtual_machine_list__vm = 2,
            isys_catg_virtual_machine_list__isys_connection__id = ' . $this->convert_sql_id($l_dao_con->add_connection($p_object_id));
    ...
    public function save($p_id, $p_status, $p_connected_obj, $p_description, $p_runs_on = null)
    {
        if (is_numeric($p_id) && $p_id > 0) {
            ...
            $l_sql = 'UPDATE isys_catg_virtual_machine_list
                SET isys_catg_virtual_machine_list__isys_connection__id = ' . $this->convert_sql_id($this->handle_connection($p_id, $p_connected_obj)) . ',
            ...
            $l_sql .= ' WHERE isys_catg_virtual_machine_list__id = ' . $this->convert_sql_id($p_id) . ';';
    ...
    public function attachObjects($p_object_id, array $p_objects)
    {
        ...
        while ($l_row = $l_tmpDao->get_row()) {
            if (!in_array($l_row["isys_obj__id"], $p_objects)) {
                ...
                // @see ID-11890 Check if the relation object still exists BEFORE attempting to delete it.
                $object = $this->get_object($l_row['isys_catg_relation_list__isys_obj__id'])->get_row();

                if (is_array($object) && $object['isys_obj__id'] > 0) {
                    $this->delete_object($object['isys_obj__id']);
                }
            }
        ...
        return [
            'connected_object' => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_g_virtual_machine::hosts'
                ],
                C__PROPERTY__DATA     => [
                ...
                if (!empty($p_cat_ids)) {
                    ...
                    foreach ($p_cat_ids as $l_cat_id) {
                        $l_sql = 'UPDATE isys_catg_virtual_machine_list
                            ...
                            isys_catg_virtual_machine_list__status = ' . $this->convert_sql_int(C__RECORD_STATUS__NORMAL) . ',
                            isys_catg_virtual_machine_list__vm = 2,
                            isys_catg_virtual_machine_list__primary = NULL
                            WHERE isys_catg_virtual_machine_list__id = ' . $this->convert_sql_id($l_cat_id) . ';';
     ...
     * @throws isys_exception_database
     */
    private function syncHelper(int $id, int $objectId, int $status, ?int $entryId = null)
    {
        $sql = 'SELECT isys_catg_virtual_machine_list__id FROM isys_catg_virtual_machine_list
						WHERE isys_catg_virtual_machine_list__isys_obj__id = ' . $this->convert_sql_id($id);
        $result = $this->retrieve($sql);
        switch ($status) {
            case isys_import_handler_cmdb::C__CREATE:
                if ($result->num_rows() > 0) {
                    $entryId = $result->get_row_value('isys_catg_virtual_machine_list__id');
                    $this->save($entryId, C__RECORD_STATUS__NORMAL, $objectId, null, null);
            ...
            case isys_import_handler_cmdb::C__UPDATE:
                if ($result->num_rows() == 0 && $objectId > 0) {
                    return $this->create(
                        $objectId,
                        C__RECORD_STATUS__NORMAL,
                        $id,
                    ...
                    $data = $result->get_row();
                    $entryId = $data['isys_catg_virtual_machine_list__id'];
                    $description = $data['isys_catg_virtual_machine_list__description'];

                    $this->save($entryId, C__RECORD_STATUS__NORMAL, $objectId, $description, null);
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        ...
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            ...
            if (is_array($p_category_data['properties']['connected_object'][C__DATA__VALUE])) {
                foreach ($p_category_data['properties']['connected_object'][C__DATA__VALUE] as $id) {
                    $return = $this->syncHelper((int)$id, (int)$p_object_id, (int)$p_status);
                }
            } else {
                $return = $this->syncHelper((int)$p_category_data['properties']['connected_object'][C__DATA__VALUE], (int)$p_object_id, (int)$p_status, (int)$p_category_data['data_id'] ?? null);
            }
        }
    ...
    public function getAttachedEntries($id, $tag = '', $asId = false): CollectionInterface
    {
        return isys_cmdb_dao_connection::instance(isys_application::instance()->container->get('database'))
            ->getConnectedObjectsReversed('isys_catg_virtual_machine_list', $id);
    }
}
