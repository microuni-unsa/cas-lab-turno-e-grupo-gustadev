...
class isys_cmdb_dao_category_s_net_zone extends isys_cmdb_dao_category_specific
{
        ...
        return [
            ...
            'range_from'      => array_replace_recursive(isys_cmdb_dao_category_pattern::text(), [
                ...
                C__PROPERTY__DATA => [
                    ...
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT isys_cats_net_zone_list__range_from FROM isys_cats_net_zone_list',
                        'isys_cats_net_zone_list',
                        'isys_cats_net_zone_list__id',
                        'isys_cats_net_zone_list__isys_obj__id',
                        '',
            ...
            'range_to'        => array_replace_recursive(isys_cmdb_dao_category_pattern::text(), [
                ...
                C__PROPERTY__DATA => [
                    ...
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT isys_cats_net_zone_list__range_to FROM isys_cats_net_zone_list',
                        'isys_cats_net_zone_list',
                        'isys_cats_net_zone_list__id',
                        'isys_cats_net_zone_list__isys_obj__id',
                        '',
            ...
            'description'     => array_replace_recursive(isys_cmdb_dao_category_pattern::commentary(), [
                ...
                C__PROPERTY__DATA => [
                    ...
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT isys_cats_net_zone_list__description FROM isys_cats_net_zone_list',
                        'isys_cats_net_zone_list',
                        'isys_cats_net_zone_list__id',
                        'isys_cats_net_zone_list__isys_obj__id',
                        '',
     ...
     *
     * @param int    $objectId
     * @param string $ipAddressFrom
     * @param string $ipAddressTo
     * @param int    $status
     *
     * @return isys_component_dao_result|null
     * @throws isys_exception_database
     */
    public function find_range_conflicts($objectId, $ipAddressFrom, $ipAddressTo, $status = C__RECORD_STATUS__NORMAL): ?isys_component_dao_result
    {
        if (empty($ipAddressFrom) || empty($ipAddressTo)) {
            return null;
        }

        $objectId = $this->convert_sql_id($objectId);
        $status = $this->convert_sql_int($status);
        $ipAddressFrom = $this->convert_sql_int(Ip::ip2long($ipAddressFrom));
        $ipAddressTo = $this->convert_sql_int(Ip::ip2long($ipAddressTo));

        $l_sql = "SELECT * FROM isys_cats_net_zone_list
            WHERE (
                (isys_cats_net_zone_list__range_from_long BETWEEN {$ipAddressFrom} AND {$ipAddressTo}) OR 
                (isys_cats_net_zone_list__range_to_long BETWEEN {$ipAddressFrom} AND {$ipAddressTo}) OR 
                (isys_cats_net_zone_list__range_from_long <= {$ipAddressFrom} AND isys_cats_net_zone_list__range_to_long >= {$ipAddressTo}) OR 
                (isys_cats_net_zone_list__range_from_long >= {$ipAddressFrom} AND isys_cats_net_zone_list__range_to_long <= {$ipAddressTo})
            )
            AND isys_cats_net_zone_list__isys_obj__id = {$objectId}
            AND isys_cats_net_zone_list__status = {$status}
     ...
     * @throws isys_exception_database
     */
    public function get_zone_by_ip($ipAddress, $netObjectId): ?int
    {
        if (empty($ipAddress)) {
            return null;
        }

        $netObjectId = $this->convert_sql_id($netObjectId);
        $ipAddress = $this->convert_sql_int(Ip::ip2long($ipAddress));

        $sql = "SELECT isys_cats_net_zone_list__isys_obj__id__zone 
            FROM isys_cats_net_zone_list
            WHERE isys_cats_net_zone_list__isys_obj__id = {$netObjectId}
            AND {$ipAddress} BETWEEN isys_cats_net_zone_list__range_from_long AND isys_cats_net_zone_list__range_to_long;";

        $result = $this->retrieve($sql);

        if (is_countable($result) && count($result)) {
            return (int)$result->get_row_value('isys_cats_net_zone_list__isys_obj__id__zone');
        }
