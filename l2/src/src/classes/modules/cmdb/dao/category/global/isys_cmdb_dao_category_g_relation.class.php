...
class isys_cmdb_dao_category_g_relation extends isys_cmdb_dao_category_global
{
     ...
     * @throws Exception
     */
    public function dynamic_property_callback_relation_overview(array $p_row): string
    {
        $return = [];
        $quickInfo = new isys_ajax_handler_quick_info();
        $language = isys_application::instance()->container->get('language');

        // @see ID-11897 Fix the main issue by fetching the correct object ID (should exist in one of these fields).
        $objectId = (int) ($p_row['isys_catg_relation_list__isys_obj__id']
            ?? $p_row['isys_cmdb_dao_category_g_relation::dynamic_property_callback_relation_overview::isys_obj__id::LC__UNIVERSAL__RELATION_OVERVIEW']
            ?? $p_row['isys_obj__id']
            ?? $p_row['__obj_id__']
            ?? $p_row['__id__']);

        $result = $this->get_data(null, $objectId);

        while ($row = $result->get_row()) {
            $master = $row['master_title'];
            $slave = $row['slave_title'];
            $relationType = $language->get($row['isys_relation_type__master']);

            $return[] = $quickInfo->get_quick_info(
                $row["isys_obj__id"],
                "<strong>{$slave}</strong> {$relationType} <strong>{$master}</strong>",
                C__LINK__OBJECT
            );
        }
    ...
    public function dynamic_property_callback_relation(array $p_row)
    {
        ...
        $l_row = isys_cmdb_dao_category_g_relation::instance($g_comp_database)
            ->get_data_by_relation_object($p_row['isys_catg_relation_list__isys_obj__id'])
            ->get_row();
    ...
    public function create_relation(
        ...
        if ($p_obj_master > 0 && $p_obj_slave > 0) {
            ...
            $relationTitle = $this->format_relation_name(
            ...
            $l_relation_obj = $this->insert_new_obj(
                $this->getObjectTypeId('C__OBJTYPE__RELATION'),
                null,
                $relationTitle,
                null,
                $p_status
            ...
            if ($l_relation_obj > 0) {
                $l_relation_id = $this->create($l_relation_obj, $p_obj_master, $p_obj_slave, $relationType['isys_relation_type__id'], null, $p_weighting, $p_status);
    ...
    public function create(
        ...
        $l_sql = 'INSERT INTO isys_catg_relation_list
            SET isys_catg_relation_list__isys_obj__id = ' . $this->convert_sql_id($p_obj_id) . ',
            ...
            isys_catg_relation_list__isys_relation_type__id = ' . $this->convert_sql_id($p_relation_type) . ',
            isys_catg_relation_list__isys_weighting__id = ' . $this->convert_sql_id($p_weighting) . ',
    ...
    {
        return [
            '_relation' => new DynamicProperty(
                'LC__UNIVERSAL__RELATION',
            ...
            '_relation_overview' => new DynamicProperty(
                'LC__UNIVERSAL__RELATION_OVERVIEW',
                'isys_catg_relation_list__isys_obj__id',
                'isys_catg_relation_list',
                [
                    $this,
    ...
    protected function properties()
    {
        return [
            'object1'       => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    C__PROPERTY__INFO__TITLE       => 'LC__CATG__RELATION__RELATION_OBJECT_MASTER',
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT      => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        ...
                        idoit\Module\Report\SqlQuery\Structure\SelectGroupBy::factory(['isys_catg_relation_list__isys_obj__id__slave'])
                    ),
            ...
            'object2'       => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    C__PROPERTY__INFO__TITLE       => 'LC__CATG__RELATION__RELATION_OBJECT_SLAVE',
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT      => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        ...
                        '',
                        idoit\Module\Report\SqlQuery\Structure\SelectCondition::factory([]),
                        idoit\Module\Report\SqlQuery\Structure\SelectGroupBy::factory(['isys_catg_relation_list__isys_obj__id__master'])
                    ),
                    C__PROPERTY__DATA__JOIN        => [
                        idoit\Module\Report\SqlQuery\Structure\SelectJoin::factory('isys_catg_relation_list', 'LEFT', 'isys_catg_relation_list__isys_obj__id', 'isys_obj__id'),
     ...
     * @throws isys_exception_database
     */
    private function isRelationFromCustomCategory(int $id)
    {
        $result = $this->retrieve(
            'SELECT isys_catg_custom_fields_list__id FROM isys_catg_custom_fields_list
         WHERE isys_catg_custom_fields_list__isys_catg_relation_list__id = ' . $this->convert_sql_id($id) . ' Limit 1;'
        );
    ...
    public function rank_records($p_relations, $p_direction = C__CMDB__RANK__DIRECTION_DELETE, $p_table = "isys_obj", $p_checkMethod = null, $p_purge = false)
    {
        ...
        // @see ID-8955 Filter relations from custom categories
        $filteredRelations = array_filter($p_relations, fn ($id) => !$this->isRelationFromCustomCategory((int) $id));

        if (count($p_relations) !== count($filteredRelations)) {
            $diff = array_diff($p_relations, $filteredRelations);

            $diff = array_map(function ($id) {
                return $this->get_data($id)->get_row_value('isys_obj__title');
            }, $diff);

            $warning = sprintf("Following relations from custom categories could not be ranked:<br />&nbsp;&nbsp; %s
                <br />Rank these relations insider their corresponding category, please.", implode("<br />&nbsp;&nbsp;", $diff));

            isys_application::instance()->container->get('notify')->warning($warning);
        }
        ...
        foreach ($filteredRelations as $l_catid) {
        ...
        if (count($l_errors) > 0) {
            isys_application::instance()->container->get('notify')->error("The following implicit relations could not be deleted:\n&nbsp;&nbsp;" .
    ...
    {
        // @see ID-10765 Escape the object value properly.
        $parentObjectId = $this->convert_sql_id($p_parent_object);
        ...
        $l_sql = "SELECT
            ...
            isys_catg_relation_list__isys_obj__id__slave,
            isys_catg_relation_list__isys_obj__id,
            CASE isys_catg_relation_list__isys_obj__id__master
                WHEN '{$parentObjectId}' THEN isys_catg_relation_list__isys_obj__id__slave
        ...
        if (!empty($status)) {
            $l_sql .= " INNER JOIN isys_obj relatedObject ON relatedObject.isys_obj__id = CASE isys_catg_relation_list__isys_obj__id__master
                WHEN '{$parentObjectId}' THEN isys_catg_relation_list__isys_obj__id__slave
                ELSE isys_catg_relation_list__isys_obj__id__master END
                INNER JOIN isys_obj parentObject ON parentObject.isys_obj__id = {$parentObjectId}";
        }

        $l_sql .= " WHERE (isys_catg_relation_list__isys_obj__id__master = {$parentObjectId}
                OR isys_catg_relation_list__isys_obj__id__slave = {$parentObjectId})";
        ...
        if (!empty($status)) {
            $l_sql .= ' AND relation_object.isys_obj__status = ' . $this->convert_sql_id($status);
        }
    ...
    {
        [$l_master, $l_slave, $l_relation_type] = $this->get_relation_members_by_obj_id($p_object_id);
    ...
    public function delete_relation($p_cat_level)
    {
        $query = 'SELECT
            ...
            isys_catg_relation_list__isys_obj__id__master,
            isys_catg_relation_list__isys_obj__id__slave,
            relationObject.isys_obj__title as relationTitle,
            objectType.isys_obj_type__id,
            isys_relation_type__title
            FROM isys_catg_relation_list
            LEFT JOIN isys_obj as relationObject on relationObject.isys_obj__id = isys_catg_relation_list__isys_obj__id
            LEFT JOIN isys_obj as slaveObject on slaveObject.isys_obj__id = isys_catg_relation_list__isys_obj__id__slave
            LEFT JOIN isys_obj_type as objectType on slaveObject.isys_obj__isys_obj_type__id = objectType.isys_obj_type__id
            INNER JOIN isys_relation_type on isys_relation_type__id = isys_catg_relation_list__isys_relation_type__id
            WHERE isys_catg_relation_list__id = ' . $this->convert_sql_id($p_cat_level) ;

        $data = $this->retrieve($query)->get_row();

        if (is_array($data)) {
            if (isys_tenantsettings::get('logbook.relations.entries', 'initiated') === 'both') {
                $objectTypeId = $data['isys_obj_type__id'];

                $typeTranslationHack = str_replace(
                    'LC__CMDB__',
                    'C__',
                    $data['isys_relation_type__title']
                );

                $changes = [
                    $typeTranslationHack => [
                        'from' => $data['relationTitle'],
                        'to'   => '-'
                    ]
                ];
                ...
                if (Context::instance()->getContextCustomer() !== Context::CONTEXT_LDAP_SYNC) {
                    isys_event_manager::getInstance()
                        ->triggerCMDBEvent(
                            ...
                            $this->m_strLogbookSQL,
                            $data['isys_catg_relation_list__isys_obj__id__slave'],
                            $objectTypeId,
                            isys_application::instance()->container->get('language')->get($data['isys_relation_type__title']),
                            serialize($changes)
                        );
                }
            }

            return $this->delete_object($data['isys_catg_relation_list__isys_obj__id']);
        } else {
            return false;
        }
    }
     ...
     * @throws isys_exception_dao
     */
    public function set_it_service($p_id, $p_it_service): bool
    {
        $l_update = 'UPDATE isys_catg_relation_list
            SET isys_catg_relation_list__isys_obj__id__itservice = ' . $this->convert_sql_id($p_it_service) . '
            WHERE isys_catg_relation_list__id = ' . $this->convert_sql_id($p_id);
     ...
     * @throws isys_exception_dao
     */
    public function set_relation_object_status($p_id, $p_status = C__RECORD_STATUS__NORMAL): bool
    {
        $l_update = 'UPDATE isys_catg_relation_list AS rel
            INNER JOIN isys_obj AS obj ON obj.isys_obj__id = rel.isys_catg_relation_list__isys_obj__id
            SET obj.isys_obj__status = ' . $this->convert_sql_int($p_status) . '
            WHERE rel.isys_catg_relation_list__id = ' . $this->convert_sql_id($p_id);
     ...
     * @throws isys_exception_database
     */
    public function has_relation_field(string $table): bool
    {
        if (!$this->table_exists($table)) {
            return false;
        }

        if ($table === 'isys_catg_relation_list') {
            return false;
        }

        $createTableQuery = $this
            ->retrieve('SHOW CREATE TABLE ' . $table . ';')
            ->get_row_value('Create Table');

        return str_contains($createTableQuery, 'isys_catg_relation_list');
    }
     ...
     * @deprecated  Please consider using `idoit\Module\System\Model\RelationType->save()`
     */
     ...
     *
     * @return isys_cmdb_dao_category_g_relation|isys_component_dao
     */
    public static function instance(isys_component_database $p_database, ?Logger $logger = null)
    {
        ...
        // @see ID-9743 reset the relation type and field from custom categories as it could affect other categories
        if ($instance->getRelationField() === 'isys_catg_custom_fields_list__isys_catg_relation_list__id') {
            $instance->reset();
        }
        return $instance;
    }
