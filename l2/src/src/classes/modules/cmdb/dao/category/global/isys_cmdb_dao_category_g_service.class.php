...
class isys_cmdb_dao_category_g_service extends isys_cmdb_dao_category_global
{
     ...
     * @deprecated Use '$this->clearAssignedServiceAliases()'. Will be removed in i-doit 40!
     */
     ...
     * @throws isys_exception_dao
     */
    public function clearAssignedServiceAliases(int $id): void
    {
        $this->update("DELETE FROM isys_catg_service_list_2_isys_service_alias WHERE isys_catg_service_list__id = {$id};");
        $this->apply_update();
    }
    ...
    public function save_element($p_cat_level, &$p_intOldRecStatus)
    {
        ...
        // @see ID-12342 Always clear alias when saving via GUI. It will be overwritten if values exist (see $this->save()).
        $this->clearAssignedServiceAliases((int)$l_list_id);
    ...
    public function save(
        ...
        if (is_array($p_service_aliase) && count($p_service_aliase)) {
            $this->clearAssignedServiceAliases((int)$p_id);
     ...
     *
     * @author  Denis Korolov <d.korolov@i-doit.de>
     * @return  array
     */
    protected function dynamic_properties()
    {
        return [
            '_alias' => new DynamicProperty(
                'LC__CMDB__CATG__SERVICE__ALIASE',
                'isys_catg_service_list__id',
                'isys_catg_service_list',
                [
                    $this,
     ...
     * @return mixed|string
     */
    public function dynamicPropertyAlias($data)
    {
        if (empty($data['isys_catg_service_list__id'])) {
            return '';
        }
        $sql = 'SELECT sa.isys_service_alias__title AS title
                FROM isys_catg_service_list AS main
                INNER JOIN isys_catg_service_list_2_isys_service_alias AS sec ON sec.isys_catg_service_list__id = main.isys_catg_service_list__id
                INNER JOIN isys_service_alias AS sa ON sa.isys_service_alias__id = sec.isys_service_alias__id
                WHERE main.isys_catg_service_list__id = ' . $this->convert_sql_int($data['isys_catg_service_list__id']);
