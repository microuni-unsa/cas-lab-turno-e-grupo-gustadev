...
class isys_cmdb_dao_category_g_raid extends isys_cmdb_dao_category_global
{
    ...
    public function get_data($p_catg_list_id = null, $p_obj_id = null, $p_condition = "", $p_filter = null, $p_status = null)
    {
        $l_sql = 'SELECT * FROM isys_catg_raid_list
            LEFT OUTER JOIN isys_obj ON isys_catg_raid_list__isys_obj__id = isys_obj__id
    ...
    {
        return [
            '_storages' => new DynamicProperty(
                'LC__CMDB__CATG__STORAGE__CONNECTED_DEVICES',
        ...
        return [
            'raid_type'     => array_replace_recursive(isys_cmdb_dao_category_pattern::dialog(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT       => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT isys_raid_type__title
                            FROM isys_catg_raid_list
                            INNER JOIN isys_raid_type ON isys_raid_type__id = isys_catg_raid_list__isys_raid_type__id',
                        'isys_catg_raid_list',
                        'isys_catg_raid_list__id',
            ...
            'title'         => array_replace_recursive(isys_cmdb_dao_category_pattern::text(), [
                ...
                C__PROPERTY__DATA => [
                    ...
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT isys_catg_raid_list__title FROM isys_catg_raid_list',
                        'isys_catg_raid_list',
                        'isys_catg_raid_list__id',
                        'isys_catg_raid_list__isys_obj__id',
                        '',
            ...
            'raid_level'    => array_replace_recursive(isys_cmdb_dao_category_pattern::dialog(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT       => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT isys_stor_raid_level__title
                            FROM isys_catg_raid_list
                            INNER JOIN isys_stor_raid_level ON isys_stor_raid_level__id = isys_catg_raid_list__isys_stor_raid_level__id',
                        'isys_catg_raid_list',
                        'isys_catg_raid_list__id',
                        'isys_catg_raid_list__isys_obj__id',
                        '',
                    ...
                    C__PROPERTY__DATA__JOIN         => [
                        ...
                        idoit\Module\Report\SqlQuery\Structure\SelectJoin::factory(
                            'isys_stor_raid_level',
                            'LEFT',
                            'isys_catg_raid_list__isys_stor_raid_level__id',
                            'isys_stor_raid_level__id'
                        )
                    ]
                ],
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT     => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT isys_catg_controller_list__title
                            FROM isys_catg_raid_list
                            INNER JOIN isys_catg_controller_list ON isys_catg_controller_list__id = isys_catg_raid_list__isys_catg_controller_list__id',
                        'isys_catg_raid_list',
                        'isys_catg_raid_list__id',
                        'isys_catg_raid_list__isys_obj__id',
                        '',
                    ...
                    C__PROPERTY__DATA__JOIN       => [
                        ...
                        idoit\Module\Report\SqlQuery\Structure\SelectJoin::factory(
                            'isys_catg_controller_list',
                            'LEFT',
                            'isys_catg_raid_list__isys_catg_controller_list__id',
                            'isys_catg_controller_list__id'
                        )
                    ],
            ...
            'description'   => array_replace_recursive(isys_cmdb_dao_category_pattern::commentary(), [
                ...
                C__PROPERTY__DATA => [
                    ...
                    C__PROPERTY__DATA__SELECT => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT isys_catg_raid_list__description FROM isys_catg_raid_list',
                        'isys_catg_raid_list',
                        'isys_catg_raid_list__id',
                        'isys_catg_raid_list__isys_obj__id',
                        '',
    ...
    public function create(
        ...
        $l_update = "INSERT INTO isys_catg_raid_list SET
            isys_catg_raid_list__isys_obj__id = " . $this->convert_sql_id($p_objID) . ",
            isys_catg_raid_list__title = " . $this->convert_sql_text($p_title) . ",
            isys_catg_raid_list__description = " . $this->convert_sql_text($p_description) . ",
            isys_catg_raid_list__status = " . $this->convert_sql_id($p_newRecStatus) . ",
            isys_catg_raid_list__isys_stor_raid_level__id = " . $this->convert_sql_id($p_raidLevelID) . ",
            isys_catg_raid_list__isys_catg_controller_list__id = " . $this->convert_sql_id($p_controllerID) . ",
            isys_catg_raid_list__isys_raid_type__id = " . $this->convert_sql_id($p_raid_typeID);

        if ($this->update($l_update) && $this->apply_update()) {
            $l_last_id = $this->get_last_insert_id();

            $l_dao_stor = isys_cmdb_dao_category_g_stor::instance($this->get_database_component());
            $l_dao_drive = isys_cmdb_dao_category_g_drive::instance($this->get_database_component());
            ...
            if ($p_raid_typeID == 1) {
                if (!empty($p_connectedDevices)) {
                    ...
                    foreach ($l_dev_array as $l_device__id) {
                        $deviceId = (int)$l_device__id;

                        $l_dao_stor->add_raid_to_item($deviceId, $l_last_id);
                        $this->writeDeviceChangeLogbook($deviceId, null, $p_title);
                    }
                }
                ...
                if (!empty($p_connectedDrives)) {
                    ...
                    foreach ($l_drive_array as $l_drive__id) {
                        $driveId = (int)$l_drive__id;

                        $l_dao_drive->add_raid_to_item($driveId, $l_last_id);
                        $this->writeDriveChangeLogbook($driveId, null, $p_title);
                    }
                }
            }
    ...
    public function save(
        ...
        $entryId = (int)$entryId;
        $objectId = (int)$this->get_data($entryId)->get_row_value('isys_obj__id');

        $l_update = "UPDATE isys_catg_raid_list SET
            isys_catg_raid_list__title = " . $this->convert_sql_text($p_title) . ",
            isys_catg_raid_list__description = " . $this->convert_sql_text($p_description) . ",
            isys_catg_raid_list__status = " . $this->convert_sql_id($p_newRecStatus) . ",
            isys_catg_raid_list__isys_stor_raid_level__id = " . $this->convert_sql_id($p_raidLevelID) . ",
            isys_catg_raid_list__isys_catg_controller_list__id = " . $this->convert_sql_id($p_controllerID) . ",
            isys_catg_raid_list__isys_raid_type__id = " . $this->convert_sql_id($p_raid_typeID) . "
            WHERE isys_catg_raid_list__id = " . $this->convert_sql_id($entryId);
        ...
        // Fetch all assigned stores and drives.
        $assignedDevices = $this->getAssignedDevices($objectId, $entryId);
        $assignedDrives = $this->getAssignedDrives($objectId, $entryId);

        // Detach all stores and drives
        $l_dao_stor->detach_raid($entryId);
        $l_dao_drive->detach_raid($entryId);
        ...
        if (!empty($p_connectedDevices)) {
            ...
            foreach ($l_connected_arr as $l_device__id) {
                ...
                if (!isset($assignedDevices[$deviceId])) {
                    $this->writeDeviceChangeLogbook($deviceId, null, $p_title);
                }

                $l_main->add_raid_to_item($deviceId, $entryId);
                unset($assignedDevices[$deviceId]);
            }
            ...
            if (!empty($p_connectedDrives)) {
                ...
                foreach ($l_drive_array as $l_drive__id) {
                    ...
                    if (!isset($assignedDrives[$driveId])) {
                        $this->writeDriveChangeLogbook($driveId, null, $p_title);
                    }

                    $l_dao_drive->add_raid_to_item($driveId, $entryId);
                    unset($assignedDrives[$driveId]);
                }
            }
        }

        foreach ($assignedDevices as $id => $title) {
            $this->writeDeviceChangeLogbook($id, $p_title, '');
        }

        foreach ($assignedDrives as $id => $title) {
            $this->writeDriveChangeLogbook($id, $p_title, '');
        }
     ...
     * @throws isys_exception_database
     */
    private function writeDeviceChangeLogbook(int $deviceId, ?string $from, string $to): void
    {
        if ($from === null) {
            $query = "SELECT isys_catg_raid_list__title AS title
                FROM isys_catg_stor_list
                LEFT JOIN isys_catg_raid_list ON isys_catg_raid_list__id = isys_catg_stor_list__isys_catg_raid_list__id
                WHERE isys_catg_stor_list__id = {$deviceId}
                LIMIT 1;";

            $from = $this->retrieve($query)->get_row_value('title');
        }

        // Get object and object type.
        $query = "SELECT isys_obj__id AS objectId, isys_obj__isys_obj_type__id AS objectTypeId
            FROM isys_catg_stor_list
        ...
        isys_event_manager::getInstance()
            ->triggerCMDBEvent(
                ...
                $row['objectTypeId'],
                'LC__STORAGE_DEVICE',
                serialize(['isys_cmdb_dao_category_g_stor::raid_group' => ['from' => $from, 'to' => $to]])
            );
    }

    /**
     * @param int $objectId
     * @param int $raidEntryId
     *
     * @return array
     * @throws isys_exception_database
     */
    private function getAssignedDevices(int $objectId, int $raidEntryId): array
    {
        $devices = [];
        $query = "SELECT isys_catg_stor_list__id AS id, isys_catg_stor_list__title AS title
            FROM isys_catg_stor_list
            WHERE isys_catg_stor_list__isys_obj__id = {$objectId}
            AND isys_catg_stor_list__isys_catg_raid_list__id = {$raidEntryId}";

        $result = $this->retrieve($query);

        while ($row = $result->get_row()) {
            $devices[(int)$row['id']] = $row['title'];
        }
     ...
     * @throws isys_exception_database
     */
    private function writeDriveChangeLogbook(int $driveId, ?string $from, string $to): void
    {
        if ($from === null) {
            $query = "SELECT isys_catg_raid_list__title AS title
                FROM isys_catg_drive_list
                LEFT JOIN isys_catg_raid_list ON isys_catg_raid_list__id = isys_catg_drive_list__id__raid_pool
                WHERE isys_catg_drive_list__id = {$driveId}
                LIMIT 1;";
        ...
        isys_event_manager::getInstance()
            ->triggerCMDBEvent(
                ...
                $row['objectTypeId'],
                'LC__STORAGE_DRIVE',
                serialize(['isys_cmdb_dao_category_g_drive::assigned_raid' => ['from' => $from, 'to' => $to]])
            );
    }
     ...
     * @throws isys_exception_database
     */
    private function getAssignedDrives(int $objectId, int $raidEntryId): array
    {
        $devices = [];
        $query = "SELECT isys_catg_drive_list__id AS id, isys_catg_drive_list__title AS title
            FROM isys_catg_drive_list
            WHERE isys_catg_drive_list__isys_obj__id = {$objectId}
            AND isys_catg_drive_list__id__raid_pool = {$raidEntryId}";
    ...
    /**
     * @param int|null $p_raid_id
     * @param int|null $p_raid_type
