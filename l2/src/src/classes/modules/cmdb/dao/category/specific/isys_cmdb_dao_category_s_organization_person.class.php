 ...
 */
class isys_cmdb_dao_category_s_organization_person extends isys_cmdb_dao_category_specific implements ObjectBrowserReceiver, ObjectBrowserAssignedEntries
{
    /**
     ...
     * @var string
     */
    protected $categoryTitle = 'LC__CMDB__CATS__ORGANIZATION_PERSONS';
     ...
     * @var string
     */
    protected $m_connected_object_id_field = 'isys_cats_person_list__isys_obj__id';
    ...
    {
        $objId = ($row['isys_cats_organization_list__isys_obj__id'] ?: ($row['isys_obj__id'] ?: null));
    ...
    {
        return [
            '_object_as_list' => new DynamicProperty(
                'LC__CMDB__CATS__ORGANIZATION_PERSONS_LIST',
        ...
        return [
            'object'  => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_s_person_master::organization'
                ],
                C__PROPERTY__DATA     => [
    ...
    public function rank_records($p_objects, $p_direction = C__CMDB__RANK__DIRECTION_DELETE, $p_table = "isys_obj", $p_checkMethod = null, $p_purge = false)
    {
        if (!empty($p_objects)) {
            foreach ($p_objects as $objectId) {
                $this->detachPerson((int)$objectId);
            }
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        ...
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            if ($p_status === isys_import_handler_cmdb::C__CREATE || $p_status === isys_import_handler_cmdb::C__UPDATE) {
                $value = is_array($p_category_data['properties']['object'][C__DATA__VALUE]) ? $p_category_data['properties']['object'][C__DATA__VALUE] :
                    [$p_category_data['properties']['object'][C__DATA__VALUE]];

                // Save category data:
                $l_indicator = $this->save($p_object_id, $value);
            }
        }
    ...
    public function attachObjects($p_object_id, array $p_objects)
    {
        ...
        while ($l_row = $l_res->get_row()) {
            ...
            if (!in_array($l_row['isys_obj__id'], $p_objects)) {
                // @see ID-10718 Use proper method to detach person and remove the relation.
                $this->detachPerson((int)$l_row['isys_obj__id']);
            }
        }
     ...
     * @throws isys_exception_dao
     */
    public function detachPerson(int $objectId): void
    {
        $daoConnection = isys_cmdb_dao_connection::instance($this->m_db);
        $personRow = isys_cmdb_dao_category_s_person_master::instance($this->m_db)->get_data_by_object($objectId)->get_row();

        if ($daoConnection->update_connection($personRow['isys_cats_person_list__isys_connection__id'], null)) {
            isys_cmdb_dao_category_g_relation::instance($this->m_db)->delete_relation($personRow["isys_cats_person_list__isys_catg_relation_list__id"]);
        }
    }

    public function attach_person($p_id, $p_orga_id)
    {
        $l_dao = isys_cmdb_dao_connection::instance($this->m_db);
        $l_data = isys_cmdb_dao_category_s_person_master::instance($this->m_db)
            ->get_data_by_object($p_id)
            ->get_row();

        $l_return = true;
        if ($l_data["isys_cats_person_list__id"] > 0) {
            $l_sql = "UPDATE isys_cats_person_list
                SET isys_cats_person_list__isys_connection__id = " . $this->convert_sql_id($l_dao->add_connection($p_orga_id)) . "
            ...
            // @see ID-8760 Show a notification why certain objects could not be attached.
            $personData = $this->get_object($p_id)->get_row();
            $language = isys_application::instance()->container->get('language');

            $message = strtr(
                $language->get('LC__CMDB__CATS__ORGANIZATION_ATTACHEMENT_FAILED'),
                [':objectId' => $p_id, ':objectTypeTitle' => $language->get($personData['isys_obj_type__title']), ':objectTitle' => $language->get($personData['isys_obj__title'])]
            );
