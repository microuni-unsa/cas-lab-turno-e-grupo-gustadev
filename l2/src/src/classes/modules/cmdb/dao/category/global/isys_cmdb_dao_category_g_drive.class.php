 ...
 */
class isys_cmdb_dao_category_g_drive extends isys_cmdb_dao_category_global implements ComparableCategory
{
    /**
    ...
    protected function dynamic_properties()
    {
        return [
            ...
            '_free_space' => (new DynamicProperty(
                'LC__CMDB__CATG__DRIVE__FREE_SPACE',
                'isys_catg_drive_list__id',
                'isys_catg_drive_list',
                [
                    $this,
            ...
            '_used_space' => (new DynamicProperty(
                'LC__CMDB__CATG__DRIVE__USED_SPACE',
                'isys_catg_drive_list__id',
                'isys_catg_drive_list',
                [
                    $this,
            ...
            '_on_device'  => new DynamicProperty(
                'LC__CATD__DRIVE_DEVICE',
                'isys_catg_drive_list__id',
                'isys_catg_drive_list',
                [
                    $this,
    ...
    public function retrieveOnDevice(array $categoryData)
    {
        ...
        switch ($categoryData["isys_catg_drive_list__const"]) {
            case 'C__CATG__RAID':
                ...
                $property = isys_cmdb_dao_category_g_raid::instance(isys_application::instance()->container->get('database'))
                    ->get_property_by_key('title');
                break;
            case 'C__CATG__LDEV_CLIENT':
                $id = $categoryData["isys_catg_drive_list__isys_catg_ldevclient_list__id"];
                $property = isys_cmdb_dao_category_g_ldevclient::instance(isys_application::instance()->container->get('database'))
                    ->get_property_by_key('title');
                break;
            case 'C__CATG__STORAGE':
                $id = $categoryData["isys_catg_drive_list__isys_catg_stor_list__id"];
                $property = isys_cmdb_dao_category_g_stor::instance(isys_application::instance()->container->get('database'))
                    ->get_property_by_key('title');
                break;
            default:
                return isys_tenantsettings::get('gui.empty_value', '-');
    ...
    {
        $capacity = new MemoryProperty(
            'C__CATD__DRIVE_CAPACITY',
            'LC__CMDB_CATG__MEMORY_CAPACITY',
            'isys_catg_drive_list__capacity',
            'isys_catg_drive_list',
            'unit'
        );
        $capacity->getData()->setCondition(function (isys_cmdb_dao_list_objects $dao, $name, $value) {
            ...
            foreach ($multiplier as $unitMultiplier) {
                $ceiled  =(int)(ceil($value * $unitMultiplier / 100) * 100);
                $conditions[] = "{$column} LIKE '%," . $ceiled . ",%'";
            }
        ...
        return [
            ...
            'filesystem'      => array_replace_recursive(isys_cmdb_dao_category_pattern::dialog_plus(), [
                C__PROPERTY__INFO => [
                    C__PROPERTY__INFO__TITLE       => 'LC__CATD__DRIVE_FILESYSTEM',
            ...
            'raid'            => array_replace_recursive(isys_cmdb_dao_category_pattern::dialog(), [
                ...
                C__PROPERTY__DATA     => [
                    C__PROPERTY__DATA__SOURCE_TABLE => 'isys_catg_raid_list',
                    C__PROPERTY__DATA__FIELD_ALIAS  => 'raid_list_unit',
                    C__PROPERTY__DATA__TABLE_ALIAS  => 'rl_units',
            ...
            'free_space'      => array_replace_recursive(isys_cmdb_dao_category_pattern::float(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__CONDITION => function (isys_cmdb_dao_list_objects $dao, $name, $value) {
                        $multiplier = [1024, 1024**2, 1024**3, 1024**4];
                        $column = $dao->get_database_component()
                            ->escapeColumnName($name);
                        $value = (int)$value;
                        $conditions = [];
            ...
            'used_space'      => array_replace_recursive(isys_cmdb_dao_category_pattern::float(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__CONDITION => function (isys_cmdb_dao_list_objects $dao, $name, $value) {
                        ...
                        foreach ($multiplier as $unitMultiplier) {
                            $ceiled  =(int)(ceil($value * $unitMultiplier / 100) * 100);
                            $floored  =(int)(floor($value * $unitMultiplier / 100) * 100);
                            $org  = $value * $unitMultiplier;
                            $conditions[] = "{$column} LIKE '%," . $ceiled . ",%'";
                            $conditions[] = "{$column} LIKE '%," . $floored . ",%'";
                            $conditions[] = "{$column} LIKE '%," . $org . ",%'";
                        }
            ...
            'firmware'     => new TextProperty(
                'C__CATG__DRIVE_FIRMWARE',
                'LC__CMDB__CATG__DRIVE__FIRMWARE',
                'isys_catg_drive_list__firmware',
                'isys_catg_drive_list'
            ),
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            $categoryData = $p_category_data['properties'];
            if (!isset($categoryData['category_const'][C__DATA__VALUE])) {
                $categoryData['category_const'][C__DATA__VALUE] = !empty($categoryData['device'][C__DATA__VALUE]) ?
                    'C__CATG__STORAGE' : (
                        !empty($categoryData['raid'][C__DATA__VALUE]) ?
                        'C__CATG__RAID' : (
                            !empty($categoryData['ldev'][C__DATA__VALUE]) ?
                            'C__CATG__LDEV_CLIENT' : null
                        )
                    );
            }
                ...
                case isys_import_handler_cmdb::C__CREATE:
                    if ($p_object_id > 0) {
                        return $this->create(
                            ...
                            $categoryData['drive_type'][C__DATA__VALUE],
                            $categoryData['mount_point'][C__DATA__VALUE],
                            $categoryData['title'][C__DATA__VALUE],
                            $categoryData['filesystem'][C__DATA__VALUE],
                            $categoryData['capacity'][C__DATA__VALUE],
                            $categoryData['unit'][C__DATA__VALUE],
                            $categoryData['device'][C__DATA__VALUE],
                            $categoryData['raid'][C__DATA__VALUE],
                            null,
                            $categoryData['ldev'][C__DATA__VALUE],
                            $categoryData['category_const'][C__DATA__VALUE],
                            $categoryData['assigned_raid'][C__DATA__VALUE],
                            $categoryData['description'][C__DATA__VALUE],
                            $categoryData['system_drive'][C__DATA__VALUE],
                            $categoryData['serial'][C__DATA__VALUE],
                            $categoryData['free_space'][C__DATA__VALUE],
                            $categoryData['free_space_unit'][C__DATA__VALUE],
                            $categoryData['used_space'][C__DATA__VALUE],
                            $categoryData['used_space_unit'][C__DATA__VALUE],
                            $categoryData['firmware'][C__DATA__VALUE]
                        );
                    }
                    break;
    ...
    public function catdata_browser($p_obj_id)
    {
        ...
        $memoryDao = isys_cmdb_dao_dialog::instance($this->m_db)->set_table('isys_memory_unit');
        ...
        while ($l_row = $l_res->get_row()) {
            ...
            $l_capacity = isys_convert::memory($l_row["isys_catg_drive_list__capacity"], $l_row["isys_catg_drive_list__isys_memory_unit__id"], C__CONVERT_DIRECTION__BACKWARD);

            if ($l_capacity > 0) {
                $memoryData = $memoryDao->get_data($l_row["isys_catg_drive_list__isys_memory_unit__id"]);
                $l_val .= ', ' . $l_capacity . ' ' . $memoryData['title'];
            }
    ...
    public function create(
        ...
        $l_update = "INSERT INTO isys_catg_drive_list SET
            ...
            isys_catg_drive_list__capacity = " . $this->convert_sql_text($p_capacity) . ",
            isys_catg_drive_list__firmware = " . $this->convert_sql_text($p_firmware) . ",
            ...
            isys_catg_drive_list__free_space = " . $this->convert_sql_text($p_free_space) . ",
            isys_catg_drive_list__free_space__isys_memory_unit__id = " . $this->convert_sql_id($p_free_space_unit) . ",
            isys_catg_drive_list__used_space = " . $this->convert_sql_text($p_used_space) . ",
