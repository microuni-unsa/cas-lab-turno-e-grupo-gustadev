...
use idoit\Module\Cmdb\Component\TitleSuffixCounter\SuffixReplacer;
...
class isys_cmdb_dao_category_g_connector extends isys_cmdb_dao_category_global
{
    ...
    public function callback_property_type(isys_request $p_request)
    {
        ...
        return [
            C__CONNECTOR__INPUT  => $lang->get('LC__CATG__CONNECTOR__INPUT'),
            C__CONNECTOR__OUTPUT => $lang->get('LC__CATG__CONNECTOR__OUTPUT')
        ];
    }
     ...
     * @throws Exception
     */
    public function get_assigned_category_title(string $constant): string
    {
        $title = 'LC__CMDB__CATG__CONNECTORS';

        if (defined($constant)) {
            $title = $this->get_catg_name_by_id_as_string(constant($constant));
        }
    ...
    public function save_element(&$p_cat_level, &$p_intOldRecStatus, $p_create)
    {
        ...
        if ($p_create) {
            ...
            $suffixReplacer = new SuffixReplacer();
            $suffixReplacer->prepareSuffixByPost('C__CATG__CONNECTOR', 'C__UNIVERSAL__TITLE');

            // If there are more than 1 connector being created then do not assign the cable because it is not possible in the current cabling model
            if ($_POST['C__CATG__CONNECTOR__SUFFIX_COUNT'] > 1) {
                $l_cableID = null;
            }

            // Iterate through connector creation.
            for ($i = 0; $i < $_POST["C__CATG__CONNECTOR__SUFFIX_COUNT"]; $i++) {
                $title = $suffixReplacer->generateCurrentTitle();
                ...
                if ($_POST["C__CATG__CONNECTOR__CREATE_APPLICABLE_OUTPUTS"] && $_POST['C__CATG__CONNECTOR__INOUT'] == C__CONNECTOR__INPUT) {
                    ...
                    $siblingTitle = str_replace("##INPUT##", $title, $l_schema);

                    // Check whether default counting mode is active
                    if ($_POST['C__CATG__CONNECTOR__SUFFIX_SCHEMA'] == '-1') {
                        // Replace ##COUNT## placeholder by extracted value
                        $siblingTitle = str_replace("##COUNT##", $suffixReplacer->getGeneratedSuffixCounter(), $siblingTitle);
                    }
    ...
    public function save(
        ...
        $l_dao->handle_cable_connection_detachment(
            (int)$p_list_id,
        ...
        $this->detach_rx_everywhere((int)$p_used_fiber_lead_tx);
        $this->detach_tx_everywhere((int)$p_used_fiber_lead_rx);
        ...
        $connectorUpdates = [
            ...
            'isys_catg_connector_list__isys_catg_connector_list__id' => $this->convert_sql_id($p_connector_sibling),
            'isys_connection__isys_obj__id'  => $this->convert_sql_id($p_connected_obj_id),
        ...
        $success = $l_dao->handleConnectorUpdate(
            $p_list_id,
            $p_is_master_obj,
            $p_connected_connector_id,
            $p_cableID,
    ...
    public function create(
        ...
        $p_used_fiber_lead_rx = (int)$p_used_fiber_lead_rx;
        $p_used_fiber_lead_tx = (int)$p_used_fiber_lead_tx;
        ...
        if ($this->update($l_strSql) && $this->apply_update()) {
            ...
            } elseif (!empty($p_cableID) && ($cableConnectionFromCable = $l_dao->detachCable($p_cableID)) !== false) {
                $l_dao->update_cable_connection_connector($l_id, $cableConnectionFromCable);
            }
    ...
    {
        // @see ID-9140 Don't apply this condition in print-view.
        if (Context::instance()->getContextCustomer() === Context::CONTEXT_EXPORT_PRINTVIEW) {
            return '';
        }
     ...
     * @return  boolean
     */
    public function pre_rank($p_list_id, $p_direction = null, $p_table = 'isys_catg_connector_list')
    {
        $l_sql = 'UPDATE isys_catg_connector_list
			SET isys_catg_connector_list__isys_catg_connector_list__id = NULL
			WHERE isys_catg_connector_list__id = ' . $this->convert_sql_id($p_list_id) . '
     ...
     * @return  isys_component_dao_result
     */
    public function get_connector_by_name_as_result($p_title, $objectID = null, $connectorType = null)
    {
        $condition = ' WHERE isys_catg_connector_list__title = ' . $this->convert_sql_text($p_title);
        if ($objectID !== null) {
            $condition .= ' AND isys_catg_connector_list__isys_obj__id = ' . $this->convert_sql_id($objectID);
        }
        if ($connectorType !== null) {
            $condition .= ' AND isys_catg_connector_list__type = ' . $this->convert_sql_id($connectorType);
        }
        return $this->retrieve('SELECT * FROM isys_catg_connector_list' . $condition . ';');
    }
    ...
    /**
     * Get Assigned object by connector ID
     *
     * @param $connector_id
     *
     * @return isys_component_dao_result
     ...
     */
    public function getAssignedObjectByConnectorId($connector_id)
    {
        $l_sql = "SELECT
            isys_obj__id AS id,
            isys_obj__id AS objID,
            isys_obj__title AS title,
            isys_obj_type__title AS type,
            connected_connector.isys_catg_connector_list__title AS connector,
            connected_connector.isys_catg_connector_list__id AS connectorID
			FROM isys_catg_connector_list
			...
			WHERE isys_catg_connector_list.isys_catg_connector_list__id = " . $this->convert_sql_int($connector_id) . '
			    AND isys_catg_connector_list.isys_catg_connector_list__status = ' . $this->convert_sql_int(C__RECORD_STATUS__NORMAL) . '
    ...
    public function dynamic_property_callback_assigned_category($p_row)
    {
        if (!empty($p_row['isys_catg_connector_list__assigned_category'])) {
            ...
            switch ($p_row['isys_catg_connector_list__assigned_category']) {
                case 'C__CATG__POWER_SUPPLIER':
                    return $language->get('LC__CMDB__CATG__POWER_CONSUMER');

                case 'C__CATG__CONNECTOR':
                    return $language->get('LC__CMDB__CATG__CONNECTORS');

                case 'C__CATG__NETWORK_PORT':
                case 'C__CMDB__SUBCAT__NETWORK_PORT': // @todo  Remove in i-doit 1.12
                    return $language->get('LC__CMDB__CATG__NETWORK_TREE_CONFIG_PORT');

                case 'C__CATG__CONTROLLER_FC_PORT':
                    return $language->get('LC__STORAGE_FCPORT');

                case 'C__CATG__UNIVERSAL_INTERFACE':
                    return $language->get('LC__CMDB__CATG__UNIVERSAL_INTERFACE');
            }
        }
    ...
    public function dynamic_property_callback_fiber_wave_lengths($p_row)
    {
        if (empty($p_row['isys_catg_connector_list__isys_connection__id'])) {
            return isys_tenantsettings::get('gui.empty_value', '-');
        }

        $query = 'SELECT isys_catg_connector_list__id AS id
            FROM isys_catg_connector_list
            WHERE isys_catg_connector_list__isys_connection__id = ' . $this->convert_sql_id($p_row['isys_catg_connector_list__isys_connection__id']) . '
            LIMIT 1';

        $entryId = (int)$this->retrieve($query)
            ->get_row_value('id');
        ...
        $result = $this->get_assigned_fiber_wave_lengths(null, $entryId);
        ...
        while ($row = $result->get_row()) {
            $wavelenths[] = $language->get($row['isys_fiber_wave_length__title']);
        }

        return implode(', ', $wavelenths);
    }

    protected function dynamic_properties()
    {
        return [
            '_assigned_category'  => new DynamicProperty(
                'LC__CATG__CONNECTOR__CATEGORY_TYPE',
            ...
            '_fiber_wave_lengths' => new DynamicProperty(
                'LC__CATG__CONNECTOR__FIBER_WAVE_LENGTHS',
                'isys_catg_connector_list__isys_connection__id',
                'isys_catg_connector_list',
                [
                    $this,
                    'dynamic_property_callback_fiber_wave_lengths'
                ]
            )
        ];
    }
        ...
        return [
            ...
            'fiber_wave_lengths' => array_replace_recursive(isys_cmdb_dao_category_pattern::multiselect(), [
                ...
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__SELECT       => idoit\Module\Report\SqlQuery\Structure\SelectSubSelect::factory(
                        'SELECT isys_catg_connector_list__id FROM isys_catg_connector_list AS c',
                        'isys_catg_connector_list',
                        ...
                        '',
                        null,
                        idoit\Module\Report\SqlQuery\Structure\SelectGroupBy::factory(['c.isys_catg_connector_list__isys_obj__id'])
                    )
                ],
                C__PROPERTY__UI       => [
                    ...
                    C__PROPERTY__UI__PARAMS  => [
                        ...
                        'placeholder'  => isys_application::instance()->container->get('language')->get('LC__CATG__CONNECTOR__FIBER_WAVE_LENGTHS'),
            ...
            'assigned_connector' => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__LINKED_PROPERTY => 'isys_cmdb_dao_category_g_connector::title'
                ],
                C__PROPERTY__DATA     => [
                    C__PROPERTY__DATA__FIELD       => 'isys_catg_connector_list__id',
    ...
    public function sync($p_category_data, $p_object_id, $p_status = 1 /* isys_import_handler_cmdb::C__CREATE */)
    {
        if (is_array($p_category_data) && isset($p_category_data['properties'])) {
            ...
            if (Context::instance()->getContextCustomer() !== Context::CONTEXT_DUPLICATE) {
            ...
            if (!empty($p_category_data['properties']['fiber_wave_lengths'][C__DATA__VALUE]) && !is_array($p_category_data['properties']['fiber_wave_lengths'][C__DATA__VALUE])) {
                ...
                } elseif (is_numeric($p_category_data['properties']['fiber_wave_lengths'][C__DATA__VALUE])) {
                    $p_category_data['properties']['fiber_wave_lengths'][C__DATA__VALUE] = [$p_category_data['properties']['fiber_wave_lengths'][C__DATA__VALUE]];
                }
            }
    ...
    private function recurse_cable_run($p_connector_id, $p_data = [])
    {
        ...
        if ($l_assigned_connector['isys_catg_connector_list__id'] > 0 &&
            ...
            isys_application::instance()->container->get('notify')->warning($l_exception_message, ['life' => 8]);
     ...
     *
     * @param int $fiberLeadRxId
     *
     * @return void
     * @throws isys_exception_dao
     */
    private function detach_rx_everywhere(int $fiberLeadRxId): void
    {
        if ($fiberLeadRxId > 0) {
            $l_sql = 'UPDATE isys_catg_connector_list
                SET isys_catg_connector_list__used_fiber_lead_rx = NULL
                WHERE isys_catg_connector_list__used_fiber_lead_rx = ' . $this->convert_sql_id($fiberLeadRxId) . ';';
     ...
     *
     * @param int $fiberLeadTxId
     *
     * @return void
     * @throws isys_exception_dao
     */
    private function detach_tx_everywhere(int $fiberLeadTxId): void
    {
        if ($fiberLeadTxId > 0) {
            $l_sql = 'UPDATE isys_catg_connector_list
                SET isys_catg_connector_list__used_fiber_lead_tx = NULL
                WHERE isys_catg_connector_list__used_fiber_lead_tx = ' . $this->convert_sql_id($fiberLeadTxId) . ';';
     ...
     * @return false|mixed
     */
    public static function getConnectorNameByConnectorId(int $connectorId, $asString = true)
    {
        try {
            $language = isys_application::instance()->container->get('language');
            ...
            $sql = 'SELECT
                    ...
                    isys_obj__title AS title,
                    isys_obj_type__title AS type,
                    isys_catg_connector_list__title AS connector,
                ...
                WHERE isys_catg_connector_list__id = ' . $dao->convert_sql_id($connectorId) .
                ' LIMIT 1;';
            $data = $dao->retrieve($sql)->get_row();

            return $asString ? $language->get($data['type']) . $separator . $data['title'] . $separator . $data['connector'] : $data;
        } catch (\Exception $ex) {
            return false;
        }
    }
