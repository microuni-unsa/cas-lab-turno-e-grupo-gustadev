 ...
 */
class isys_cmdb_dao_category_s_group extends isys_cmdb_dao_category_specific implements ObjectBrowserReceiver, ObjectBrowserAssignedEntries
{
    /**
    ...
    {
        $this->m_category = 'group';
        $this->m_multivalued = true;

        parent::__construct($database);

        $this->categoryTitle = 'LC__OBJTYPE__GROUP';
        $this->m_connected_object_id_field = 'isys_connection__isys_obj__id';
        $this->m_entry_identifier = 'object';
        $this->m_has_relation = true;
        $this->m_object_browser_category = true;
        $this->m_object_browser_property = 'object';
        $this->m_object_id_field = 'isys_cats_group_list__isys_obj__id';
    }
        ...
        return [
            'object'      => array_replace_recursive(isys_cmdb_dao_category_pattern::object_browser(), [
                C__PROPERTY__INFO     => [
                    ...
                    C__PROPERTY__INFO__BACKWARD_PROPERTY => 'isys_cmdb_dao_category_g_group_memberships::connected_object'
                ],
                C__PROPERTY__DATA     => [
                    ...
                    C__PROPERTY__DATA__RELATION_HANDLER => new isys_callback([
                        ...
                        'isys_cmdb_dao_category_s_group',
                        false // @see ID-11955 Do not swap master/slave.
                    ]),
    ...
    {
        $l_indicator = false;
        if (is_array($p_category_data) && $p_status === isys_import_handler_cmdb::C__CREATE && isset($p_category_data['properties']['object'][C__DATA__VALUE])) {
            $objects = $p_category_data['properties']['object'][C__DATA__VALUE];

            // @see ID-9484 Convert the passed object to always be an array.
            if (!is_array($objects)) {
                $objects = [$objects];
            }

            $unassignedObjects = $this->filterUnassignedEntries($p_object_id, $objects);
            ...
            foreach ($unassignedObjects as $object) {
                $p_category_data['data_id'] = $this->create($p_object_id, C__RECORD_STATUS__NORMAL, $object, '');
    ...
    public function attachObjects($object_id, array $objects)
    {
        ...
        if (is_array($objects)) {
            $l_dao = $this->get_data(null, $object_id);
            while ($l_row = $l_dao->get_row()) {
                if (!in_array($l_row["isys_connection__isys_obj__id"], $objects)) {
            ...
            foreach ($objects as $l_popup_object) {
                if (!isset($l_members[$l_popup_object]) && !$l_members[$l_popup_object]) {
                    $l_id = $this->create($object_id, C__RECORD_STATUS__NORMAL, $l_popup_object, "");
                }
     ...
     */
    public function create($objectId, $status, $connectedObjectId, $description)
    {
        $connectionId = isys_cmdb_dao_connection::instance($this->m_db)->add_connection($connectedObjectId);

        $insertQuery = 'INSERT INTO isys_cats_group_list SET
            isys_cats_group_list__description = ' . $this->convert_sql_text($description) . ',
            isys_cats_group_list__isys_obj__id = ' . $this->convert_sql_id($objectId) . ',
            isys_cats_group_list__isys_connection__id = ' . $this->convert_sql_id($connectionId) . ',
            isys_cats_group_list__status = ' . $this->convert_sql_id($status) . ';';

        if ($this->update($insertQuery) && $this->apply_update()) {
            $this->m_strLogbookSQL .= $insertQuery;

            $entryId = $this->get_last_insert_id();

            (new isys_cmdb_dao_category_g_relation($this->get_database_component()))
                ->handle_relation(
                    $entryId,
                    "isys_cats_group_list",
                    defined_or_default('C__RELATION_TYPE__GROUP_MEMBERSHIPS'),
                    $this->get_data($entryId)->get_row_value('isys_cats_group_list__isys_catg_relation_list__id'),
                    $objectId, // @see ID-9956 Group as master.
                    $connectedObjectId // @see ID-9956 Assigned object as slave.
                );
