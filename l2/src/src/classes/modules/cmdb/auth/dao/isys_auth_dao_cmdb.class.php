...
class isys_auth_dao_cmdb extends isys_auth_module_dao
{
     ...
     */
    public static function objectRelatedAuthenticationRules(isys_cmdb_dao $cmdbDao, $direction, array $objectIds)
    {
        ...
        if (count($objectIds) > 0) {
            ...
            foreach ($objectIds as $objectId) {
                try {
                    $currentStatus = $cmdbDao->get_object_status_by_id($objectId);
                    ...
                    // ID-8079 rank all auth paths to the corresponding object
                    if ($currentStatus > 0) {
                        $update = 'UPDATE isys_auth SET isys_auth__status = ' . $cmdbDao->convert_sql_int($currentStatus) .
                            ' WHERE isys_auth__isys_obj__id = ' . $cmdbDao->convert_sql_id($objectId);

                        $cmdbDao->update($update);
                        $affectedRows += $cmdbDao->affected_after_update();
                        $cmdbDao->apply_update();
                    }
                } catch (Exception $e) {
