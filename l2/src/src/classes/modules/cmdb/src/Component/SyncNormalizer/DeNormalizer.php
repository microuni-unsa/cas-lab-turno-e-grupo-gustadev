...
class DeNormalizer extends AbstractNormalizer
{
    ...
    public function normalizeData(int $objectId, ?int $entryId = null): array
    {
        ...
        $categoryData = [];
        $allowedProperties = self::getAllowedProperties(get_class($dao));
        ...
        $data = array_filter(
            $data,
            fn ($key) => (isset($properties[$key]) && $properties[$key]->getProvides()->isImport()) || in_array($key, $allowedProperties),
        ...
        foreach ($data as $propertyKey => &$value) {
            ...
            $dataNormalizerProvider->denormalizeData($propertyKey, $data, $value);
        }
