...
class DateType extends AbstractType implements TypeInterface
{
    ...
    public function isApplicable(Property $property, string $tag, ?string $class)
    {
        ...
        return $property->getInfo()->getType() === Property::C__PROPERTY__INFO__TYPE__DATE &&
    ...
    private function dataHandlerHelper($currentObjectId, $currentPropertyTag, $oldValue, $newValue)
    {
        ...
        if ($oldValue !== '') {
            $oldValue = isys_application::instance()->container->get('locales')->fmt_date(date('Y-m-d', $oldValue));
        }

        if ($newValue !== '') {
            $newValue = isys_application::instance()->container->get('locales')->fmt_date(date('Y-m-d', $newValue));
        }
