...
class CiMatcher extends AbstractMatcher implements Matchable
{
     ...
     * @var CiIdentifiers
     */
    protected CiIdentifiers $filters;

    /**
     * @return CiIdentifiers
     */
    public function getFilters(): CiIdentifiers
    {
        return $this->filters;
    }
    ...
    public function match(array $matchKeywords)
    {
        ...
        if (strlen($unionSelect) > 0 && trim($unionSelect) !== '') {
            ...
            while ($row = $resultSet->get_row()) {
                ...
                $matchResult[] = MatchData::factory()
                    ->setId($row['id'])
                    ->setTitle($row['title']);
            }
        }

        return MatchData::factory()
            ->setId($objectid)
            ->setTitle($title)
    ...
    public function __construct(MatchConfig $config)
    {
        ...
        $this->filters = new CiIdentifiers($config->getFilter());
    }
}
