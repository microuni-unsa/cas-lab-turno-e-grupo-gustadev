...
class MatchDao extends Base
{
     ...
     */
    public function getMatchProfileByTitle(string $title)
    {
        $sql = 'SELECT * FROM isys_obj_match WHERE isys_obj_match__title = ' . $this->convert_sql_text($title);
     ...
     * @param int|null $filterMinMatch
     *
     * @return bool
     * @throws \isys_exception_dao
     * @author   Van Quyen Hoang <qhoang@i-doit.com>
     */
    public function saveMatchProfile($id, $title, $bits, $minMatch, $filter = null, $filterMinMatch = null)
    {
        $sql = 'UPDATE isys_obj_match SET
          ...
          isys_obj_match__filter = ' . $this->convert_sql_int($filter) . ',
          isys_obj_match__filter_min = ' . $this->convert_sql_int($filterMinMatch) . '
     ...
     * @param $minMatch
     * @param $filter
     * @param $filterMinMatch
     *
     * @return bool
     * @throws \isys_exception_dao
     * @author   Van Quyen Hoang <qhoang@i-doit.com>
     */
    public function createMatchProfile($title, $bits, $minMatch, $filter = null, $filterMinMatch = null)
    {
        return $this->update(
            <<<SQL
            INSERT INTO isys_obj_match (
                isys_obj_match__title,
                isys_obj_match__bits,
                isys_obj_match__min_match,
                isys_obj_match__filter,
                isys_obj_match__filter_min
            ) VALUES (
                {$this->convert_sql_text($title)},
                {$this->convert_sql_int($bits)},
                {$this->convert_sql_int($minMatch)},
                {$this->convert_sql_int($filter)},
                {$this->convert_sql_int($filterMinMatch)}
            )
            SQL
        );
    }
