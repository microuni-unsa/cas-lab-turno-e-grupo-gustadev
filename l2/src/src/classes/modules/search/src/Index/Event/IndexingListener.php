...
class IndexingListener implements EventSubscriberInterface
{
    ...
    public function onError(GenericEvent $event)
    {
        ...
        // Fixing an issue when passing the trace directly to 'writeln'.
        $this->output->writeln(['<error>There was a problem</error>', $exception->getMessage()], OutputInterface::VERBOSITY_DEBUG);

        foreach ($exception->getTrace() as $i => $trace) {
            $this->output->writeln(
                '<info>#' . $i . '</info>: ' . $trace['class'] . $trace['type'] . $trace['function'] . ' (in <comment>' . $trace['file'] . '</comment>)',
