...
class TenantExist implements Step, Undoable
{
    ...
    {
        $exists = $this->dao
            ->retrieve("SELECT COUNT(1) AS exist FROM isys_mandator WHERE isys_mandator__db_name = '{$this->dbName}' OR isys_mandator__title = '{$this->title}';")
            ->get_row_value('exist') > 0;

        $messages->addMessage(new StepMessage($this, $exists ? 'exists' : 'does not exist', $exists ? ErrorLevel::ERROR : ErrorLevel::INFO));
