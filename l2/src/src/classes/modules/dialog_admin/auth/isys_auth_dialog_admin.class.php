...
class isys_auth_dialog_admin extends isys_auth implements isys_auth_interface
{
        ...
        switch ($p_method) {
            case 'dialog_tables':
                ...
                foreach ($l_tables as $l_table) {
                    $l_data[$l_table] = strip_tags(isys_application::instance()->container->get('language')->get($l_table));
                }
            ...
            case 'custom_dialog_tables':
                ...
                // Bring the tables in the needed syntax.
                foreach ($l_tables as $l_table) {
                    $l_data[strtolower($l_table['identifier'])][] = $l_table['fieldTitle'];
                }

                foreach ($l_data as $key => $value) {
                    $l_data[$key] = implode(', ', $value)  . ' (' . $key . ')';
                }
     ...
     * @throws Exception
     */
    public function custom($right, $identifier)
    {
        if (!$this->is_auth_active()) {
            return true;
        }
        ...
        // @see ID-10054
        return $this->check_module_rights($right, 'custom', strtolower($identifier), new isys_exception_auth(isys_application::instance()->container->get('language')
            ->get('LC__AUTH__DIALOG_ADMIN_EXCEPTION__MISSING_RIGHT_FOR_DIALOG_IDENTIFIER', [
                isys_auth::get_right_name($right),
                $identifier
            ])));
    }
}
