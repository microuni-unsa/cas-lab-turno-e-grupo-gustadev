...
use idoit\Component\Helper\Purify as Purify;
...
class isys_helper_link
{
     ...
     * @return string
     */
    public static function get_base(bool $forceHttps = false): string
    {
        // @see ID-9951 Always check if HTTPS should be used.
        $useHttps = $forceHttps || (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on');

        // @see ID-4952  It is now possible to define a i-doit URL in the settings.
        if ('' !== trim($systemBase = Environment::getBaseUri())) {
            ...
            return ($useHttps ? str_replace('http://', 'https://', $url) : $url);
        }
        ...
        return 'http' . ($useHttps ? 's' : '') . '://' . $host . $g_config['www_dir'];
    }

    public static function getCurrentDomain(bool $forceHttps = false): string
    {
        global $g_config;
        $useHttps = $forceHttps || (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on');
        $host = $_SERVER['HTTP_HOST'];
    ...
    {
        [$l_base, $l_path] = explode('?', $p_url);

        // This function works like "http_build_query()" - but backwards.
        parse_str((string)$l_path, $l_path_array);

        if (empty($l_path_array)) {
            return $l_base;
        }
    ...
    public static function get_url_variables($p_obj_id)
    {
        ...
        return [
            ...
            '%objectname_formatted%'           => Purify::formatUrlPart($l_obj_data['isys_obj__title']),
            '%objectname_lowercase_formatted%' => strtolower(Purify::formatUrlPart($l_obj_data['isys_obj__title'])),
            '%objectname_uppercase_formatted%' => strtoupper(Purify::formatUrlPart($l_obj_data['isys_obj__title'])),
            ...
            '%model_title%'                    => $l_model_data['isys_model_title__title'],
            '%model_manufacturer%'             => $l_model_data['isys_model_manufacturer__title'],
    ...
    /**
     * Method for formatting a URL parameter: "Peter Griffin! äöüß" -> "Peter-Griffin-aeoeuess".
     *
     * @param string $parameter
     *
     * @return string
     * @deprecated Please use 'Purify::formatUrlPart()'
     */
    public static function format_url_param($parameter): string
    {
        return Purify::formatUrlPart($parameter);
    }
