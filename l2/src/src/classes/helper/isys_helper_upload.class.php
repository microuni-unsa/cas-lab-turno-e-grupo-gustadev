...
class isys_helper_upload
{
     ...
     * @author  Leonard Fischer <lfischer@i-doit.org>
     */
    public static function save(array $p_file, $p_filename = null, $p_directory = null, $p_chmod = 0664)
    {
        ...
        $l_filename = realpath($p_directory) . '/' . $p_filename;
     ...
     *
     * @return string
     * @deprecated Please use 'Purify::formatFilename' instead.
     */
    public static function prepare_filename(string $filename)
    {
        return Purify::formatFilename($filename);
    }
     ...
     * @return string
     */
    public static function append_hash_prefix($filename): string
    {
        $prefix = substr(md5($filename . microtime(true)), 0, 8);

        return Purify::formatFilename("{$prefix}_{$filename}");
    }
}
