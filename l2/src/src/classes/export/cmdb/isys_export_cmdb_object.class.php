...
class isys_export_cmdb_object extends isys_export_cmdb
{
     ...
     */
    public function export($p_object_ids, $p_categories = [], $p_record_status = C__RECORD_STATUS__NORMAL, $p_duplicate = false, $p_merge = false, $p_overview_only = false, bool $skipEmptyValues = false)
    {
        Context::instance()
        ...
        try {
            ...
            foreach ($l_object_ids as $l_object_id) {
                ...
                $this->m_export[$l_object_id] = $this->export_categories($l_object_id, $p_categories, $p_record_status, $p_duplicate, $p_merge, $p_overview_only, $skipEmptyValues);
     ...
     * @return  array
     */
    public function export_properties($p_object_properties, $p_row, $p_cattype, $p_disable_provides = false, bool $skipEmptyValues = false)
    {
        $l_data = [];
        ...
        foreach ($p_object_properties as $l_tag => $l_property) {
            ...
            if (!isset($l_property[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK]) || empty($l_property[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK])) {
                // No helper class.
                ...
                if (class_exists($l_property[C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][0])) {
                    ...
                    if ($l_helper instanceof isys_global_custom_fields_export_helper) {
                        $l_helper->setSkipEmptyValues($skipEmptyValues);
                    }
            ...
            if ($skipEmptyValues) {
                ...
                if (is_string($l_new['value']) && trim($l_new['value']) === '') {
                    continue;
                }
            }
     ...
     * @return  array
     */
    private function export_categories($p_object_id, $p_categories, $p_record_status = C__RECORD_STATUS__NORMAL, $p_duplicate = false, $p_merge = false, $p_overview_only = false, bool $skipEmptyValues = false)
    {
        ...
        foreach ([
            ...
            try {
                if ($l_cattype == C__CMDB__CATEGORY__TYPE_GLOBAL) {
                    $l_all_categories = $l_cat->get_categories_as_array($l_object_type, $p_object_id, $l_cattype, C__RECORD_STATUS__NORMAL, $p_overview_only);
            ...
            if (is_array($l_all_categories) && count($l_all_categories)) {
                foreach ($l_all_categories as $l_category_id => $l_data) {
                    ...
                    if (is_countable($p_categories[$l_cattype]) && count($p_categories[$l_cattype]) !== 0) {
                        ...
                        if ($l_cattype == C__CMDB__CATEGORY__TYPE_GLOBAL && $p_categories && !isset($p_categories[$l_cattype][$l_category_id])) {
                            continue;
                        }
                    } else {
                        // Skip category
                    ...
                    if (!$l_category_dao instanceof isys_cmdb_dao_category) {
                        continue;
                    }
                    ...
                    if (Context::instance()->getContextCustomer() === Context::CONTEXT_EXPORT_PRINTVIEW && $l_category_dao instanceof Printable) {
                        ...
                        // @see ID-8634 in some cases the status is being checked on isys_obj instead of the category table
                        if ($l_dataresult->num_rows() <= 0) {
                            $l_dataresult = $l_category_dao->get_data(null, $p_object_id, $l_condition, null, C__RECORD_STATUS__TEMPLATE);
                        }
                    }
                    ...
                    if ($l_cattype == C__CMDB__CATEGORY__TYPE_CUSTOM) {
                        $report = $l_category_dao->getReport($p_object_id);

                        if ($report !== false && Context::instance()->getGroup() !== Context::CONTEXT_GROUP_TEMPLATE) {
                            if ($report !== null) {
                                $l_output[$l_cattype][$l_category_id]['head']['title'] = $report['isys_report__title'];
                                ...
                                while ($l_row = $l_dataresult->get_row()) {
                                    ...
                                    foreach ($l_row as $key => $value) {
                                        $l_output[$l_cattype][$l_category_id][$id][] = [
                                            'title' => $key,
                                            'value' => $value,
                                            'tag'   => strtolower($key)
                                        ];
                            ...
                            foreach ($l_merged_arr as $l_data_key_id => $l_merge_test) {
                                $l_prop_export = $this->export_properties($l_category_data_information, $l_merge_test, $l_cattype, false, $skipEmptyValues);
    ...
    public function __construct($p_export_type = 'isys_export_type_xml', isys_component_database|null $p_database = null)
    {
        self::initSkippedCategories();
