...
class isys_specific_net_export_helper extends isys_export_helper
{
    ...
    public function exportGateway_import($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            $result = $daoIpAddress->get_ips_by_obj_id(
                $objectId,
                false,
                false,
                null,
                ' AND isys_cats_net_ip_addresses_list__title = ' . $daoIpAddress->convert_sql_text($data['ref_title'])
            );
    ...
    public function exportDnsServer_import($importData)
    {
        ...
        foreach ($data as $value) {
            ...
            $result = $daoIp->get_ips_by_obj_id(
                $objectId,
                false,
                false,
                null,
                ' AND isys_cats_net_ip_addresses_list__title = ' . $daoIp->convert_sql_text($value['ref_title'])
            );
