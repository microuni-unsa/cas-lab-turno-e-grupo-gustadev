...
class isys_export_type_xml extends isys_export_type
{
        ...
        if (isset($p_data['title'])) {
            ...
            foreach ($p_data as $l_dkey => $l_dval) {
                ...
                if (!is_null($l_dval) && $l_dval != "") {
                    if (!is_numeric($l_dkey)) {
                        $l_xml .= " " . $l_dkey . "=\"" . htmlspecialchars($l_dval, ENT_COMPAT, BASE_ENCODING) . "\"";
                    } else {
                        $l_xml .= " key-" . $l_dkey . "=\"" . htmlspecialchars($l_dval, ENT_COMPAT, BASE_ENCODING) . "\"";
                    }
                }
            }
            ...
            if ($l_sub_tag) {
                ...
                if (trim($l_sub_tag . $p_tabs) == '') {
                    $l_sub_string = htmlspecialchars($l_title, ENT_COMPAT, BASE_ENCODING);
                }
                $l_xml .= ' title="' . htmlspecialchars($l_title, ENT_COMPAT, BASE_ENCODING) . '">';
                $l_xml .= $l_sub_string;
            } else {
                ...
                if (isset($p_title)) {
                    $l_xml .= ' title="' . htmlspecialchars($p_title, ENT_COMPAT, BASE_ENCODING) . '"';
                }
                ...
                if (strstr($l_title, "CDATA")) {
                    ...
                    $l_xml .= htmlspecialchars($l_title, ENT_COMPAT, BASE_ENCODING);
                }
            }
            $l_xml .= "</" . $p_tag . ">\n";
        } else {
                ...
                if (is_object($l_val) && method_exists($l_val, "get_data")) {
                    ...
                    if (in_array($l_key, $this->m_special_leveled)) {
                        ...
                        if (is_array($l_data[0]) && count($l_data[0]) > 0) {
                            ...
                            $l_xml .= " >";
                            $l_xml .= htmlspecialchars($l_dtitle, ENT_COMPAT, BASE_ENCODING);
                            $l_xml .= "</" . $l_key . ">";
                        }
                    }
                    ...
                    $l_xml .= "\n" . $p_tabs . "<" . $l_key . ">" . htmlspecialchars($l_val, ENT_COMPAT, BASE_ENCODING) . "</" . $l_key . ">";
                }
            }
            $l_xml .= $p_tabs . "\n";
    ...
    public function parse($p_array, $p_export_format = null, $p_translate = false)
    {
        ...
        $l_xml .= "\t<head>\n" . "\t\t<datetime>" . date("Y-m-d H:i:s") . "</datetime>\n" . "\t\t<mandator language=\"" . htmlentities($session->get_language(), ENT_COMPAT, BASE_ENCODING) .
            "\" id=\"" . intval($session->get_mandator_id()) . "\">" . $this->cdata($session->get_mandator_name()) . "</mandator>\n" . "\t\t<type>" .
            get_class($this) . "</type>\n";
    ...
    public function cdata($p_value)
    {
        ...
        return htmlspecialchars($p_value, ENT_COMPAT, BASE_ENCODING);
    }
        ...
        foreach ($p_sub as $l_catid => $l_catdata) {
            ...
            if (is_array($l_cathead) && count($l_cathead) > 0) {
                foreach ($l_cathead as $l_cathead_key => $l_cathead_value) {
                    if ($l_cathead_key === 'title') {
                        if ($p_translate === true) {
                            $l_cathead_value = $language->get($l_cathead_value);
                        }
                    }

                    // Somehoe the key could be an integer 0 has maybe something to do with VirtualProperties in properties
                    if ($l_cathead_key === 0) {
                        continue;
                    }

                    $l_xml .= ' ' . $l_cathead_key . '="' . htmlspecialchars($l_cathead_value ?? '', ENT_COMPAT, BASE_ENCODING) . '"';
                }
            }
            ...
            foreach ($l_catdata as $l_category_id => $l_properties) {
                ...
                foreach ($l_properties as $l_property) {
                    $l_title = null;
                    $l_tag = preg_replace('/\W/', '', $l_property[C__DATA__TAG]);
                    ...
                    switch (gettype($l_property[C__DATA__VALUE])) {
                        case 'string':
                            ...
                            if ($isBrowserFirefox && empty($_POST)) {
                                ...
                                $l_property[C__DATA__VALUE] = html_entity_decode(htmlspecialchars($l_property[C__DATA__VALUE]), ENT_COMPAT, BASE_ENCODING);
                            }
                            ...
                            $l_xml .= '>';
                            if ($l_tag == 'description' || strstr($l_property[C__DATA__VALUE], "\n")) {
                                $l_xml .= '<![CDATA[' . $l_property[C__DATA__VALUE] . ']]>';
                            } else {
                                $l_xml .= htmlspecialchars($l_property[C__DATA__VALUE], ENT_COMPAT, BASE_ENCODING);
                            }
                            $l_xml .= '</' . $l_tag . ">\n";
                            break;
                                ...
                                case 'isys_export_data':
                                    ...
                                    if (is_array($l_data) && count($l_data) > 0) {
                                        foreach ($l_data as $l_key => $l_item) {
                                            if (is_numeric($l_key)) {
                                                if (!$l_parent) {
                                                    $l_xml .= "\t\t\t\t\t\t" . "<" . $l_tag . " title=\"" . htmlspecialchars($l_title, ENT_COMPAT, BASE_ENCODING) . "\">\n";
                                                }

                                                $l_parent = true;

                                                $l_xml .= $this->iterate_dialog('sub_' . $l_tag, $l_item, "\t\t\t\t\t\t\t", $l_title);
                                            } else {
                                                $l_key = preg_replace('/\W/', '', $l_key);
                                                $l_xml .= "\t\t\t\t\t\t" . '<' . $l_key;
                        ...
                        case 'boolean':
                        default:
                            if (strpos($l_tag, 'html_') === 0) {
                                if ($isBrowserFirefox && empty($_POST)) {
                                    // @see  ID-826  Fixes the print view in firefox. The print view is a pure GET request without POST data.
                                    $l_title = html_entity_decode(htmlspecialchars($l_title), ENT_COMPAT, BASE_ENCODING);
                                }
                                $l_xml .= "\t\t\t\t\t\t";
                                $l_xml .= '<' . $l_tag . '>';
                                $l_xml .= '<![CDATA[' . $l_title . ']]>';
                ...
                if (!is_array(($l_pval))) {
                    ...
                    if (isset($l_pval["attributes"])) {
                        foreach ($l_pval["attributes"] as $l_akey => $l_aval) {
                            ...
                            if (!empty($l_aval)) {
                                $l_attributes .= " " . $l_akey . "=\"" . htmlspecialchars($l_aval, ENT_COMPAT, BASE_ENCODING) . "\"";
                            }
                        }
