        ...
        try {
            switch ($this->m_get['func']) {
                case 'create_zone_range':
                    $l_return['data'] = $this->create_zone_range(
                        $this->m_get[C__CMDB__GET__OBJECT],
                        substr($this->m_post['type'], 9),
                        $this->m_post['from'],
                        $this->m_post['to']
                    );
                    break;
    ...
    protected function retrieve_zone_ranges($p_net_object)
    {
        ...
        while ($l_zone_row = $l_zone_res->get_row()) {
            ...
            $l_return[] = [
                ...
                'color'   => isys_helper_color::unifyHexColor((string)$l_zone_options['isys_catg_net_zone_options_list__color']),
