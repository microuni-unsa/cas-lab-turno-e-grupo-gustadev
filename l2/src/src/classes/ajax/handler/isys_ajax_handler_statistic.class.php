...
class isys_ajax_handler_statistic extends isys_ajax_handler
{
    ...
    protected function getRackStatistics($rackObjectId)
    {
        ...
        while ($row = $result->get_row()) {
            ...
            if ($row['isys_catg_location_list__option'] == C__RACK_INSERTION__HORIZONTAL && $row['isys_catg_location_list__insertion'] != null &&
                ...
                } elseif ($row['isys_catg_location_list__insertion'] == C__INSERTION__REAR) {
     ...
     */
    protected function get_consumption_of_electricity($objectId)
    {
        $l_return = [
            'watt' => 0,
            'btu'  => 0
        ...
        $recordStatus = $dao->convert_sql_int(C__RECORD_STATUS__NORMAL);
        $objectId = $dao->convert_sql_id($objectId);

        $l_sql = "SELECT isys_catg_pc_list__watt, isys_catg_pc_list__btu, content_obj.isys_obj__id, isys_catg_pc_list__active, content_type.isys_obj_type__container
			...
			AND rack_obj.isys_obj__id = {$objectId}
			AND content_obj.isys_obj__status = {$recordStatus}
			AND isys_catg_pc_list__status = {$recordStatus};";

        $l_res = $dao->retrieve($l_sql);

        while ($l_row = $l_res->get_row()) {
            if ($l_row['isys_catg_pc_list__active']) {
                $l_return['watt'] += (float)$l_row['isys_catg_pc_list__watt'];
                $l_return['btu'] += (float)$l_row['isys_catg_pc_list__btu'];
            }
    ...
    public function getOwnConsumptionOfElectricity($objectId)
    {
        ...
        $sql = "SELECT SUM(isys_catg_pc_list__watt) AS total_watt, SUM(isys_catg_pc_list__btu) AS total_btu
            FROM isys_obj
            INNER JOIN isys_catg_location_list location ON location.isys_catg_location_list__isys_obj__id = isys_obj__id
            INNER JOIN isys_catg_pc_list ON isys_catg_pc_list__isys_obj__id = isys_obj__id
            WHERE location.isys_catg_location_list__status = {$recordStatus}
            AND isys_obj__id = {$objectId}
            AND isys_catg_pc_list__status = {$recordStatus} 
            AND isys_catg_pc_list__active = 1;";
