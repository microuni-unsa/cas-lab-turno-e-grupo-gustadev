...
class isys_protocol_soap extends isys_protocol
{
     ...
     */
    public static function get_instance(string $host = 'localhost', int $port = 80, string $protocol = self::C__HTTP): static
    {
    ...
    public function request($p_method, $p_params = [])
    {
        ...
        if (isset($this->m_options['stream_context'])) {
            $l_options['stream_context'] = $this->m_options['stream_context'];
        }
    ...
    public function __construct($p_host, $p_port, $p_protocol)
    {
        ...
        $this->m_options['user_agent'] = 'i-doit ' . $g_product_info['version'];
    }
}
