...
abstract class isys_exception extends Exception
{
    ...
    public function get(Exception $exception)
    {
        return isys_application::instance()->container->get('template')
            ...
            ->assign('g_error', $exception->getMessage())
    ...
    {
        $log = Logger::factory('exception', BASE_DIR . '/log/exception.log');

        // Also put the stack trace in the log.
        $logMessage = [
            $this->getMessage(),
        ...
        foreach ($this->getTrace() as $frame) {
            ...
            $logMessage[] = '- ' . $frame['class'] . $frame['type'] . $frame['function'];
            $logMessage[] = "  in file: {$frame['file']} (line: {$frame['line']})";
        }
