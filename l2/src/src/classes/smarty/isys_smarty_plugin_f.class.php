...
class isys_smarty_plugin_f implements isys_smarty_plugin
{
    ...
    protected function attach_wiki($p_params)
    {
        ...
        if ($this->m_strPluginName && !$p_params['p_bInvisible']) {
            ...
            if (!empty($l_wiki_url) && empty($p_params["nowiki"]) && is_null($p_params["p_bDisabled"])) {
                ...
                $title = isys_application::instance()->container->get('language')->get('LC__UNIVERSAL__WIKI');

                return ' <a target="_blank" href="' . $l_wiki_url . $this->m_strPluginName . '" class="btn wiki-link" data-tooltip="1" title="' . $title . '">' .
                    '<img src="' . $g_dirs["images"] . 'axialis/web-email/internet-network-green.svg" />' .
                    '</a>';
            }
        }
    ...
    protected function prepare_input_group(array $params = [])
    {
        if (!isset($params['disableInputGroup']) || !$params['disableInputGroup']) {
            ...
            // @see ID-8783 Fix deprecation issues if the value contains 'null'.
            if (!isset($params["p_strClass"])) {
                $params["p_strClass"] = '';
            }
