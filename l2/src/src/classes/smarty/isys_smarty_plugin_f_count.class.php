...
class isys_smarty_plugin_f_count extends isys_smarty_plugin_f implements isys_smarty_plugin
{
    ...
    public function navigation_edit(isys_component_template $p_tplclass, $p_params = null)
    {
        ...
        $decreaseButton = '<button type="button" class="btn" title="' . $language->get('LC__UNIVERSAL__DECREASE') . '" onClick="$(\'' . $l_strID . '\').setValue(decreaseNumber($F(\'' . $l_strID . '\'))).simulate(\'change\');' . $l_onchange . '" ' . $p_params['p_onAlter'] . ' data-tooltip="1">' .
            '<img src="' . $g_dirs['images'] . 'axialis/basic/symbol-remove.svg" />' .
            '</button>';
        ...
        $increaseButton = '<button type="button" class="btn" title="' . $language->get('LC__UNIVERSAL__INCREASE') . '" onClick="$(\'' . $l_strID . '\').setValue(increaseNumber($F(\'' . $l_strID . '\'))).simulate(\'change\'); ' . $l_onchange . '" ' . $p_params['p_onAlter'] . ' data-tooltip="1">' .
            '<img src="' . $g_dirs['images'] . 'axialis/basic/symbol-add.svg" />' .
            '</button>';
