...
class isys_smarty_plugin_f_property_selector extends isys_smarty_plugin_f implements isys_smarty_plugin
{
    ...
    public function navigation_edit(isys_component_template $p_tplclass, $p_params = null)
    {
        ...
        if (isset($p_params['preselection']) && isys_format_json::is_json($p_params['preselection'])) {
            $preselectionDecoded = isys_format_json::decode($p_params['preselection'], true);
            $l_preselection = $this->handle_preselection($preselectionDecoded);
            $completePath['root'] = $preselectionDecoded;
        }
        ...
        if (isset($p_params['preselection_lvls']) && $p_params['preselection_lvls'] !== 'null') {
            if (!isys_format_json::is_json($p_params['preselection_lvls'])) {
                ...
                $decodedPreselectionLvls = $p_params['preselection_lvls'];
            } else {
                $l_preselection_lvls = $p_params['preselection_lvls'];
                $decodedPreselectionLvls = isys_format_json::decode($p_params['preselection_lvls'], true);
            }

            foreach ($decodedPreselectionLvls as $item) {
                $completePath[key($item)] = current($item);
            }
        }

        if (!empty($completePath)) {
            $l_js_params['completePath'] = isys_format_json::encode($completePath);
        }
        ...
        $l_left_box = '<tr><td class="vat"><div style="width:' . $l_min_width . '" class="border fl mr20 bg-white">
				<ul class="m0 bg-neutral-200 browser-tabs" style="position:relative" id="' . $p_params['name'] . '_tabs">
					...
					' . $l_catg_custom_li . (($p_params['searchable']) ? '<li style="position:absolute;right:5px;" class="mr5">
                        	<a href="#' . $p_params['name'] . '_search" id="' . $p_params['name'] . '_search_tab" class="mouse-pointer">
                                <img src="' . $wwwPath . 'images/axialis/basic/zoom.svg" alt="search" />
                                <div id="' . $p_params['name'] . '_search_controls" class="input-group input-size-mini" style="display:none;">
                                    <input type="text" id="' . $p_params['name'] . '_search_input" placeholder="' . $language->get('LC__PROPERTY_SELECTOR__SEARCH_IN_PROPERTIES') . '" class="input" />
                                    <button type="button" id="' . $p_params['name'] . '_search_button" class="btn" disabled="disabled">
                                        <img src="' . $wwwPath . 'images/axialis/basic/zoom.svg" alt="search" />
                                        <img src="' . $wwwPath . 'images/axialis/user-interface/loading.svg" class="animation-rotate" style="display:none" alt="" />
                                    </button>
                                </div>
                            </a>
                        </li>' : '') . '
				...
				</div>' . $l_catg_custom_div . '<div class="m10 mt20" id="' . $p_params['name'] . '_search" style="display:none;">
				    <div id="' . $p_params['name'] . '_search_result_container"></div>
                    <div id="' . $p_params['name'] . '_search_count_container" class="mt10 text-blue display-flex align-items-center" style="display:none;">
                        <img src="' . $wwwPath . 'images/axialis/basic/button-info.svg" class="mr5" alt="">
                        <span id="' . $p_params['name'] . '_search_count" class="mr5"></span>
                        <span>' . $language->get('LC__PROPERTY_SELECTOR__ATTRIBUTES_FOUND') . '</span>
                    </div>
				</div>
				<div class="error m10 p5" id="' . $p_params['name'] . '_attr_error" style="display:none;"></div>
			</div></td>';

        $l_right_box = '<td class="vat"><div style="width:' . $l_min_width . '" class="border bg-white fl">' .
            '<div class="m0 p10 bg-neutral-200 browser-tabs" style="box-sizing: border-box; height:33px; border-bottom:1px solid #888;">' .
            '<span>' . $language->get('LC__REPORT__INFO__CHOSEN_PROPERTIES_TEXT') . '</span>';
        ...
        if (isset($p_params['default_sorting'])) {
            ...
            $l_right_box = '<td class="vat"><div style="width:' . $l_min_width . '" class="border bg-white fl">' .
                '<div class="m0 p10 bg-neutral-200 browser-tabs" style="box-sizing: border-box; height:33px; border-bottom:1px solid #888;">' .
                '<strong class="text-neutral-600" style="font-size:10px;">' . $language->get('LC__REPORT__INFO__CHOSEN_PROPERTIES_TEXT') . '</strong>' .
                '<strong class="fr text-neutral-600" style="font-size:10px;">' . $language->get('LC__REPORT__INFO__DEFAULT_SORTING') . '</strong>';
        }
				...
				new Tabs('" . $p_params['name'] . "_tabs', {
					...
					onTabSelect: function(el) {
                        ...
                        if (el.getAttribute('href') == '#" . $p_params['name'] . "_search' && !$('" . $p_params['name'] . "_search_controls').visible()) {
                            ...
                            $('" . $p_params['name'] . "_search_tab').select('img')[0].show();
                        }
					}
				});
			}";
    ...
    public function get_catg($p_provides, $p_dynamic_properties, $p_consider_rights = true, $p_obj_type_id = null)
    {
        ...
        while ($l_row = $l_catg_res->get_row()) {
            ...
            $className = $l_row['isysgui_catg__class_name'];
            ...
            if (isset($featureCheck[$className]) && $featureCheck[$className] === false) {
                continue;
            }

            if (!isset($featureCheck[$className])) {
                $interfaces = class_implements($className);
                $featureCheck[$className] = true;
                if (in_array(FeatureCheckInterface::class, $interfaces) && !$className::isFeatureEnabled()) {
                    $featureCheck[$className] = false;
                    continue;
                }
            }
            ...
            if ($l_obj_type_catg[$l_row['isysgui_catg__id']]['isysgui_catg__parent'] > 0) {
                ...
                if (isset($l_obj_type_catg[$l_obj_type_catg[$l_row['isysgui_catg__id']]['isysgui_catg__parent']]) &&
                    ...
                    $l_parent = $language->get($l_obj_type_catg[$l_obj_type_catg[$l_row['isysgui_catg__id']]['isysgui_catg__parent']]['isysgui_catg__title']) . ' > ';
                }
            }

            $l_catg[$l_row['isys_property_2_cat__cat_const']] = $l_parent . $language->get($l_row['isysgui_catg__title']);
        }
    ...
    public function get_cats($p_provides, $p_dynamic_properties, $p_consider_rights = true, $p_obj_type_id = null)
    {
        ...
        while ($l_row = $l_cats_res->get_row()) {
            $className = $l_row['isysgui_cats__class_name'];
            ...
            // @see ID-9125 We use a 'proper' way to get the parent category name.
            $sql = 'SELECT isysgui_cats__title
                FROM isysgui_cats
                INNER JOIN isysgui_cats_2_subcategory ON isysgui_cats__id = isysgui_cats_2_subcategory__isysgui_cats__id__parent
                WHERE isysgui_cats_2_subcategory__isysgui_cats__id__child = ' . $l_dao->convert_sql_id($l_row['isysgui_cats__id']) . '
                LIMIT 1;';

            $parentCategoryName = $l_dao->retrieve($sql)->get_row_value('isysgui_cats__title');

            if (is_string($parentCategoryName) && trim($parentCategoryName) !== '') {
                $parentCategoryName = $language->get($parentCategoryName) . ' > ';
            }
            ...
            foreach ($l_obj_type_row as $l_obj_type) {
                if (!empty($l_obj_type['isys_obj_type__title']) && $language->get($l_obj_type['isys_obj_type__title']) != $language->get($l_row['isysgui_cats__title'])) {
                    $l_obj_types[] = $language->get($l_obj_type['isys_obj_type__title']);
                }
            }

            $l_cats[$l_row['isys_property_2_cat__cat_const']] = $parentCategoryName . $language->get($l_row['isysgui_cats__title']);
    ...
    public function handle_preselection($p_data)
    {
        ...
        while ($l_row = $l_res->get_row()) {
            ...
            // @see ID-11771 Allow all non-virtual properties to be sortable.
            $l_sortable = $l_property[C__PROPERTY__DATA][C__PROPERTY__DATA__INDEX] || !$l_property[C__PROPERTY__PROVIDES][C__PROPERTY__PROVIDES__VIRTUAL];
            ...
            if ($l_row['isys_property_2_cat__isysgui_catg__id'] !== null) {
                if (isset($l_cat['g'][$l_row[$l_catg_key]]) && is_array($l_cat['g'][$l_row[$l_catg_key]])) {
                    if (in_array($l_row['isys_property_2_cat__prop_key'], $l_cat['g'][$l_row[$l_catg_key]])) {
                        $l_return[$l_row['isys_property_2_cat__id']] = [
                            'cat_type'      => 'g',
                            'cat_title'     => $language->get($l_row['title']),
                            ...
                            'prop_title'    => $language->get($l_row['isys_property_2_cat__prop_title']),
                ...
                if (isset($l_cat['g_custom'][$l_row[$l_catg_custom_key]]) && is_array($l_cat['g_custom'][$l_row[$l_catg_custom_key]])) {
                    if (in_array($l_row['isys_property_2_cat__prop_key'], $l_cat['g_custom'][$l_row[$l_catg_custom_key]])) {
                        $l_return[$l_row['isys_property_2_cat__id']] = [
                            'cat_type'      => 'g_custom',
                            'cat_title'     => isys_glob_htmlentities($language->get($l_row['title'])),
                            'cat_const'     => $l_row['isys_property_2_cat__cat_const'],
                            'prop_id'       => $l_row['isys_property_2_cat__id'],
                            'prop_key'      => $l_row['isys_property_2_cat__prop_key'],
                            'prop_title'    => isys_glob_htmlentities($language->get($l_row['isys_property_2_cat__prop_title'])),
                            'prop_type'     => $l_property[C__PROPERTY__INFO][C__PROPERTY__INFO__TYPE],
                            'prop_sortable' => $l_sortable,
                            'prop_multi'    => $l_property_type_addition
                        ];
