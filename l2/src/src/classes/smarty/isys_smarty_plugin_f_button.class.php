...
class isys_smarty_plugin_f_button extends isys_smarty_plugin_f implements isys_smarty_plugin
{
    ...
    public function navigation_view(isys_component_template $p_tplclass, $p_params = null)
    {
        ...
        if ($p_params["p_bDisabled"] == "0") {
            $disabled = '';
        } elseif ($p_params["p_bDisabled"] == "1" || $this->m_bEditMode == false) {
            // @todo  Check if the CSS class "disabled" is used in JS context - otherwise remove it.
        ...
        if (isset($p_params['icon'])) {
            $icon = '<img src="' . $p_params['icon'] . '" />';
        }

        $tooltip = (empty($l_value) && !empty($icon) || $p_params['force-tooltip']) ? 'data-tooltip="1"' : '';

        $attributes = implode(' ', array_filter([
            $p_params["name"],
            $p_params["type"],
            $p_params["p_strAccessKey"],
            $p_params["p_strID"],
            $p_params["p_strTitle"],
            $p_params["p_strClass"],
            $p_params["p_strStyle"],
        ...
        return $this->getInfoIcon($p_params) . "<button " . $attributes . '>' .
            $icon . (!empty($l_value) ? '<span>' . $l_value . '</span>' : '') .
            '</button>';
    }
