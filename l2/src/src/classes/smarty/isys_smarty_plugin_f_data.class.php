...
class isys_smarty_plugin_f_data extends isys_smarty_plugin_f implements isys_smarty_plugin
{
    ...
    public function navigation_view(isys_component_template $p_tplclass, $p_params = null)
    {
        ...
        if ($l_style . $l_id . $l_class) {
            return (string) $this->getInfoIcon($p_params) . "<span" . $l_style . $l_id . $l_class . ">" . $p_params["p_strValue"] . "</span>";
        } else {
            return (string) $this->getInfoIcon($p_params) . $p_params["p_strValue"];
        }
    }
    ...
    {
        return (string)$this->navigation_view($p_tplclass, $p_params);
    }
}
