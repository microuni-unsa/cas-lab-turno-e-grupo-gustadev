...
class isys_request
{
    ...
    public function __construct($p_data = [])
    {
        ...
        if (!$this->m_object_id && !isset($_GET[C__CMDB__GET__OBJECT]) && isset($_POST[C__CMDB__POST__OBJECTS]) && $_POST[C__CMDB__POST__OBJECTS]) {
            $objIds = explode(',', trim($_POST[C__CMDB__POST__OBJECTS], '[]'));
            if (count($objIds) == 1) {
                $this->m_object_id = $objIds[0];
            }
        }
    }
}
