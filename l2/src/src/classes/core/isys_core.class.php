...
class isys_core
{
     ...
     * @return mixed
     */
    public static function request_url($p_include_querystring = false)
    {
     ...
     * @throws  Exception
     */
    public static function status($p_code)
    {
     ...
     */
    public static function expire($p_secs = 0)
    {
        if (PHP_SAPI != 'cli' && !headers_sent()) {
     ...
     */
    public static function warning_handler($p_errno, $p_errstr, $p_errfile, $p_errline)
    {
    ...
    public static function is_webserver_module_installed($p_module)
    {
        ...
        } elseif (isset($_SERVER['HTTP_' . strtoupper($p_module)])) {
        ...
        if (self::is_webserver_module_installed($p_module)) {
            switch ($p_module) {
                case 'mod_rewrite':
                    ...
                    $response = file_get_contents(self::request_url() . 'system/rewrite-check', false, $context);
