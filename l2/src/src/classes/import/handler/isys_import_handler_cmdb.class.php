...
class isys_import_handler_cmdb extends isys_import_handler
{
     ...
     * @var array
     */
    private static array $alreadyUpdatedObjects = [];
    ...
    private $m_possible_empty_values = [
        '0000-00-00 00:00:00',
        C__FALLBACK_EMPTY_DATETIME,
        C__FALLBACK_EMPTY_DATETIME_TZ
    ];
    ...
    {
        try {
            $this->log()->info('Parsing the import file...');
            ...
            if ($this->m_object_counter === 1) {
                $this->log()->info('There is 1 object waiting...');
            } else {
                $this->log()->info('There are ' . $this->m_object_counter . ' objects waiting...');
            }

            $this->log()->debug('Parsing objects...');
            ...
            foreach ($l_xml_data->objects->object as $l_object) {
                ...
                $this->log()->debug(sprintf('Found object %s (%s) of type %s.', $l_title, $l_object_id, $l_type[C__DATA__VALUE]));
                $this->log()->debug('Parsing categories...');
                ...
                foreach ($l_object->data->category as $l_category) {
                    ...
                    if (!defined($l_cat_const)) {
                        // @see ID-10402 Use 'sprintf' instead of 'printf'.
                        $this->log()->notice(sprintf('Category %s\'s constant has not been defined yet. Skipping.', $l_cat_const));
                        continue;
                    }
                    ...
                    $this->log()->debug('Parsing category data...');
    ...
    public function prepare()
    {
        try {
            ...
            foreach ($this->m_data as $l_object_id => $l_object_values) {
                ...
                foreach ($l_object_values[self::C__CATEGORY_TYPES] as $l_category_type_id => $l_categories) {
                    ...
                    foreach ($l_categories as $l_category_id => $l_category_values) {
                        if ($l_category_id === defined_or_default('C__CATG__LOCATION') && $l_category_values[self::C__CATEGORY_ENTITIES] === null &&
                            Context::instance()->getContextCustomer() === Context::CONTEXT_DUPLICATE) {
                            $this->m_trigger_location_fix = false;
                        }
                                            ...
                                            if (isset($l_ref['ref_id']) && isset($l_ref['ref_type']) && isset($l_ref['ref_title'])) {
                                                ...
                                                if ($l_found_category_datas !== false) {
                                                    ...
                                                    foreach ($l_found_category_datas as $l_found_category_data) {
                                                        ...
                                                        $categoryPriorities[$l_category_type_id . '_' . $l_category_id][] = $l_found_category_data['category_type_id'] . '_' .
                                                            $l_found_category_data['category_id'];
                                                    }
                                                    ...
                                                    if (!isset($this->m_cached_category_const[C__CMDB__CATEGORY__TYPE_GLOBAL][$l_ref['ref_type']]) &&
                                                        ...
                                                        $l_found_category_type = ($this->m_cached_category_const[C__CMDB__CATEGORY__TYPE_GLOBAL][$l_ref['ref_type']]
                                                            ? C__CMDB__CATEGORY__TYPE_GLOBAL
                                                            : ($this->m_cached_category_const[C__CMDB__CATEGORY__TYPE_SPECIFIC][$l_ref['ref_type']]
                                                                ? C__CMDB__CATEGORY__TYPE_SPECIFIC
                                                                : ($this->m_cached_category_const[C__CMDB__CATEGORY__TYPE_CUSTOM][$l_ref['ref_type']]
                                                                    ? C__CMDB__CATEGORY__TYPE_CUSTOM :
                                                                    false)));
                                                    }
                                                    ...
                                                    $categoryPriorities[$l_found_category_type . '_' . constant($l_ref['ref_type'])][] = $l_found_category_type . '_' . constant($l_ref['ref_type']);
                                                }
                                            }
                        ...
                        $categoryId = (int)$l_category_id;

                        // Hack for database and operating system. The category application has to be handled before the databases and operating system
                        if (($categoryId === defined_or_default('C__CATG__APPLICATION') ||
                                $categoryId === defined_or_default('C__CATG__DATABASE_TABLE') ||
                                $categoryId === defined_or_default('C__CATG__OPERATING_SYSTEM')) &&
                            $l_category_type_id === defined_or_default('C__CMDB__CATEGORY__TYPE_GLOBAL')
                        ) {
                            $applicationKey = $l_category_type_id . '_' . defined_or_default('C__CATG__APPLICATION');
                            $databaseKey = $l_category_type_id . '_' . defined_or_default('C__CATG__DATABASE_SA');
                            $operatingSystemKey = $l_category_type_id . '_' . defined_or_default('C__CATG__OPERATING_SYSTEM');

                            if (!isset($this->m_prepared[$l_object_id][self::C__CATEGORIES][$applicationKey])) {
                                continue;
                            }

                            $applicationPriority = $this->m_prepared[$l_object_id][self::C__CATEGORIES][$applicationKey][self::C__PRIORITY];

                            if (isset($this->m_prepared[$l_object_id][self::C__CATEGORIES][$databaseKey]) &&
                                $this->m_prepared[$l_object_id][self::C__CATEGORIES][$databaseKey][self::C__PRIORITY] >= $applicationPriority) {
                                $this->m_prepared[$l_object_id][self::C__CATEGORIES][$applicationKey][self::C__PRIORITY] +=
                                    $this->m_prepared[$l_object_id][self::C__CATEGORIES][$databaseKey][self::C__PRIORITY] + 1;
                                continue;
                            }

                            if (isset($this->m_prepared[$l_object_id][self::C__CATEGORIES][$operatingSystemKey]) &&
                                $this->m_prepared[$l_object_id][self::C__CATEGORIES][$operatingSystemKey][self::C__PRIORITY] >= $applicationPriority) {
                                $this->m_prepared[$l_object_id][self::C__CATEGORIES][$applicationKey][self::C__PRIORITY] +=
                                    $this->m_prepared[$l_object_id][self::C__CATEGORIES][$operatingSystemKey][self::C__PRIORITY] + 1;
                            }
                        }
                    } // foreach categories
                } // foreach category types
                foreach ($categoryPriorities as $category => $referencedCategories) {
                    foreach ($referencedCategories as $referencedCategory) {
                        if (isset($categoryPriorities[$referencedCategory])) {
                            continue;
                        }

                        $this->m_prepared[$l_object_id][self::C__CATEGORIES][$referencedCategory][self::C__PRIORITY] +=
                            $this->m_prepared[$l_object_id][self::C__CATEGORIES][$category][self::C__PRIORITY];
                    }
                }
                ...
                if (isset($this->m_prepared[$l_object_id][self::C__CATEGORIES]) && is_array($this->m_prepared[$l_object_id][self::C__CATEGORIES])) {
                    $this->log()->info('Sorting categories by priority...');
            ...
            $this->log()->info('Sorting objects by priority...');
            ...
            unset($this->m_data);
            $this->log()->debug('Optimizing data is done.');
        } catch (Exception $e) {
            isys_notify::warning('Error: ' . $e->getMessage());
        }
    ...
    // @see ID-10109 Declare dynamic props.
    private $m_actual_values = null;
     ...
     */
    public function handle_update_globals(&$p_object_values, $p_object_title)
    {
        ...
        // @see  ID-8377  validate globals
        $global_dao = new isys_cmdb_dao_category_g_global($this->m_db);
        $validationErrors = $global_dao->validate([
            'title'       => $p_object_title,
            'sysid'       => $p_object_values['sysid'],
            'description' => $p_object_values['description']
        ]);
        if (!empty($validationErrors) && is_array($validationErrors) && (bool)isys_tenantsettings::get('import.validation.break-on-error', true)) {
            ...
            throw new isys_exception_validation(
                isys_application::instance()->container->get('language')->get($global_dao->getCategoryTitle()) . ' &raquo; ' .
    ...
    public function set_mode($p_mode)
    {
        ...
        switch ($p_mode) {
            case self::C__APPEND:
                $this->log()->notice('Export data will be appended to existing data.');
                break;

            case self::C__MERGE:
                $this->log()->notice('Export data will be merged with existing data.');
                break;

            case self::C__OVERWRITE:
                $this->log()->notice('Export data will overwrite existing data.');
                break;

            case self::C__USE_IDS:
                $this->log()->notice('Export data will update existing data by identifiers.');
                break;
    ...
    public function set_empty_fields_mode($p_mode)
    {
        ...
        switch ($p_mode) {
            case self::C__KEEP:
                $this->log()->info('Ignore empty fields');
                break;

            case self::C__CLEAR:
                $this->log()->info('Clear fields');
                break;
    ...
    public function set_multivalue_categories_mode($p_mode)
    {
        ...
        switch ($p_mode) {
            case self::C__UNTOUCHED:
                $this->log()->info('Keep category entries untouched');
                break;

            case self::C__APPEND:
                $this->log()->info('Add category entries');
                break;

            case self::C__OVERWRITE:
                $this->log()->info('Delete existing category entries before adding new ones');
                break;

            case self::C__UPDATE:
                $this->log()->info('Update existing category entries');
                break;

            case self::C__UPDATE_ADD:
                $this->log()->info('Update existing category entries and add new ones');
                break;
    ...
    public function fireDisconnectedSignals()
    {
        if (!empty($this->m_cat_info)) {
            ...
            foreach ($categoryIds as $categoryIdString) {
                [$categoryType, $categoryId] = explode('_', $categoryIdString);
            ...
            $this->log()->notice('Index Signals triggered');
    ...
    public function import()
    {
        ...
        $this->log()->debug('Importing data to database...');
        $this->log()->flush_verbosity(true, false);

        // Check import mode:
        if (!isset($this->m_mode)) {
            // Try to set it automaticly.
            $this->log()->debug('Import mode is not set.');
            if ($this->m_mandator[C__DATA__VALUE] == $g_mandator_info['isys_mandator__title'] && $this->m_mandator['id'] == $g_mandator_info['isys_mandator__id']) {
                $this->log()->debug('The export source is the same like this system.');
                $this->set_mode(self::C__USE_IDS);
            } else {
                $this->log()->debug('The export source is another one then this system.');
        ...
        if (!isset($this->m_empty_fields)) {
            $this->log()->debug('Mode for empty fields is not set.');
        ...
        $this->log()->notice('Tenant: ' . $this->m_mandator[C__DATA__VALUE]);
        $this->log()->notice('Version: ' . $this->m_version);
        $this->log()->notice('Exported at: ' . date($this->m_date_format, $this->m_scantime));
        $this->log()->notice('Export-Type: ' . $this->m_type);
        ...
        $this->log()->notice('Import was successful.');
        ...
        if (is_countable($this->m_logbook_entries) && count($this->m_logbook_entries) > 0) {
            $this->log()->debug('Writing ' . intval($this->m_logbook_entries) . ' logbook entries.');
        ...
        $this->log()->info('Updating changed_by attribute and deleting quickinfo cache for ' . $countOfChangedObjects . ' object(s)');
        $this->log()->debug(' -> ' . implode(', ', $l_changed_objects));
        ...
        if (!empty($l_changed_objects) && $this->m_options['update-object-changed']) {
            foreach ($l_changed_objects as $l_obj_id) {
                if (!isset(static::$alreadyUpdatedObjects[$l_obj_id])) {
                    $this->m_cmdb_dao->object_changed($l_obj_id, $this->m_import_started_by, true);
                    static::$alreadyUpdatedObjects[$l_obj_id] = true;
                }
            }
        }
    ...
    public function __destruct()
    {
        ...
        try {
            ...
            $this->log()->error('Location could not be regenerated with message: ' . $e->getMessage());
        }
        ...
        if (is_countable($this->m_object_ids) && count($this->m_object_ids) > 0) {
            try {
                ...
                $this->log()->warning(sprintf('Could not clear cache files for %sauth-* with message: ' . $e->getMessage(), isys_glob_get_temp_dir()));
            }
        }
    }
    ...
    {
        $this->log()->debug('Parsing properties...');
        ...
        switch (gettype($p_value)) {
            ...
            default:
                $this->log()->warning(sprintf('Unknown property type %s.', gettype($p_value)));
                break;
        }
    ...
    {
        $this->log()->debug(sprintf('Look for object %s of type %s with sysid %s and title %s...', $p_object_id, $p_type, $p_sysid, $p_value, $p_current_object_id));
                ...
                foreach ($l_categories as $l_category_id => $l_category_values) {
                    ...
                    foreach ($l_category_values[self::C__CATEGORY_ENTITIES] as $l_category_data_id => $l_category_data_values) {
                        ...
                        if ($l_found_value === true) {
                            $this->log()->debug(sprintf('Match found: category %s in category type %s', $l_category_id, $l_category_type_id));
                            $l_result[] = [
    ...
    private function sort_categories($p_a_key, $p_b_key)
    {
        ...
        [$l_a_category_type_id, $l_a_category_id] = explode('_', $p_a_key);
        [$l_b_category_type_id, $l_b_category_id] = explode('_', $p_b_key);
    ...
    private function import_dummy_objects()
    {
        ...
        if (is_array($this->m_info[self::C__OBJECTS])) {
            foreach ($this->m_info[self::C__OBJECTS] as $l_object_id => $l_object_info) {
                // @see ID-10906 Check if there is at least 4 MB of memory available.
                Memory::instance()->outOfMemoryBreak(1024 * 1024 * 4);
                ...
                if ($this->m_found_objects[$l_object_id] === self::C__COMPARISON__DIFFERENT) {
                    $this->log()->debug('Looks like current object is unknown, but we also take a look at the SYSID.');
                ...
                // Second try:
                if ($this->m_found_objects[$l_object_id] === self::C__COMPARISON__DIFFERENT) {
                    $this->log()->debug('Looks like current object is unknown, but we also take a look at the Title and Type.');
                ...
                if ($this->m_found_objects[$l_object_id] === self::C__COMPARISON__SAME) {
                    $this->log()->debug('  Match found in database.');
                    ...
                    $this->log()->debug('  Match found in database, but it\'s not exactly the same. Updating.');
                    ...
                    try {
                        ...
                        if ($l_status === false) {
                            $this->log()->error('Failed to update database. Aborting.');
                    ...
                    $this->log()->debug('  No match found in database.');
                    $this->log()->debug(sprintf('  Check whether object type %s exists.', $l_object_info[self::C__PROPERTIES]['type']));
                    ...
                    if (is_numeric($l_object_info[self::C__PROPERTIES]['type_id']) && isset($this->m_object_types[$l_object_info[self::C__PROPERTIES]['type_id']])) {
                        ...
                        $this->log()->debug('  Object type found: ' . $this->m_object_type_ids[$l_object_id]);
                    } elseif (in_array($l_object_info[self::C__PROPERTIES]['type'], $this->m_object_types)) {
                        $this->log()->debug('  Object type already exists.');
                        $this->m_object_type_ids[$l_object_id] = $l_object_info[self::C__PROPERTIES]['type'];
                    } else {
                        $this->log()->debug('  Object type is unknown. A dummy will be now created.');
                        ...
                        $this->log()->debug(sprintf('  New object type %s created as a dummy.', $l_object_info[self::C__PROPERTIES]['type']));
                        // Log:
                        $this->m_event_manager->triggerImportEvent(
                    ...
                    $this->log()->debug('Now it\'s time to create new object.');
                    $l_objtype = ((!isset($l_new_obj_type_id)) ? constant($this->m_object_type_ids[$l_object_id]) : $l_new_obj_type_id);
                    $objTitle = ((!isset($l_object_info[self::C__PROPERTIES][C__DATA__VALUE])) ? $l_object_info[self::C__PROPERTIES][C__DATA__TITLE] : $l_object_info[self::C__PROPERTIES][C__DATA__VALUE]);
                    ...
                    $this->log()->info('Dummy object "' . $l_object_info[self::C__PROPERTIES][C__DATA__VALUE] . '" [' . $this->m_object_ids[$l_object_id] . '] created.');
                    // Log:
                    $this->m_event_manager->triggerImportEvent(
    ...
    private function import_objects()
    {
        ...
        foreach ($this->m_prepared as $l_object_id => $l_object_values) {
            $this->log()->info(sprintf('Importing object #%s: %s (%s)', $l_object_id, $l_object_values[C__DATA__TITLE], $l_object_values['sysid']));
            // @see ID-10906 Check if there is at least 4 MB of memory available.
            Memory::instance()->outOfMemoryBreak(1024 * 1024 * 4);
            ...
            $config = ReplacerConfig::factory(
                ...
                null,
                '',
                isset($l_object_values[self::C__CATEGORIES][C__CMDB__CATEGORY__TYPE_GLOBAL . '_' . C__CATG__GLOBAL])
            );

            // Replace default values
            $l_object_title = isys_application::instance()->container->get('idoit.component.placeholder-replacer')->replacePlaceholder(
                $l_object_title ?? '',
                $config
            );
            ...
            if ($this->m_mode !== self::C__APPEND) {
                ...
                // Second try:
                if ($this->m_found_objects[$l_object_id] === self::C__COMPARISON__DIFFERENT) {
                    $this->log()->debug('Looks like current object is unknown, but we also take a look at the Object Title and Object Type.');
                ...
                if ($this->m_found_objects[$l_object_id] === self::C__COMPARISON__SAME) {
                    ...
                    $this->log()->debug('No match found in database. Create new object in a moment.');
                    $this->log()->debug(sprintf('Check whether object type %s exists.', $l_object_values['type']['const']));

                    if (in_array($l_object_values['type']['const'], $this->m_object_types)) {
                        $this->log()->debug('Object type already exists.');
                        $this->m_object_type_ids[$l_object_id] = $l_object_values['type']['const'];
                    } else {
                        $this->log()->debug('Object type is unknown. It will be now created.');
                    ...
                    $this->log()->debug('Now it\'s time to create new object.');
                    $this->handle_update_globals($l_object_values, $l_object_title);
                    $previousObjectTitle = $l_object_title;
                    $this->m_object_ids[$l_object_id] = $this->m_cmdb_dao->insert_new_obj(
                        constant($this->m_object_type_ids[$l_object_id]),
                        false,
                        $l_object_title,
                    ...
                    $l_object_title = $this->m_cmdb_dao->obj_get_title_by_id_as_string($this->m_object_ids[$l_object_id]);

                    if ($previousObjectTitle !== $l_object_title) {
                        $this->updateObjectTitleInImport($l_object_id, $l_object_title);
                    }

                    $this->log()->info('<<<< New Object ' . $l_object_title . ' (' . $this->m_object_ids[$l_object_id] . ') created. >>>>');
                    // Log:
                    $this->m_event_manager->triggerImportEvent(
                        'C__LOGBOOK_EVENT__OBJECT_CREATED',
                        'CMDB-Import',
                ...
                $this->log()->debug('Create new object in a moment.');
                $this->log()->debug(sprintf('Check whether object type %s exists.', $l_object_values['type']['const']));
                if (in_array($l_object_values['type']['const'], $this->m_object_types)) {
                    $this->log()->debug('Object type already exists.');
                    $this->m_object_type_ids[$l_object_id] = $l_object_values['type']['const'];
    ...
    private function import_categories()
    {
        ...
        // Iterate through objects:
        foreach ($this->m_prepared as $l_object_id => $l_object_values) {
            $this->log()->debug("Importing object $l_object_id: " . $l_object_values[C__DATA__TITLE] . " (" . $l_object_values['sysid'] . ")");

            /* Store OBJECTID */
            self::store__objectID($l_object_id);

            // @see ID-8833 Check if imported object is a template.
            $isTemplateObject = in_array(
                $this->m_cmdb_dao->get_object($l_object_id)->get_row_value('isys_obj__status'),
                [C__RECORD_STATUS__TEMPLATE, C__RECORD_STATUS__MASS_CHANGES_TEMPLATE],
                false
            );
            ...
            if (is_array($l_object_values[self::C__CATEGORIES])) {
                // @see ID-10961 Removed ID-9541 as it is already sorted by priority
                $generalCategory = $l_object_values[self::C__CATEGORIES][C__CMDB__CATEGORY__TYPE_GLOBAL . '_' . C__CATG__GLOBAL];
                unset($l_object_values[self::C__CATEGORIES][C__CMDB__CATEGORY__TYPE_GLOBAL . '_' . C__CATG__GLOBAL]);
                $reversedCategoryOrder = array_reverse($l_object_values[self::C__CATEGORIES], true);
                $reversedCategoryOrder[C__CMDB__CATEGORY__TYPE_GLOBAL . '_' . C__CATG__GLOBAL] = $generalCategory;
                $l_object_values[self::C__CATEGORIES] = array_reverse($reversedCategoryOrder);
                ...
                foreach ($l_object_values[self::C__CATEGORIES] as $l_category_key => $l_category_values) {
                    ...
                    // @see  ID-8007 ID-8008 ID-8015  Skip categories that have no numeric ID.
                    if (!is_numeric($l_category_id)) {
                        $this->log()->warning("  The category #{$l_category_id} (of type '{$l_category_type_id}') could not be processed. Skipping.");
                        continue;
                    }

                    // Translate category name:
                    $l_category_name = $language->get($this->m_all_categories[$l_category_type_id][$l_category_id]['title']);

                    $this->log()->info("  Importing category \"$l_category_name\"");

                    // Check category type id:
                    if (!array_key_exists($l_category_type_id, $this->m_all_categories)) {
                        $this->log()->debug('Category type ' . $l_category_type_id . ' does not exist. Skipping.');
                        continue;
                    }

                    // @see ID-8833 Check if we are about to import a 'auth' related category.
                    if ($isTemplateObject) {
                        if ($l_category_type_id == C__CMDB__CATEGORY__TYPE_GLOBAL && $l_category_id == defined_or_default(C__CATG__VIRTUAL_AUTH)) {
                            $this->log()->debug('Skipping auth related categories when importing template objects.');
                            continue;
                        }

                        if ($l_category_type_id == C__CMDB__CATEGORY__TYPE_SPECIFIC && $l_category_id == defined_or_default(C__CATS__BASIC_AUTH)) {
                            $this->log()->debug('Skipping auth related categories when importing template objects.');
                            continue;
                        }
                    }
                    ...
                    if (!empty($this->m_ignored_categories) && isset($this->m_ignored_categories[$l_category_type_id]) &&
                        ...
                        $this->log()->debug('Category is black-listed. Skipping.');
                        continue;
                    }
                    ...
                    if (isset($l_daos_processed[$l_cat_class]) && $l_cat_class !== isys_cmdb_dao_category_g_custom_fields::class) {
                        $this->log()->debug("Category $l_cat_class ($l_category_id) was already processed by the import. Skipping.");

                        continue;
                    }

                    // We need the category DAO:
                    if ($l_cat_class === isys_cmdb_dao_category_g_custom_fields::class) {
                        $l_cat_dao = new isys_cmdb_dao_category_g_custom_fields($this->m_db);
                        $l_cat_dao->set_catg_custom_id((int)$l_category_id);
                    } else {
                    ...
                    if ($l_cat_dao instanceof isys_cmdb_dao_category) {
                    ...
                    // Collect information about this category for better performance:
                    ...
                    if ($l_cat_dao instanceof isys_cmdb_dao_category_g_custom_fields) {
                        $this->log()->debug('Handle custom category ' . $l_category_values['title']);
                        ...
                        foreach ($l_category_values[self::C__CATEGORY_ENTITIES] as $l_category_data_values) {
                            if (isset($l_category_data_values['properties']) && is_array($l_category_data_values['properties']) &&
                                ...
                                foreach ($l_category_data_values['properties'] as $l_tag => $l_values) {
                                    ...
                                    [$l_type, $l_key] = isys_cmdb_dao_category_g_custom_fields::getTypeAndKey($l_tag);
                        ...
                        if (is_countable($l_module_data) && count($l_module_data) > 0) {
                            foreach ($l_module_data as $l_candidate) {
                                if ($l_candidate['isysgui_catg_custom__id'] == $l_catg_custom_id) {
                                    ...
                                    $customConfig = Unserialize::toArray($l_candidate['isysgui_catg_custom__config']);
                        ...
                        if ($l_found === false) {
                            $this->log()->notice('Custom category ' . $l_category_values['title'] . ' not found. Creating...');
                            ...
                            if ($l_catg_custom_id === false) {
                                $this->log()->warning('Failed to create custom category ' . $l_category_values['title'] . ' [' . $l_category_values['const'] . ']. Skipping.');
                                continue;
                            }

                            $l_status = $l_custom_module->assign($l_catg_custom_id, constant($this->m_object_type_ids[$l_object_id]));
                            if ($l_status === false) {
                                $this->log()->warning('Failed to assign custom category ' . $l_category_values['title'] . ' [' . $l_category_values['const'] .
                    ...
                    $objectId = $this->m_object_ids[$l_object_id];
                    $objectTypeId = (!is_numeric($this->m_object_type_ids[$l_object_id]) ? constant($this->m_object_type_ids[$l_object_id]) : $this->m_object_type_ids[$l_object_id]);
                    ...
                    if (is_array($l_category_values[self::C__CATEGORY_ENTITIES])) {
                        ...
                        foreach ($l_category_values[self::C__CATEGORY_ENTITIES] as $l_category_data_id => $l_category_data_values) {
                            ...
                            if (!is_array($l_category_data_values[self::C__PROPERTIES]) || count($l_category_data_values[self::C__PROPERTIES]) === 0) {
                            ...
                            foreach ($l_category_data_values[self::C__PROPERTIES] as $l_key => $l_value) {
                                ...
                                foreach ($this->m_cat_info[$l_category_key]['property_info'] as $l_tag => $l_info) {
                                    ...
                                    if ($l_tag == $l_key && $l_info[C__PROPERTY__PROVIDES][C__PROPERTY__PROVIDES__IMPORT] === false) {
                                        unset($l_category_data_values[self::C__PROPERTIES][$l_key]);
                                        continue 2;
                                    }
                                ...
                                if ($l_class && $l_method) {
                                    ...
                                    if (!method_exists($l_class, $l_import_method)) {
                                        ...
                                        if ($l_helper instanceof isys_export_helper) {
                                        ...
                                        if ((($l_import_method == 'connection_import' || $l_import_method == 'object_import') && is_array($l_value)) || $l_import_method == 'model_title_import' ||
                                            ...
                                            $l_import_method == 'object_image_import' || (array_key_exists('ref_id', $l_category_data_values[self::C__PROPERTIES][$l_key]) &&
                                                ($l_category_data_values[self::C__PROPERTIES][$l_key]['ref_id'] === null ||
                                                    !is_numeric($l_category_data_values[self::C__PROPERTIES][$l_key][C__DATA__VALUE]))) || $l_mode === self::C__MERGE) {
                                            $l_transformed = $l_helper->$l_import_method($l_value);
                                            $importMethodApplied = true;
                            ...
                            if ($l_mode === self::C__APPEND) {
                                $l_category_data_status = self::C__CREATE;
                                $this->log()->debug('Ignore data from database.');
                            } elseif ($l_mode === self::C__OVERWRITE) {
                                $l_category_data_status = self::C__CREATE;

                                if ($l_already_overwritten === false && $l_cat_class != 'isys_cmdb_dao_category_g_global') {
                                    $this->log()->debug('Deleting existing category data from database...');

                                    $l_cleared = $l_cat_dao->clear_data($objectId, $this->m_cat_info[$l_category_key]['table']);

                                    if ($l_cleared === true) {
                                        $this->log()->info('Category data deleted.');
                                    } else {
                                        $this->log()->warning(sprintf(
                                ...
                                if (is_countable($l_object_category_dataset) && count($l_object_category_dataset) == 0) {
                                    ...
                                    $this->log()->debug('Checking whether one of the datasets matches the properties of category "' . $l_category_name . '"...');
                                    ...
                                    if ($this->m_cat_info[$l_category_key]['can_compare']) {
                                        if ($l_cat_dao instanceof ComparableCategory) {
                            ...
                            if ($l_category_data_status !== self::C__KEEP) {
                                if ($l_category_data_status === self::C__CREATE) {
                                    if (!$this->m_cat_info[$l_category_key]['is_multi-valued']) {
                                         ...
                                         */
                                        $l_res = $l_cat_dao->get_data(null, $objectId, '', null, C__RECORD_STATUS__NORMAL);

                                        if ($l_res->num_rows() == 1 && $l_cat_class != 'isys_cmdb_dao_category_g_global') {
                                            $l_category_data_status = self::C__UPDATE;
                                            $this->log()->debug('  Dataset already exists; update current dataset.');
                                            // Merge existing data
                                            $l_row = $l_res->get_row();
                                            ...
                                            if ($l_cat_class === 'isys_cmdb_dao_category_g_accounting' &&
                                                defined($this->m_object_type_ids[$objectId])) {
                                                $l_obj_type_id = constant($this->m_object_type_ids[$objectId]);
                                            ...
                                            $this->log()->debug('  Dataset will be created.');
                                        }
                                    }
                                } //if create dataset
                                            ...
                                            if (!method_exists($l_used_properties[$l_key][C__PROPERTY__FORMAT][C__PROPERTY__FORMAT__CALLBACK][0], $l_method)) {
                                                ...
                                                if (isset($l_category_data_values[self::C__PROPERTIES][$l_key]['sysid'])) {
                                                    ...
                                                    if (is_array($l_category_data_values[self::C__PROPERTIES][$l_key][C__DATA__VALUE])) {
                                                        ...
                                                        $this->log()->info('It is a category reference.');
                                                    } else {
                                ...
                                if (is_countable($l_category_data_values[self::C__PROPERTIES]) && count($l_category_data_values[self::C__PROPERTIES]) > 0) {
                                    ...
                                    $l_category_data_values = $l_cat_dao->replacePlaceholdersWithSyncData($l_category_data_values, $objectId, $objectTypeId, $l_object_values[C__DATA__TITLE], $l_object_values['sysid']);
                                    ...
                                    if ($l_category_data_status === self::C__UPDATE) {
                                        $this->log()->debug('== Dataset updated.');
                                        ...
                                        $changes = $this->getCategoryChanges($l_cat_dao, $this->m_object_ids[$l_object_id], $l_category_data_values, $l_category_data_values[Config::CONFIG_DATA_ID]);
                                        $l_category_changes = $changes->getCurrentReformatedChanges();
                                        ...
                                        $this->log()->debug('  Dataset created.');

                                        $changes = $this->getCategoryChanges($l_cat_dao, $this->m_object_ids[$l_object_id], $l_category_data_values);
                                        $l_category_changes = $changes->getCurrentReformatedChanges();
                                    ...
                                    try {
                                        ...
                                        $l_cat_dao->set_object_id($objectId);
                                        // Set Object Type ID in category dao
                                        $l_cat_dao->set_object_type_id($objectTypeId);
                                        ...
                                        } elseif ($logbookEntry !== null && is_array($logbookEntry)) {
                                        ...
                                        if (is_array($validationErrors) && count($validationErrors)) {
                                            $this->recordValidationError($l_cat_dao, $validationErrors);

                                            if (isys_tenantsettings::get('import.validation.break-on-error', true)) {
                                                $logbookEntry = null;
                                                throw new isys_exception_validation(
                                                    $language->get($l_cat_dao->getCategoryTitle()) . ' &raquo; ' .
                                                    implode('<br/>', $validationErrors),
                                                    $validationErrors
                                                );
                                            }
                                        ...
                                        // Sync category:
                                        $l_new_category_data_id = $l_cat_dao->sync($l_category_data_values, $objectId, $l_category_data_status);
                                        ...
                                        if ($l_category_data_status === self::C__CREATE) {
                                            $this->m_signals->emit(
                                                ...
                                                $objectId,
                                                $l_category_data_values,
                                                !empty($l_collected_category_changes) ? $l_collected_category_changes : $l_category_changes
                                            );

                                            isys_component_signalcollection::get_instance()
                                                ->emit(
                                                    "mod.cmdb.afterCreateCategoryEntry",
                                                    $l_cat_dao->get_category_id(),
                                                    $l_new_category_data_id,
                                                    true,
                                                    $objectId,
                                                    $l_cat_dao,
                                    ...
                                    $this->log()->debug(sprintf('== There is nothing to sync for the category %s skipping...', $l_category_name));
                                }
                            }

                            if ($l_category_data_status === self::C__KEEP) {
                                $l_new_category_data_id = $l_category_data_values['data_id'];
                                $this->log()->debug('== Dataset kept.');
                            }
                    ...
                    if ($l_changed_categories === true && $this->m_found_objects[$l_object_id] === self::C__COMPARISON__SAME) {
                        $this->setChangesInObjectIds($objectId);
                        $this->importCounters['category_updated']++;
    ...
    private function compare_objects($p_database_object, &$p_import_object, &$p_object_id, &$p_object_status = null)
    {
        ...
        if ($p_database_object->num_rows() !== 1) {
            $this->log()->debug('No object found.');
            $l_badness += 1000;
        }
        ...
        if ((int)$l_existing_object['isys_obj__id'] !== (int)$p_import_object['id']) {
            $this->log()->debug('No object with that ID found.');
            $l_badness++;
        }
        ...
        if ($l_existing_object['isys_obj__isys_obj_type__id'] != $l_type) {
            $this->log()->debug('Object type is different.');
            $l_badness += 1000;
        } else {
            $l_title_type_exists++;
        }
        ...
        if (strcasecmp($l_existing_object['isys_obj__title'], $l_title) !== 0) {
            $this->log()->debug('Object title is different.');
            $l_badness++;
        } else {
            $l_title_type_exists++;
        }
        ...
        if (strcasecmp($l_existing_object['isys_obj__sysid'], $p_import_object['sysid']) !== 0) {
            $this->log()->debug('Object SYSID is different.');
        ...
        if (defined('C__CATG__GLOBAL') && isset($p_import_object['category_types'][C__CMDB__CATEGORY__TYPE_GLOBAL][constant('C__CATG__GLOBAL')][self::C__CATEGORY_ENTITIES][$p_object_id][self::C__PROPERTIES]['description'][C__DATA__VALUE]) &&
            ...
            $this->log()->debug('Object description is different.');
            $l_badness++;
        }
        ...
        if (isset($p_import_object['status']) && ((bool)isys_tenantsettings::get('import.object.keep-status')) === false) {
            if ($l_existing_object['isys_obj__status'] != $p_import_object['status']) {
                $this->log()->debug('Object status is different.');
    ...
    private function validateCategoryDataBeforeSync(isys_cmdb_dao_category $dao, array &$categoryData, array &$categoryChanges)
    {
        ...
        foreach ($validationErrors as $propertyKey => $validationMessage) {
            ...
            $categoryData['properties'][$propertyKey]['ref_title'] = null;
    ...
    /**
     * @param int    $objectId
     * @param string $newObjectTitle
     *
     * @return void
     */
    private function updateObjectTitleInImport(int $objectId, string $newObjectTitle): void
    {
        $index = key($this->m_prepared[$objectId][self::C__CATEGORIES][C__CMDB__CATEGORY__TYPE_GLOBAL . '_' . C__CATG__GLOBAL][self::C__CATEGORY_ENTITIES]);
        $this->m_prepared[$objectId][self::C__CATEGORIES][C__CMDB__CATEGORY__TYPE_GLOBAL . '_' . C__CATG__GLOBAL][self::C__CATEGORY_ENTITIES][$index][self::C__PROPERTIES]['title']['value'] =
            $newObjectTitle;
    }
     ...
     * @return Changes|null
     */
    private function getCategoryChanges(isys_cmdb_dao_category $p_category_dao, int $objectId, array $syncData = [], ?int $entryId = null): ?Changes
    {
        if (empty($syncData)) {
            return null;
        }
        ...
        if (isset($syncData[self::C__PROPERTIES]) && isset($currentData[self::C__PROPERTIES])) {
            $currentDataValues = $syncDataValues = [];
            foreach ($syncData[self::C__PROPERTIES] as $index => $value) {
                $syncDataValues[$index] = empty($value['value']) ? null : $value['value'];
            }
            foreach ($currentData[self::C__PROPERTIES] as $index => $value) {
