...
class isys_component_dao_object_table_list extends isys_component_dao
{
    ...
    /**
     * Helper-method to get the current lists configuration.
     *
     * @return Config
     * @see ID-11048
     */
    ...
    {
        return isys_component_list::instance($this->m_db)
            ->set_listdao($this)
            ->get_table_config();
    }
    ...
    public function get_result($p_strTableName = null, $p_object_id = null, $p_cRecStatus = null)
    {
        ...
        if (!$this->m_cat_dao instanceof isys_cmdb_dao_category) {
            throw new isys_exception_dao_cmdb(get_class($this->m_cat_dao) . ' needs to extend "isys_cmdb_dao_category".');
        }

        if ($p_object_id === null || $p_object_id <= 0) {
    ...
    /**
     * Return array data for the status select box.
     *
     * @return array
     * @throws Exception
     */
    public function get_rec_array()
    {
        ...
        $recordCounts = $this->get_rec_counts();
        ...
        if (defined("C__TEMPLATE__STATUS") && C__TEMPLATE__STATUS == 1) {
            $map[C__RECORD_STATUS__TEMPLATE] = 'LC__CMDB__RECORD_STATUS__TEMPLATE';
        }

        foreach ($map as $id => $title) {
            $return[$id] = $languageManager->get($title) . " (" . ($recordCounts[$id] ?? 0) . ")";
        }
