 ...
 * i-doit
 *
 * Sends a CSV file with the table contents to the browser.
 * FYI: this class will be used when exporting MV categories. Object lists will be exported by 'isys_component_table_csv'.
 *
 * @package     i-doit
...
class isys_component_list_csv extends isys_component_list
{
    ...
    public function createTempTable()
    {
        ...
        $l_csv->insertOne(array_map(function ($column) use ($language) {
            return $language->get($column);
        }, array_values($l_header)));

        $removeObjectIds = (bool)isys_tenantsettings::get('cmdb.csv-export.remove-object-ids', false);

        if ($this->m_resData) {
            $this->write_generic_category_rows($l_csv, $l_header, $removeObjectIds);
        }
        ...
        if ($this->m_arData && is_array($this->m_arData) && count($this->m_arData)) {
            ...
            $this->write_custom_category_rows($l_csv, $l_header, $removeObjectIds);
        }
     ...
     * @param bool      $removeObjectIds
     *
     * @throws CannotInsertRecord
     */
    protected function write_generic_category_rows(CsvWriter $p_csv, array $p_header, bool $removeObjectIds)
    {
        $language = isys_application::instance()->container->get('language');
            ...
            foreach ($p_header as $l_key => $l_field) {
                ...
                if (is_scalar($l_val)) {
                    if (strpos($l_val, "LC") === 0) {
                        $l_val = $language->get($l_val);
                    }
                } elseif (is_array($l_val)) {
                    // @see ID-11048 Always fall back to 'comma separated' for CSV.
                    $l_val = implode(', ', $l_val);
                    ...
                    $parameters = $l_val->set_parameter('p_bEditMode', false)
                        ...
                        ->get_parameter();
                    $l_val = $l_val->navigation_view(isys_application::instance()->container->get('template'), $parameters);
                }

                $l_csv_row[$l_key] = trim(strip_tags(isys_helper_textformat::remove_scripts(html_entity_decode($l_val, ENT_COMPAT, BASE_ENCODING))));
            }

            $l_csv_row = array_values($l_csv_row);

            if (empty($l_filter) || stripos(implode(' ', $l_csv_row), $l_filter) !== false) {
                // @see  ID-8370  We remove the object ID from values like 'title {123}'.
                if ($removeObjectIds) {
                    $l_csv_row = array_map(function ($value) {
                        return preg_replace('/([^\{\>]+) \{([0-9]+)\}/i', '$1', $value);
                    }, $l_csv_row);
                }
     ...
     */
    protected function write_custom_category_rows(CsvWriter $p_csv, array $p_header, bool $removeObjectIds)
    {
        $language = isys_application::instance()->container->get('language');
        $l_filter = isys_glob_get_param('filter');
        ...
        foreach ($this->m_arData as $l_row) {
            ...
            foreach ($p_header as $l_identifier => $l_field) {
                $l_csv_row[] = trim(strip_tags(isys_helper_textformat::remove_scripts(html_entity_decode($language->get($l_row[$l_identifier]), ENT_COMPAT, BASE_ENCODING))));
            }
    ...
    protected function get_csv_filename()
    {
        ...
        } elseif (method_exists($this->m_listdao, 'get_dao_category') && method_exists($this->m_listdao->get_dao_category(), 'get_object_id')) {
        ...
        if (isset($l_gets[C__CMDB__GET__CATG]) || isset($l_gets[C__CMDB__GET__CATS])) {
            ...
            if ($l_gets[C__CMDB__GET__CATG] == defined_or_default('C__CATG__CUSTOM_FIELDS') && isset($l_gets[C__CMDB__GET__CATG_CUSTOM])) {
                $l_name = isys_cmdb_dao_category_g_custom_fields::instance($database)->get_cat_custom_name_by_id_as_string($l_gets[C__CMDB__GET__CATG_CUSTOM]);
            } elseif (isset($l_gets[C__CMDB__GET__CATG]) && $l_gets[C__CMDB__GET__CATG] > 1) {
                $l_name = isys_cmdb_dao::instance($database)->get_catg_name_by_id_as_string($l_gets[C__CMDB__GET__CATG]);
            } elseif (isset($l_gets[C__CMDB__GET__CATS]) && $l_gets[C__CMDB__GET__CATS] > 1) {
                $l_name = isys_cmdb_dao::instance($database)->get_cats_name_by_id_as_string($l_gets[C__CMDB__GET__CATS]);
            }
        } elseif (isset($l_gets[C__CMDB__GET__OBJECTTYPE])) {
            // Object type context...
            $l_name = isys_cmdb_dao::instance($database)->get_objtype_name_by_id_as_string($l_gets[C__CMDB__GET__OBJECTTYPE]);
        }

        return isys_helper_textformat::clean_string(date('Y_m_d') . ($l_object_id !== null ? '__' . $l_object_id : '') . '__' .
                $language->get($l_name)) . '.csv';
    }
}
