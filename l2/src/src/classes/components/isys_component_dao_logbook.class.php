...
class isys_component_dao_logbook extends isys_component_dao
{
    ...
    {
        $l_changes = empty($p_changes_binary) ? [] : Unserialize::toArray($p_changes_binary);
     ...
     * @throws isys_exception_database
     */
    public function get_result_latest_entry(): isys_component_dao_result
    {
        $logbookTable = $this->getTableName('isys_logbook');
        $logbookLevelTable = $this->getTableName('isys_logbook_level');

        $query = "SELECT isys_logbook__id, isys_logbook__date, isys_logbook__isys_obj__id, isys_logbook__event_static, isys_logbook__obj_name_static, isys_logbook__category_static, isys_logbook__obj_type_static, isys_logbook_level__const, isys_logbook__user_name_static
            FROM {$logbookTable}
            INNER JOIN {$logbookLevelTable} ON isys_logbook__isys_logbook_level__id = isys_logbook_level__id
    ...
    {
        $l_query = 'SELECT isys_logbook__description
            FROM ' . $this->getTableName('isys_logbook') . '
            WHERE isys_logbook__id = ' . $this->convert_sql_id($p_logbookID) . ';';

        $l_result = $this->retrieve($l_query);

        if ($l_row = $l_result->get_row()) {
            return $l_row['isys_logbook__description'];
        }
    ...
    {
        $l_query = 'SELECT isys_logbook__changes
            FROM ' . $this->getTableName('isys_logbook') . '
            WHERE isys_logbook__id = ' . $this->convert_sql_id($p_logbookID) . ';';
    ...
    public function get_object_id_by_logbook_id($p_nLogbookID)
    {
        if (is_numeric($p_nLogbookID)) {
            $l_strSQL = 'SELECT isys_catg_logb_list__isys_obj__id
                FROM ' . $this->getTableName('isys_catg_logb_list') . '
                JOIN ' . $this->getTableName('isys_logbook') . ' ON isys_catg_logb_list__isys_logbook__id = ' . $this->convert_sql_id($p_nLogbookID) . ';';

            return $this->retrieve($l_strSQL)->get_row_value('isys_catg_logbook_list__isys_obj__id');
        }
    ...
    public function set_entry(
        ...
        $l_strQuery = "";
        $l_nUserID = isys_application::instance()->container->get('session')->get_user_id();
        $l_strUserName = "";
        $l_nSourceID = 1;
        ...
        $l_strQuery .= "INSERT INTO isys_logbook SET
			...
			isys_logbook__status = {$statusNormal},
        ...
        // @see ID-9167 Fetch the latest ID from the logbook table.
        $lastInsertedId = (int)$this
            ->retrieve('SELECT MAX(isys_logbook__id) AS id FROM isys_logbook;')
            ->get_row_value('id');

        if ($l_bAllGood) {
            $this->set_logbook_id($lastInsertedId);
        }
        ...
        if (is_numeric($p_nObjectID) && $p_strConstEvent != "C__LOGBOOK_EVENT__OBJECT_PURGED") {
            ...
            if ($l_bAllGood && isys_cmdb_dao::instance($this->m_db)
                ...
                $l_strQuery = "INSERT INTO isys_catg_logb_list
                    SET isys_catg_logb_list__isys_obj__id = " . $this->convert_sql_id($p_nObjectID) . ",
                    isys_catg_logb_list__isys_logbook__id = {$lastInsertedId},
                    isys_catg_logb_list__status = {$statusNormal};";
    ...
    public function getAllCategories(): array
    {
        ...
        $result = $this->retrieve(
            <<<SQL
        SELECT isysgui_catg__title AS 'title' FROM isysgui_catg
            UNION SELECT isysgui_cats__title AS 'title' FROM isysgui_cats
            UNION SELECT isysgui_catg_custom__title AS 'title' FROM isysgui_catg_custom;
        SQL
        );
     ...
     * @throws isys_exception_database
     */
    public function get_data(?string $fromDate = null, int $limit = 100, int $lastId = 0): isys_component_dao_result
    {
        $condition = '';
        $logbookTable = $this->getTableName('isys_logbook');
        $logbookCategoryTable = $this->getTableName('isys_catg_logb_list');

        if ($fromDate !== null) {
            $condition = "AND isys_logbook__date < " . $this->convert_sql_datetime($fromDate);
        }

        $l_query = "SELECT *
            FROM {$logbookTable}
            LEFT JOIN {$logbookCategoryTable} ON isys_logbook__id = isys_catg_logb_list__isys_logbook__id
            WHERE isys_logbook__id > {$lastId}
            {$condition}
    ...
    /**
     * Count existing logbook entries by given interval
     *
     * @param int $interval
     *
     * @return int
     * @throws isys_exception_database
     */
    public function countEntriesByInterval(int $interval = 0): int
    {
        $condition = '';
        $logbookTable = $this->getTableName('isys_logbook');
        $logbookCategoryTable = $this->getTableName('isys_catg_logb_list');
        ...
        if ($interval > 0) {
            $dateTime = date('Y-m-d H:i:s', strtotime('-' . $interval . ' days'));

            $condition = "WHERE isys_logbook__date < '{$dateTime}'";
        }

        $sql = "SELECT count(1) as entryCount
            FROM {$logbookTable}
            LEFT JOIN {$logbookCategoryTable} ON isys_logbook__id = isys_catg_logb_list__isys_logbook__id
            {$condition} LIMIT 1;";

        return (int)$this->retrieve($sql)->get_row_value('entryCount');
    }
     ...
     * @throws isys_exception_dao
     */
    public function save_logbook_config($p_id, $p_type, ?string $p_placeholder_string = null)
    {
     ...
     * @throws isys_exception_dao
     */
    public function create_logbook_config($p_type = 0, ?string $p_placeholder_string = null)
    {
        $l_update = 'INSERT INTO isys_logbook_configuration SET
            isys_logbook_configuration__type = ' . $this->convert_sql_int($p_type) . ',
            isys_logbook_configuration__placeholder_string = ' . $this->convert_sql_text($p_placeholder_string) . ';';

        if ($this->update($l_update) && $this->apply_update()) {
            // @see ID-9167 Fetch the latest ID from the logbook configuration table.
            return $this
                ->retrieve('SELECT MAX(isys_logbook_configuration__id) AS id FROM isys_logbook_configuration;')
                ->get_row_value('id');
        }
