...
class isys_component_dao_user_settings extends isys_component_dao_abstract_settings
{
    ...
    {
        if (!is_array($this->m_cached_settings) || count($this->m_cached_settings) === 0) {
            $this->m_cached_settings = [];

            $userId = $this->convert_sql_id(isys_application::instance()->container->get('session')->get_user_id());

            $query = "SELECT * 
                FROM isys_settings 
                WHERE isys_settings__isys_obj__id = {$userId} 
                ORDER BY isys_settings__key ASC;";
