...
class isys_component_calendar extends isys_component
{
     ...
     * @throws Exception
     */
    public static function factory(string $p_name, array $p_options = []): self
    {
     ...
     *
     * @param isys_component_calendar_event $p_event
     * @return self
     */
    public function add_event(isys_component_calendar_event $p_event): self
    {
     ...
     *
     * @param int|null $p_year
     * @return isys_component_calendar
     * @throws Exception
     * @deprecated Use a third party library like 'azuyalabs/yasumi' instead. Will be removed in i-doit 40!
     */
    public function add_holidays(?int $p_year = null): isys_component_calendar
    {
        ...
        return $this
            ->add_event(isys_component_calendar_event::factory($this->language->get('LC__CALENDAR_EVENT__NEW_YEAR'), 1, 1))
            ->add_event(isys_component_calendar_event::factory($this->language->get('LC__CALENDAR_EVENT__VALENTINES_DAY'), 14, 2))
            ->add_event(isys_component_calendar_event::factory($this->language->get('LC__CALENDAR_EVENT__SAINT_PATRICKS_DAY'), 17, 3))
            ->add_event(isys_component_calendar_event::factory($this->language->get('LC__CALENDAR_EVENT__EASTER'), $l_day, $l_month, $l_year))
            ->add_event(isys_component_calendar_event::factory($this->language->get('LC__CALENDAR_EVENT__HALLOWEEN'), 31, 10))
            ->add_event(isys_component_calendar_event::factory($this->language->get('LC__CALENDAR_EVENT__CHRISTMAS'), 24, 12));
    }
     ...
     * @return string|array Depending on the parameter, you'll get a rendered calendar or the raw data.
     * @throws Exception
     */
    public function render(bool $p_render_template = true): string|array
    {
        $l_data = [];
        ...
        $l_month_names = array_map(fn (string $month) => $this->language->get($month), [
            'LC__UNIVERSAL__CALENDAR__MONTHS_JANUARY',
            'LC__UNIVERSAL__CALENDAR__MONTHS_FEBRUARY',
            'LC__UNIVERSAL__CALENDAR__MONTHS_MARCH',
            'LC__UNIVERSAL__CALENDAR__MONTHS_APRIL',
            'LC__UNIVERSAL__CALENDAR__MONTHS_MAY',
            'LC__UNIVERSAL__CALENDAR__MONTHS_JUNE',
            'LC__UNIVERSAL__CALENDAR__MONTHS_JULY',
            'LC__UNIVERSAL__CALENDAR__MONTHS_AUGUST',
            'LC__UNIVERSAL__CALENDAR__MONTHS_SEPTEMBER',
            'LC__UNIVERSAL__CALENDAR__MONTHS_OCTOBER',
            'LC__UNIVERSAL__CALENDAR__MONTHS_NOVEMBER',
            'LC__UNIVERSAL__CALENDAR__MONTHS_DECEMBER',
        ]);
     ...
     * @return self
     */
    public function set_options(array $p_options): self
    {
     ...
     * @return self
     */
    public function merge_options(array $p_options): self
    {
     ...
     * @return array
     */
    private function retrieve_day_data(int $p_timestamp): array
    {
