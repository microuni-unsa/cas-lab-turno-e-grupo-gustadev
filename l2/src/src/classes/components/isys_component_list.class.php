...
class isys_component_list extends isys_cmdb_dao_list_objects
{
     ...
     * @var bool
     */
    private bool $resetFilter = false;
     ...
     * @param string|null $p_method
     *
     * @return $this
     */
    public function set_row_modifier($p_object, $p_method = null)
    {
     ...
     * @return $this
     */
    public function set_listdao($listDao): self
    {
    ...
    public function createTempTable()
    {
        ...
        if ($this->m_resData) {
            ...
            foreach ($l_header as $l_field => $l_field_text) {
                ...
                if (strstr($l_field, "__id") || in_array($l_field, ["object_id", "object_count", 'counter']) || strpos($l_field, "_count") === strlen($l_field) - strlen('_count')) {
                    $l_type = "INT(10)";
                } else {
                    $l_type = "TEXT";
                }
        ...
        if ($l_bEverythingGood) {
            ...
            if ($this->m_resData) {
                ...
                if ($this->m_listdao != null && $this->m_listdao instanceof isys_component_dao_object_table_list) {
                    ...
                    $listConfiguration = $this->m_listdao->getConfiguration();
                }
                    ...
                    foreach ($l_header as $l_key => $l_field_text) {
                        if ($l_key == $this->m_id) {
                            ...
                            if (is_scalar($l_row)) {
                                ...
                                if (is_array($l_row)) {
                                    // @ID-11048 Consider the defined grouping.
                                    if ($listConfiguration->getGroupingType() === isys_cmdb_dao_category_property_ng::C__GROUPING__LIST) {
                                        $l_row = '<ul class="list-style-none m0"><li>' . implode('</li><li>', $l_row) . '</li></ul>';
                                    } else {
                                        $l_row = implode(', ', $l_row);
                                    }
                                } else {
     ...
     * @return bool
     */
    public function isResetFilter(): bool
    {
    ...
    public function render()
    {
        ...
        $sessionSource = Configuration::initSessionSource($cacheKey);

        if (!isset($_GET['suggestion']) && !$this->isResetFilter()) {
            if (empty($filterValues)) {
                $filterValues = $sessionSource->get();
            } else {
        ...
        foreach ($tableConfig->getProperties() as $i => $property) {
            $header[$property->getPropertyKey()] = $lang->get($property->getName());
        ...
        $routeParams = array_filter(
            $routeParams,
            fn ($item) => !in_array($item, $this->routeRenderBlockList),
        ...
        $options = [
            ...
            'resizeTableAjaxURL'    => isys_helper_link::create_url([
                C__GET__AJAX      => 1,
                C__GET__AJAX_CALL => 'table',
                'func'            => 'saveTableWidth',
                'identifier'      => 'cmdb.obj-' . $cacheKey . '.table-width'
            ]),
            'columnSizes'           => isys_usersettings::get('cmdb.obj-' . $cacheKey . '.table-columns', []),
            'tableSize'             => isys_usersettings::get('cmdb.obj-' . $cacheKey . '.table-width', 0),
            'rowsPerPage'           => (int)$_GET['rowsPerPage'] ?: \isys_usersettings::get('gui.objectlist.rows-per-page', 50),
            'replacerOptions'       => $this->getReplacerOptions($tableConfig),
        ...
        // @see  ID-6902  We merge the basic options with custom ones.
        $options = array_replace_recursive($options, $this->options);

        // @see  ID-6902  We had to move this down so it's still possible to modify the data.
        $options['replacerOptions'] = isys_format_json::encode($options['replacerOptions']);
    ...
    protected function getReplacerOptions(Config $tableConfig)
    {
        ...
        return [
            'memoryUnit'               => $tableConfig->getAdvancedOptionMemoryUnit(),
            ...
            'linkEmailAddresses' => $tableConfig->getShowEmailLinks()
        ];
    }
    ...
    {
        if (empty($this->getTempTableName())) {
            return '';
        }
     ...
     */
    public function getTenantTableConfig(?string $constant = null): ?Config
    {
        $category = $constant ?? $this->get_key() ?? null;
        if ($category && isys_tenantsettings::has("cmdb.multilist-table.default-config.{$category}")) {
            $config = isys_tenantsettings::get("cmdb.multilist-table.default-config.{$category}");

            // @see ID-10384 Use new JSON format.
            if (is_string($config) && trim($config) !== '' && isys_format_json::is_json_array($config)) {
                $config = isys_format_json::decode($config);
            }

            if (is_array($config) && isset($config['isys_obj_type_list__table_config']) && isys_format_json::is_json_array($config['isys_obj_type_list__table_config'])) {
                $config = Config::fromJson($config['isys_obj_type_list__table_config']);
            }

            // @see ID-11972 Verify that we return a 'Config' object.
    ...
    {
        return $this->getTenantTableConfig() ?? $this->getConfigWithAllProperties();
    }
    ...
    public function getConfigWithAllProperties()
    {
        ...
        if ($this->m_listdao instanceof isys_component_dao_category_table_list) {
            $categoryId = $this->m_listdao->get_dao_category()->get_category_id();
            $categoryTypeId = $this->m_listdao->get_dao_category()->get_category_type();
            $unfilterable = $this->m_listdao->getUnfilterableProps();
        ...
        foreach ($fields as $property => $header) {
            ...
            // @see ID-9761 There is no way to properly fetch the correct property data here, so we'll do it hardcoded right now.
            if (in_array($property, $unfilterable, true)) {
    ...
    {
        $uid = isys_application::instance()->container->get('session')->get_user_id();

        $keys = [
            'cmdb.multilist-table.config.' . $uid . '.' . $this->get_key(),
            'cmdb.multilist-table.default-config.' . $this->get_key()
        ];

        foreach ($keys as $key) {
            $data = isys_tenantsettings::get($key);

            if (is_array($data) && count($data)) {
                return $data;
            }

            if (trim($data) !== '' && isys_format_json::is_json_array($data)) {
    ...
    {
        $data = [
            'isys_obj_type_list__table_config'  => $tableConfig->toJson($this->get_key()),
            'isys_obj_type_list__row_clickable' => $tableConfig->isRowClickable()
        ];
        isys_tenantsettings::set('cmdb.multilist-table.default-config.' . $this->get_key(), isys_format_json::encode($data));
    }
    ...
    public function save_table_config(Config $tableConfig, $uid = null)
    {
        ...
        $uid = $uid ?: isys_application::instance()->container->get('session')->get_user_id();
        isys_tenantsettings::set('cmdb.multilist-table.config.' . $uid . '.' . $this->get_key(), isys_format_json::encode($data));
    }
     ...
     * @return $this
     */
    public function setRouteRenderBlockList(array $blockList): isys_component_list
    {
        $this->routeRenderBlockList = $blockList;
        return $this;
    }
     ...
     * @return $this
     */
    public function addToRouteRenderBlockList(array $blockList): isys_component_list
    {
        $this->routeRenderBlockList = array_merge($this->routeRenderBlockList, $blockList);
        return $this;
    }
    ...
    {
        // @see ID-11507 Prioritize category list logic, if available.
        if ($this->m_listdao && $this->m_listdao instanceof isys_component_dao_category_table_list) {
            return $this->m_listdao->get_rec_counts();
        }

        return parent::get_rec_counts();
    }
}
