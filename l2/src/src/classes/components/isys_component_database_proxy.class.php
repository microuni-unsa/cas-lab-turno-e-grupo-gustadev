...
class isys_component_database_proxy extends isys_component_database
{
    ...
    {
        if ($this->db instanceof isys_component_database) {
            $this->db->close();
        }

        // @see ID-9614 Do not allow a recursion!
        if ($database !== $this) {
            $this->db = $database;
        }
    }
     ...
     * @see ID-9614 When setting the database, we need this in order to not set the proxy!
     */
    public function getDatabase(): ?isys_component_database
    {
        if ($this->db && $this->db !== $this && $this->db instanceof isys_component_database) {
            return $this->db;
        }
    ...
    {
        if ($this->db !== null) {
            return $this->db->format_number($p_value);
        }
