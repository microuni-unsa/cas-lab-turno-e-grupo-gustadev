...
class isys_component_menu extends isys_component
{
    ...
    public static function get_default_menu_as_constant()
    {
        ...
        // @see ID-12228 Prevent false menu highlight for 'Extras > Services'.
        if (isset($l_gets[C__GET__MODULE_ID]) && $l_gets[C__GET__MODULE_ID] != C__MODULE__CMDB) {
            return false;
        }
    ...
    public function set_objtype_group_menu()
    {
        ...
        if ($l_cache === false) {
            try {
                ...
                $orphanedObjectTypeGroup = $cmdbDao->convert_sql_id(defined_or_default('C__OBJTYPE_GROUP__ORPHANED'));

                if (is_array($l_allowed_objecttypes) && count($l_allowed_objecttypes) > 0) {
                    $sql = 'SELECT DISTINCT(isys_obj_type_group__id), isys_obj_type_group__title, isys_obj_type_group__const
                        FROM isys_obj_type
                        INNER JOIN isys_obj_type_group ON isys_obj_type__isys_obj_type_group__id = isys_obj_type_group__id
                        WHERE isys_obj_type_group__status = ' . $cmdbDao->convert_sql_int(C__RECORD_STATUS__NORMAL) . '
                        AND isys_obj_type__id ' . $cmdbDao->prepare_in_condition($l_allowed_objecttypes) . '
                        AND isys_obj_type_group__id != ' . $orphanedObjectTypeGroup;
                    ...
                    $sql = 'SELECT isys_obj_type_group__id, isys_obj_type_group__title, isys_obj_type_group__const
                        FROM isys_obj_type_group
                        WHERE ' . ($l_allowed_objects_condition === true ? 'TRUE' : 'FALSE') . '
                        AND isys_obj_type_group__id != ' . $orphanedObjectTypeGroup;
                }

                $l_res = $cmdbDao->retrieve($sql . ' ORDER BY isys_obj_type_group__sort ASC');
    ...
    public function set_other_menu()
    {
        ...
        try {
            ...
            if ($l_cache === false) {
                ...
                $l_menu_item[C__MAINMENU__ADDONS] = $this->m_other_menu[C__MAINMENU__ADDONS] = [
                    '#',
                    'LC__UNIVERSAL__ADD_ONS',
                    null,
                    'add-ons'
                ];
     ...
     * @throws Exception
     */
    private function hasExtrasMenu(): bool
    {
        try {
            $moduleManager = isys_application::instance()->container->get('moduleManager');
            $moduleResult = $moduleManager->get_modules();

            // Iterate through the modules and display each.
            while ($module = $moduleResult->get_row()) {
                if (!class_exists($module['isys_module__class'])) {
                    continue;
                }

                $isCmdb = $module['isys_module__const'] === 'C__MODULE__CMDB';
                $showInMainMenu = constant($module['isys_module__class'] . '::DISPLAY_IN_MAIN_MENU');
                $authInstance = $moduleManager->get_module_auth($module['isys_module__id']);

                if (!$isCmdb && $showInMainMenu && $authInstance && $authInstance->has_any_rights_in_module()) {
                    return true;
                }

                $additionalEntries = $module['isys_module__class']::get_additional_links();

                foreach ($additionalEntries as $key => $l_content) {
                    if ($key == 'RELATION') {
                        $cmdbAuth = isys_auth_cmdb::instance();
                        $allowedToViewRelations = $cmdbAuth->is_allowed_to(isys_auth::VIEW, 'OBJ_IN_TYPE/C__OBJTYPE__RELATION');
                        $allowedToViewParallelRelations = $cmdbAuth->is_allowed_to(isys_auth::VIEW, 'OBJ_IN_TYPE/C__OBJTYPE__PARALLEL_RELATION');

                        if ($allowedToViewRelations || $allowedToViewParallelRelations) {
                            return true;
                        }
                    } elseif ($authInstance && $authInstance->is_allowed_to(isys_auth::VIEW, $key)) {
                        return true;
                    }
                }
            }
    ...
    public function set_my_doit()
    {
        ...
        if (defined('C__MAINMENU__MYDOIT') && $userSettings->get('gui.show-my-doit', 0)) {
    ...
    {
        $objectTypeGroupDropDown = [
            'object-type-group' => [
                '#',
                isys_application::instance()->container->get('language')->get('LC__CMDB__OBJTYPE__GROUP'),
                null,
                'object-type-group hide'
            ]
        ];

        // @see ID-9008 Always show everything, the frontend will hide whatever is necessary.
        return $this->m_mainmenu + $this->get_objecttype_group_menu() + $objectTypeGroupDropDown + $this->m_other_menu;
    }
