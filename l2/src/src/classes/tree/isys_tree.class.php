...
class isys_tree extends RecursiveArrayIterator implements JsonSerializable
{
    ...
    public function remove_node(isys_tree_node $p_node, $p_recursive = false)
    {
        ...
        foreach ($this->m_children as $l_index => $l_node) {
            if ($l_node === $p_node) {
                ...
                return true;
            } elseif ($p_recursive && $l_node->has_children()) {
                $l_mynodes[] = $l_node;
            }
        }
    ...
    /**
     * @param bool $p_include_childs
     *
     * @return int
     */
    public function count($p_include_childs = false): int
    {
        if ($p_include_childs) {
     ...
     * @return  boolean
     */
    public function offsetExists($p_key): bool
    {
     ...
     * @param  mixed $value
     */
    public function offsetSet($p_key, $value): void
    {
     ...
     * @param  mixed $p_key
     */
    public function offsetUnset($p_key): void
    {
