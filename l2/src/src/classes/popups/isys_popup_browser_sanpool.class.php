    ...
    public function handle_smarty_include(isys_component_template $template, $p_params)
    {
        ...
        if (isys_glob_is_edit_mode()) {
            ...
            return $l_objPlugin->navigation_edit($template, $p_params) .
                '<button type="button" title="' . $this->language->get('LC_SANPOOL_POPUP__SELECT_SAN') . '" class="btn" onClick="' . $this->process_overlay('', 800, 360, $p_params) . '" data-tooltip="1">' .
                '<img src="' . isys_application::instance()->www_path . 'images/axialis/basic/link.svg" class="greyscale" alt="" />' .
                '</button>' .
                '<button type="button" title="' . $this->language->get('LC__UNIVERSAL__DETACH') . '" class="btn" onClick="' . $l_onclick_detach . '" data-tooltip="1">' .
                '<img src="' . isys_application::instance()->www_path . 'images/axialis/basic/link-broken.svg" class="greyscale" alt="" />' .
                '</button>' .
                $l_hidden_devices .
                $l_hidden_raids;
        }
