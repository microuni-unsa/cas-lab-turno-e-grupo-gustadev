...
class isys_popup_browser_cable_connection_ng extends isys_popup_browser_object_ng
{
    ...
    public function &handle_module_request(isys_module_request $p_modreq)
    {
        ...
        $this->template
            ...
            ->assign('creatableObjectTypes', $this->getCreatableObjectTypes(substr($this->m_params['hidden'], 0, -8)))
    ...
    protected function handle_preselection($preselection, $unused = null)
    {
        ...
        // @see  ID-7114  Instead of calling `parent::handle_preselection()` we do the logic here, since there'll surely be a reason why we don't call the parent method.
    ...
    public function handle_smarty_include(isys_component_template $template, $parameters)
    {
        ...
        if (str_contains($this->m_params['name'], '[') && str_contains($this->m_params['name'], ']')) {
        ...
        $l_strHiddenField = '<input id="' . $l_attr['hidden'] . '" name="' . $l_attr['hidden'] . '" class="' . $this->m_params['hidden_class'] . '" type="hidden" value="' . $l_port_id . '" />' .
            '<input id="' . $l_attr['cable_name'] . '" name="' . $l_attr['cable_name'] . '" type="hidden" value="" />';

        $l_detach_callback = $this->m_params[self::C__CALLBACK__DETACH] ?? "";
        ...
        if (isys_glob_is_edit_mode() || $this->m_params[self::C__EDIT_MODE]) {
            ...
            // @see ID-11141 Add 'data-hidden-field' and 'data-last-value' attributes for list edit.
            $this->m_params['p_additional'] = ($this->m_params['p_additional'] ?? '') . ' data-hidden-field="' . str_replace('"', "'", $l_hidden) . '" data-last-value="' . $this->m_params['p_strValue'] . '"';

            $textfield = $l_objPlugin->navigation_edit($this->template, $this->m_params);

            $onClick = $this->process_overlay("live_preselection=' + \$F('" . $l_attr['hidden'] . "') + '", '80%', '90%', $this->m_params, null, 1000, 300, 1600, 800);

            $attachButton = '<button type="button" title="' . $this->language->get('LC__UNIVERSAL__SELECT') . '" class="' . $this->m_params['name'] . ' attach btn" onclick="' . $onClick . '" data-tooltip="1">' .
                '<img src="' . $imageDirectory . 'axialis/basic/link.svg" class="greyscale" alt="" />' .
                '</button>';

            $detachButton = '<button type="button" title="' . $this->language->get('LC__UNIVERSAL__DESELECT') . '" class="' . $this->m_params['name'] . ' detach btn" onclick="' . $l_onclick_detach . ';" data-tooltip="1">' .
                '<img src="' . $imageDirectory . 'axialis/basic/link-broken.svg" class="greyscale" alt="" />' .
                '</button>';

            return $textfield . $attachButton . $detachButton . $l_strHiddenField;
        }

        $this->m_params['p_bHtmlDecode'] = true;
        $this->m_params['p_strValue'] = $this->format_selection($l_port_id);

        return $l_objPlugin->navigation_view($this->template, $this->m_params) . $l_strHiddenField;
    }

    /**
     * @param bool $m_only_logical_ports
     *
     * @return isys_popup_browser_cable_connection_ng
     */
    public function setOnlyLogicalPorts(bool $m_only_logical_ports): isys_popup_browser_cable_connection_ng
    {
        $this->m_only_logical_ports = $m_only_logical_ports;
        return $this;
    }
}
