...
<h3 class="border-top border-bottom bg-neutral-200 p10 mt5">[{isys type="lang" ident="LC__CMDB__CATS__LAYER2_NET__IP_HELPER_ADDRESS"}]</h3>
...
<div id="new_iphelper_template" class="hide">
    [{isys
        ...
        p_strTable="isys_layer2_iphelper_type"
        name="{type_name}"
        id="{type_id}"
        p_strClass="input-small ip_helper_types"
        p_bInfoIconSpacer=0
        callback_accept="idoit.callbackManager.triggerCallback('l2net_update_ip_helper');"}]
    [{isys
        type="f_text"
        name="{ip_name}"}]
</div>
            ...
            add_iphelper.on('click', function () {
                ...
                l_new_row
                    ...
                    .insert(new Element('td').update(new Element('button', {type:'button', className:'btn remove ml10 mr10'})
                        .update(new Element('img', {src:'[{$dir_images}]axialis/basic/symbol-cancel.svg', alt:''}))
