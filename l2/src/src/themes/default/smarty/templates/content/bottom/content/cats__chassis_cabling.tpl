...
<div id="C__CATS__CHASSIS_CABLING">
	...
	<div data-obj-id="[{$object.id}]" class="chassis-cabling-box [{if ! $smarty.foreach.cabling_objects.last}]mb15[{/if}]">
		<h3 class="p5 border-top">[{$object.title}] ([{$object.type_title}])</h3>
		...
		<button type="button" class="btn m10 new-log-port-button">
			<img src="[{$dir_images}]axialis/basic/symbol-add.svg" alt="" /><span>[{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_CABLING__CREATE_NEW_CONNECTION_TO"}]</span>
		</button>
				...
				<tr>
					...
					<td class="value">
						<button type="button" class="btn create-new-log-port-button ml20 mr5" data-obj-id="[{$object.id}]">
							<img src="[{$dir_images}]axialis/basic/symbol-ok.svg" alt="w" /><span>[{isys type="lang" ident="LC__UNIVERSAL__BUTTON_ADD"}]</span>
						</button>
						<button type="button" class="btn cancel-new-log-port-button">
							<img src="[{$dir_images}]axialis/basic/symbol-cancel.svg" alt="w" /><span>[{isys type="lang" ident="LC__NAVIGATION__NAVBAR__CANCEL"}]</span>
						</button>
					</td>
				</tr>
			</table>
		...
		window.create_remove_buttons = function () {
			...
			$$('.log-ports tbody tr').each(function($el) {
				var cat_id = $el.readAttribute('data-log-port-id'),
					td = $el.down('td'),
					button = new Element('button', {type:'button', className: 'btn remove', 'data-log-port-id': cat_id}).update(new Element('img', {src:'[{$dir_images}]axialis/basic/symbol-cancel.svg'}));
