...
<table class="contentTable m0">
	...
	<tr>
		...
		<td class="value">
			<button type="button" id="net-collision-checker" class="btn ml20">
				<img src="[{$dir_images}]axialis/basic/zoom.svg" alt="" /><span>[{isys type="lang" ident="LC__CMDB__CATS__NET__CHECK"}]</span>
			</button>
	...
	<tr class="IPv6 IP">
		...
		<td class="value">
            ...
            <span class="[{if isys_glob_is_edit_mode()}]fl text-center[{else}]ml5 mr5[{/if}]" [{if isys_glob_is_edit_mode()}]style="width:20px; height:32px;padding: 8px 0;"[{/if}]>/</span>
...
<script>
	(function () {
		...
		var $net_v4 = $('C__CATS__NET__NET_V4'),
            ...
            $addressV6_range_from = $('C__CATS__NET__ADDRESS_RANGE_FROM'),
			$addressV6_range_to = $('C__CATS__NET__ADDRESS_RANGE_TO'),
		...
		var setNetArea = function () {
			...
			if ($net_v4_cidr.getValue() === '31') {
				...
				if (ip_tmp.from_long > ip_tmp.to_long) {
					$address_range_from.setValue(ip_tmp.to).simulate('change');
					$address_range_to.setValue(ip_tmp.from).simulate('change');
				} else {
					$address_range_from.setValue(ip_tmp.from).simulate('change');
					$address_range_to.setValue(ip_tmp.to).simulate('change');
				}
			...
			if ($net_v4_cidr.getValue() === '32') {
				$address_range_from.setValue($net_v4.getValue()).simulate('change');

				$address_range_to.setValue($address_range_from.getValue()).simulate('change');
				return;
			}
		...
		const calcNetMaskBit = function (p_val) {
			let bit = 7;
			let counter_bits = 7;
			let bit_value = 0;
		...
		if ($net_collision_check) {
			$net_collision_check.on('click', function () {
                let ip_from = $address_range_from.getValue();
                let ip_to = $address_range_to.getValue();

                if ($net_type.getValue() === '[{$smarty.const.C__CATS_NET_TYPE__IPV6}]') {
                    ip_from = $addressV6_range_from.getValue();
                    ip_to = $addressV6_range_to.getValue();

                    if (ip_from.blank() || ip_to.blank()) {
                        idoit.Notify.info('[{isys type="lang" ident="LC__CMDB__CATS__NET__CHECK_NO_ADDRESS"}]', { life: 10 });
                        $net_v6.focus();
                        return;
                    }
                }
				...
				new Ajax.Request(idoit.Router.getRoute('cmdb.category.net.check-net-collision'),
					{
						parameters: {
							from: ip_from,
							to: ip_to,
							networkType: $net_type.getValue(),
							objectId: '[{$net_id}]'
						},
						method: 'POST',
						onSuccess: function (transport) {
							...
							$net_collision_check
								.down('img').removeClassName('animation-rotate').writeAttribute('src', '[{$dir_images}]axialis/basic/zoom.svg')
							...
							if (json.success) {
								if (json.data === null) {
                                    ...
                                    let netCount = json.data.plain.length;
                                    let netRemaining = 0;
                                    let additionalNote = '';

                                    if (netCount > 3) {
                                        netRemaining = netCount - 3;
                                        additionalNote = '[{isys type="lang" ident="LC__CMDB__CATS__NET__CHECK_COLLISIONS_ADDITION"}]'.replace('%s', '' + netRemaining);
                                    }

									$net_collision_check_result
										.addClassName('box-red')
										.writeAttribute('title', json.data.plain.join("\n"))
										.update('[{isys type="lang" ident="LC__CMDB__CATS__NET__CHECK_COLLISIONS_DETECTED"}]: ' + json.data.html.slice(0, 3).join(', ') + additionalNote);
								}
        ...
        idoit.callbackManager
            .registerCallback('cmdb-cats-net-dns_domain-update', function (selected) {
                ...
                try {
                    $('C__CATS__NET__DNS_DOMAIN_chosen').remove();
                } catch (e) {
                    // Do nothing.
                }
                ...
                $('C__CATS__NET__DNS_DOMAIN_chosen').setStyle({ width: '575px'});
            });
        [{/if}]

        // @see ID-12157 Fill both fields, so that they appear in the logbook changes.
        if ($address_range_from) {
            $address_range_from.on('change', () => $addressV6_range_from.setValue($address_range_from.getValue()));
        }

        if ($address_range_to) {
            $address_range_to.on('change', () => $addressV6_range_to.setValue($address_range_to.getValue()));
        }
    })();
</script>
