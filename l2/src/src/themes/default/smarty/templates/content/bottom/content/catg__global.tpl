<table class="contentTable">
	<tr>
		...
		<td class="value">
            ...
            [{if $placeholders_g_global && $editMode}]
				...
				<button type="button" class="btn" onclick="Effect.toggle('placeholderHelper_g_global', 'slide', {duration:0.2});">
					<img src="[{$dir_images}]axialis/basic/button-help.svg" alt="" />
				</button>
			</div>
	...
	<tr>
		<td class="key">[{isys type="f_label" name="C__OBJ__ID" ident="LC__UNIVERSAL__OBJECT_ID"}]</td>
	...
	(function () {
		...
		var updateObjectTitle = function(ev) {
			...
			if ($titleField) {
				...
				if ($('navMode').getValue() !== '[{$smarty.const.C__NAVMODE__EDIT}]') {
					$titleField.select();
				}
