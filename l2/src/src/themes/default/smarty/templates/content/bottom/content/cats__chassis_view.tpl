...
<table id="chassis">
	<tr>
		<td class="vat">
			<div class="border mb15 mr5" style="width:auto;">
				<h3 class="p5 bg-neutral-200 border-bottom">[{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_ENCLOSURE"}] [{isys type="lang" ident="LC__UNIVERSAL__FRONT"}]</h3>
				<div id="chassis_front" class="canvas"></div>
			</div>
			<div class="border mb15 mr5" style="width:auto;">
				<h3 class="p5 bg-neutral-200 border-bottom">[{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_ENCLOSURE"}] [{isys type="lang" ident="LC__UNIVERSAL__REAR"}]</h3>
				<div id="chassis_rear" class="canvas"></div>
			</div>
		</td>
		<td class="vat" style="width: 250px">
			<div id="notification" class="p5 mb15"></div>

			<div class="border mb15">
				<h3 class="p5 bg-neutral-200 border-bottom">[{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_VIEW__AVAILABLE_SLOTS"}]</h3>

				<div id="unassigned-slot-list" class="p5">
				[{foreach from=$slots item=slot}]
					<button type="button" class="unassigned-slot btn btn-secondary btn-block mb5" data-assigned="[{intval($slot.assigned)}]" data-insertion="[{$slot.isys_cats_chassis_slot_list__insertion}]" data-slot-css="m[{$slot.isys_cats_chassis_slot_list__y_from}]-[{$slot.isys_cats_chassis_slot_list__x_from}]" data-slot-id="[{$slot.isys_cats_chassis_slot_list__id}]" data-slot-title="[{$slot.isys_cats_chassis_slot_list__title}]">
						<img src="[{$dir_images}]icons/silk/brick_[{if $slot.assigned === false}]add[{else}]edit[{/if}].png" alt="Add" class="vam" /><span>[{$slot.isys_cats_chassis_slot_list__title}] ([{if $slot.isys_cats_chassis_slot_list__insertion == $smarty.const.C__INSERTION__FRONT}][{isys type="lang" ident="LC__UNIVERSAL__FRONT"}][{else}][{isys type="lang" ident="LC__UNIVERSAL__REAR"}][{/if}])</span>
					</button>
				[{/foreach}]
				</div>
			</div>
			...
			<div id="slot_options" class="box">
				<h3 class="p5 bg-neutral-200 border-bottom">[{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_VIEW__SLOT_OPTIONS"}]</h3>
				<div class="m5">
					<button type="button" id="option-reassign" class="btn btn-secondary btn-block">
                        <img src="[{$dir_images}]axialis/basic/sort.svg" alt="" /><span>[{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_VIEW__CHANGE_POSITION"}]</span>
                    </button>
					<button type="button" id="option-remove" class="btn btn-secondary btn-block">
                        <img src="[{$dir_images}]axialis/basic/symbol-cancel.svg" alt="" /><span>[{isys type="lang" ident="LC__UNIVERSAL__RESET_ASSIGNMENT"}]</span>
                    </button>
					<button type="button" id="option-save" class="btn btn-secondary btn-block">
                        <img src="[{$dir_images}]axialis/basic/symbol-ok.svg" alt="" /><span>[{isys type="lang" ident="LC__NAVIGATION__NAVBAR__SAVE"}]</span>
                    </button>

					<p id="option-assign-description" class="m5 p5 display-flex align-items-center">
                        <img src="[{$dir_images}]axialis/basic/button-info.svg" class="mr5" alt="" />
                        <span>[{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_VIEW__DESCRIPTION"}]</span>
                    </p>
            ...
            <div id="slot_assigned_devices" class="box mt15">
                <h3 class="p5 bg-neutral-200 border-bottom">[{isys type="lang" ident="LC__CMDB__CATS__CHASSIS_DEVICES"}]</h3>
...
<script type="text/javascript">
	var matrix_front = JSON.parse('[{$matrix_front|escape:"quotes"}]'),
		devices_front = JSON.parse('[{$devices_front|escape:"quotes"}]'),
		matrix_rear = JSON.parse('[{$matrix_rear|escape:"quotes"}]'),
		devices_rear = JSON.parse('[{$devices_rear|escape:"quotes"}]'),
		editmode = !! $('C__CMDB__CATS__CHASSIS_VIEW__X_FRONT'), // This is more reliable than the "is_edit_mode()" function.
		unassigned_slots = $$('.unassigned-slot');

	$('matrix-size-notice').hide();

	idoit.Require.require('chassis', function () {
        ...
        window.reset_slot_options = function () {
            ...
            $$('.unassigned-slot.pressed').invoke('removeClassName', 'pressed');
            $('info').update().hide();
        };
        ...
        window.infobox = function (ev) {
            ...
            if ($trigger.tagName.toLowerCase() === 'button') {
                ...
                $td = $('chassis').down('td[data-slot-id=' + $trigger.readAttribute('data-slot-id') + ']');
            } else {
                $td = $trigger.up('td');
            }
        ...
        window.display_slot_options = function () {
            ...
            unassigned_slots.invoke('removeClassName', 'pressed');
                    ...
                    onSuccess: function (transport) {
                        ...
                        if(json.success) {
                            json.devices.each(function(ele){
                                $('assigned_devices').insert(new Element('a', { href: '?[{$smarty.const.C__CMDB__GET__OBJECT}]=' + ele.id })
                                    .update(new Element('img', { src: window.dir_images + 'axialis/basic/link.svg', className: 'mr5' }))
                                    .insert(new Element('span').update(ele.title)));
                            });
                        } else {
                            $('assigned_devices').insert(new Element('div', { className: 'box-red p5'}).update(json.message));
                        }
                    }
                });
        };
        ...
        window.save_slot_position = function () {
            var slot_id = this.readAttribute('data-slot-id'),
                ...
                insertion = $$('.unassigned-slot[data-slot-id=' + slot_id + ']')[0].readAttribute('data-insertion');
            ...
            new Ajax.Request('?ajax=1&call=chassis&func=assign_slot_position',
                {
                    ...
                    onSuccess:function (transport) {
                        ...
                        if (json.success) {
                            ...
                            $$('.unassigned-slot[data-slot-id=' + slot_id + ']')[0]
        ...
        window.remove_slot_position = function (ev) {
            ...
            $('slot_options').show()
                .down('h3').update($('unassigned-slot-list').down('.unassigned-slot[data-slot-id=' + slot_id + ']').readAttribute('data-slot-title'));
            ...
            new Ajax.Request('?ajax=1&call=chassis&func=remove_slot_position',
                {
                    ...
                    onSuccess:function (transport) {
                        ...
                        if (json.success) {
                            ...
                            // Update the data attribute, so the new positioned slot will be highlighted.
                            var slot = $$('.unassigned-slot[data-slot-id=' + slot_id + ']')[0]
        ...
        window.options_callback = function (ev) {
            var $contextWrapper = $('chassisContextPopup'),
                ...
                slot_title = $$('.unassigned-slot[data-slot-id=' + slot_id + ']')[0].readAttribute('data-slot-title'),
            ...
            $ul_options.insert(
                new Element('li').update(
                    new Element('button', {id: 'slot-option-reassign', 'data-slot-id': slot_id, className: 'btn btn-block', type: 'button'})
                        .update(new Element('img', {src: '[{$dir_images}]axialis/basic/sort.svg'}))
            ...
            $ul_options.insert(
                new Element('li').update(
                    new Element('button', {id: 'slot-option-remove', 'data-slot-id': slot_id, className: 'btn btn-block', type: 'button'})
                        .update(new Element('img', {src: '[{$dir_images}]axialis/basic/symbol-cancel.svg'}))
            ...
            if (!!element.down('div.device')) {
                $ul_options.insert(
                    new Element('li').update(
                        new Element('button', {id: 'option-information', 'data-slot-id': slot_id, className: 'btn btn-small btn-block', type: 'button'})
                            .update(new Element('img', {src: '[{$dir_images}]axialis/basic/button-info.svg'}))
            ...
            $contextWrapper = new Element('div', {id: 'chassisContextPopup', className:'popup', style: 'top: ' + y + 'px; left: ' + x + 'px;'})
                .update(new Element('div', { className: 'bg-neutral-200 p5 border-bottom' }).update(new Element('strong').update(slot_title)))
                .insert($ul_options);
