<div id="catg-vrrp-view" class="p5">
	...
	<div class="border bg-white">
		<h3 class="p5 border-bottom">[{$vrrp.info.url}]</h3>
		...
		<fieldset class="overview">
			...
			<div><a href="[{$port.url}]"><img src="[{$dir_images}]icons/silk/link.png" class="vam mr5" />[{$port.title}]</a>[{if $port.mac}]<em class="text-neutral-400 ml5">[{$port.mac}]</em>[{/if}]
			[{if $port.parent}], [{isys type="lang" ident="LC__CMDB__CATG__INTERFACE_L__PARENT"}]: <a href="[{$port.parent.url}]"><img src="[{$dir_images}]icons/silk/link.png" class="vam mr5" />[{$port.parent.title}]</a>[{if $port.parent.mac}]<em class="text-neutral-400 ml5">[{$port.parent.mac}]</em>[{/if}][{/if}]
			</div>
			[{/foreach}]
		</fieldset>
	</div>
	[{foreachelse}]
		<p class="m5 p5 text-blue display-flex align-items-center">
            <img src="[{$dir_images}]axialis/basic/button-info.svg" class="mr5" /><span>[{isys type="lang" ident="LC__CATG__VRRP_VIEW__INFORMATION"}]</span>
        </p>
	[{/foreach}]
</div>
