<div id="catg-images">
	<div class="display-flex align-items-center p5">
        <div id="catg-images-droppable"></div>
        <div id="catg-images-options" class="ml-auto">
            <button type="button" data-action="order-toggle" class="btn mr10"><img src="[{$dir_images}]axialis/basic/sort.svg"></button>

            <div class="btn-group">
                <button type="button" data-action="zoom-out" class="btn"><img src="[{$dir_images}]axialis/basic/symbol-remove.svg"></button>
                <button type="button" data-action="zoom-null" class="btn"><img src="[{$dir_images}]axialis/basic/zoom.svg"></button>
                <button type="button" data-action="zoom-in" class="btn"><img src="[{$dir_images}]axialis/basic/symbol-add.svg"></button>
            </div>
        </div>
    </div>
    ...
    (function () {
        ...
        idoit.Require.require('fileUploader', function () {
            var $container        = $('catg-images'),
                ...
                $options          = $('catg-images-options'),
                $gallery          = $('catg-images-gallery'),
                images            = '[{$images}]'.evalJSON(),
                thumb_size        = 300,
                $orderButton      = $options.down('[data-action="order-toggle"]'),
                $orderPopup       = new Element('div', {id: 'catg-images-order-popup', className: 'hide popup'}),
                $zoom_out_button  = $options.down('[data-action="zoom-out"]'),
                $zoom_null_button = $options.down('[data-action="zoom-null"]'),
                $zoom_in_button   = $options.down('[data-action="zoom-in"]'),
                i;

            $container.insert($orderPopup);
            ...
            var imageOrderPopup = Class.create({
                ...
                buildPopup: function () {
                    this.$popupContainer
                        .update(new Element('div', {className: 'popup-header-ng'}).update(new Element('h1').update('[{isys type="lang" ident="LC__CATG__IMAGES__ORDER_IMAGES"}]')))
                        .insert(new Element('div', {className: 'popup-content'})
                            .update(new Element('ul', {className: 'list-style-none m0'})))
                        .insert(new Element('div', {className: 'popup-footer-ng'})
                ...
                attachObserver: function () {
                    this.$popupContainer.down('.popup-footer-ng .btn').on('click', this.saveImageOrder.bindAsEventListener(this));
                ...
                saveImageOrder: function (ev) {
                    ...
                    $button.disable()
                        ...
                        .next('span')
                        .update('[{isys type="lang" ident="LC__UNIVERSAL__LOADING"}]');
                    ...
                    new Ajax.Request('?ajax=1&call=images&func=saveImagesOrder', {
                        ...
                        onComplete: function (xhr) {
                            ...
                            $button.enable()
                                ...
                                .next('span')
                                .update('[{isys type="lang" ident="LC__CATG__IMAGES__SAVE_ORDER"}]');
                        }.bind(this)
                    });
                },
            ...
            new qq.FileUploader({
                ...
                allowedExtensions:                 window.allowedImageExtensions,
                ...
                uploadButtonText:                  '<img src="[{$dir_images}]axialis/basic/zoom.svg" alt="" /><span>[{isys type="lang" ident="LC__UNIVERSAL__FILE_ADD"}]</span>',
                cancelButtonText:                  '&nbsp;',
            ...
            function render_thumb(id) {
                var $thumb    = new Element('div', { id: 'thumb-' + id, className: 'gallery-item', 'data-image-id': 0, style: 'width:' + thumb_size + 'px; height:' + thumb_size + 'px' }),
                    ...
                    $loader   = new Element('img', { src: window.dir_images + 'axialis/user-interface/loading.svg', className: 'loader animation-rotate' }),
...
<style type="text/css">
    ...
    #catg-images-gallery div.gallery-item button:hover {
		background-color: #e3e3e3; /* Hover color without transparency */
    }

	#catg-images-gallery div.gallery-item button:active {
		background-color: #b9b9b9; /* Active color without transparency */
	}
	...
	#catg-images-order-popup li:hover {
