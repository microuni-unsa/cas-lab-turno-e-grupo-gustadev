...
<div id="contentBottom">
	<div id="contentBottomContent">
		<div id="scroller" [{if $scrollerVisible}]class="overflow-scroller"[{/if}]>
			...
			<table class="contentTable commentaryTable">
	            <tr>
	                <td class="key vat">[{isys type="f_label" name="C__CMDB__CAT__COMMENTARY_$commentarySuffix" ident="LC__CMDB__CAT__COMMENTARY"}]</td>
	...
	<script type="text/javascript">
		var $contentbottomcontent = $('contentBottomContent'),
            ...
            $idField = $('isys_form').down('[name="id"]');
        ...
        // @see ID-11322 ALWAYS set a entry ID (0 as default).
        $idField.setValue('[{$id|default:0}]');

        [{if !empty($attributeVisibilityConfig)}]
        idoit.Require.require('attributeVisibility', function () {
            (new AttributeVisibility(JSON.parse('[{$attributeVisibilityConfig|json_encode|escape:"javascript"}]'))).hideAttributes();
        });
        [{/if}]
		...
		[{if isys_glob_is_edit_mode()}]
            ...
            idoit.Require.require('validation', function () {
                $contentbottomcontent.on('change', '[data-validation-rule="1"]', function (ev) { // .validate-rule
                    ...
                    if (identifier == null || identifier.blank()) {
                    ...
                    new Ajax.Request('[{$validationUrl}]', {
                        ...
                        parameters: {
                            ...
                            objectTypeId: parseInt('[{$smarty.get.objTypeID|strip_tags|escape}]' || '[{$object_type_id}]'),
                            objectId: parseInt('[{$smarty.get.objID|strip_tags|escape}]' || '[{$object_id}]'),
                            customCategoryId: parseInt('[{$smarty.get.customID|strip_tags|escape|default:'0'}]'),
                            categoryEntryId: parseInt('[{$smarty.get.cateID|strip_tags|escape}]' || '[{$category_entry_id}]')
                        },
