...
<script type="text/javascript">
	(function () {
		...
		var $ipTableBody        = $('ip-table-body'),
		    ...
		    hosts               = JSON.parse('[{$hosts|json_encode|escape:"javascript"}]'),
		    non_addressed_hosts = JSON.parse('[{$non_addressed_hosts|json_encode|escape:"javascript"}]'),
		    class_name          = null,
		    zones               = JSON.parse('[{$zones|json_encode|escape:"javascript"}]');
		...
		var render_list = function () {
			...
			for (i in hosts)
			{
				...
				for (i2 in hosts[i])
				{
					...
					action = new Element('button', {type: 'button', onClick: 'idoit.callbackManager.triggerCallback(\'iplist_disconnect\', ' + hosts[i][i2].list_id + ')', className:'btn btn-mini', title:'[{isys type="lang" ident="LC__CMDB__CATS__NET_IP_ADDRESSES__DISCONNECT" p_bHtmlEncode=false}]', 'data-tooltip': 1})
							.update(new Element('img', {src:window.dir_images + 'axialis/basic/link-broken.svg'}));
			...
			for (i in non_addressed_hosts)
			{
				...
				action = new Element('button', {type: 'button', onClick: 'idoit.callbackManager.triggerCallback(\'iplist_disconnect\', ' + non_addressed_hosts[i].list_id + ')', className:'btn btn-mini', title:'[{isys type="lang" ident="LC__CMDB__CATS__NET_IP_ADDRESSES__DISCONNECT" p_bHtmlEncode=false}]', 'data-tooltip': 1})
						.update(new Element('img', {src:window.dir_images + 'axialis/basic/link-broken.svg'}));
		...
		$('contentWrapper').on('scroll', function() {
			...
			if (top > scroll_at) {
				...
				$boxScroller.setStyle({top: scroll_at + 'px'});
			} else {
