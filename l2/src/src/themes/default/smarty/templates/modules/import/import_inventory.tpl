...
<table class="mainTable" cellpadding="0" cellspacing="0">
    ...
    [{foreach from=$import_files item="im"}]
        ...
        <tr id="[{$im.stripped}]">
            <td class="mouse-pointer" onclick="$('currentImportFileInventory').update('[{$im.filename}] : ');select_importfile('inventory', '[{$cmdbImportPath}][{$im.filename}]', $(this));">
            ...
            <td>
                <button type="button" class="fr btn btn-small mr5" onclick="delete_import('[{$im.filename}]');new Effect.SlideUp('[{$im.stripped}]')">
                    <img src="[{$dir_images}]axialis/industry-manufacturing/waste-bin.svg" alt="" class="mr5" /><span>[{isys type="lang" ident="LC__UNIVERSAL__DELETE_FILE"}]</span>
                </button>

                <a href="[{$im.download}]" class="fr btn btn-small mr5">
                    <img src="[{$dir_images}]axialis/basic/symbol-download.svg" alt="" class="mr5" /><span>[{isys type="lang" ident="LC__UNIVERSAL__DOWNLOAD_FILE"}]</span>
                </a>
            </td>
        </tr>
        [{/if}]
...
<pre class="m5 mt15 bg-neutral-200 border" id="import_result_inventory" style="height:400px;display:none;overflow:scroll;font-family:Courier New, Monospace;"></pre>
...
<div class="m5 mt15">
    ...
    [{/if}]
    <a href="[{$inventoryDownloadUrl}]" target="_blank" class="btn">
