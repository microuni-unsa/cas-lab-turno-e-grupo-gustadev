...
Browser.portList = Class.create(Browser.objectList, {
    ...
    addButton:function (values, view) {
        ...
        if (this.options.multiselection) {
            ...
            return '<button type="button" class="btn btn-small btn-block" onclick="' + func_string + '">' +
                   '<img src="' + window.dir_images + 'axialis/basic/symbol-add.svg" alt="" />' +
    ...
    removeButton:function (values, view) {
        ...
        if (this.options.multiselection) {
            ...
            return '<button type="button" class="btn btn-small btn-block" onclick="' + func_string + '">' +
                   '<img src="' + window.dir_images + 'axialis/basic/symbol-cancel.svg" alt="" />' +
    ...
    if (window.browserPreselection.isMultiselection()) {
        ...
        if (selection[0]) {
            ...
            window.browserPreselection.getObjectMetaData([selection[0]], function (metaData) {
                $(viewElement).setValue(metaData[0].objectTypeTitle + ' >> ' + metaData[0].objectTitle);
...
window.browserPreselection = new Browser.preselection('objectPreselection', {
    ...
    secondList:new Browser.portList('portList', {
        listOptions:{
            colgroup:'<colgroup><col style="width:120px" /><col /><col width="130" /></colgroup>',
            search:false,
            filter:false,
...
Browser.relationList = Class.create(Browser.objectList, {
    createFirstRow:function (obj) {
        ...
        this.tableColumnsName.each(function (i) {
            ...
            if (i == '__checkbox__') {
                var all_button = '<button type="button" id="' + this.table.id + '-add_all" class="btn btn-small mr5" title="' + idoit.Translate.get('LC__CMDB__OBJECT_BROWSER__ADD_ALL_BY_FILTER') + '">' +
                    '<img src="' + window.dir_images + 'axialis/basic/symbol-add.svg" alt="" /><span>' + idoit.Translate.get('LC__UNIVERSAL__ALL') + '</span>' +
                    '</button>' +
                    '<button type="button" id="' + this.table.id + '-add_page" class="btn btn-small" title="' + idoit.Translate.get('LC__CMDB__OBJECT_BROWSER__ADD_ALL_ON_PAGE') + '">' +
                    '<img src="' + window.dir_images + 'axialis/basic/symbol-add.svg" alt="" /><span>' + idoit.Translate.get('LC__UNIVERSAL__PAGE') + '</span>' +
                    '</button>';
    ...
    createRow:function (obj, index) {
        ...
        this.tableColumnsName.each(function (s, index) {
            ...
            if (s == '__checkbox__') {
                ...
                if (!is_relation && JSON.parse('[{$relation_only}]') && this.options.firstSelection) {
...
window.browserList = new Browser.relationList('objectList', {
    ...
    listOptions:{
        colgroup:'<colgroup><col style="width:140px;" /><col /><col style="width:130px;" /><col style="width:130px;" /></colgroup>',
        multiselection:JSON.parse('[{if $multiselection}]true[{else}]false[{/if}]'),
        firstSelection:true,
        secondSelection:false,
        secondSelectionExists:JSON.parse('[{$secondSelection|default:"false"}]'),
