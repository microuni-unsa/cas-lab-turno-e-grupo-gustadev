...
<p><b>Tenant licenses</b>: [{$totalTenants}], In use: [{$totalTenants - $remainingTenants}], Free: [{$remainingTenants}]</p>
<p><b>Object licenses</b>: <span id="total_license_objects">[{$totalLicenseObjects}]</span>, In use: [{$totalLicenseObjects - $remaningLicenseObjects}], Free: [{$remaningLicenseObjects}]</p>
<p><b>Assigned object licenses</b>: [{$assignedObjectLicenses}]</p>
<p><b>Automatic object license distribution</b>: <input type="checkbox" id="active_license_distribution" name="active_license_distribution" [{if $activeLicenseDistribution}]checked[{/if}] /></p>
...
<table cellpadding="2" cellspacing="0" width="100%" class="sortable mt10" id="list">
	...
	[{foreach $tenantsList as $row}]
		<tr class="[{cycle values="even,odd"}] mandator-row[{if $mandatorObjectCount[$row.isys_mandator__id] > $row.isys_mandator__license_objects && $row.isys_mandator__active}] invalid[{/if}]">
            ...
            <td>
                Object licenses in use: [{$mandatorObjectCount[$row.isys_mandator__id]}], Assigned licenses:
                [{if $row.isys_mandator__active}]
                <input type="number" class="mandator_license_objects" data-mandator-id="[{$row.isys_mandator__id}]" name="license_objects[]" value="[{$row.isys_mandator__license_objects}]" [{if $activeLicenseDistribution}]disabled="disabled"[{/if}]/>
                [{else}]
                0
                <input type="hidden" data-mandator-id="[{$row.isys_mandator__id}]" name="license_objects[]" value="0" />
                [{/if}]
            </td>
		</tr>
	[{/foreach}]
