        ...
        case 'lazyinstall':
            try {
                if (isset($_POST['module']) && isset($_POST['tenant'])) {
                    if (!file_exists($g_absdir . '/src/classes/modules/' . $_POST['module'] . '/package.json')) {
                        throw new Exception('Can not find installed module in directory: ' . $g_absdir . '/src/classes/modules/' . $_POST['module']);
                    }
            ...
            try {
                ...
                if ($_FILES['module_file']['tmp_name']) {
                    ...
                    if (install_module_by_zip($_FILES['module_file']['tmp_name'], $_POST['mandator'])) {
                        ...
                        // At the end we'll clear the temp directory.
            ...
            if (is_dir($l_directory . $l_file) && strpos($l_file, '.') !== 0 && !isOpenModuleIdentifier($l_file)) {
                if (file_exists($l_directory . $l_file . '/package.json')) {
                    ...
                    if ($l_package && $l_module_id) {
                        ...
                        // @see ID-9614 Loading module details is not necessary - the data is never used.
